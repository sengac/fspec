...
async fn test_read_pdf_default_visual_mode_renders_pages_as_images() {
    ...
    let output = result.expect("PDF read with default visual mode should succeed");
    ...
    assert!(
        parsed.get("pages").is_some() || parsed.get("total_pages").is_some(),
    ...
    if let Some(pages) = parsed.get("pages").and_then(|p| p.as_array()) {
        for page in pages {
            ...
            assert_eq!(
                media_type,
                Some("image/png"),
                "Media type should be image/png"
            );
...
async fn test_read_pdf_text_mode_extracts_searchable_text() {
    ...
    assert!(
        page1_pos < page2_pos && page2_pos < page3_pos,
...
async fn test_read_pdf_images_mode_extracts_embedded_images() {
    ...
    assert!(
        !output.is_empty(),
        "Output should not be empty for images mode"
    );
}
...
async fn test_reject_password_protected_pdf_with_clear_error() {
    ...
    // @step And the error message should mention password protection
    assert!(
...
async fn test_text_mode_handles_scanned_pdfs_gracefully() {
    ...
    assert!(
        result.is_ok(),
        "Text mode should succeed even with no extractable text"
    );
    let output = result.unwrap();

    // @step And the output should still include page separators
    assert!(
        output.contains("Page") || output.contains("page"),
...
async fn test_pdf_modes_exempt_from_token_limits() {
    ...
    assert!(
        result.is_ok(),
        "PDF should succeed despite low token limit: {:?}",
        result.err()
    );
    ...
    assert!(
        output.contains("page") || output.contains("content"),
...
async fn test_invalid_pdf_mode_falls_back_to_visual() {
    ...
    let output = result.expect("Invalid mode should fall back to visual mode");
