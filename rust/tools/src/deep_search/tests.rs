...
mod tests {
    ...
    fn test_tool_trait_name_and_args() {
        // @step Given the DeepSearch tool struct exists in the rust/tools/src/deep_search module
    ...
    fn test_narrow_scope_restricts_sub_agent() {
        ...
        // @step When the parent agent calls DeepSearch with scope ["rust/tools/src/read.rs"]
        let scope = vec!["rust/tools/src/read.rs".to_string()];
        let prompt = build_system_prompt(&scope, false);

        // @step Then the sub-agent's system prompt describes only "rust/tools/src/read.rs" as in scope
        assert!(prompt.contains("rust/tools/src/read.rs"));
