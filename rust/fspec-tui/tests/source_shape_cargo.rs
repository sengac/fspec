//! Source-shape regression tests for rust/fspec-tui Cargo.toml + workspace
...
//!   - "rust/fspec-tui is a fifth RPC-family workspace member with no
//!     binary entry point"
//!   - "rust/fspec-tui production dependencies include only RPC seam
//!     crates plus ratatui dependencies"
//!   - "rust/fspec-tui dev-dependencies allow codelet-core for fixtures
//!     but never codelet-napi"
//!   - "rust/fspec-tui contains no own-runtime construction calls"
...
fn codelet_fspec_tui_is_a_workspace_member_with_no_binary_entry_point() {
    ...
    // @step When I add rust/fspec-tui to rust/Cargo.toml's [workspace] members and create rust/fspec-tui/Cargo.toml plus rust/fspec-tui/src/lib.rs
    ...
    assert!(
        ...
        "rust/Cargo.toml [workspace] members must include \"fspec-tui\". Got:\n{workspace_cargo_toml}"
    );

    // @step And rust/fspec-tui/Cargo.toml contains a [lib] section but NO [[bin]] section
    ...
    // @step And running "cargo build -p codelet-fspec-tui" from rust/ exits with status code 0
    ...
    assert!(
        lib_rs.exists(),
        "expected rust/fspec-tui/src/lib.rs to exist"
    );
}
...
fn codelet_fspec_tui_contains_no_own_runtime_construction_calls() {
    // @step Given a source-shape integration test that scans rust/fspec-tui/src/
    let src_dir = common::workspace_root().join("fspec-tui").join("src");
    let rs_files = common::collect_rs_files(&src_dir);
    assert!(
        !rs_files.is_empty(),
        "expected at least one .rs file under rust/fspec-tui/src/"
    );
    ...
    assert!(
        violations.is_empty(),
        "rust/fspec-tui MUST NOT construct its own tokio runtime. \
...
fn rpc_embedded_scenario_7_is_widened_to_scan_fspec_tui() {
    // @step Given the existing test rust/rpc-embedded/tests/architecture_invariants.rs::scenario_7_embedded_transport_requires_tokio_handle_at_construction
