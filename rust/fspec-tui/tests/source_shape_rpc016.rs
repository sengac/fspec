...
fn work_unit_info_gains_the_last_state_change_at_field() {
    // @step Given rust/rpc-types/src/lib.rs after RPC-016 lands
    ...
    assert!(
        ...
        "rust/rpc-types/src/lib.rs must declare `pub last_state_change_at: Option<String>` on WorkUnitInfo after RPC-016"
    );
}
...
fn codelet_core_work_units_reads_state_history_into_last_state_change_at() {
    // @step Given rust/core/src/work_units.rs after RPC-016 lands
    ...
    assert!(
        ...
        "rust/core/src/work_units.rs must reference stateHistory (or state_history serde rename) after RPC-016"
    );
    // @step And the file contains the substring "last_state_change_at"
    assert!(
        body.contains("last_state_change_at"),
        "rust/core/src/work_units.rs must populate `last_state_change_at` after RPC-016"
    );
}
...
fn action_enum_gains_the_four_new_viewport_variants() {
    // @step Given rust/fspec-tui/src/components/mod.rs after RPC-016 lands
...
fn boardstore_declares_scroll_offsets_field_and_viewport_methods() {
    // @step Given rust/fspec-tui/src/store/board.rs after RPC-016 lands
...
fn viewport_painter_module_exists_as_separate_file() {
    // @step Given the rust/fspec-tui crate after RPC-016 lands
    // @step When a developer scans the views/board/ directory
    let viewport = src_dir().join("views").join("board").join("viewport.rs");
    // @step Then the file rust/fspec-tui/src/views/board/viewport.rs exists
    assert!(
        viewport.exists(),
        "rust/fspec-tui/src/views/board/viewport.rs must exist after RPC-016"
    );
}
...
fn new_and_modified_board_modules_stay_under_300_lines() {
    // @step Given the directory rust/fspec-tui/src/views/board/ plus the views/board.rs orchestrator and store/board.rs
    ...
    let mut targets: Vec<std::path::PathBuf> = vec![board_orchestrator, store_board];
...
fn rpc013_rpc014_rpc015_invariants_preserved() {
    // @step Given rust/fspec-tui/src/views/board.rs after RPC-016 lands
