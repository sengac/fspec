...
//! engine out of `rust/napi/src/scheduler/` and into
//! `rust/core/src/scheduler/`, plus the new `SchedulerHooks` trait
//! that replaces direct `crate::session_bindings::SessionManager`
//! references. The pure CRUD helpers from
//! `rust/napi/src/schedule_handler.rs` lift into
//! `rust/core/src/scheduler/crud.rs`.
//!
...
fn scheduler_core_dir() -> PathBuf {
    workspace_root().join("rust/core/src/scheduler")
}
...
/// Scenario: Scheduler engine modules live under rust/core/src/scheduler/
#[test]
fn scheduler_engine_modules_live_under_codelet_core() {
    // @step Given the directory rust/core/src/scheduler/ exists
    let dir = scheduler_core_dir();
    assert!(
        dir.is_dir(),
        "rust/core/src/scheduler/ directory should exist"
    );
    ...
    for file in [
        ...
        assert!(
            path.is_file(),
            "rust/core/src/scheduler/{file} should exist"
        );
    }
}
...
fn codelet_core_declares_scheduler_hooks_trait() {
    // @step Given the file rust/core/src/scheduler/mod.rs is compiled
...
fn spawn_scheduler_takes_arc_dyn_scheduler_hooks() {
    // @step Given the file rust/core/src/scheduler/engine.rs is compiled
...
fn lifted_engine_has_no_session_bindings_references() {
    ...
    // @step Then no file under rust/core/src/scheduler/ contains the text "crate::session_bindings"
    // @step And no file under rust/core/src/scheduler/ contains the text "session_bindings::SessionManager"
...
/// Scenario: rust/napi/src/scheduler/mod.rs is a thin re-export shim
#[test]
fn napi_scheduler_mod_is_thin_reexport_shim() {
    // @step Given the file rust/napi/src/scheduler/mod.rs is compiled
    let path = workspace_root().join("rust/napi/src/scheduler/mod.rs");
...
fn crud_helpers_live_in_codelet_core_scheduler_crud() {
    // @step Given the file rust/core/src/scheduler/crud.rs is compiled
...
/// from rust/napi/src/scheduler/ into rust/core/src/scheduler/
...
fn lifted_scheduler_engine_and_cron_utils_use_captured_identifier_format_args() {
    ...
    // (verified transitively by rust/sessions/tests/skeleton_invariants.rs::scenario_workspace_lints_are_inherited_and_clippy_passes)

    // @step Given rust/core/src/scheduler/engine.rs and rust/core/src/scheduler/cron_utils.rs live at their post-RPC-058 location
    let engine_path = workspace_root().join("rust/core/src/scheduler/engine.rs");
    let cron_utils_path = workspace_root().join("rust/core/src/scheduler/cron_utils.rs");
    assert!(
        engine_path.exists(),
    ...
    assert!(
        ...
        "RPC-058 retro: rust/core/src/scheduler/engine.rs must use captured-identifier format args (got {} violations):\n{}",
        engine_violations.len(),
        engine_violations.join("\n")
    );
    assert!(
        cron_violations.is_empty(),
        "RPC-058 retro: rust/core/src/scheduler/cron_utils.rs must use captured-identifier format args (got {} violations):\n{}",
        cron_violations.len(),
