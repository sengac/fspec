//! MUX-006 — mux focus flash: a 350ms purple background scan (full-height
//! 2-column strip, right-to-left) painted over the ENTIRE newly-selected pane.
//!
...
//! animation (no burst ring, no rain): a full-height 2-column scan strip
//! sweeping from the pane's RIGHT edge to the LEFT over 350ms. It is
//! re-armed on every focus change while mux is enabled, advances through
//! a render-driven clock (+16ms per rendered mux frame — the flash paints
//! at the current clock then advances, so frame 1 shows the right edge
//! and frame 22 the left edge), and keeps the run-loop draw gate
//! (`tick_should_draw`, 5th operand) open for its 350ms window.
...
/// frame 22 paints at clock 336 (left edge), then the advance to 352 ends
/// the flash.
...
fn symbols_in(buf: &ratatui::buffer::Buffer, x0: u16, y0: u16, w: u16, h: u16) -> Vec<String> {
    (0..h)
        .flat_map(|dy| {
            (0..w).map(move |dx| buf[(x0 + dx, y0 + dy)].symbol().to_string())
        })
        .collect()
}
...
// ─────────────────────────────────────────────────────────────────────────
// Scenario: focusing a pane scans a purple strip from the right edge to
// the left
// ─────────────────────────────────────────────────────────────────────────

/// Scenario: focusing a pane scans a purple strip from the right edge to the left
#[test]
fn focusing_a_pane_scans_a_purple_strip_from_right_edge_to_left() {
    ...
    assert_eq!(paint_clock, 0, "the first frame must paint at the armed clock 0 (right edge)");
    let buf = render_frames(&mut nav, &board, &mut agent, 1);
    let agent_rect = nav.mux.pane_rects()[1];
    // @step Then the Agent pane rect shows the dark purple background (RGB 74, 44, 112) as a full-height 2-column strip at the pane's right edge
    ...
    assert_eq!(
        ...
        "frame 1 must paint exactly the deterministic right-edge strip"
    );
    assert!(!painted.is_empty(), "the flash must paint cells at the right edge");
    let left_col = painted.iter().map(|&(x, _)| x).min().expect("non-empty");
    assert_eq!(
        left_col,
        agent_rect.x + agent_rect.width - 2,
        "the scan must START at the pane's right edge (2 columns wide)"
    );
    // Full-height: every row of the pane is tinted.
    let rows: Vec<u16> = painted.iter().map(|&(_, y)| y).collect();
    assert_eq!(
        rows.iter().min().copied().expect("rows"),
        agent_rect.y,
        "strip must span the pane from its top"
    );
    assert_eq!(
        rows.iter().max().copied().expect("rows"),
        agent_rect.y + agent_rect.height - 1,
        "strip must span the pane to its bottom"
    );
    ...
    // @step And rendering a few frames later the strip has moved LEFT (a mid-scan position, still 2 columns wide)
    ...
    assert!(!mid_painted.is_empty(), "the strip must still be visible mid-scan");
    let mid_left = mid_painted.iter().map(|&(x, _)| x).min().expect("non-empty");
    assert!(
        mid_left < left_col,
        "mid-scan left column {mid_left} must be LEFT of the start column {left_col}"
    );
    let mid_right = mid_painted.iter().map(|&(x, _)| x).max().expect("non-empty");
    assert_eq!(
        (mid_right - mid_left) + 1,
        2,
        "the strip must stay exactly 2 columns wide mid-scan"
    );
    // @step And rendering to the end of the 350ms window shows the strip at the pane's left edge
    // 9 frames have rendered so far; 13 more land on frame 22, which
    // paints at clock 21*16 = 336 (LAST_PAINT_MS — the left edge) and
    // whose post-paint advance (352 ≥ 350) ends the flash.
    let last_buf = render_frames(&mut nav, &board, &mut agent, ELAPSE_FRAMES - 9);
    let end_painted = purple_cells(&last_buf);
    assert!(!end_painted.is_empty(), "the strip must still be visible on the final frame");
    let end_left = end_painted.iter().map(|&(x, _)| x).min().expect("non-empty");
    assert_eq!(
        end_left,
        agent_rect.x,
        "the scan must END at the pane's left edge"
    );
    // MUX-007: the 350ms scan animation is over, but its FINAL frame —
    // the full-height strip parked at the left edge — settles: it keeps
    ...
    // @step And after 350ms of rendered frames the scan animation has ended but the final left-edge strip stays painted on the focused pane only
    let settled = purple_cells(&after);
    let expected_settled = flash_cells(agent_rect, FLASH_MS);
    assert!(!expected_settled.is_empty(), "settled strip must exist");
    ...
    assert_eq!(
        sorted, exp_sorted,
        "after the window elapses the focused pane must keep exactly the settled left-edge strip"
    );
    for (x, y) in &settled {
        assert!(
            (*x as i32) >= (agent_rect.x as i32)
                && *x < agent_rect.x + 2,
            "settled strip must sit at the pane's left edge at ({x}, {y})"
        );
        assert!(
            !board_rect.contains(ratatui::layout::Position { x: *x, y: *y }),
            "settled strip leaked into the Board pane at ({x}, {y})"
        );
    }
}
...
fn the_flash_stays_inside_the_focused_pane_and_keeps_content_readable() {
    ...
    let symbols1 = symbols_in(&buf1, agent_rect.x, agent_rect.y, agent_rect.width, agent_rect.height);
    ...
    assert!(!painted.is_empty(), "the flash must be painting during the window");
    for (x, y) in &painted {
        assert!(
    ...
    let symbols2 = symbols_in(&buf2, agent_rect.x, agent_rect.y, agent_rect.width, agent_rect.height);
    assert_eq!(
        symbols1, symbols2,
...
fn every_focus_change_path_re_arms_the_flash_on_the_newly_focused_pane() {
    ...
    assert_eq!(nav.mux.focus(), 1, "Shift+Right must move focus to the agent pane");
    assert_eq!(nav.mux.flash_pane(), Some(1), "flash must be armed on the agent pane");
    assert_eq!(nav.mux.flash_clock_ms(), 0, "flash clock must restart at 0 (right edge)");
    ...
    assert_eq!(nav.mux.flash_pane(), Some(0), "flash must be armed on the board pane");
    assert_eq!(nav.mux.flash_clock_ms(), 0, "flash clock must restart at 0 (right edge)");
    // @step When I press Enter on the selected work unit
    let result = nav.handle_event(&key(KeyCode::Enter, KeyModifiers::NONE), &board);
    assert!(result.is_consumed(), "Enter on the focused board pane must be consumed");
    // @step Then the Agent pane is focused and its flash is re-armed (restarted at the right edge)
    ...
    // @step Then the Agent pane is focused and its flash is re-armed (restarted at the right edge)
    assert_eq!(nav.mux.focus(), 1, "MuxEnterWorkUnit must focus the agent pane");
    assert_eq!(nav.mux.flash_pane(), Some(1), "flash must re-arm on the agent pane");
    assert_eq!(nav.mux.flash_clock_ms(), 0, "flash clock must restart at 0 (right edge)");
    ...
    assert_eq!(nav.mux.focus(), 0, "BackToBoard must focus the board pane in-grid");
    assert_eq!(nav.mux.flash_pane(), Some(0), "flash must re-arm on the board pane");
    assert_eq!(nav.mux.flash_clock_ms(), 0, "flash clock must restart at 0 (right edge)");
}
...
fn the_flash_keeps_the_render_tick_redrawing_while_idle_and_stops_after_the_window() {
    ...
    assert!(nav.is_mux_flash_active(), "Navigator must report the mux flash as active");
    ...
    // MUX-007: the settled strip is REPAINT content, not an animation —
    // the gate stays closed but the next (triggered) frame still paints
    // the left-edge strip on the focused pane.
    let agent_rect = nav.mux.pane_rects()[1];
    let after = render_buffer(&mut nav, &board, &mut agent);
    let settled = purple_cells(&after);
    assert_eq!(
        ...
        "with the gate closed the settled left-edge strip must still be painted"
    );
}
...
fn no_flash_and_no_purple_cells_when_mux_is_off() {
    ...
    assert!(!nav.is_mux_flash_active(), "mux-flash gate operand must be false with mux off");
    assert!(nav.mux.flash_pane().is_none(), "no flash may be armed with mux off");
}
...
fn the_flash_pattern_is_deterministic_for_a_given_clock_and_never_persisted() {
    ...
    nav.mux.set_focus(0);
    nav.mux.set_focus(1);
    assert_eq!(nav.mux.flash_clock_ms(), 0, "re-arming must reset the clock");
    let buf_b = render_buffer(&mut nav, &board, &mut agent);
    let clock_b = nav.mux.flash_clock_ms();
    let set_b = purple_cells(&buf_b);
    assert_eq!(clock_a, clock_b, "both renders must land at the same clock value");
