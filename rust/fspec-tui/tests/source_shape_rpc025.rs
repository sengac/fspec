...
//! recall state):
//!   * `rust/core/src/persistence/{mod.rs, history.rs}` exist with the
...
//!   * No file under `rust/fspec-tui/src/views/` imports forbidden
...
fn lifted_history_module_exists_under_core_with_right_shape() {
    ...
    assert!(
        mod_lines < 100,
        "rust/core/src/persistence/mod.rs has {mod_lines} lines, must be < 100"
    );
    // @step And a file exists at rust/core/src/persistence/history.rs
    assert!(
        history_path.is_file(),
        "{} must exist (lifted HistoryStore)",
        history_path.display()
    );
    // @step And rust/core/src/persistence/history.rs is under 300 lines
    let hist_lines = count_lines_path(&history_path);
    assert!(
        hist_lines < 300,
        "rust/core/src/persistence/history.rs has {hist_lines} lines, must be < 300"
    );
}
...
fn history_nav_state_module_exists_under_100_loc() {
    // @step Given the rust/fspec-tui crate
    let path = fspec_tui_src()
        .join("store")
        .join("agent_view")
        .join("history_state.rs");
    // @step Then a file exists at rust/fspec-tui/src/store/agent_view/history_state.rs
...
fn agent_view_store_stays_under_300_loc_with_history_fields() {
    // @step Given rust/fspec-tui/src/store/agent_view.rs after RPC-025 lands
...
fn napi_persistence_surface_becomes_thin_delegate_layer() {
    // @step Given rust/napi/src/persistence/mod.rs after the lift
    let napi_persistence_dir = napi_src().join("persistence");
    let napi_mod = read_raw(&napi_persistence_dir.join("mod.rs"));

    // @step Then rust/napi/src/persistence/history.rs does NOT exist (persistence types live in codelet_core)
    assert!(
        !napi_persistence_dir.join("history.rs").exists(),
        "rust/napi/src/persistence/history.rs must NOT exist — persistence types live in codelet_core after RPC-035"
    );

    // @step And rust/napi/src/persistence/mod.rs flat re-exports codelet_core::persistence
    let stripped_mod = common::strip_rust_comments(&napi_mod);
    assert!(
        stripped_mod.contains("pub use codelet_core::persistence::*"),
        "rust/napi/src/persistence/mod.rs must flat re-export codelet_core::persistence"
    );

    // @step And rust/napi/src/persistence/napi_bindings.rs::persistence_add_history delegates to history::add
    ...
    // @step And rust/napi/src/persistence/napi_bindings.rs::persistence_get_history delegates to history::get
    assert!(
    ...
    // @step And rust/napi/src/persistence/napi_bindings.rs::persistence_search_history delegates to history::search
    assert!(
...
fn history_match_declared_in_rpc_types_with_expected_fields() {
    // @step Given the rust/rpc-types crate
    let body = read_raw(&rpc_types_src().join("lib.rs"));
    // @step Then rust/rpc-types/src/lib.rs declares "pub struct HistoryMatch"
    assert!(
        body.contains("pub struct HistoryMatch"),
        "rust/rpc-types/src/lib.rs must declare `pub struct HistoryMatch`"
    );
    ...
    assert!(
        body.contains("napi"),
        "rust/rpc-types/src/lib.rs must continue to use the `napi` feature for shared types"
    );
}
...
fn no_view_file_imports_forbidden_crates() {
    ...
    for path in &files {
        let body = read_raw(path);
        // @step Then no file under rust/fspec-tui/src/views/ imports codelet_core
        // @step And no file under rust/fspec-tui/src/views/ imports codelet_napi
        // @step And no file under rust/fspec-tui/src/views/ imports tarpc
        // @step And no file under rust/fspec-tui/src/views/ imports tokio_tungstenite
        for needle in [
            "codelet_core::",
            "codelet_napi::",
            "tarpc::",
