...
fn new_shared_types_live_in_rpc_types() {
    // @step Given rust/rpc-types/src/lib.rs after RPC-022 lands
...
fn fspec_service_trait_gains_five_new_rpc_methods() {
    // @step Given rust/rpc/src/lib.rs after RPC-022 lands
...
fn session_manager_handle_trait_gains_new_methods_with_default_impls() {
    // @step Given rust/core/src/session_manager_handle.rs after RPC-022 lands
...
fn fspec_backend_trait_declares_five_new_methods() {
    // @step Given rust/fspec-tui/src/transport/mod.rs after RPC-022 lands
...
fn both_transports_implement_five_new_backend_methods() {
    // @step Given the rust/fspec-tui crate after RPC-022 lands
    ...
    // @step Then rust/fspec-tui/src/transport/embedded.rs contains the substring "async fn list_providers"
    assert!(embedded.contains("async fn list_providers"));
    // @step And rust/fspec-tui/src/transport/embedded.rs contains the substring "async fn set_session_model"
    assert!(embedded.contains("async fn set_session_model"));
    // @step And rust/fspec-tui/src/transport/embedded.rs contains the substring "async fn set_thinking_level"
    assert!(embedded.contains("async fn set_thinking_level"));
    // @step And rust/fspec-tui/src/transport/embedded.rs contains the substring "async fn get_session_role"
    assert!(embedded.contains("async fn get_session_role"));
    // @step And rust/fspec-tui/src/transport/embedded.rs contains the substring "async fn set_session_role"
    assert!(embedded.contains("async fn set_session_role"));
    // @step And rust/fspec-tui/src/transport/websocket.rs contains the substring "async fn list_providers"
    assert!(websocket.contains("async fn list_providers"));
    // @step And rust/fspec-tui/src/transport/websocket.rs contains the substring "async fn set_session_model"
    assert!(websocket.contains("async fn set_session_model"));
    // @step And rust/fspec-tui/src/transport/websocket.rs contains the substring "async fn set_thinking_level"
    assert!(websocket.contains("async fn set_thinking_level"));
    // @step And rust/fspec-tui/src/transport/websocket.rs contains the substring "async fn get_session_role"
    assert!(websocket.contains("async fn get_session_role"));
    // @step And rust/fspec-tui/src/transport/websocket.rs contains the substring "async fn set_session_role"
...
fn new_modal_dialog_modules_and_dispatch_helper_exist() {
    // @step Given the rust/fspec-tui crate after RPC-022 lands
    let src = src_dir();
    // @step Then the file rust/fspec-tui/src/components/thinking_level_dialog.rs exists
    assert!(src
        .join("components")
        .join("thinking_level_dialog.rs")
        .exists());
    // @step And the file rust/fspec-tui/src/views/agent/role_banner.rs exists
    assert!(src
        .join("views")
        .join("agent")
        .join("role_banner.rs")
        .exists());
    // @step And the file rust/fspec-tui/src/app/dispatch_model_thinking_dialogs.rs exists
    assert!(src
        .join("app")
...
fn new_rpc022_modules_stay_under_300_lines() {
    ...
    // @step Then rust/fspec-tui/src/components/thinking_level_dialog.rs has fewer than 300 lines
    assert!(
        lines_thinking < 300,
        "thinking_level_dialog.rs has {lines_thinking} lines (>= 300)"
    );
    // @step And rust/fspec-tui/src/views/agent/role_banner.rs has fewer than 300 lines
    assert!(
        lines_role < 300,
        "role_banner.rs has {lines_role} lines (>= 300)"
    );
    // @step And rust/fspec-tui/src/app/dispatch_model_thinking_dialogs.rs has fewer than 300 lines
    assert!(
        lines_dispatch < 300,
        "dispatch_model_thinking_dialogs.rs has {lines_dispatch} lines (>= 300)"
    );
}
...
fn priority_enum_gains_foreground_variant_numbered_900() {
    // @step Given rust/fspec-tui/src/components/mod.rs after RPC-022 lands
