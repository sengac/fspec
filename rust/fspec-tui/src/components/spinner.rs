//! TUI-106 — shared braille-dots spinner painter (lifted from `views/agent/spinner.rs`).
//!
//! MIRORS `src/tui/components/ThinkingIndicator.tsx:19-22` (the
//! `'dots'` spinner style) byte-for-byte:
//!
//! - 10 frames: `⠋ ⠙ ⠹ ⠸ ⠼ ⠴ ⠦ ⠧ ⠇ ⠏`
//! - 80 ms per frame
//!
//! TUI-106 moved this pure module into `components/` so that the lazy
//! mode-views (TUI-107 Checkpoints / TUI-108 Changed Files) can import
//! the spinner directly instead of reaching through `views::agent`
//! (a wrong dependency direction — sibling view modules must not
//! import each other). `views/agent/spinner.rs` now re-exports this
//! module so every existing agent-view call site
//! (`input_transition.rs`, `transition_driver.rs`) compiles unchanged
...
//! orchestrator computes `elapsed_ms = now - spinner_started_at` and
//! forwards it here. `LoadingDialog` (TUI-106) feeds the same elapsed
//! value for the mode-view dialog spinner.
//!
