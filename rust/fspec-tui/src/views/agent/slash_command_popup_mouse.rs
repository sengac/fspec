//! RPC-028 + TUI-103 — mouse routing for [`super::slash_command_popup::SlashCommandPopup`].
//!
//! Feature: spec/features/rpc028-scroll-mouse-wrap-parity.feature
//!
//! Extracted from `slash_command_popup.rs` so the popup file stays
//! under the 300-LoC source-shape ceiling (rpc018/rpc020 pins).
...
/// shrink-to-content dialog rect for `area`. The body content starts at
/// `d_rect.y + 4` (border + padding + title + gap) and spans
/// `d_rect.height - 4` rows; the scrollbar gutter is the rightmost
/// body column.
