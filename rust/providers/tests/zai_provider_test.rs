...
use codelet_providers::{LlmProvider, ZAIProvider};
use std::env;
use std::sync::Mutex;
...
fn test_error_when_zai_api_key_missing() {
    ...
    assert!(
        env::var("ZAI_API_KEY").is_err(),
        "ZAI_API_KEY should not be set"
    );
    assert!(
        env::var("ZAI_PLAN_API_KEY").is_err(),
        "ZAI_PLAN_API_KEY should not be set"
    );
    ...
    assert!(
        error_msg.contains("ZAI_API_KEY") || error_msg.contains("zai"),
