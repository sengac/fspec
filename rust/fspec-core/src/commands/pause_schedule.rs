//! `pause-schedule` — Rust port of `src/commands/schedule/pause-schedule.ts`
//! (RPC-254).
//!
//! Sets a schedule's `status` to `"paused"` in `spec/schedules.json`. The
//! schedule remains on disk but will not trigger until resumed. Both the
//! LLM-facing dispatcher AND the standalone Rust binary's clap subcommand
//! converge on this single `run` function (RPC-003 §7/§11 two-front-doors).
//!
//! ## Behaviour parity with TypeScript (`pause-schedule.ts:23-43`)
//!
//! * Missing schedule → error `Schedule '<name>' does not exist`.
//! * Already-paused schedule → error `Schedule '<name>' is already paused`.
//! * Otherwise the target's `status` becomes `"paused"`; every other field of
...
/// CLI arguments accepted by `pause-schedule`. Mirrors the TS
/// `ScheduleNameOptions` shape (`src/types/schedule.ts`) — a single required
/// schedule `name`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct PauseScheduleArgs {
    name: String,
}
...
pub async fn run(args_json: &str, project_root: &Path) -> Result<String, FspecCoreError> {
    ...
    // Validate the schedule exists (parity with pause-schedule.ts:31-33).
    let schedule =
        data.schedules
            .get_mut(&args.name)
    ...
    // Validate it is not already paused (parity with pause-schedule.ts:35-37).
    let current_status = schedule.get("status").and_then(Value::as_str).unwrap_or("");
    if current_status == "paused" {
        return Err(FspecCoreError::InvalidArgs {
            command: "pause-schedule",
            reason: format!("Schedule '{}' is already paused", args.name),
        });
    }
    ...
    let result = json!({
        "success": true,
        "message": format!("✓ Schedule '{}' paused successfully", args.name),
    });
...
mod tests {
    ...
    #[test]
    fn args_parse_name() {
        let a: PauseScheduleArgs = serde_json::from_str(r#"{"name":"nightly"}"#).unwrap();
