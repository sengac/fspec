...
// Feature: spec/features/add-command-rust-port.feature
//
// Validates the acceptance criteria for the Rust port of `add-command` (RPC-174).
...
#[test]
fn first_add_seeds_event_storm_and_appends_command_id_zero() {
    ...
    // @step When I dispatch add-command with workUnitId='AUTH-001' and text='PlaceOrder'
    let result = dispatch_command(req(
        tmp.path(),
        json!({"workUnitId": "AUTH-001", "text": "PlaceOrder"}),
    ));

    // @step Then the dispatcher returns success=true
    assert!(result.success, "expected success=true, got {result:?}");

    // @step And the returned data contains commandId=0
    let data: Value = serde_json::from_str(&result.data).expect("parse data json");
    assert_eq!(data["commandId"].as_u64(), Some(0));
    ...
    // @step And spec/work-units.json on disk shows AUTH-001.eventStorm.items[0].type='command'
    assert_eq!(items[0]["type"].as_str(), Some("command"));

    // @step And spec/work-units.json on disk shows AUTH-001.eventStorm.items[0].color='blue'
    assert_eq!(items[0]["color"].as_str(), Some("blue"));
...
fn second_add_appends_with_auto_incremented_id() {
    ...
    // @step When I dispatch add-command with workUnitId='AUTH-001' and text='CancelOrder'
    let result = dispatch_command(req(
        tmp.path(),
        json!({"workUnitId": "AUTH-001", "text": "CancelOrder"}),
    ));

    // @step Then the dispatcher returns success=true
    assert!(result.success, "expected success=true, got {result:?}");

    // @step And the returned data contains commandId=1
    let data: Value = serde_json::from_str(&result.data).expect("parse data json");
    assert_eq!(data["commandId"].as_u64(), Some(1));
    ...
    // @step And spec/work-units.json on disk shows AUTH-001.eventStorm.items[1].text='CancelOrder'
    assert_eq!(items[1]["text"].as_str(), Some("CancelOrder"));

    // @step And spec/work-units.json on disk shows AUTH-001.eventStorm.nextItemId=2
    assert_eq!(
        v["workUnits"]["AUTH-001"]["eventStorm"]["nextItemId"].as_u64(),
        Some(2)
    );
}

#[test]
fn optional_actor_is_stored_on_the_command_item() {
    // @step Given a project root tempdir with spec/work-units.json containing AUTH-001 status=specifying and no eventStorm field
    let tmp = TempDir::new().expect("tempdir");
    write_work_units(tmp.path(), &seed_units(&[("AUTH-001", "specifying")]));

    // @step When I dispatch add-command with workUnitId='AUTH-001' text='PlaceOrder' and actor='Customer'
    let result = dispatch_command(req(
        tmp.path(),
        json!({"workUnitId": "AUTH-001", "text": "PlaceOrder", "actor": "Customer"}),
    ));
    ...
    let v = read_work_units(tmp.path());
    assert_eq!(
        v["workUnits"]["AUTH-001"]["eventStorm"]["items"][0]["actor"].as_str(),
        Some("Customer")
    );
}
...
fn missing_work_unit_surfaces_canonical_error() {
    ...
    // @step When I dispatch add-command with workUnitId='NOPE-001' and text='Anything'
    let result = dispatch_command(req(
        tmp.path(),
...
fn done_or_blocked_status_is_rejected_verbatim_and_disk_is_untouched() {
    ...
    let tmp = TempDir::new().expect("tempdir");
    write_work_units(tmp.path(), &seed_units(&[("AUTH-001", "blocked")]));
    let pre_bytes = fs::read(tmp.path().join("spec/work-units.json")).unwrap();

    // @step When I dispatch add-command with workUnitId='AUTH-001' and text='Anything'
    let result = dispatch_command(req(
        tmp.path(),
