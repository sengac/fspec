...
//! a codelet-core dep on rust/rpc, while still forbidding codelet-napi.
...
fn scenario_default_fixture_is_unreachable_from_production_code() {
    // @step Given the rust/rpc crate after the watcher lift
    ...
    assert!(
        ...
        "default_fixture must be renamed/gated; found surviving `pub fn default_fixture` in rust/rpc/src/lib.rs"
    );
    assert!(
    ...
    assert!(
        has_cfg_test_gate,
        "rust/rpc/src/lib.rs must contain at least one #[cfg(test)] (or #[cfg(any(test, ...))]) gate around the test fixture"
    );
}

#[test]
fn scenario_embedded_transport_reuses_host_runtime_handle_for_fan_out() {
    // @step Given the rust/rpc-embedded crate after RPC-006
    ...
    assert!(
        ...
        "rust/rpc-embedded must not construct its own tokio runtime. \
         Found forbidden Builder/Runtime::new usage in: {forbidden_calls:?}"
    );
    assert!(
        uses_handle_spawn,
        "rust/rpc-embedded must spawn at least one task via `self.handle.spawn(...)` so the runtime invariant is exercised, not just satisfied vacuously"
    );
}

#[test]
fn scenario_codelet_rpc_may_depend_on_codelet_core_but_not_on_codelet_napi() {
    // @step Given rust/rpc/Cargo.toml after the watcher lift
    let cargo_path = workspace_root().join("rpc").join("Cargo.toml");
    let cargo = read_to_string_or_panic(&cargo_path);

    // @step When I inspect its [dependencies] table and the rust/rpc/src/ source for use statements naming codelet_napi or codelet_core
    ...
    assert!(
        ...
        "rust/rpc/Cargo.toml MUST depend on codelet-core after the watcher lift (RPC-006)"
    );
    assert!(
        !has_codelet_napi_dep,
        "rust/rpc/Cargo.toml MUST NOT depend on codelet-napi (RPC-006 dependency-arrow invariant)"
    );
    assert!(
        napi_imports.is_empty(),
        "rust/rpc/src must not import codelet_napi (RPC-006). Found: {napi_imports:?}"
    );
}
...
fn scenario_work_unit_info_continues_to_be_defined_exactly_once_in_rpc_types() {
    ...
    // @step When I inspect rust/napi and rust/core for any local definition of a struct named WorkUnitInfo and run cargo check on the workspace
    ...
    // @step Then no other crate defines WorkUnitInfo locally, both crates re-export the type from rust/rpc-types where they reference it, and the workspace builds successfully
    assert!(
        local_definitions.is_empty(),
        "Neither rust/napi nor rust/core may redefine WorkUnitInfo. \
...
fn scenario_embedded_push_path_contains_no_bincode_encode_call() {
    // @step Given the rust/rpc-embedded crate after RPC-006 exposes work_units_rx via a broadcast::Receiver
    let src_dir = workspace_root().join("rpc-embedded").join("src");
    let rs_files = collect_rs_files(&src_dir);
    ...
    assert!(
        ...
        "rust/rpc-embedded must not bincode-encode the push path (zero-cost embedded). \
         Found: {bincode_calls:?}"
    );
}
