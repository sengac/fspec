...
fn scenario_rpc_007_source_shape_invariants() {
    // @step Given the workspace contains rust/rpc, rust/rpc-types, rust/rpc-server, rust/rpc-embedded, rust/napi, and rust/core
    ...
    for crate_name in [
        ...
        assert!(
            ...
            "expected rust/{crate_name}/Cargo.toml to exist",
        );
    }

    // @step When cargo metadata is queried for rust/rpc/Cargo.toml dependencies
    ...
    assert!(
        ...
        "rust/rpc/Cargo.toml MUST NOT depend on codelet-napi (RPC-007 source-shape preserves the RPC-006 invariant)",
    );
    assert!(
        napi_imports.is_empty(),
        "rust/rpc/src must not import codelet_napi. Found: {napi_imports:?}",
    );
    ...
    // @step Then each type has exactly one definition site, located in rust/rpc-types
    for (ty, sites) in &definition_sites {
        ...
        assert!(
            ...
            "type {ty} must be defined in rust/rpc-types. Found: {}",
            sites[0],
        );
    }

    // @step And rust/napi re-exports each type via the existing #[cfg_attr(feature = "napi", napi(...))] pattern
    ...
    for ty in lifted_types {
        ...
        assert!(
            appears_via_use,
            "rust/napi must re-export {ty} from codelet_rpc_types (look for `pub use codelet_rpc_types::{ty}` or grouped re-export)",
        );
    }
}
