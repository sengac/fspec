...
//! `rust/{fspec, fspec-tui, sessions}/tests/no_napi_dependency.rs`,
...
/// Resolve the workspace root (`rust/`) by walking up from the test
/// crate's `CARGO_MANIFEST_DIR`.
///
/// Tests invoke the helpers from `rust/<crate>/tests/*.rs`. Their
/// `CARGO_MANIFEST_DIR` is `rust/<crate>/`, so the workspace root is
...
/// (i.e. `rust/<test_crate>/..` resolves to `rust/`). This works
...
/// `rust/{core, rpc-types, fspec, fspec-tui, sessions}/tests/no_napi_dependency.rs`.
///
/// # Panics
///
...
/// Walk every `.rs` file under `rust/<crate_dir_name>/src/`, strip
...
/// # Panics
///
/// - If `rust/<crate_dir_name>/src/` is not a directory.
/// - If `rust/<crate_dir_name>/src/` contains zero `.rs` files (so
...
pub fn assert_no_import_in_sources_with_manifest(
    ...
    assert!(
        src_dir.is_dir(),
        "rust/{crate_dir_name}/src must be a directory; got {}",
        src_dir.display()
    );
    ...
    assert!(
        !rs_files.is_empty(),
        "rust/{crate_dir_name}/src must contain at least one .rs file"
    );
    ...
    assert!(
        offenders.is_empty(),
        "rust/{crate_dir_name}/src MUST NOT import {forbidden_module} (forbidden-arrow invariant). Offending files: {offenders:?}"
    );
}
