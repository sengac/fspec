...
// reducing rust/napi/src/persistence/ to a pure NAPI adapter
...
//
// Tests live under rust/core/tests/ so they are a downstream consumer
...
//   - rust/napi/src/persistence/ still contains tests.rs and lazy_init_tests.rs
//   - rust/napi/src/persistence/mod.rs still has the 6 surviving pub fn helpers
//   - rust/napi/src/persistence/napi_bindings.rs still opens with `use super::*;`
//   - rust/core/src/persistence/tests.rs and lazy_init_tests.rs do not exist yet
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR points to rust/core; the workspace root is two levels up.
...
fn napi_persistence_dir() -> PathBuf {
    workspace_root().join("rust/napi/src/persistence")
}

fn core_persistence_dir() -> PathBuf {
    workspace_root().join("rust/core/src/persistence")
}
...
fn test_napi_persistence_directory_contains_only_mod_and_napi_bindings() {
    ...
    // @step When std::fs::read_dir lists the entries of rust/napi/src/persistence/
    ...
    assert_eq!(
        actual, expected,
        "rust/napi/src/persistence/ must contain ONLY mod.rs + napi_bindings.rs after RPC-035 (found {actual:?})"
    );

    // @step And rust/napi/src/persistence/mod.rs is at most 20 lines long
    let mod_rs = read_to_string_at(&dir.join("mod.rs"));
    let line_count = mod_rs.lines().count();
    assert!(
        line_count <= 20,
        "rust/napi/src/persistence/mod.rs must be at most 20 lines after RPC-035 (got {line_count})"
    );

    // @step And rust/napi/src/persistence/mod.rs contains no pub fn declarations
    assert!(
        !mod_rs.contains("pub fn "),
        "rust/napi/src/persistence/mod.rs must contain no `pub fn` declarations after RPC-035"
    );

    // @step And rust/napi/src/persistence/mod.rs contains exactly one pub use line: `pub use codelet_core::persistence::*;`
    ...
    assert!(
        pub_use_count >= 1,
        "rust/napi/src/persistence/mod.rs must re-export codelet_core::persistence via `pub use codelet_core::persistence::*;`"
    );
    assert!(
        mod_rs.contains("pub use codelet_core::persistence::*"),
        "rust/napi/src/persistence/mod.rs must contain the line `pub use codelet_core::persistence::*;`"
    );
}
...
fn test_napi_bindings_uses_explicit_codelet_core_persistence_imports() {
    // @step Given the engineer opens rust/napi/src/persistence/napi_bindings.rs
    ...
    assert!(
        ...
        "rust/napi/src/persistence/napi_bindings.rs must import its persistence symbols via `use codelet_core::persistence::{{ ... }};` (explicit, not via `use super::*;`)"
    );
...
fn test_relocated_codelet_core_persistence_tests_module_exists_with_48_tests() {
    // @step Given rust/core/src/persistence/tests.rs has been added with the relocated 48 tests
    let tests_path = core_persistence_dir().join("tests.rs");
    assert!(
        tests_path.is_file(),
        "rust/core/src/persistence/tests.rs must exist after RPC-035 relocates the 48-test suite from codelet-napi"
    );
    ...
    assert!(
        test_count >= 48,
        "rust/core/src/persistence/tests.rs must contain at least 48 #[test] attributes (the byte-identical baseline from codelet-napi pre-relocation, got {test_count})"
    );
    for fn_name in [
    ...
    // @step And rust/core/src/persistence/mod.rs declares `#[cfg(test)] mod tests;`
    let mod_rs = read_to_string_at(&core_persistence_dir().join("mod.rs"));
    assert!(
        mod_rs.contains("mod tests"),
        "rust/core/src/persistence/mod.rs must declare `#[cfg(test)] mod tests;`"
    );

    // @step And `cargo test -p codelet-napi --lib persistence::tests` reports zero tests because rust/napi/src/persistence/tests.rs no longer exists
    let napi_tests_path = napi_persistence_dir().join("tests.rs");
    assert!(
        !napi_tests_path.exists(),
        "rust/napi/src/persistence/tests.rs must not exist after RPC-035 relocates the 48-test suite to codelet-core (so `cargo test -p codelet-napi --lib persistence::tests` reports zero tests)"
    );
...
fn test_relocated_codelet_core_persistence_lazy_init_tests_module_exists_with_9_tests() {
    // @step Given rust/core/src/persistence/lazy_init_tests.rs has been added with the relocated 9 tests
    let lazy_path = core_persistence_dir().join("lazy_init_tests.rs");
    assert!(
        lazy_path.is_file(),
        "rust/core/src/persistence/lazy_init_tests.rs must exist after RPC-035 relocates the 9-test BUG-122 lazy-init suite"
    );
    ...
    assert_eq!(
        test_count, 9,
        "rust/core/src/persistence/lazy_init_tests.rs must contain exactly 9 #[test] attributes (the BUG-122 lazy-init invariants, got {test_count})"
    );
    for fn_name in [
    ...
    // @step And rust/core/src/persistence/mod.rs declares `#[cfg(test)] mod lazy_init_tests;`
    let mod_rs = read_to_string_at(&core_persistence_dir().join("mod.rs"));
    assert!(
        mod_rs.contains("mod lazy_init_tests"),
        "rust/core/src/persistence/mod.rs must declare `#[cfg(test)] mod lazy_init_tests;`"
    );
...
fn test_codelet_core_persistence_surface_still_exposes_expected_symbols() {
    // @step Given rust/rpc-embedded depends on codelet-core but is forbidden from depending on codelet-napi by rpc_006_source_shape.rs
    // @step When a downstream consumer in rust/rpc-embedded writes `use codelet_core::persistence::{create_session, load_session, append_message_with_metadata, fork_session, store_blob, get_blob, blob_exists, process_envelope_for_blob_storage, rehydrate_envelope_blobs, ensure_directories, history};`
    ...
    //         (asserted separately by rust/rpc-embedded/tests/rpc_006_source_shape.rs)
}
...
// ============================================================================
//
// The pre-card SHA-256 of rust/napi/index.d.ts is captured here. This
...
fn test_index_dts_remains_byte_identical_to_pre_card_snapshot() {
    // @step Given the SHA-256 hash H1 of rust/napi/index.d.ts captured before RPC-035 is committed as a fixture
    ...
    // @step And napi-rs regenerates rust/napi/index.d.ts from the #[napi] attributes
    let index_dts = workspace_root().join("rust/napi/index.d.ts");
    assert!(
        index_dts.is_file(),
        "rust/napi/index.d.ts must exist (regenerated by `cargo build -p codelet-napi --release` via napi-rs)"
    );

    // @step Then `sha256sum rust/napi/index.d.ts` returns the same hash H1
    let actual_sha256 = sha256_of(&index_dts);
    assert_eq!(
        actual_sha256, pre_card_sha256,
        "rust/napi/index.d.ts SHA-256 must equal the pre-card fixture {pre_card_sha256} after RPC-035 (got {actual_sha256})"
    );
    ...
    for required in [
        ...
        assert!(
            ...
            "rust/napi/index.d.ts must still export `{required}` after RPC-035"
        );
    }
...
fn test_napi_bridge_integration_tests_remain_in_codelet_napi_with_rewritten_setup() {
    // @step Given the NAPI-bridge integration tests session_persistence_test.rs and subordinate_session_persistence_test.rs stay in rust/napi/tests/
    let napi_tests = workspace_root().join("rust/napi/tests");
    assert!(
        napi_tests.join("session_persistence_test.rs").is_file(),
        "rust/napi/tests/session_persistence_test.rs must stay in codelet-napi (NAPI-bridge integration test)"
    );
    assert!(
        napi_tests.join("subordinate_session_persistence_test.rs").is_file(),
        "rust/napi/tests/subordinate_session_persistence_test.rs must stay in codelet-napi (NAPI-bridge integration test)"
    );

    // @step And rust/napi/src/test_support.rs::setup_test_env is rewritten to call codelet_common::set_data_directory + codelet_core::persistence::reset_stores_for_tests + crate::credentials::reset_credential_store + crate::graph::reset_graph_db inline
    let test_support =
        read_to_string_at(&workspace_root().join("rust/napi/src/test_support.rs"));
    assert!(
    ...
    assert!(
        ...
        "rust/napi/tests/session_persistence_test.rs must retain at least 23 #[test] attributes after RPC-035 (got {session_test_count})"
    );
    ...
    assert!(
        sub_test_count >= 4,
        "rust/napi/tests/subordinate_session_persistence_test.rs must retain at least 4 #[test] attributes after RPC-035 (got {sub_test_count})"
    );
...
fn test_relocated_setup_test_env_uses_codelet_core_primitives_only() {
    // @step Given the engineer opens rust/core/src/persistence/tests.rs
    let tests_path = core_persistence_dir().join("tests.rs");
    let tests = read_to_string_at(&tests_path);
...
fn test_both_codelet_napi_feature_builds_succeed_after_relocation() {
    // @step Given rust/napi/src/persistence/mod.rs no longer declares mod tests or mod lazy_init_tests and contains no pub fn declarations
    let mod_rs = read_to_string_at(&napi_persistence_dir().join("mod.rs"));
    assert!(
        !mod_rs.contains("mod tests"),
        "rust/napi/src/persistence/mod.rs must NOT declare `mod tests;` after RPC-035 (tests.rs is relocated to codelet-core)"
    );
    assert!(
        !mod_rs.contains("mod lazy_init_tests"),
        "rust/napi/src/persistence/mod.rs must NOT declare `mod lazy_init_tests;` after RPC-035"
    );
    assert!(
        !mod_rs.contains("pub fn "),
        "rust/napi/src/persistence/mod.rs must contain no `pub fn` declarations after RPC-035 (helpers absorbed into napi_bindings.rs)"
    );
    ...
    assert!(
        napi_bindings.contains("use codelet_core::persistence::"),
        "rust/napi/src/persistence/napi_bindings.rs must import its callees via `use codelet_core::persistence::{{...}};`"
    );
    ...
    // both features.
    let napi_cargo_toml = read_to_string_at(&workspace_root().join("rust/napi/Cargo.toml"));
    assert!(
        napi_cargo_toml.contains("napi"),
        "rust/napi/Cargo.toml must continue to declare the `napi` feature"
    );
    assert!(
        napi_cargo_toml.contains("noop"),
        "rust/napi/Cargo.toml must continue to declare the `noop` feature"
    );
    assert!(
        mod_rs.contains("#[cfg(not(feature = \"noop\"))]")
            || mod_rs.contains("#[cfg(not(feature=\"noop\"))]"),
        "rust/napi/src/persistence/mod.rs must preserve the `#[cfg(not(feature = \"noop\"))] mod napi_bindings;` gate so the noop feature build excludes the NAPI shims"
    );
    ...
    // @step And the napi build emits the unchanged rust/napi/index.d.ts with SHA-256 hash equal to the pre-card snapshot
    let index_dts = workspace_root().join("rust/napi/index.d.ts");
    if index_dts.is_file() {
        let actual_sha256 = sha256_of(&index_dts);
        assert_eq!(
            actual_sha256, PRE_CARD_INDEX_DTS_SHA256,
            "rust/napi/index.d.ts SHA-256 must equal the pre-card fixture {PRE_CARD_INDEX_DTS_SHA256} (got {actual_sha256}). Field reorder in Napi* wire structs is the most common cause."
        );
    }
}
