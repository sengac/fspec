...
//!
//! Lifted out of `rust/napi/src/work_units_watcher.rs` so that it can
//! be reached from the shared `FspecService` implementation in
//! `rust/rpc` without dragging the NAPI runtime into the dependency
//! arrow. The legacy NAPI shim remains in `rust/napi` and is now a
...
/// `rust/fspec-tui/src/store/board.rs` and `STATES` in
