...
// via setup_test_env().
//
// RPC-035: relocated from rust/napi/src/persistence/tests.rs into
...
/// Reads `rust/napi/src/persistence/napi_bindings.rs` from the
/// workspace directly so we don't need a NAPI dev-dep here.
#[test]
fn rpc049_napi_binding_is_thin_delegate() {
    // @step Given the rust/napi/src/persistence/napi_bindings.rs file after RPC-049
