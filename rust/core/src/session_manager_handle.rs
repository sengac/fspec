...
//! layer (rust/rpc) consumes via dependency injection. The concrete
//! 8,649 LOC `SessionManager` implementation lives in `rust/napi/src/
//! session_manager.rs` — rust/core defines only the trait surface so
//! that rust/rpc never imports rust/napi (rpc → napi forbidden;
//! rpc → core permitted; napi → core permitted).
//!
//! ## NAPI shared contract invariant
//!
//! The trait surface here, plus the five new types in rust/rpc-types
//! (SessionId, SessionInfo, SessionStatus, StreamChunk, LogRecord), are
//! the contract that all three frontends consume identically:
//!   * the JS frontend via rust/napi's #[napi] re-exports,
...
/// Trait implemented by the concrete `SessionManager` in rust/napi
...
pub trait SessionManagerHandle: Send + Sync + 'static {
    ...
    /// — compile without per-test wiring. The concrete rust/napi
    ...
    /// `ThinkingLevel::Off`; the rust/napi `SessionManager` overrides
    ...
    /// without per-test wiring. The concrete rust/napi
    ...
    /// without per-test wiring. The rust/napi `SessionManager`
    ...
    /// The rust/sessions `SessionManager` overrides this to delegate to
    ...
    /// without per-test wiring. The rust/sessions `SessionManager`
    ...
    /// rust/sessions override delegates to
    ...
    /// Default returns `Ok(())`; the rust/sessions override delegates to
    ...
    /// rust/sessions `SessionManager` overrides this to delegate to
    ...
    /// compile without per-test wiring. The rust/sessions `SessionManager`
    ...
    /// Default returns `Ok(())` (silent no-op). The rust/napi
    ...
    /// `Ok(())` (silent no-op). The rust/napi override forwards
    ...
    /// rust/napi override forwards to the existing
    ...
    // unchanged. The rust/napi `SessionManager` will override these in
    ...
    // impl lives in `rust/sessions/src/handle_impl.rs` and routes
