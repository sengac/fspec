...
- `rust/fspec-tui/src/views/agent/header_build.rs::thinking_label` →
...
- RPC path: `get_thinking_level` → `Action::ThinkingLevelLoaded` →
  ...
  `rust/fspec-tui/src/app/dispatch_session_chrome.rs`.
- The `/thinking` dialog already emits `Action::SetThinkingLevelDefault`, routed
  in `rust/fspec-tui/src/app/dispatch_model_thinking_dialogs.rs::handle_set_thinking_level_default`
...
`rust/sessions/src/handle_impl.rs` (~line 844):
...
3. **On `SessionCreated`**, load the persisted default and apply it to the new
   ...
   in `rust/fspec-tui/src/app/dispatch_session_chrome.rs` (or the
...
| Purpose | Path |
|---|---|
| Stub to extend (server) | `rust/sessions/src/handle_impl.rs` (`set_thinking_level_default`) |
| New persistence helpers | `rust/sessions/src/` (new module or existing config) |
| Apply on session create | `rust/fspec-tui/src/app/dispatch_session_chrome.rs` and/or session-creation path |
| Renderer (reference only) | `rust/fspec-tui/src/views/agent/header_build.rs` |
