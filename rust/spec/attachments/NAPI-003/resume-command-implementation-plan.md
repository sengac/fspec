...
**Key Types** (`rust/napi/src/persistence/types.rs`):
```rust
...
**TypeScript Interface** (`rust/napi/index.d.ts`):
```typescript
...
From `rust/napi/src/persistence/napi_bindings.rs`:
...
In `rust/napi/src/session.rs`, add a method to inject messages:
...
| `rust/napi/src/session.rs` | (Optional) Add loadSessionMessages method |
| `rust/napi/index.d.ts` | (Optional) Add loadSessionMessages type definition |
