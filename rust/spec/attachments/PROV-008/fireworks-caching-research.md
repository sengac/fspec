...
Investigation into how prompt caching works with Fireworks.ai (`https://app.fireworks.ai/models/fireworks/kimi-k2p5`) when accessed via rust's OpenAI-compatible provider, and what changes are needed to maximize cache efficiency.
...
- `rust/providers/src/openai.rs` — OpenAI provider, `from_api_key_with_options()`, `create_rig_agent()`
- `rust/patches/rig-core/src/providers/openai/client.rs` — rig OpenAI client builder
- `rust/patches/rig-core/src/providers/openai/completion/mod.rs` — `Usage`, `PromptTokensDetails`, `CompletionRequest`
- `rust/patches/rig-core/src/providers/openai/completion/streaming.rs` — Streaming usage extraction
- `rust/patches/rig-core/src/client/mod.rs` — `ClientBuilder::http_headers()`, header propagation
- `rust/providers/src/caching_client.rs` — Anthropic-specific cache_control transforms
- `rust/providers/src/cache_token_extractor.rs` — Anthropic SSE cache token extraction
- `rust/providers/src/lib.rs` — `LlmProvider` trait, `supports_caching()`
- `rust/providers/src/manager.rs` — Provider manager, `get_openai()`
