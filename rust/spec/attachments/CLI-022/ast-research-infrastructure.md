...
### Reusable Patterns
- Uses `dirs::home_dir()` for cross-platform path: `~/.rust/logs/`
...
- Reuse same `dirs::home_dir()` pattern for `~/.rust/debug/`
...
Create singleton manager mirroring rust's debug-capture.ts:
