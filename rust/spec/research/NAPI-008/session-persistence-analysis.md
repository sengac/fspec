...
**File:** `rust/napi/src/session.rs` (lines 269-316)
...
**File:** `rust/napi/src/session.rs` (lines 377-414)
...
**Provider types** (from `ProviderType` enum in `rust/providers/src/manager.rs`):
...
From `rig` crate source (rust/patches/rig-core/src/completion/message.rs):
