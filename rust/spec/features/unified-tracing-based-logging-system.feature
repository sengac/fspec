...
Feature: Unified Tracing-Based Logging System
  ...
  """
  Architecture notes:
  - Rust port of rust's Winston logging system using tracing + tracing-subscriber + tracing-appender
  - Creates dedicated logging module (src/logging/mod.rs) with init_logging() function
  - Logs to ~/.rust/logs/ with daily rotation (tracing_appender::rolling::daily)
  ...
  #   2. Logs must be written to files in ~/.rust/logs/ directory (not stdout) to avoid interfering with CLI output
  ...
  #
  # EXAMPLES:
  #   1. Initialize logging in main.rs with tracing_subscriber, setting up JSON file output to ~/.rust/logs/ with daily rotation
  ...
  #
  # QUESTIONS (ANSWERED):
  #   Q: Should we keep the current simple tracing_subscriber::fmt() setup in main.rs, or implement a full logger module (like rust's logger.ts) for easier initialization and configuration?
  #   A: Create a dedicated logging module (src/logging/mod.rs) with init_logging() function for clean initialization, similar to rust's logger.ts pattern. This encapsulates configuration and makes testing easier.
  #
  ...
  #   A: Keep stdout completely clean (file-only logging). CLI output should only show user-facing information. This matches rust's approach and avoids interference with ratatui/crossterm UI rendering.
  #
  #   Q: Should we add tracing spans for performance tracking (like rust's token tracking), or keep it simple with just event logging (info\!, debug\!, etc.)?
  ...
  Scenario: Initialize logging module with daily file rotation
    ...
    And I set up tracing_appender::rolling::daily to ~/.rust/logs/
    When the logging system initializes in main.rs
    Then logs should be written to ~/.rust/logs/codelet-YYYY-MM-DD.log
