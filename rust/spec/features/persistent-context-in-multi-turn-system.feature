...
Feature: Persistent Context in Multi-Turn System
  ...
  #   A: Option B: SessionWrapper - Create a new Session struct that owns ProviderManager, messages Vec, ContextManager, and TokenTracker. This matches rust's REPL scope pattern exactly and works perfectly with rig's stateless agent design requiring caller-managed history via with_history(&mut Vec<Message>)
  #
