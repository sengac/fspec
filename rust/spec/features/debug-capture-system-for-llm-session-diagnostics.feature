...
Feature: Debug capture system for LLM session diagnostics
  ...
  Reuse dirs::home_dir() pattern from logging/mod.rs for ~/.rust/debug/ path
  ...
  Create new src/debug_capture.rs module as singleton manager (mirrors rust's debug-capture.ts)
  """
  ...
  #   2. When enabled, a new session file is created in ~/.rust/debug/ with JSONL format
  ...
  #   14. This is an EXACT one-for-one port of rust's /debug feature - no modifications, no shortcuts, no stubs
  #
  # EXAMPLES:
  #   1. User types /debug when disabled -> debug capture starts, returns 'Debug capture enabled. Session file: ~/.rust/debug/session-2024-01-15T10-30-00.jsonl'
  #   2. User types /debug when enabled -> debug capture stops, generates summary, returns 'Debug capture disabled. Session saved to ~/.rust/debug/session-2024-01-15T10-30-00.jsonl'
  ...
  #
  # QUESTIONS (ANSWERED):
  #   Q: Should the debug directory use ~/.rust/debug/ (like codelet) or ~/.rust/debug/ to match the Rust project naming?
  #   A: Use ~/.rust/debug/ exactly as in the original codelet implementation - one for one copy
  #
  ...
  Scenario: Enable debug capture with /debug command
    ...
    And the debug directory "~/.rust/debug/" should exist
  ...
  Scenario: Create debug directory with secure permissions
    ...
    Then the directory "~/.rust/debug/" should be created
