...
Feature: Context Compaction with Anchoring System
  ...
  #   A: Use 50,000 tokens (50k) matching rust's AUTOCOMPACT_BUFFER constant. This reserves buffer space for compaction operations and summary generation.
  #
  #   Q: Should compaction emit user-visible notifications like '[Generating summary...]' and '[Context compacted]' during execution?
  #   A: Yes - emit '[Generating summary...]' when compaction starts and '[Context compacted]' with metrics when complete. Match rust's user-visible notifications (lines 844, 930-942 in runner.ts) for transparency.
  #
  #   Q: Should the LLM summary provider use the current provider from ProviderManager or always use a specific provider?
  #   A: Use current provider from ProviderManager, matching rust's design (llm-summary-provider.ts:88). This ensures summary generation uses the same model the user selected for the session.
  #
