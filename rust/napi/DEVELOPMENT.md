...
```
fspec monorepo
├── package.json                 # workspaces: ["rust/napi"]
...
- **During development**: npm resolves `@sengac/codelet-napi` to `rust/napi/` via workspace symlink
...
2. Updates `rust/napi/package.json` version
...
| `rust/napi/package.json` | npm package config |
| `rust/napi/src/lib.rs` | Main Rust NAPI bindings |
| `rust/Cargo.toml` | Rust workspace config |
...
2. Common issues:
   - Rust compilation errors in `rust/napi/src/`
   - Missing dependencies in `rust/Cargo.toml`
