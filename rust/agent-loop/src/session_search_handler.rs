...
fn get_user_message_previews(
    session: &codelet_core::persistence::SessionManifest,
...
/// this module — Rule [6].
fn extract_work_unit_id(session: &codelet_core::persistence::SessionManifest) -> Option<String> {
...
pub fn resolve_message_content(msg: &StoredMessage) -> String {
    ...
    if is_blob_reference(&msg.content) {
        if let Some(hash) = codelet_core::persistence::extract_blob_hash(&msg.content) {
