...
//! rust/{core,sessions,fspec,fspec-tui,rpc-types}/tests/no_napi_dependency.rs.
...
fn no_codelet_napi_in_transitive_dependency_graph() {
    ...
    // @step When I add `codelet-graph = { workspace = true }` to rust/agent-loop/Cargo.toml
...
fn no_codelet_napi_import_in_source() {
    ...
    // @step When I scan rust/agent-loop/src for codelet_napi imports
    // @step Then no .rs file under rust/agent-loop/src/ contains the substring `codelet_napi`
    //
    // RPC-072 Phase A foundation refit contract:
