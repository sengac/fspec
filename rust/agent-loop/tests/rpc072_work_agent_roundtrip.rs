...
//! `rust/fspec/tests/cross_frontend_parity.rs`. After RPC-072 lands,
...
async fn stub_provider_input_to_reply() {
    ...
    // (Asserted by `[features] test-support` in rust/agent-loop/Cargo.toml
...
fn no_codelet_napi_in_transitive_dependency_graph() {
    // @step Given the codelet-agent-loop crate exists under rust/agent-loop/
...
fn no_codelet_napi_import_in_source() {
    // @step And no .rs file under rust/agent-loop/src/ contains the substring "codelet_napi"
...
// independently in `rust/fspec/tests/no_napi_dependency.rs`. We re-assert
...
// ===========================================================================
//
// Source-shape regression: pin the call sites in `rust/fspec/src/common.rs`
...
fn build_service_installs_fspec_agent_hooks() {
    ...
    let src = std::fs::read_to_string(&common_rs).unwrap_or_else(|e| {
        panic!(
            "must be able to read rust/fspec/src/common.rs at {}: {e}",
            common_rs.display()
        )
    });
