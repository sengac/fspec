...
//! recovery helper wiring in `rust/cli/src/interactive/`. Pins the
...
fn workspace_root() -> PathBuf {
    // tests run with CWD = rust/cli — walk up two levels to repo root.
    let manifest = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above rust/cli")
        .to_path_buf()
}
...
fn interactive_mod_declares_classifier_and_six_recovery_modules() {
    // @step Given the source file rust/cli/src/interactive/mod.rs
    let body = read_source("rust/cli/src/interactive/mod.rs");
...
fn stall_classifier_uses_prefix_as_single_source_of_truth() {
    ...
    // @step And the source file rust/cli/src/interactive/error_classifiers.rs body contains the substring "super::recovery_stall::STALL_TIMEOUT_ERROR_PREFIX"
    let body = read_source("rust/cli/src/interactive/error_classifiers.rs");
    assert!(
...
fn stream_loop_wires_every_classifier_predicate() {
    // @step Given the source file rust/cli/src/interactive/stream_loop.rs
    let body = read_source("rust/cli/src/interactive/stream_loop.rs");
