...
impl RecordingOutput {
    ...
    /// The (position) of the first `CompactionFailed` and the first
    /// `CompactionComplete` — both `Some` iff both were emitted, in which
    /// case the ordering assertion compares them directly.
    fn compaction_failed_then_complete_positions(&self) -> (Option<usize>, Option<usize>) {
        let events = self.events.lock().unwrap();
        let failed = events
            .iter()
            .position(|e| matches!(e, StreamEvent::CompactionFailed { .. }));
        let complete = events
            .iter()
            .position(|e| matches!(e, StreamEvent::CompactionComplete { .. }));
        (failed, complete)
    }
}
...
fn seed_turns(session: &mut Session, n: usize) {
    for i in 0..n {
        session.messages.push(Message::User {
            content: OneOrMany::one(UserContent::text(format!("user turn {i} do some work"))),
        });
...
async fn sub_agent_timeout_pins_a_free_fallback_dag() {
    ...
    assert_eq!(
        *llm_calls.lock().unwrap(),
        1,
        "no further LLM call after the timeout"
    );

    // @step And the failure is surfaced with exactly one structured CompactionFailed lifecycle event carrying the timeout reason, emitted before the CompactionComplete
    let (failed_at, complete_at) = output.compaction_failed_then_complete_positions();
    assert!(
        failed_at.is_some(),
        "exactly one structured CompactionFailed event must surface the timeout \
         (got {})",
    ...
    assert_eq!(
        output.count(|e| matches!(e, StreamEvent::CompactionFailed { .. })),
        1,
        "exactly one CompactionFailed event"
    );
    assert!(
        complete_at.is_some() && failed_at.unwrap() < complete_at.unwrap(),
        "the CompactionFailed event must be emitted BEFORE the CompactionComplete"
    );
    let reason = output
        .events
        .lock()
        ...
        .iter()
        .find_map(|e| match e {
            StreamEvent::CompactionFailed { reason } => Some(reason.clone()),
            _ => None,
        })
        .expect("CompactionFailed must carry the reason");
    assert!(
        reason.contains("timed out"),
        "the CompactionFailed reason must name the timeout, got: {reason}"
    );
...
async fn unparseable_sub_agent_output_pins_a_fallback_dag() {
    ...
    set_compactor_sub_agent_handler(
        ...
        Some(Arc::new(|_parent, _| {
            Box::pin(async {
                Ok("I looked at the session but could not build any nodes at all".to_string())
            })
        })),
    );
    ...
    // @step And the failure is surfaced with exactly one structured CompactionFailed lifecycle event carrying the unparseable-output reason, emitted before the CompactionComplete
    let (failed_at, complete_at) = output.compaction_failed_then_complete_positions();
    assert_eq!(
        output.count(|e| matches!(e, StreamEvent::CompactionFailed { .. })),
        1,
        "exactly one CompactionFailed event must surface the unparseable output"
    );
    assert!(
        complete_at.is_some() && failed_at.unwrap() < complete_at.unwrap(),
        "the CompactionFailed event must be emitted BEFORE the CompactionComplete"
    ...
    let reason = output
        ...
        .expect("CompactionFailed must carry the reason");
    assert!(
        reason.contains("no parseable <dag-node> blocks"),
        "the CompactionFailed reason must name the unparseable output, got: {reason}"
    );
...
async fn partial_dag_node_blocks_are_recovered() {
    ...
    // The handler models the timeout: it resolves to `Err` and threads the
    // sub-agent's partial output (complete dag-node blocks emitted before
    // the timeout) into the failure reason — the contract the spawner's
    // timeout path must fulfil so `pin_fallback_dag` can recover them.
    let session_id = Uuid::new_v4();
...
...truncated mid-survey"#;
    set_compactor_sub_agent_handler(
        session_id,
        Some(Arc::new(|_parent, _| {
            Box::pin(async { Err(partial.to_string()) })
        })),
    );
    ...
    // @step And the failure is surfaced with exactly one structured CompactionFailed lifecycle event carrying the partial-recovery reason, emitted before the CompactionComplete
    let (failed_at, complete_at) = output.compaction_failed_then_complete_positions();
    assert_eq!(
        output.count(|e| matches!(e, StreamEvent::CompactionFailed { .. })),
        1,
        "exactly one CompactionFailed event must surface the partial recovery"
    );
    assert!(
        complete_at.is_some() && failed_at.unwrap() < complete_at.unwrap(),
        "the CompactionFailed event must be emitted BEFORE the CompactionComplete"
    ...
    let reason = output
        ...
        .expect("CompactionFailed must carry the reason");
    assert!(
        reason.contains("compactor sub-agent failed"),
        "the CompactionFailed reason must name the partial-recovery failure, got: {reason}"
    );
...
async fn missing_compactor_handler_still_reduces_the_session() {
    ...
    // CMPCT-044 rule [9] (review finding W2): the fallback outcome
    // surfaces the structured `CompactionFailed(reason)` lifecycle event
    // BEFORE the honest post-pin `CompactionComplete` — the session IS
    // reduced (the convergence guarantee), but the sub-agent path's
    // failure must be visible to frontends.
    let (failed_at, complete_at) = output.compaction_failed_then_complete_positions();
    assert!(
        failed_at.is_some(),
        "a structured CompactionFailed event must surface the missing-handler \
         failure"
    );
    assert!(
        complete_at.is_some() && failed_at.unwrap() < complete_at.unwrap(),
    ...
    let reason = output
        ...
        .expect("CompactionFailed must carry the reason");
    assert!(
        reason.contains("no compactor sub-agent handler registered"),
        "the CompactionFailed reason must name the missing handler, got: {reason}"
    );

    // @step And the CompactionComplete event is still emitted exactly once after the pin with the recalculated post-pin token basis
    let complete = output.count(|e| matches!(e, StreamEvent::CompactionComplete { .. }));
    assert_eq!(
        complete, 1,
        "the reduced-session guarantee must still be reflected in exactly one \
         CompactionComplete event with the recalculated basis (got {complete})"
    );
}
