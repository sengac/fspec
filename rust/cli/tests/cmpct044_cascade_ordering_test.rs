...
fn overflow_check_runs_before_network_retry_arm() {
    ...
    let recovery_at = src[overflow_at..]
        ...
        .unwrap_or_else(|| panic!("CMPCT-044: no begin_compaction_recovery after the overflow classifier"));
    assert!(
...
fn sub_agent_escalation_shares_the_compaction_retry_budget() {
    ...
    assert!(macro_def_at < macro_end_at, "sanity: macro def precedes the primary loop");

    let compactor_at = find(&src, "crate::compactor_sub_agent::run_compactor_sub_agent_round");
    assert!(
