...
pub type CompactorSubAgentHandler = Arc<
    dyn Fn(Uuid, Option<(String, usize)>)
...
static COMPACTOR_SUB_AGENT_HANDLERS:
    once_cell::sync::Lazy<RwLock<HashMap<Uuid, CompactorSubAgentHandler>>> =
...
pub async fn run_compactor_sub_agent_round<O: StreamOutput>(
    ...
    let run_result: Result<String, String> = match handler {
        ...
        None => {
            ...
            Err("compactor sub-agent handler not configured for this session".to_string())
        }
    };
...
fn emit_compaction_complete<O: StreamOutput>(
    output: &O,
    original_tokens: u64,
    session: &Session,
) {
    ...
    async fn successful_dag_is_pinned_to_the_session() {
        ...
        set_compactor_sub_agent_handler(
            ...
            Some(Arc::new(|_parent, existing| {
                ...
                Box::pin(async {
                    Ok(
                        r#"<dag-node depth="D1" turns="0-5" label="Work arc">
- completed the task
</dag-node>"#
                            .to_string(),
                    )
