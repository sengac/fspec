//! `restore-example` shell-facing CLI bridge (RPC-289).
//!
//! Feature: spec/features/restore-example-cli-subcommand.feature
//!
//! Two-front-doors pattern (RPC-003 §7/§11):
//!   - Shell argv         → clap → this module → fspec_core::commands::restore_example::run
//!   - LLM tool call JSON → fspec_core::dispatch::dispatch_command → fspec_core::commands::restore_example::run
//!
//! Exit-code contract:
//!   - 0 on success; the rendered text is written to stdout.
//!   - 1 on any [`codelet_fspec_core::FspecCoreError`]; the message is
//!     written to stderr prefixed with `✗ Failed to restore example:`
//!     (parity with the TS `output.error('✗ Failed to restore example:', error.message)`
//!     path at `src/commands/restore-example.ts:107`).
//!
//! ## CLI surface asymmetry vs. dispatcher
//!
//! The captured TS `--help` fixture advertises a `--ids` bulk flag for
//! symmetry with `restore-rule`, but `src/commands/restore-example.ts`'s
//! `registerRestoreExampleCommand` does NOT actually wire `--ids` on the
//! Commander.js subcommand (lines 88-110 — only the two positionals). The
//! flag is purely documentation. We mirror that verbatim: the clap layer
//! exposes only `<workUnitId>` and `<index>`, so `--ids` falls through to
//! clap's unknown-flag error (exit code 2). The bulk branch exists in
//! `restore_rule`'s core impl and is dispatcher-only.
//!
...
//! `NaN`, and the downstream `find(e => e.id === NaN)` always returns
//! `undefined`, so the canonical `Example with ID NaN not found` error is
//! surfaced (unless the examples array is empty, in which case the
//! `Work unit X has no examples` guard fires first). To preserve byte
//! parity we accept the index as a raw `String` at the clap layer and
//! marshal it through [`parse_ts_int_radix10`] — finite integers go through
//! as JSON numbers, everything else becomes the literal JSON string
//! `"NaN"` which the core deserialiser treats as TS NaN.

use std::env;
use std::path::PathBuf;

use anyhow::{Context, Result};
use codelet_fspec_core::commands::restore_example;
...
pub async fn run(args: CliArgs) -> Result<u8> {
    ...
    match restore_example::run(&args_json, &project_root).await {
        Ok(rendered) => {
            print!("{rendered}");
...
/// into a shared helper) because each CLI bridge owns its own marshalling
/// surface — sharing would couple unrelated commands and pull
/// `parse_ts_int_radix10` into a common module that none of the existing
/// remove-* bridges agreed to depend on (the playbook explicitly approves
/// the inline duplication for the restore-* pair).
