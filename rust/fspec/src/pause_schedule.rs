//! `pause-schedule` shell-facing CLI bridge (RPC-254).
//!
...
//! parses argv (the `Mode::PauseSchedule` clap variant in [`crate::main`])
//! and delegates to the single source-of-truth in
//! [`codelet_fspec_core::commands::pause_schedule::run`] — the SAME function
//! the LLM-facing agent_loop dispatcher invokes.
//!
//! Two-front-doors pattern:
//!   - Shell argv         → clap → this module → fspec_core::commands::pause_schedule::run
//!   - LLM tool call JSON → fspec_core::dispatch::dispatch_command → fspec_core::commands::pause_schedule::run
//!
//! Both call sites pass a JSON-encoded args shape and a `project_root: &Path`.
//! The CLI surface resolves project_root from CWD (parity with the TypeScript
//! `process.cwd()` default at `src/commands/schedule/pause-schedule.ts:24`).
//!
//! Exit-code contract:
...
//!     `output.log(...)` path at `pause-schedule.ts:84`).
//!   - 1 on any [`codelet_fspec_core::FspecCoreError`]; the message is written
//!     to stderr prefixed with `✗ Failed to pause schedule:` (parity with the
//!     TS `output.error('✗ Failed to pause schedule:', err.message)` path at
//!     `pause-schedule.ts:86`). The dispatcher-only
//!     `"Invalid args for fspec command pause-schedule: "` envelope is stripped
...
use codelet_fspec_core::commands::pause_schedule;
use serde_json::{json, Value};

use crate::common::render_core_error;

/// Strongly-typed args mirrored from the TS Commander.js positional set for
/// `pause-schedule` (`src/commands/schedule/pause-schedule.ts:78-83`). NO
...
/// Entry point invoked from `main.rs` for the `pause-schedule` clap
...
pub async fn run(args: CliArgs) -> Result<u8> {
    ...
    // Marshal the JSON args shape fspec_core::commands::pause_schedule::run
    ...
    match pause_schedule::run(&args_json, &project_root).await {
        Ok(data) => {
            ...
            let parsed: Value =
                serde_json::from_str(&data).context("parse fspec_core pause-schedule result")?;
        ...
        Err(err) => {
            eprintln!("✗ Failed to pause schedule: {}", render_core_error(&err));
            Ok(1)
        }
    }
}
