//! `add-domain-event-to-foundation` shell-facing CLI bridge (RPC-180).
//!
...
//! parses argv (the `Mode::AddDomainEventToFoundation` clap variant in
//! [`crate::main`]) and delegates to the SAME source-of-truth the
//! LLM-facing dispatcher uses — `add-domain-event-to-foundation` routed
...
/// registration at `src/commands/add-domain-event-to-foundation.ts:138-162`.
#[derive(Debug)]
pub struct CliArgs {
...
/// `add-domain-event-to-foundation` clap subcommand.
pub async fn run(args: CliArgs) -> Result<u8> {
    ...
    // core. The optional flag is only inserted when present.
    let mut body = Map::new();
    body.insert("contextName".to_string(), Value::String(args.context_name));
    body.insert("eventName".to_string(), Value::String(args.event_name));
