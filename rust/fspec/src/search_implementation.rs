//! `search-implementation` shell-facing CLI bridge (RPC-296).
//!
//! Feature: spec/features/search-implementation-cli-subcommand.feature
//!
//! Thin façade: marshal CLI args into JSON, delegate to
//! `fspec_core::commands::search_implementation::run`, then:
//!   * `--json` → print the rendered JSON envelope verbatim
//!   * default → print the `message` summary line emitted by the core
//!
//! This bridge contains NO inline coverage reading, file scanning, or summary
...
use codelet_fspec_core::commands::search_implementation;
...
/// Entry point for the `search-implementation` clap subcommand.
pub async fn run(args: CliArgs) -> Result<u8> {
    let project_root: PathBuf = env::current_dir().context("resolve current working directory")?;

    let mut payload: Map<String, Value> = Map::new();
    payload.insert("function".to_string(), Value::String(args.function.clone()));
    payload.insert(
        "showWorkUnits".to_string(),
        Value::Bool(args.show_work_units),
    );
    payload.insert("json".to_string(), Value::Bool(args.json));
    let args_json = json!(payload).to_string();

    match search_implementation::run(&args_json, &project_root).await {
        Ok(rendered) => {
            if args.json {
                println!("{rendered}");
            } else {
        ...
        Err(err) => {
            ...
            // path. Strip the `InvalidArgs` wrapper so the printed message
            // matches the bare TS Error.message.
            match &err {
