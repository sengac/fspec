//! `add-command-to-foundation` shell-facing CLI bridge (RPC-175).
//!
...
//! parses argv (the `Mode::AddCommandToFoundation` clap variant in
//! [`crate::main`]) and delegates to the SAME source-of-truth the
//! LLM-facing dispatcher uses — `add-command-to-foundation` routed through
...
/// registration at `src/commands/add-command-to-foundation.ts:138-156`.
#[derive(Debug)]
pub struct CliArgs {
...
/// Entry point invoked from `main.rs` for the `add-command-to-foundation`
/// clap subcommand.
