//! `delete-step` shell-facing CLI bridge (RPC-221).
//!
...
//! parses argv (the `Mode::DeleteStep` clap variant in [`crate::main`]) and
//! delegates to the single source-of-truth in
//! [`codelet_fspec_core::commands::delete_step::run`] — the SAME function
//! the LLM-facing agent_loop dispatcher invokes.
//!
//! Two-front-doors pattern:
//!   - Shell argv         → clap → this module → fspec_core::commands::delete_step::run
//!   - LLM tool call JSON → fspec_core::dispatch::dispatch_command → fspec_core::commands::delete_step::run
//!
//! Both call sites pass a JSON-encoded args shape and a `project_root: &Path`.
//! The CLI surface resolves project_root from CWD (parity with the TS
//! `process.cwd()` default at `src/commands/delete-step.ts:25`).
//!
//! Exit-code contract:
//!   - 0 on success; `✓ <message>` is written to stdout.
...
/// Entry point invoked from `main.rs` for the `delete-step` clap subcommand.
pub async fn run(args: CliArgs) -> Result<u8> {
    ...
    match delete_step::run(&args_json, &project_root).await {
        Ok(data_json) => {
            let parsed: Value =
