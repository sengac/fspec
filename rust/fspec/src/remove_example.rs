//! `remove-example` shell-facing CLI bridge (RPC-273).
//!
//! Feature: spec/features/remove-example-cli-subcommand.feature
//!
//! Two-front-doors pattern (RPC-003 §7/§11):
//!   - Shell argv         → clap → this module → fspec_core::commands::remove_example::run
//!   - LLM tool call JSON → fspec_core::dispatch::dispatch_command → fspec_core::commands::remove_example::run
//!
//! Exit-code contract:
//!   - 0 on success; the rendered text is written to stdout.
//!   - 1 on any [`codelet_fspec_core::FspecCoreError`]; the message is
//!     written to stderr prefixed with `✗ Failed to remove example:` (parity
//!     with the TS `output.error('✗ Failed to remove example:', error.message)`
//!     path at `src/commands/remove-example.ts:104`).
//!
...
use codelet_fspec_core::commands::remove_example;
...
pub async fn run(args: CliArgs) -> Result<u8> {
    ...
    match remove_example::run(&args_json, &project_root).await {
        Ok(rendered) => {
            print!("{rendered}");
        ...
        Err(err) => {
            // Mirror TS `output.error('✗ Failed to remove example:', error.message)`
            // at src/commands/remove-example.ts:104.
            eprintln!("✗ Failed to remove example: {}", render_core_error(&err));
            Ok(1)
        }
    }
}
...
/// example ids using strict equality, so a `"NaN"` value never matches an
/// integer id and the canonical `Example with ID NaN not found` error is
/// surfaced — byte-parity with the TS CLI's behaviour on non-numeric input.
