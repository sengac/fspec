...
fn scenario_clap_exposes_remove_capability_with_positional_arg_in_help() {
    ...
    // @step When I run `./rust/target/release/fspec remove-capability --help`
...
fn scenario_cli_removes_capability_and_prints_success_line() {
    ...
    // @step When I run `./rust/target/release/fspec remove-capability "Search"`
...
fn scenario_cli_prints_no_capabilities_detail_and_exits_1() {
    ...
    // @step When I run `./rust/target/release/fspec remove-capability "X"`
...
fn scenario_cli_lists_available_capabilities_and_exits_1() {
    ...
    // @step When I run `./rust/target/release/fspec remove-capability "Login"`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And running `./rust/target/release/fspec remove-capability "B"` afterwards exits 0
    ...
    // @step And the CLI bridge module rust/fspec/src/remove_capability.rs contains NO inline draft probing, matching, or JSON-mutation logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/remove_capability.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/remove_capability.rs must exist; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_remove_capability_help_matches_fixture() {
    ...
    // @step When I run `./rust/target/release/fspec remove-capability --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("remove-capability")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/remove-capability.txt
