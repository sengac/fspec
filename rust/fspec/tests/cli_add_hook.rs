...
//!   - `rust/fspec/src/main.rs` does not yet register an `add-hook`
//!     clap subcommand (clap exits 2 for unrecognized subcommand), and
//!   - `rust/fspec-core/src/commands/add_hook.rs` is still a
//!     NotYetPorted stub.
...
fn scenario_add_hook_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step Then stdout is byte-for-byte identical to the captured TS help fixture at rust/fspec/tests/fixtures/help/add-hook.txt
    assert_eq!(
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "hooks": {
    ...
    // @step Then the CLI bridge module rust/fspec/src/add_hook.rs contains NO inline parsing or write logic — its only computation is JSON arg marshalling
