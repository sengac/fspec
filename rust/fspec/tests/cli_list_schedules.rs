...
//! `rust/fspec-core/src/commands/list_schedules.rs` is ported, the
//! clap subcommand is wired into `rust/fspec/src/main.rs`, and the
//! bridge module `rust/fspec/src/list_schedules.rs` translates the
...
fn scenario_cli_against_empty_directory_prints_sentinel_and_no_files() {
    ...
    // @step When I run `./rust/target/release/fspec list-schedules` from that directory
...
fn scenario_cli_text_output_renders_populated_case() {
    ...
    // @step When I run `./rust/target/release/fspec list-schedules`
    let (code, stdout, stderr) = run_list_schedules(ws.path(), &[]);
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/list_schedules.rs contains NO inline schedule-aggregation, filter, or rendering logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/list_schedules.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/list_schedules.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
///    > rust/fspec/tests/fixtures/help/list-schedules.txt`
...
fn scenario_list_schedules_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec list-schedules --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("list-schedules")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the TS reference output at rust/fspec/tests/fixtures/help/list-schedules.txt
    assert_eq!(
