...
//! `rust/fspec/tests/fixtures/cross_frontend_run.jsonl`.
//!
...
//!     inside `rust/fspec/src/common.rs::build_service`
//!   - `rust/fspec/tests/README.md`
//!
...
//!     end-to-end (currently a no-op in `rust/fspec/src/session_hooks.rs`).
...
fn scenario_workspace_registers_the_stub_provider() {
    ...
    // rust/providers/src/stub_provider.rs. The runtime path is
...
async fn scenario_scripted_run_matches_golden() {
    ...
    let body = async {
        ...
        // @step And the golden file at rust/fspec/tests/fixtures/cross_frontend_run.jsonl exists
...
fn scenario_regenerate_env_var_recorded_in_test_source() {
    ...
    //
    // @step Given the file rust/fspec/tests/fixtures/cross_frontend_run.jsonl does not exist
...
fn scenario_regression_catch_documented() {
    ...
    // @step When a developer edits rust/sessions/src/background_session.rs to substitute StreamChunk::text("oops") for the forwarded provider chunk
...
// ---------------------------------------------------------------------
// Scenario: rust/fspec/tests/README.md documents regeneration and
// the deferred TS-fixture path
// ---------------------------------------------------------------------

#[test]
fn scenario_tests_readme_documents_regeneration() {
    // @step Given the rust/fspec/tests/README.md file exists after this card
    let readme = fspec_crate_root().join("tests").join("README.md");
    assert!(
        readme.exists(),
        "rust/fspec/tests/README.md must exist (RPC-066 deliverable)"
    );
