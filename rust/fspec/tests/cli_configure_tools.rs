...
fn scenario_configure_tools_help_matches_ts_fixture() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/configure-tools.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function() {
    ...
    // @step And the CLI bridge module rust/fspec/src/configure_tools.rs contains NO inline config-merge or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/configure_tools.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/configure_tools.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
