...
fn scenario_cli_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step And stdout matches the captured fixture at rust/fspec/tests/fixtures/help/add-scenario.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/add_scenario.rs contains NO inline gherkin parsing or insertion logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_scenario.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/add_scenario.rs must exist as the CLI bridge module"
    );
    let bridge_src =
