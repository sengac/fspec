...
//! `rust/fspec-core/src/commands/add_schedule.rs` is still a
//! `NotYetPorted` stub, the clap `Mode::AddSchedule` variant is NOT yet
//! wired into `rust/fspec/src/main.rs`, the `intercept_ts_help`
//! arm does NOT exist, and the bridge module
//! `rust/fspec/src/add_schedule.rs` has NOT been created. Every
...
fn scenario_add_schedule_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step Then stdout is byte-for-byte identical to the TS reference output at rust/fspec/tests/fixtures/help/add-schedule.txt
    assert_eq!(
...
fn scenario_cli_registers_agent_schedule_and_delegates_to_fspec_core() {
    ...
    // @step Then the CLI bridge module rust/fspec/src/add_schedule.rs contains NO validation, schedule-construction, or file-writing logic beyond JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_schedule.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/add_schedule.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
