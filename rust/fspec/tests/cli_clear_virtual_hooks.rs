...
//! `rust/fspec/src/main.rs` does not yet register a `clear-virtual-hooks`
...
fn scenario_cli_prints_success_message_when_clearing_hooks() {
    ...
    // @step When I run `./rust/target/release/fspec clear-virtual-hooks AUTH-001` in that project root
...
fn scenario_cli_exits_1_when_work_unit_does_not_exist() {
    ...
    // @step When I run `./rust/target/release/fspec clear-virtual-hooks AUTH-999` in that project root
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/clear_virtual_hooks.rs contains NO inline rendering, file IO, or work-unit-lookup logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/clear_virtual_hooks.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/clear_virtual_hooks.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_clear_virtual_hooks_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec clear-virtual-hooks --help` piped to non-TTY
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/clear-virtual-hooks.txt
