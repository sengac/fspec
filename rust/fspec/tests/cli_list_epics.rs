...
//!   - `rust/fspec/src/main.rs` does not yet register a `list-epics`
//!     clap subcommand (clap returns exit code 2 for "unrecognized
//!     subcommand").
//!   - `rust/fspec-core/src/commands/list_epics.rs` is still a
//!     NotYetPorted stub.
...
fn scenario_clap_exposes_list_epics_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec list-epics --help` from a shell
...
fn scenario_cli_against_empty_directory_prints_sentinel_and_no_files() {
    ...
    // @step When I run `./rust/target/release/fspec list-epics` from that directory
...
fn scenario_cli_text_output_renders_epic_progress() {
    ...
    // @step When I run `./rust/target/release/fspec list-epics`
    let (code, stdout, stderr) = run_list_epics(ws.path(), &[]);
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "states": {
    ...
    // @step Then the CLI bridge module rust/fspec/src/list_epics.rs contains NO inline epic-aggregation, filter, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/list_epics.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/list_epics.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
// ─────────────────────────────────────────────────────────────────────────
// Scenario: list-epics --help is byte-for-byte identical to TS reference (RPC-243)
// ─────────────────────────────────────────────────────────────────────────

const TS_HELP_FIXTURE_LE: &str = include_str!("fixtures/help/list-epics.txt");

#[test]
fn scenario_list_epics_help_matches_ts_formatcommandhelp_reference() {
    // @step Given the fspec Rust binary at rust/target/release/fspec has been compiled

    // @step When I run `./rust/target/release/fspec list-epics --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("list-epics")
        .arg("--help")
        ...
        .env("NO_COLOR", "1")
        .output()
        .expect("spawn list-epics --help");
    let code = output.status.code().unwrap_or(-1);
    let stdout = String::from_utf8_lossy(&output.stdout).into_owned();
    let stderr = String::from_utf8_lossy(&output.stderr).into_owned();

    // @step Then the command exits 0
    assert_eq!(code, 0, "list-epics --help must exit 0; stderr={stderr}");

    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-epics.txt
    assert_eq!(stdout, TS_HELP_FIXTURE_LE);

    // @step And stdout starts with a blank line followed by 'LIST-EPICS'
