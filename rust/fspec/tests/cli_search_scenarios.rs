...
fn clap_exposes_search_scenarios_as_a_subcommand_and_prints_flag_help() {
    ...
    // @step When I run `./rust/target/release/fspec search-scenarios --help` from a shell
...
fn cli_without_query_flag_exits_non_zero() {
    ...
    // @step When I run `./rust/target/release/fspec search-scenarios` from that directory
...
fn cli_default_output_prints_the_green_summary_line() {
    ...
    // @step When I run `./rust/target/release/fspec search-scenarios --query Login` from that workspace
...
fn cli_json_prints_the_2_space_json_envelope_to_stdout() {
    ...
    // @step When I run `./rust/target/release/fspec search-scenarios --query Login --json` from that workspace
...
fn cli_regex_sets_search_mode_to_regex() {
    ...
    // @step When I run `./rust/target/release/fspec search-scenarios --query valid.* --regex --json` from that workspace
...
fn cli_exits_non_zero_on_an_invalid_regex_pattern() {
    ...
    // @step When I run `./rust/target/release/fspec search-scenarios --query [ --regex` from that workspace
...
fn cli_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec search-scenarios --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("search-scenarios")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/search-scenarios.txt
...
fn cli_delegates_to_the_same_fspec_core_function_used_by_the_dispatcher() {
    ...
    // @step And I run `./rust/target/release/fspec search-scenarios --query Login --json` against the same workspace
    ...
    // @step And the CLI bridge module rust/fspec/src/search_scenarios.rs contains NO inline filtering or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/search_scenarios.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/search_scenarios.rs must exist as the CLI bridge module"
    );
