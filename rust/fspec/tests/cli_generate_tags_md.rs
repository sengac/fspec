...
fn scenario_generate_tags_md_help_matches_ts_fixture() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/generate-tags-md.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function() {
    ...
    // @step And the CLI bridge module rust/fspec/src/generate_tags_md.rs contains NO inline markdown rendering, schema validation, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/generate_tags_md.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/generate_tags_md.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
