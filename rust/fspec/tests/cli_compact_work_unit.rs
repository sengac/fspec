...
fn scenario_clap_exposes_compact_work_unit_with_arg_and_force_flag() {
    ...
    // @step When I run `./rust/target/release/fspec compact-work-unit --help`
...
fn scenario_cli_compacts_done_work_unit_and_prints_summary() {
    ...
    // @step When I run `./rust/target/release/fspec compact-work-unit AUTH-001`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/compact_work_unit.rs contains NO inline file-read, mutation, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/compact_work_unit.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/compact_work_unit.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_compact_work_unit_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec compact-work-unit --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("compact-work-unit")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/compact-work-unit.txt
