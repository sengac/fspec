...
//!   - `rust/fspec/src/main.rs` does not yet register a `remove-attachment`
...
fn scenario_remove_attachment_help_matches_ts_fixture_byte_for_byte() {
    ...
    // @step And stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/remove-attachment.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/remove_attachment.rs contains NO splice, file unlink, or atomic write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/remove_attachment.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/remove_attachment.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
