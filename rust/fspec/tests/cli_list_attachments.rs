...
//!   - `rust/fspec/src/main.rs` does not yet register a `list-attachments`
//!     clap subcommand (clap returns exit code 2 for "unrecognized
//!     subcommand").
//!   - `rust/fspec-core/src/commands/list_attachments.rs` is still a
//!     NotYetPorted stub.
...
fn scenario_clap_exposes_list_attachments_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec list-attachments --help` piped to non-TTY (no color codes)
...
fn scenario_cli_missing_positional_exits_2() {
    ...
    // @step When I run `./rust/target/release/fspec list-attachments` (no positional) from that directory
...
fn scenario_cli_prints_empty_sentinel_and_exits_0() {
    ...
    // @step When I run `./rust/target/release/fspec list-attachments AUTH-001`
...
fn scenario_cli_unknown_work_unit_exits_1_with_stderr() {
    ...
    // @step When I run `./rust/target/release/fspec list-attachments NONEXISTENT-001`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step Then the CLI bridge module rust/fspec/src/list_attachments.rs contains NO inline rendering, file-stat, or work-unit-lookup logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/list_attachments.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/list_attachments.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
