...
//! rust/fspec/tests/cli_add_command_to_foundation.rs (RPC-175).
...
fn scenario_add_domain_event_to_foundation_help_matches_ts_fixture() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/add-domain-event-to-foundation.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function() {
    ...
    // @step And the CLI bridge module rust/fspec/src/add_domain_event_to_foundation.rs contains NO inline item construction, context lookup, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_domain_event_to_foundation.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/add_domain_event_to_foundation.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
