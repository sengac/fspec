...
fn scenario_clap_exposes_create_prefix_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec create-prefix --help` from a shell
...
fn scenario_cli_creates_prefixes_file_and_prints_success() {
    ...
    // @step When I run `./rust/target/release/fspec create-prefix AUTH "Auth features"` from that directory
...
fn scenario_cli_rejects_lowercase_prefix() {
    ...
    // @step When I run `./rust/target/release/fspec create-prefix auth "bad case"` from that directory
...
fn scenario_cli_rejects_duplicate_prefix() {
    ...
    // @step When I run `./rust/target/release/fspec create-prefix AUTH "Different desc"` from that directory
...
fn scenario_cli_missing_positional_fails() {
    ...
    // @step When I run `./rust/target/release/fspec create-prefix` from that directory
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step Then the CLI bridge module rust/fspec/src/create_prefix.rs contains NO inline validation, file IO, or rendering logic — its only computation is JSON arg marshalling and stdout/stderr printing
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/create_prefix.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/create_prefix.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_create_prefix_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec create-prefix --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("create-prefix")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/create-prefix.txt
