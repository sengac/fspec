...
fn scenario_remove_command_from_foundation_help_matches_ts_fixture() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/remove-command-from-foundation.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function() {
    ...
    // @step And the CLI bridge module rust/fspec/src/remove_command_from_foundation.rs contains NO inline context lookup, command match, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("src/remove_command_from_foundation.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/remove_command_from_foundation.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
