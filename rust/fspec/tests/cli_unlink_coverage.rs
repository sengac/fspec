...
fn clap_exposes_unlink_coverage_as_a_subcommand_and_prints_flag_help() {
    ...
    // @step When I run `./rust/target/release/fspec unlink-coverage --help` from a shell
...
fn cli_all_empties_the_scenario_mappings_and_prints_the_success_message() {
    ...
    // @step When I run `./rust/target/release/fspec unlink-coverage user-login --scenario Login --all` from that workspace
...
fn cli_without_all_or_test_file_exits_1() {
    ...
    // @step When I run `./rust/target/release/fspec unlink-coverage user-login --scenario Login` from that workspace
...
fn cli_exits_1_when_the_coverage_file_is_missing() {
    ...
    // @step When I run `./rust/target/release/fspec unlink-coverage user-login --scenario Login --all` from that directory
...
fn cli_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec unlink-coverage --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("unlink-coverage")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/unlink-coverage.txt
...
fn cli_delegates_to_the_same_fspec_core_function_used_by_the_dispatcher() {
    ...
    // @step And I run `./rust/target/release/fspec unlink-coverage user-login --scenario Login --all` against an identical workspace
    ...
    // @step And the CLI bridge module rust/fspec/src/unlink_coverage.rs contains NO inline mutation or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/unlink_coverage.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/unlink_coverage.rs must exist as the CLI bridge module"
    );
