...
//! `mod common;`. Mirrors the rust/rpc-server/tests/common helpers
//! pattern (see rust/rpc-server/tests/common/mod.rs) so the
...
/// Project root containing `rust/`. Walks up from the test binary's
/// `CARGO_MANIFEST_DIR` (which is `<root>/rust/fspec`) twice.
...
/// `rust/` directory (the cargo workspace root).
pub fn codelet_root() -> PathBuf {
    project_root().join("rust")
}

/// `rust/fspec/` directory.
