...
fn scenario_clap_exposes_remove_architecture_note_with_positional_args_in_help() {
    ...
    // @step When I run `./rust/target/release/fspec remove-architecture-note --help`
...
fn scenario_cli_soft_deletes_architecture_note_and_prints_success_line() {
    ...
    // @step When I run `./rust/target/release/fspec remove-architecture-note AUTH-001 0`
...
fn scenario_cli_rejects_unknown_work_unit_with_exit_1() {
    ...
    // @step When I run `./rust/target/release/fspec remove-architecture-note MISSING-001 0`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/remove_architecture_note.rs contains NO inline soft-delete, ID-lookup, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/remove_architecture_note.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/remove_architecture_note.rs must exist; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_remove_architecture_note_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec remove-architecture-note --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("remove-architecture-note")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/remove-architecture-note.txt
