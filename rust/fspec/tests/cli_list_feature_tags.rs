...
//! `rust/fspec/src/main.rs` does not yet register a `list-feature-tags`
//! clap subcommand (clap returns exit code 2 for "unrecognized
//! subcommand"). The bridge module
//! `rust/fspec/src/list_feature_tags.rs` exists and is ready to be
...
fn scenario_clap_exposes_list_feature_tags_with_show_categories() {
    ...
    // @step When I run `./rust/target/release/fspec list-feature-tags --help` from a shell
...
fn scenario_cli_happy_path_prints_flat_declaration_tag_list() {
    ...
    // @step When I run `./rust/target/release/fspec list-feature-tags spec/features/user-auth.feature` from the project root
    let output = Command::new(fspec_bin())
        .arg("list-feature-tags")
...
fn scenario_cli_show_categories_flag_emits_categorized_pairs() {
  ...
  "categories": [
    ...
    // @step When I run `./rust/target/release/fspec list-feature-tags spec/features/user-auth.feature --show-categories`
    let output = Command::new(fspec_bin())
        .arg("list-feature-tags")
        .arg("spec/features/user-auth.feature")
        .arg("--show-categories")
...
fn scenario_cli_bridge_module_embeds_no_duplicated_business_logic() {
    // @step Given the CLI bridge module rust/fspec/src/list_feature_tags.rs is the only shell-facing entry point for list-feature-tags
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/list_feature_tags.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/list_feature_tags.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_cli_missing_file_routes_error_to_stderr_with_exit_1() {
    ...
    // @step When I run `./rust/target/release/list_feature_tags_bin spec/features/missing.feature` from that directory
    let output = Command::new(fspec_bin())
        .arg("list-feature-tags")
...
fn scenario_cli_missing_file_with_show_categories_also_routes_error_to_stderr() {
    ...
    // @step When I run `./rust/target/release/list_feature_tags_bin spec/features/missing.feature --show-categories` from that directory
    let output = Command::new(fspec_bin())
        .arg("list-feature-tags")
        .arg("spec/features/missing.feature")
        .arg("--show-categories")
...
fn scenario_cli_invalid_gherkin_routes_error_to_stderr_with_exit_1() {
    ...
    // @step When I run `./rust/target/release/list_feature_tags_bin spec/features/broken.feature` from that directory
    let output = Command::new(fspec_bin())
        .arg("list-feature-tags")
...
///    > rust/fspec/tests/fixtures/help/list-feature-tags.txt`
...
fn scenario_list_feature_tags_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec list-feature-tags --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("list-feature-tags")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the TS reference fixture at rust/fspec/tests/fixtures/help/list-feature-tags.txt
    assert_eq!(
