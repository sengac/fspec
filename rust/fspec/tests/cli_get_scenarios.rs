...
fn scenario_clap_exposes_get_scenarios_with_help() {
    ...
    // @step When I run `./rust/target/release/fspec get-scenarios --help` from a shell
...
fn scenario_cli_format_json_prints_a_json_array_of_scenario_objects() {
    ...
    // @step When I run `./rust/target/release/fspec get-scenarios --tag @auth --format json` from that workspace
...
fn scenario_cli_default_text_output_prints_count_message_and_groups_by_feature() {
    ...
    // @step When I run `./rust/target/release/fspec get-scenarios --tag @auth` from that workspace
...
fn scenario_cli_against_workspace_with_no_spec_features_exits_1() {
    ...
    // @step When I run `./rust/target/release/fspec get-scenarios` from that directory
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step Then the CLI bridge module rust/fspec/src/get_scenarios.rs contains NO inline parsing, filtering, or rendering logic — its only computation is JSON arg marshalling and stdout printing
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/get_scenarios.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/get_scenarios.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_get_scenarios_help_matches_ts_reference() {
    ...
    // @step When I run `./rust/target/release/fspec get-scenarios --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("get-scenarios")
        .arg("--help")
    ...
    // @step Then stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/get-scenarios.txt
