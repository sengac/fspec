...
//!
//! Red phase: these tests MUST fail today because `rust/fspec/src/main.rs`
...
/// `rust/fspec-core/tests/list_work_units.rs`. AUTH-001 (backlog, epic ux),
...
fn scenario_standalone_fspec_binary_exposes_list_work_units_as_a_clap_subcommand() {
    ...
    // @step When I run `./rust/target/release/fspec list-work-units --help` from a shell
...
fn scenario_cli_against_empty_dir_creates_defaults_and_prints_sentinel() {
    ...
    // @step When I run `./rust/target/release/fspec list-work-units` from that directory
...
fn scenario_cli_emits_two_space_indented_json_for_format_json() {
  ...
  "workUnits": {
    ...
    // @step When I run `./rust/target/release/fspec list-work-units --format=json`
...
fn scenario_cli_status_filter_matches_dispatcher_behavior() {
    ...
    // @step When I run `./rust/target/release/fspec list-work-units --status=backlog --format=json`
...
fn scenario_cli_malformed_work_units_exits_1_with_stderr() {
    ...
    // @step When I run `./rust/target/release/fspec list-work-units`
    let (code, stdout, stderr) = run_list_work_units(ws.path(), &[]);
...
fn scenario_cli_tolerates_unknown_work_unit_type_values() {
  ...
  "workUnits": {
    ...
    // @step When I run `./rust/target/release/fspec list-work-units --type=story --format=json`
...
//
// Defect: rust/fspec/src/main.rs declares `--workspace` with
...
fn scenario_list_work_units_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec list-work-units --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("list-work-units")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-work-units.txt
