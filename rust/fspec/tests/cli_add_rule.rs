...
fn scenario_add_rule_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/add-rule.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/add_rule.rs contains NO inline rule construction, status guard, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_rule.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/add_rule.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
