...
//! `rust/fspec/src/main.rs` does not yet register a `remove-virtual-hook`
...
fn scenario_clap_exposes_remove_virtual_hook_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec remove-virtual-hook --help` from a shell
...
fn scenario_cli_removes_existing_hook_and_prints_canonical_success_lines() {
    ...
    // @step When I run `./rust/target/release/fspec remove-virtual-hook AUTH-001 eslint`
...
fn scenario_cli_fails_with_exit_1_when_work_unit_does_not_exist() {
    ...
    // @step When I run `./rust/target/release/fspec remove-virtual-hook AUTH-999 eslint`
...
fn scenario_cli_fails_with_exit_1_when_named_hook_not_found() {
    ...
    // @step When I run `./rust/target/release/fspec remove-virtual-hook AUTH-001 missing`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/remove_virtual_hook.rs contains NO inline script-removal, retain, or work-unit-lookup logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/remove_virtual_hook.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/remove_virtual_hook.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_remove_virtual_hook_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec remove-virtual-hook --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("remove-virtual-hook")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/remove-virtual-hook.txt
