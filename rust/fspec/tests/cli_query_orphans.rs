...
fn scenario_clap_exposes_query_orphans_with_help() {
    ...
    // @step When I run `./rust/target/release/fspec query-orphans --help` from a shell
...
fn scenario_cli_no_options_empty_workspace_prints_success() {
    ...
    // @step When I run `./rust/target/release/fspec query-orphans` from that directory
...
fn scenario_cli_output_json_empty_workspace_prints_empty_array() {
    ...
    // @step When I run `./rust/target/release/fspec query-orphans --output json` from that directory
...
fn scenario_cli_text_output_lists_orphan_with_suggested_actions() {
    ...
    // @step When I run `./rust/target/release/fspec query-orphans` from that workspace
    let (code, stdout, stderr) = run_query(ws.path(), &[]);
...
fn scenario_cli_exclude_done_flag_suppresses_done_orphans() {
    ...
    // @step When I run `./rust/target/release/fspec query-orphans --exclude-done --output json` from that workspace
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And I run `./rust/target/release/fspec query-orphans --output json` against the same workspace
    ...
    // @step And the CLI bridge module rust/fspec/src/query_orphans.rs contains NO inline orphan-detection or rendering logic — its only computation is JSON arg marshalling and stdout printing
...
fn scenario_query_orphans_help_matches_ts_reference() {
    ...
    // @step When I run `./rust/target/release/fspec query-orphans --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("query-orphans")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/query-orphans.txt
