...
//! `rust/fspec/src/main.rs` does not yet register a `copy-virtual-hooks`
...
fn scenario_cli_prints_success_message_when_copying_all_hooks() {
    ...
    // @step When I run `./rust/target/release/fspec copy-virtual-hooks --from AUTH-001 --to AUTH-002` in that project root
...
fn scenario_cli_prints_success_message_when_copying_named_hook() {
    ...
    // @step When I run `./rust/target/release/fspec copy-virtual-hooks --from AUTH-001 --to AUTH-002 --hook-name eslint` in that project root
...
fn scenario_cli_exits_1_when_from_is_omitted() {
    ...
    // @step When I run `./rust/target/release/fspec copy-virtual-hooks --to AUTH-002` in that project root
...
fn scenario_cli_exits_1_when_to_is_omitted() {
    ...
    // @step When I run `./rust/target/release/fspec copy-virtual-hooks --from AUTH-001` in that project root
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/copy_virtual_hooks.rs contains NO inline rendering, file IO beyond cwd resolution, or work-unit-lookup logic — its only computation is JSON arg marshalling plus the --from/--to presence guard
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/copy_virtual_hooks.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/copy_virtual_hooks.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_copy_virtual_hooks_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec copy-virtual-hooks --help` piped to non-TTY
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/copy-virtual-hooks.txt
