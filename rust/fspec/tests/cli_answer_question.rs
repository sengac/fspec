...
fn scenario_answer_question_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/answer-question.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/answer_question.rs contains NO inline question lookup, status guard, RuleItem construction, or file-write logic — its only computation is clap parsing + JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/answer_question.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/answer_question.rs must exist as the CLI bridge module"
    );
