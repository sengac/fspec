...
fn scenario_clap_exposes_show_deleted_with_positional_work_unit_id_and_no_flags() {
    ...
    // @step When I run `./rust/target/release/fspec show-deleted --help` from a shell
...
fn scenario_cli_prints_sentinel_when_no_deleted_items() {
    ...
    // @step When I run `./rust/target/release/fspec show-deleted AUTH-001` from that directory
...
fn scenario_cli_text_output_renders_header_and_item_lines() {
    ...
    // @step When I run `./rust/target/release/fspec show-deleted AUTH-001`
    let (code, stdout, stderr) = run_show_deleted(ws.path(), &["AUTH-001"]);
...
fn scenario_cli_unknown_work_unit_exits_1_with_stderr() {
    ...
    // @step When I run `./rust/target/release/fspec show-deleted UNKNOWN-999`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step Then the CLI bridge module rust/fspec/src/show_deleted.rs contains NO inline deleted-item collection, filter, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/show_deleted.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/show_deleted.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_show_deleted_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec show-deleted --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("show-deleted")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-deleted.txt
