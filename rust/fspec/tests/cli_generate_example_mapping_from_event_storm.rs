...
fn scenario_generate_help_matches_ts_fixture() {
    ...
    // @step When I run `./rust/target/release/fspec generate-example-mapping-from-event-storm --help` from a shell
    ...
    // @step Then stdout is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/generate-example-mapping-from-event-storm.txt
...
fn scenario_cli_transforms_and_prints_summary() {
    ...
    // @step When I run `./rust/target/release/fspec generate-example-mapping-from-event-storm AUTH-001` from that directory
