...
fn scenario_clap_exposes_add_capability_with_positional_args_in_help() {
    ...
    // @step When I run `./rust/target/release/fspec add-capability --help`
...
fn scenario_cli_adds_capability_and_prints_success_block() {
    ...
    // @step When I run `./rust/target/release/fspec add-capability "Search" "Full text search"`
...
fn scenario_cli_writes_to_draft_and_reports_draft_file_name() {
    ...
    // @step When I run `./rust/target/release/fspec add-capability "Data Export" "Export to CSV"`
...
fn scenario_cli_prints_placeholder_removal_line() {
    ...
    // @step When I run `./rust/target/release/fspec add-capability "Login" "Authenticate users"`
...
fn scenario_cli_fails_with_exit_1_when_foundation_missing() {
    ...
    // @step When I run `./rust/target/release/fspec add-capability "X" "Y"`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And running `./rust/target/release/fspec add-capability "Via CLI" "d"` afterwards exits 0
    ...
    // @step And the CLI bridge module rust/fspec/src/add_capability.rs contains NO inline draft probing, placeholder detection, or JSON-mutation logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_capability.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/add_capability.rs must exist; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_add_capability_help_matches_fixture() {
    ...
    // @step When I run `./rust/target/release/fspec add-capability --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("add-capability")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/add-capability.txt
