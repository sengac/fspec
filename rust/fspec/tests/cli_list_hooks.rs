...
//!   - `rust/fspec/src/main.rs` does not yet register a `list-hooks`
//!     clap subcommand (clap returns exit code 2 for "unrecognized
//!     subcommand").
//!   - `rust/fspec-core/src/commands/list_hooks.rs` is still a
//!     NotYetPorted stub.
...
///    > rust/fspec/tests/fixtures/help/list-hooks.txt`
...
fn scenario_clap_exposes_list_hooks_with_flag_aware_help() {
    ...
    // (Fixture captured at rust/fspec/tests/fixtures/help/list-hooks.txt — 51 lines, 1237 bytes.)

    // @step When I run `./rust/target/release/fspec list-hooks --help` piped to non-TTY (no color codes)
...
fn scenario_cli_against_empty_directory_writes_zero_bytes_and_no_files() {
    ...
    // @step When I run `./rust/target/release/fspec list-hooks` from that directory
...
fn scenario_cli_writes_zero_bytes_regardless_of_populated_hooks() {
    ...
    // @step When I run `./rust/target/release/fspec list-hooks`
    let (code, stdout, stderr) = run_list_hooks(ws.path(), &[]);
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "hooks": {
    ...
    // @step Then the CLI bridge module rust/fspec/src/list_hooks.rs contains NO inline event-aggregation or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/list_hooks.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/list_hooks.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
