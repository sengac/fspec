...
//!   - `rust/fspec/src/main.rs` does not yet register a `list-tags`
//!     clap subcommand (clap returns exit code 2 for "unrecognized
//!     subcommand").
//!   - `rust/fspec-core/src/commands/list_tags.rs` is still a
//!     NotYetPorted stub.
...
fn scenario_clap_exposes_list_tags_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec list-tags --help` from a shell
...
fn scenario_cli_against_empty_directory_auto_creates_tags_file() {
    ...
    // @step When I run `./rust/target/release/fspec list-tags` from that directory
...
fn scenario_cli_text_output_renders_alphabetically_sorted_tags() {
  ...
  "categories": [
    ...
    // @step When I run `./rust/target/release/fspec list-tags`
    let (code, stdout, stderr) = run_list_tags(ws.path(), &[]);
...
fn scenario_cli_category_filter_restricts_output() {
    ...
    // @step When I run `./rust/target/release/fspec list-tags --category 'Phase Tags'`
...
fn scenario_cli_category_filter_unknown_exits_1() {
    ...
    // @step When I run `./rust/target/release/fspec list-tags --category 'No Such Category'`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "categories": [
    ...
    // @step Then the CLI bridge module rust/fspec/src/list_tags.rs contains NO inline category-filter, tag-sorting, or rendering logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/list_tags.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/list_tags.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
    ...
    // @step And the CLI bridge module rust/fspec/src/list_tags.rs contains NO inline category-filter, tag-sorting, or rendering logic
...
fn scenario_list_tags_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec list-tags --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("list-tags")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-tags.txt
