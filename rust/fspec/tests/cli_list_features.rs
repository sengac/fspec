...
//!   - `rust/fspec/src/main.rs` does not yet register a `list-features`
//!     clap subcommand (clap returns exit code 2 for "unrecognized
//!     subcommand").
//!   - `rust/fspec-core/src/commands/list_features.rs` is still a
//!     NotYetPorted stub.
...
fn scenario_clap_exposes_list_features_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec list-features --help` from a shell
...
fn scenario_cli_against_no_spec_dir_exits_2_with_directory_not_found() {
    ...
    // @step When I run `./rust/target/release/fspec list-features` from that directory
...
fn scenario_cli_text_output_renders_feature_listing_populated_case() {
    ...
    // @step When I run `./rust/target/release/fspec list-features`
    let (code, stdout, stderr) = run_list_features(ws.path(), &[]);
...
fn scenario_cli_tag_filter_narrows_results_and_updates_summary() {
    ...
    // @step When I run `./rust/target/release/fspec list-features --tag @critical`
...
fn scenario_cli_bridge_module_embeds_no_duplicated_business_logic() {
    // @step Given the file rust/fspec/src/list_features.rs exists as the CLI bridge module
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/list_features.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/list_features.rs must exist as the CLI bridge module; missing: {}",
        bridge_path.display()
    );
...
fn scenario_list_features_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec list-features --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("list-features")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-features.txt
