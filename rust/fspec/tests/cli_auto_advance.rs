...
fn scenario_auto_advance_help_matches_ts_format_command_help_reference() {
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/auto-advance.txt
    assert_eq!(
...
fn scenario_cli_bridge_delegates_to_same_fspec_core_function() {
    // @step Given the CLI bridge module rust/fspec/src/auto_advance.rs
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/auto_advance.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/auto_advance.rs must exist as the CLI bridge module; missing: {}",
        bridge_path.display()
    );
