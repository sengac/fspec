...
fn clap_exposes_search_implementation_as_a_subcommand_and_prints_flag_help() {
    ...
    // @step When I run `./rust/target/release/fspec search-implementation --help` from a shell
...
fn cli_without_function_flag_exits_non_zero() {
    ...
    // @step When I run `./rust/target/release/fspec search-implementation` from that directory
...
fn cli_default_output_prints_the_green_summary_line() {
    ...
    // @step When I run `./rust/target/release/fspec search-implementation --function loadConfig` from that workspace
...
fn cli_json_prints_the_2_space_json_envelope_to_stdout() {
    ...
    // @step When I run `./rust/target/release/fspec search-implementation --function loadConfig --json` from that workspace
...
fn cli_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec search-implementation --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("search-implementation")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/search-implementation.txt
...
fn cli_delegates_to_the_same_fspec_core_function_used_by_the_dispatcher() {
    ...
    // @step And I run `./rust/target/release/fspec search-implementation --function loadConfig --json` against the same workspace
    ...
    // @step And the CLI bridge module rust/fspec/src/search_implementation.rs contains NO inline filtering or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/search_implementation.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/search_implementation.rs must exist as the CLI bridge module"
    );
