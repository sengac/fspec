...
fn scenario_clap_exposes_delete_work_unit_with_arg_and_flags() {
    ...
    // @step When I run `./rust/target/release/fspec delete-work-unit --help`
...
fn scenario_cli_deletes_existing_leaf_work_unit_and_prints_success_line() {
    ...
    // @step When I run `./rust/target/release/fspec delete-work-unit AUTH-001`
...
fn scenario_cli_exits_1_when_work_unit_does_not_exist() {
    ...
    // @step When I run `./rust/target/release/fspec delete-work-unit MISSING-999`
...
fn scenario_cli_cascades_dependencies_and_prints_blocks_warning() {
  ...
  "states": {
    ...
    // @step When I run `./rust/target/release/fspec delete-work-unit AUTH-001 --cascade-dependencies`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And running `./rust/target/release/fspec delete-work-unit DASH-001` afterwards exits 0
    ...
    // @step And the CLI bridge module rust/fspec/src/delete_work_unit.rs contains NO inline file-read, mutation, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/delete_work_unit.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/delete_work_unit.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_delete_work_unit_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec delete-work-unit --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("delete-work-unit")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/delete-work-unit.txt
