...
fn scenario_clap_exposes_init_subcommand_and_prints_help() {
    ...
    // @step When I run `./rust/target/release/fspec init --help` from a shell
...
fn scenario_cli_installs_the_claude_agent_and_prints_the_success_summary() {
    ...
    // @step When I run `./rust/target/release/fspec init --agent claude` from that directory
...
fn scenario_cli_installs_multiple_agents_from_repeated_agent_flags() {
    ...
    // @step When I run `./rust/target/release/fspec init --agent claude --agent cursor` from that directory
...
fn scenario_cli_without_agent_fails_because_shell_is_non_tty() {
    ...
    // @step When I run `./rust/target/release/fspec init` from that directory
...
fn scenario_cli_rejects_an_unknown_agent_id() {
    ...
    // @step When I run `./rust/target/release/fspec init --agent bogus` from that directory
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/init.rs must exist as the CLI bridge module"
    );
...
fn scenario_init_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec init --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("init")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/init.txt
