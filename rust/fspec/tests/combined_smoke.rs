...
fn scenario_combined_uses_tokio_runtime_handle_current_for_embedded_backend() {
    // @step Given the file `rust/fspec/src/combined.rs` exists
    let combined_rs = fspec_crate_root().join("src").join("combined.rs");
    let body = std::fs::read_to_string(&combined_rs).expect("read rust/fspec/src/combined.rs");
...
fn scenario_combined_and_daemon_share_the_same_bind_and_serve_function() {
    // @step Given the file `rust/fspec/src/combined.rs` exists
    // @step And the file `rust/fspec/src/daemon.rs` exists
    let combined_rs = fspec_crate_root().join("src").join("combined.rs");
    ...
    // @step And no other file under `rust/fspec/src/` calls `bind_and_serve(`
    ...
    assert!(
        ...
        "rust/rpc-server/src/server.rs must still expose pub async fn bind_and_serve(...)"
    );
}
