...
//! `rust/fspec-core/src/commands/pause_schedule.rs` is still a stub, the
//! clap subcommand is NOT yet wired into `rust/fspec/src/main.rs`, and
//! the bridge module `rust/fspec/src/pause_schedule.rs` does not exist.
...
fn scenario_pause_schedule_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step And stdout is byte-for-byte identical to the TS reference output at rust/fspec/tests/fixtures/help/pause-schedule.txt
    assert_eq!(
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/pause_schedule.rs contains NO inline schedule-mutation, validation, or file-writing logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/pause_schedule.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/pause_schedule.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
