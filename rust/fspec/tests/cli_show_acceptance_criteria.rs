...
fn rust_port_shared_infrastructure_module_is_registered() {
    ...
    // @step When I inspect rust/fspec-core/src/commands/show_acceptance_criteria.rs
    let ws = tempfile::tempdir().expect("tempdir");
    mkdir(ws.path(), "spec/features");
    let result = dispatch(ws.path(), "{}");
...
fn cli_clap_exposes_subcommand_with_flag_help() {
    ...
    // @step When I run `./rust/target/release/fspec show-acceptance-criteria --help` from a shell
...
fn cli_against_workspace_with_no_spec_features_exits_1() {
    ...
    // @step When I run `./rust/target/release/fspec show-acceptance-criteria` from that directory
...
fn cli_default_text_output_prints_feature_name_and_scenario_steps() {
    ...
    // @step When I run `./rust/target/release/fspec show-acceptance-criteria --tag @auth` from that workspace
...
fn cli_format_markdown_prints_h1_h2_bullet_step_output() {
    ...
    // @step When I run `./rust/target/release/fspec show-acceptance-criteria --tag @auth --format markdown` from that workspace
...
fn cli_format_json_prints_two_space_json_array() {
    ...
    // @step When I run `./rust/target/release/fspec show-acceptance-criteria --tag @auth --format json` from that workspace
...
fn cli_output_writes_file_and_prints_message_without_dumping_content() {
    ...
    // @step When I run `./rust/target/release/fspec show-acceptance-criteria --tag @auth --format markdown --output out.md` from that workspace
...
fn cli_tag_matching_zero_features_prints_no_features_found() {
    ...
    // @step When I run `./rust/target/release/fspec show-acceptance-criteria --tag @missing` from that workspace
...
fn cli_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec show-acceptance-criteria --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("show-acceptance-criteria")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-acceptance-criteria.txt
...
fn cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And I run `./rust/target/release/fspec show-acceptance-criteria --tag @auth --format json` against the same workspace
    ...
    // @step And the CLI bridge module rust/fspec/src/show_acceptance_criteria.rs contains NO inline gherkin parsing, filter, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/show_acceptance_criteria.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/show_acceptance_criteria.rs must exist as the CLI bridge module"
    );
