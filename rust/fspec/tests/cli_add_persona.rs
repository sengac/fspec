...
fn scenario_help_output_matches_ts_fixture() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/add-persona.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/add_persona.rs contains NO inline placeholder, file-read, or file-write logic — its only computation is JSON arg marshalling and stdout rendering
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_persona.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/add_persona.rs must exist; got missing: {}",
        bridge_path.display()
    );
