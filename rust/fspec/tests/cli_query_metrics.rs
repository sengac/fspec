...
fn scenario_cli_exposes_query_metrics_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec query-metrics --help` with NO_COLOR=1
    ...
    // @step Then stdout is byte-for-byte identical to the captured fixture at rust/fspec/tests/fixtures/help/query-metrics.txt
...
fn scenario_cli_against_missing_work_units_exits_1_with_query_failed() {
    ...
    // @step When I run `./rust/target/release/fspec query-metrics` from that directory
...
fn scenario_cli_json_output_matches_dispatcher_output() {
  ...
  "states": {{
    ...
    // @step When I run `./rust/target/release/fspec query-metrics --work-unit-id AUTH-001 --format json` against that workspace
...
fn scenario_cli_text_output_for_aggregate_renders_project_metrics_block() {
  ...
  "states": {{
    ...
    // @step When I run `./rust/target/release/fspec query-metrics`
    let (code, stdout, stderr) = run_query_metrics(ws.path(), &[]);
...
fn scenario_cli_bridge_module_delegates_with_no_inline_aggregation() {
    // @step Given the file rust/fspec/src/query_metrics.rs exists as the CLI bridge module
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/query_metrics.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/query_metrics.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );

    // @step When I read the source of rust/fspec/src/query_metrics.rs
