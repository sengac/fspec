...
fn scenario_cli_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step And stdout matches the captured fixture at rust/fspec/tests/fixtures/help/create-feature.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/create_feature.rs contains NO inline template, kebab-case, coverage, or prefill logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/create_feature.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/create_feature.rs must exist as the CLI bridge module"
    );
    let bridge_src =
