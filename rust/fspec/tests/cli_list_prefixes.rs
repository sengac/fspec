...
//! (`Mode::ListPrefixes` in `rust/fspec/src/main.rs`) and the ported
//! `rust/fspec-core/src/commands/list_prefixes.rs` implementation.
...
fn scenario_clap_exposes_list_prefixes_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec list-prefixes --help` from a shell
...
fn scenario_cli_against_empty_directory_prints_sentinel_and_no_files() {
    ...
    // @step When I run `./rust/target/release/fspec list-prefixes` from that directory
...
fn scenario_cli_text_output_renders_prefix_progress() {
    ...
    // @step When I run `./rust/target/release/fspec list-prefixes`
    let (code, stdout, stderr) = run_list_prefixes(ws.path(), &[]);
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "states": {
    ...
    // @step Then the CLI bridge module rust/fspec/src/list_prefixes.rs contains NO inline prefix-aggregation, filter, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/list_prefixes.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/list_prefixes.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_list_prefixes_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec list-prefixes --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("list-prefixes")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-prefixes.txt
