...
fn scenario_clap_exposes_suggest_dependencies_with_help() {
    ...
    // @step When I run `./rust/target/release/fspec suggest-dependencies --help` from a shell
...
fn scenario_cli_without_options_prints_empty_sentinel_against_empty_workspace() {
    ...
    // @step When I run `./rust/target/release/fspec suggest-dependencies` from that directory
...
fn scenario_cli_with_output_json_prints_empty_suggestions_array() {
    ...
    // @step When I run `./rust/target/release/fspec suggest-dependencies --output json` from that directory
...
fn scenario_cli_text_output_lists_a_sequential_suggestion() {
    ...
    // @step When I run `./rust/target/release/fspec suggest-dependencies` from that workspace
    let (code, stdout, stderr) = run_suggest(ws.path(), &[]);
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And I run `./rust/target/release/fspec suggest-dependencies --output json` against the same workspace
    let (code, stdout, stderr) = run_suggest(ws.path(), &["--output", "json"]);
    ...
    // @step And the CLI bridge module rust/fspec/src/suggest_dependencies.rs contains NO inline suggestion logic — its only computation is JSON arg marshalling and stdout printing
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/suggest_dependencies.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/suggest_dependencies.rs must exist as the CLI bridge module"
    );
...
fn scenario_suggest_dependencies_help_matches_ts_reference() {
    ...
    // @step When I run `./rust/target/release/fspec suggest-dependencies --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("suggest-dependencies")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/suggest-dependencies.txt
