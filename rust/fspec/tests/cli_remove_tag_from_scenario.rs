...
fn scenario_remove_tag_from_scenario_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/remove-tag-from-scenario.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/remove_tag_from_scenario.rs contains NO inline scenario lookup, line-walk filter, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/remove_tag_from_scenario.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/remove_tag_from_scenario.rs must exist; missing: {}",
        bridge_path.display()
    );
