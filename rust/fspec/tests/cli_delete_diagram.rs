...
fn scenario_clap_exposes_delete_diagram_with_positional_args_in_help() {
    ...
    // @step When I run `./rust/target/release/fspec delete-diagram --help`
...
fn scenario_cli_removes_diagram_by_title_and_prints_success_block() {
    ...
    // @step When I run `./rust/target/release/fspec delete-diagram Architecture "Component Flow"`
...
fn scenario_cli_fails_with_exit_1_when_foundation_missing() {
    ...
    // @step When I run `./rust/target/release/fspec delete-diagram Architecture "X"`
...
fn scenario_cli_fails_with_exit_1_when_title_not_found() {
    ...
    // @step When I run `./rust/target/release/fspec delete-diagram Architecture "Missing"`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And running `./rust/target/release/fspec delete-diagram Architecture "B"` afterwards exits 0
    ...
    // @step And the CLI bridge module rust/fspec/src/delete_diagram.rs contains NO inline file IO, JSON-parse, or splice logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/delete_diagram.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/delete_diagram.rs must exist; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_delete_diagram_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec delete-diagram --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("delete-diagram")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/delete-diagram.txt
