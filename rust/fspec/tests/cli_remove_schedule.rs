...
//! `rust/fspec-core/src/commands/remove_schedule.rs` is still a
...
//! `rust/fspec/src/remove_schedule.rs` has NOT been created. Every
...
fn scenario_remove_schedule_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step Then stdout is byte-for-byte identical to the TS reference output at rust/fspec/tests/fixtures/help/remove-schedule.txt
    assert_eq!(
...
fn scenario_cli_removes_schedule_and_delegates_to_fspec_core() {
    ...
    // @step Then the CLI bridge module rust/fspec/src/remove_schedule.rs contains NO schedule-mutation or file-writing logic beyond JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/remove_schedule.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/remove_schedule.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
