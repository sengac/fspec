...
//!
//! The fix in `rust/fspec/src/common.rs::env_filter` pins
...
fn default_env_filter_suppresses_tarpc_info_keeps_warn_and_other_info() {
    ...
    // in rust/fspec/src/common.rs. The companion source-text
...
fn common_rs_defines_tarpc_quiet_directives_constant() {
    ...
    let common_rs = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/src/common.rs"))
        .expect("read rust/fspec/src/common.rs");

    // @step Then the file declares a TARPC_QUIET_DIRECTIVES constant
    assert!(
        common_rs.contains("TARPC_QUIET_DIRECTIVES"),
        "rust/fspec/src/common.rs MUST define a TARPC_QUIET_DIRECTIVES constant — regressed LOG-005"
    );

    // @step And the constant contains tarpc::client=warn
    assert!(
        common_rs.contains("\"tarpc::client=warn\""),
        "rust/fspec/src/common.rs MUST list \"tarpc::client=warn\" in TARPC_QUIET_DIRECTIVES — regressed LOG-005"
    );

    // @step And the constant contains tarpc::server=warn
    assert!(
        common_rs.contains("\"tarpc::server=warn\""),
        "rust/fspec/src/common.rs MUST list \"tarpc::server=warn\" in TARPC_QUIET_DIRECTIVES — regressed LOG-005"
    );
    ...
    assert!(
        ...
        "rust/fspec/src/common.rs::env_filter MUST iterate TARPC_QUIET_DIRECTIVES and add each directive — regressed LOG-005"
    );
}
