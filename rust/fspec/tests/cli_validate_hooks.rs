...
fn scenario_validate_hooks_help_matches_ts_reference() {
    ...
    // @step When I run `./rust/target/release/fspec validate-hooks --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("validate-hooks")
        .arg("--help")
    ...
    assert_eq!(
        code, 0,
        "validate-hooks --help must exit 0; stderr={stderr}"
    );

    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/validate-hooks.txt
...
fn scenario_cli_all_scripts_exist_prints_success_exits_0() {
    ...
    // @step When I run `./rust/target/release/fspec validate-hooks`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step When I dispatch validate-hooks through fspec_core::dispatch::dispatch_command and also run `./rust/target/release/fspec validate-hooks` against the same on-disk state
    ...
    // @step Then the CLI bridge module rust/fspec/src/validate_hooks.rs contains NO inline validation logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/validate_hooks.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/validate_hooks.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
