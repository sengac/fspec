...
//! (`Mode::ShowFeature` in `rust/fspec/src/main.rs`) and the ported
//! `rust/fspec-core/src/commands/show_feature.rs` implementation.
...
fn scenario_clap_exposes_show_feature_with_flag_aware_help() {
    ...
    // @step When I run `./rust/target/release/fspec show-feature --help` from a shell
...
fn scenario_show_feature_against_empty_workspace_exits_1_with_not_found() {
    ...
    // @step When I run `./rust/target/release/fspec show-feature missing` from that directory
...
fn scenario_cli_text_output_renders_feature_contents_and_work_units_none() {
    ...
    // @step When I run `./rust/target/release/fspec show-feature login` from that workspace
...
fn scenario_cli_text_output_renders_work_unit_progress_block() {
  ...
  "states": {
    ...
    // @step When I run `./rust/target/release/fspec show-feature auth` from that workspace
...
fn scenario_show_feature_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec show-feature --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("show-feature")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-feature.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "states": {
    ...
    // @step And I run `./rust/target/release/fspec show-feature auth` against the same workspace
    let (code, stdout, stderr) = run_show_feature(ws.path(), &["auth"]);
    ...
    // @step And the CLI bridge module rust/fspec/src/show_feature.rs contains NO inline gherkin parsing, work-unit aggregation, or text rendering — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/show_feature.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/show_feature.rs must exist as the CLI bridge module"
    );
