...
fn scenario_add_command_help_matches_ts_fixture() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/add-command.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/add_command.rs contains NO inline item construction, status guard, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_command.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/add_command.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
