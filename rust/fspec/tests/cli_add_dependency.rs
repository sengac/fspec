...
fn scenario_add_dependency_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step And the stdout matches the canonical help fixture at rust/fspec/tests/fixtures/help/add-dependency.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/add_dependency.rs contains NO inline edge-add, status guard, cycle, or file-write logic — its only computation is shorthand resolution + conflict pre-check + JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_dependency.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/add_dependency.rs must exist as the CLI bridge module"
    );
