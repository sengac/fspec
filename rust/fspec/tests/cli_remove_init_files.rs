...
fn scenario_clap_exposes_remove_init_files_with_help() {
    ...
    // @step When I run `./rust/target/release/fspec remove-init-files --help` from a shell
...
fn scenario_cli_removes_claude_agent_files_and_prints_the_success_summary() {
    ...
    // @step When I run `./rust/target/release/fspec remove-init-files --no-keep-config` from that workspace
...
fn scenario_cli_keep_config_preserves_spec_fspec_config_json() {
    ...
    // @step When I run `./rust/target/release/fspec remove-init-files --keep-config` from that workspace
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module rust/fspec/src/remove_init_files.rs contains NO inline detection or deletion logic — its only computation is JSON arg marshalling and stdout printing
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/remove_init_files.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/remove_init_files.rs must exist as the CLI bridge module"
    );
...
fn scenario_remove_init_files_help_matches_ts_reference() {
    ...
    // @step When I run `./rust/target/release/fspec remove-init-files --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("remove-init-files")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/remove-init-files.txt
