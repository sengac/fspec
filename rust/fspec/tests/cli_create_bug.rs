...
fn scenario_clap_exposes_create_bug_with_args_and_option_flags() {
    ...
    // @step When I run `./rust/target/release/fspec create-bug --help`
...
fn scenario_cli_creates_minimal_bug_and_prints_success_block() {
    ...
    // @step When I run `./rust/target/release/fspec create-bug BUG "Login crash"`
...
fn scenario_cli_creates_bug_with_description_epic_parent() {
  ...
  "prefixCounters": {"BUG": 1}
    ...
    // @step When I run `./rust/target/release/fspec create-bug BUG "Login crash" -d "Crashes on submit" -e auth -p BUG-001`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And running `./rust/target/release/fspec create-bug BUG "Second"` afterwards exits 0
    ...
    // @step And the CLI bridge module rust/fspec/src/create_bug.rs contains NO inline validation, id-generation, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/create_bug.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/create_bug.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_create_bug_help_matches_fixture() {
    ...
    // @step When I run `./rust/target/release/fspec create-bug --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("create-bug")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/create-bug.txt
