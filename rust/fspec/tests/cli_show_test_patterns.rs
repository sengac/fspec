...
fn rust_port_shared_infrastructure_module_is_registered() {
    ...
    // @step When I inspect rust/fspec-core/src/commands/show_test_patterns.rs
    let ws = tempfile::tempdir().expect("tempdir");
    write_work_units(ws.path(), wus_one_cli());
    let result = dispatch(ws.path(), r#"{"tag":"@cli"}"#);
...
fn cli_clap_exposes_subcommand_with_flag_help() {
    ...
    // @step When I run `./rust/target/release/fspec show-test-patterns --help` from a shell
...
fn cli_without_tag_flag_exits_non_zero() {
    ...
    // @step When I run `./rust/target/release/fspec show-test-patterns` from that directory
...
fn cli_default_output_prints_green_summary_line() {
    ...
    // @step When I run `./rust/target/release/fspec show-test-patterns --tag @cli` from that workspace
...
fn cli_json_flag_prints_json_envelope_to_stdout() {
    ...
    // @step When I run `./rust/target/release/fspec show-test-patterns --tag @cli --json` from that workspace
...
fn cli_include_coverage_includes_deduplicated_test_file_paths() {
    ...
    // @step When I run `./rust/target/release/fspec show-test-patterns --tag @cli --include-coverage --json` from that workspace
...
fn cli_exits_1_when_work_units_json_missing() {
    ...
    // @step When I run `./rust/target/release/fspec show-test-patterns --tag @cli` from that directory
...
fn cli_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./rust/target/release/fspec show-test-patterns --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("show-test-patterns")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-test-patterns.txt
...
fn cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And I run `./rust/target/release/fspec show-test-patterns --tag @cli --json` against the same workspace
    ...
    // @step And the CLI bridge module rust/fspec/src/show_test_patterns.rs contains NO inline filtering or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/show_test_patterns.rs");
    assert!(
        bridge_path.exists(),
        "rust/fspec/src/show_test_patterns.rs must exist as the CLI bridge module"
    );
