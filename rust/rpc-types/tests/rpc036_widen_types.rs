...
fn phase_21_per_session_derived_state_types_have_documented_shape() {
    // @step Given the engineer opens rust/rpc-types/src/lib.rs after RPC-036 is implemented
...
fn all_new_types_round_trip_through_serde_json() {
    // @step Given a test suite under #[cfg(test)] in rust/rpc-types/src/lib.rs
    ...
    // @step And serde_json is declared in rust/rpc-types/Cargo.toml under [dev-dependencies]
...
fn both_feature_gates_compile_and_dev_deps_are_isolated() {
    ...
    let cargo_toml =
        std::fs::read_to_string(env!("CARGO_MANIFEST_DIR").to_string() + "/Cargo.toml")
            .expect("read rust/rpc-types/Cargo.toml");
...
fn isolation_state_change_constructor_keeps_two_arg_signature() {
    ...
    // @step And rust/napi/src/types.rs::stream_chunk_to_json_value destructures the IsolationStateChange variant with the new base_commit field accounted for
