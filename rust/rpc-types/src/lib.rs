...
//! cross the JS boundary so that `rust/napi` can re-export them
...
//!
//! RPC-005 lifted only `WorkUnitInfo` from `rust/napi/src/types.rs:182`.
...
impl SessionStatus {
    ...
    /// Lifted from `rust/napi/src/session_manager.rs` as part of the
    /// RPC-007 type-uniqueness rule so callers in `rust/napi` can use
...
impl From<u8> for SessionStatus {
    ...
    /// `rust/napi/src/session_manager.rs` so the historical
...
/// rust/napi/src/session_manager.rs:380) so that rust/napi can
...
/// `@sengac/codelet-napi` (`rust/napi/src/thinking_config.rs`).
#[cfg_attr(feature = "napi", napi_derive::napi(string_enum))]
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// TS `NapiModelInfo` shape (rust/napi/src/models/napi_bindings.rs)
...
/// TS `NapiProviderModels` shape (rust/napi/src/models/napi_bindings.rs)
...
/// `ScheduleEntry` from rust/napi/src/scheduler/types.rs but flattens
...
//   rust/napi/src/types.rs:217 with #[napi(discriminant = "type")] and
...
//
// The integration-test counterpart at rust/rpc-types/tests/rpc036_widen_types.rs
