...
//!
//! Integration tests for isolated-session creation via create_worktree /
//! create_worktree_at_ref with real git repositories.
//! Tests use fixtures (real temp repos) - NO MOCKING.
//!
//! WT-011: the dead IsolatedSessionInfo wrapper was deleted;
//! create_worktree/create_worktree_at_ref are the only isolation-info paths.

mod common;

use codelet_git::{
    create_worktree, create_worktree_at_ref, GitError, WorktreeCreateResult, FSPEC_WORKTREES_DIR,
};
use std::fs;
use std::path::Path;
...
fn test_create_isolated_session_with_worktree() {
    ...
    // @step When I create a session with id "abc123" and isolated=true
    let info: WorktreeCreateResult =
        create_worktree(repo_path, "abc123").expect("Failed to create isolated session");
    ...
    assert_eq!(
        ...
        "session should track worktree_path at the expected location"
    );

    // @step And the session state should include base_commit
    assert_eq!(info.base_commit, head_sha, "base_commit should match HEAD");
}
...
fn test_create_non_isolated_session_without_worktree() {
    ...
    // @step When I create a session with id "def456" and isolated=false
    // Non-isolated: no worktree creation call; the effective working
    // directory is the project root itself.
    let effective_cwd = repo_path.to_path_buf();
    ...
    assert_eq!(
        ...
        "non-isolated session should use the project root as its working dir"
    );
}
...
fn test_default_session_creation_is_non_isolated() {
    ...
    // @step When I create a session with id "ghi789" without specifying isolation
    // Using the project root as the effective working directory (no
    // worktree creation call) is the default factory.
    let effective_cwd = repo_path.to_path_buf();
...
fn test_effective_cwd_returns_worktree_path_for_isolated_session() {
    ...
    // @step And an isolated session "abc123" with worktree at ".fspec/worktrees/abc123/"
    let info = create_worktree(repo_path, "abc123").expect("Failed to create isolated session");

    let expected_path = repo_path.join(FSPEC_WORKTREES_DIR).join("abc123");

    // @step When I call effective_cwd on the session
    // For isolated sessions the worktree path is the effective cwd.
    let result = info.info.path;
...
fn test_effective_cwd_returns_project_root_for_non_isolated_session() {
    ...
    // @step And a non-isolated session "def456"
    // Non-isolated sessions run in the project root.
    let effective_cwd = repo_path.to_path_buf();
    ...
    assert_eq!(
        effective_cwd,
        repo_path.to_path_buf(),
        "effective_cwd should return the project root for non-isolated sessions"
    );
}
...
fn test_create_isolated_session_fails_if_worktree_exists() {
    ...
    // @step And a worktree already exists for session "abc123"
    let _first =
        create_worktree(repo_path, "abc123").expect("Failed to create first isolated session");

    // @step When I try to create a session with id "abc123" and isolated=true
    let result = create_worktree(repo_path, "abc123");
