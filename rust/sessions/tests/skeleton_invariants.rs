...
//!
//! Pattern borrowed from `rust/rpc-embedded/tests/rpc_006_source_shape.rs`.
...
fn scenario_cargo_workspace_recognises_the_new_codelet_sessions_crate() {
    // @step Given the root `Cargo.toml` lists `rust/sessions` as a workspace member and `codelet-sessions` as a workspace dependency
    //
    // NOTE: workspace.members paths are relative to the workspace root
    // (`rust/Cargo.toml`), so the literal in the manifest is "sessions"
    // even though the directory's repo-relative path is `rust/sessions/`.
    let root_manifest = read(&workspace_root().join("Cargo.toml"));
    assert!(
        root_manifest.contains("\"sessions\""),
        "root Cargo.toml workspace.members must include \"sessions\" (which resolves to rust/sessions/)"
    );
    assert!(
    ...
    // @step Then the output JSON includes a package named `codelet-sessions` at version 0.1.0 with manifest path ending in `rust/sessions/Cargo.toml`
    assert!(
        stdout.contains("\"name\":\"codelet-sessions\""),
        "cargo metadata must list a `codelet-sessions` package"
    );
    assert!(
        stdout.contains("rust/sessions/Cargo.toml"),
        "cargo metadata manifest_path must point at rust/sessions/Cargo.toml"
    );
}
...
fn scenario_codelet_sessions_builds_standalone_against_empty_modules() {
    ...
    assert!(
        bg.exists(),
        "rust/sessions/src/background_session.rs must exist"
    );
    assert!(
        sm.exists(),
        "rust/sessions/src/session_manager.rs must exist"
    );
...
fn scenario_codelet_sessions_has_no_transitive_dependency_on_codelet_napi() {
    ...
    assert!(
        ...
        "rust/sessions/Cargo.toml must depend on codelet-rpc-types"
    );
    ...
    for line in sessions_manifest.lines() {
        ...
        if trimmed.contains("codelet-rpc-types") && trimmed.contains("\"napi\"") {
            panic!(
                "rust/sessions/Cargo.toml must NOT enable the napi feature on codelet-rpc-types: {trimmed}"
            );
        }
    }
    ...
    // rust/tools/Cargo.toml lines 80-81). Removing them is a separate
    ...
    if transitive_names.contains("napi") || transitive_names.contains("napi-derive") {
        ...
        // Tracking note for future readers: see rust/tools/Cargo.toml.
        eprintln!(
            "[RPC-038] note: transitive napi crate is present via a non-local inbound dep; this is documented as out-of-scope (see rust/tools/Cargo.toml). transitive_names contains: napi={}, napi-derive={}",
...
fn scenario_smoke_test_runs_and_passes() {
    // @step Given `rust/sessions/tests/smoke.rs` contains a `crate_compiles` test
    ...
    assert!(
        smoke.contains("#[test]"),
        "rust/sessions/tests/smoke.rs must mark `crate_compiles` with #[test]"
    );
...
fn scenario_lib_rs_declares_placeholder_modules_for_later_rpc_cards() {
    ...
    assert!(lib_path.exists(), "rust/sessions/src/lib.rs must exist");

    // @step When I read `rust/sessions/src/lib.rs`
    let lib = read(&lib_path);

    // @step Then it declares `pub mod background_session;`
    assert!(
        lib.contains("pub mod background_session;"),
        "rust/sessions/src/lib.rs must declare `pub mod background_session;`"
    );

    // @step And it declares `pub mod session_manager;`
    assert!(
        lib.contains("pub mod session_manager;"),
        "rust/sessions/src/lib.rs must declare `pub mod session_manager;`"
    );
...
fn scenario_workspace_lints_are_inherited_and_clippy_passes() {
    // @step Given `rust/sessions/Cargo.toml` declares `[lints]` with `workspace = true`
    ...
    assert!(
        ...
        "rust/sessions/Cargo.toml must declare a [lints] section"
    );
    assert!(
        has_workspace_lints,
        "rust/sessions/Cargo.toml [lints] must inherit `workspace = true`"
    );
