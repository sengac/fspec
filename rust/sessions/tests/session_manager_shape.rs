...
//!
//! Pattern borrowed from `rust/sessions/tests/background_session_shape.rs`
//! (RPC-039).
...
/// inside `rust/napi/src/session_manager.rs` (pre-RPC-043). Returns the
...
/// rust/napi/src/session_manager.rs broke the original single-file
...
fn scenario_codelet_sessions_builds_standalone_with_session_manager_chain_of_command_navigation_credentials_and_the_hooks_trait_at_their_new_home(
    ...
    // @step Given the SessionManager and ChainOfCommand structs have been moved into rust/sessions/src/
    let sm = read(&moved_sm_path());
    assert!(
        sm.contains("pub struct SessionManager"),
        "rust/sessions/src/session_manager.rs must define `pub struct SessionManager`"
    );
    let coc = read(&moved_coc_path());
    assert!(
        coc.contains("pub struct ChainOfCommand"),
        "rust/sessions/src/chain_of_command.rs must define `pub struct ChainOfCommand`"
    );

    // @step And the crate::navigation module has been lifted to rust/sessions/src/navigation.rs
    let nav = read(&moved_nav_path());
    assert!(
        nav.contains("pub fn build_navigation_list"),
        "rust/sessions/src/navigation.rs must define `pub fn build_navigation_list`"
    );

    // @step And the NAPI-free portion of crate::credentials has been lifted to rust/sessions/src/credentials/
    ...
    assert!(
        ...
        "rust/sessions/src/credentials/mod.rs must re-export the lifted submodules"
    );

    // @step And the SessionManagerHooks trait plus NoopSessionManagerHooks default impl have been added to rust/sessions/src/session_manager.rs
    assert!(
        sm.contains("pub trait SessionManagerHooks"),
        "rust/sessions/src/session_manager.rs must define `pub trait SessionManagerHooks`"
    );
    assert!(
        sm.contains("pub struct NoopSessionManagerHooks") || sm.contains("NoopSessionManagerHooks"),
        "rust/sessions/src/session_manager.rs must define `NoopSessionManagerHooks`"
    );
...
{
    // @step Given rust/napi/src/session_manager.rs now `pub use`s SessionManager and ChainOfCommand from codelet-sessions
    let napi = read_napi_shell();
    assert!(
        napi.contains("pub use codelet_sessions::session_manager::SessionManager"),
        "rust/napi/src/session_manager.rs must `pub use codelet_sessions::session_manager::SessionManager`"
    );
    assert!(
        ...
        "rust/napi/src/session_manager.rs must `pub use codelet_sessions::chain_of_command::ChainOfCommand`"
    );
    ...
    for sym in [
        ...
        assert!(
            napi.contains(sym),
            "rust/napi/src/session_manager.rs must still expose the #[napi] free function `{sym}`"
        );
    }
}
...
fn scenario_the_moved_session_manager_rs_and_chain_of_command_rs_have_no_napi_references() {
    // @step Given the SessionManager and ChainOfCommand code has been moved into rust/sessions/src/
    let sm = read(&moved_sm_path());
    let coc = read(&moved_coc_path());
    assert!(sm.contains("pub struct SessionManager"));
    assert!(coc.contains("pub struct ChainOfCommand"));

    // @step When I grep rust/sessions/src/session_manager.rs and rust/sessions/src/chain_of_command.rs for the regex `napi::|use napi|#\[napi`
...
fn scenario_the_moved_session_manager_rs_has_no_crate_references_to_napi_private_modules_or_free_functions(
    ...
    // @step Given the SessionManager code has been moved into rust/sessions/src/session_manager.rs
    let sm = read(&moved_sm_path());
    assert!(sm.contains("pub struct SessionManager"));
    ...
    // @step Then I find zero matches in rust/sessions/src/session_manager.rs
    assert!(
        violations.is_empty(),
...
fn scenario_create_isolated_session_with_id_returns_the_wire_type_isolated_session_info_and_the_napi_wrapper_converts_to_isolated_session_result(
    ...
    let napi = read_napi_shell();
    // @step And the napi-side #[napi(object)] IsolatedSessionResult struct at rust/napi/src/session_manager.rs:5258-5267 stays in NAPI carrying the JS-bridge shape
    assert!(
        napi.contains("pub struct IsolatedSessionResult"),
        "rust/napi/src/session_manager.rs must still own `pub struct IsolatedSessionResult`"
    );
    assert!(
...
fn scenario_codelet_sessions_has_no_transitive_dependency_on_codelet_napi() {
    ...
    // @step And the existing dependency-rule test at rust/sessions/tests/skeleton_invariants.rs::scenario_codelet_sessions_has_no_transitive_dependency_on_codelet_napi still passes
...
{
    // @step Given the ChainOfCommand code has been moved into rust/sessions/src/chain_of_command.rs
    let coc = read(&moved_coc_path());
    assert!(coc.contains("pub struct ChainOfCommand"));

    // @step And rust/napi/src/session_manager.rs `pub use codelet_sessions::chain_of_command::ChainOfCommand;`
    let napi = read_napi_shell();
    assert!(
        napi.contains("pub use codelet_sessions::chain_of_command::ChainOfCommand")
            || napi.contains("pub use codelet_sessions::chain_of_command::"),
    ...
    // split rust/napi/src/session_manager.rs into seven siblings).
...
fn scenario_codelet_sessions_tests_assert_session_manager_is_reachable_from_its_new_home_and_the_hooks_design_is_sound(
    ...
    // @step And a new integration test rust/sessions/tests/session_manager_shape.rs has been added
...
fn scenario_the_napi_side_installs_its_hooks_at_startup_so_existing_ts_behaviour_is_preserved() {
    ...
    // @step And NapiSessionManagerHooks::spawn_agent_loop delegates to `tokio::spawn(async move { agent_loop(session, input_rx, mcp_injection_rx).await })` (where agent_loop is still the napi-side free function at rust/napi/src/session_manager.rs:3620)
    assert!(
    ...
    // @step And NapiSessionManagerHooks::spawn_footer_poller delegates to the existing napi-side spawn_footer_poller free function at rust/napi/src/session_manager.rs:5104
    assert!(
        combined.contains("fn spawn_footer_poller"),
        "NapiSessionManagerHooks must implement `spawn_footer_poller`"
    );

    // @step And NapiSessionManagerHooks::stop_footer_poller delegates to the existing napi-side stop_footer_poller free function at rust/napi/src/session_manager.rs:5223
    assert!(
...
fn scenario_session_manager_passes_broadcast_senders_to_background_session() {
    // @step Given the SessionManager code lives at rust/sessions/src/session_manager.rs
    let sm = read(&moved_sm_path());
...
fn scenario_session_manager_hooks_trait_no_longer_carries_emit_isolation_state_change() {
    // @step Given the SessionManagerHooks trait is defined in rust/sessions/src/session_manager.rs
    let sm = read(&moved_sm_path());
    let sm_code = strip_line_comments(&sm);
    ...
    // @step And the napi-side `NapiSessionManagerHooks` in rust/napi/src/session_manager.rs implements the same six methods only and no longer references `GLOBAL_CHUNK_CALLBACK`
    let napi = read_napi_shell();
...
fn scenario_session_set_global_chunk_callback_subscribes_to_chunks_tx_and_fans_into_tsfn() {
    // @step Given the napi free function `session_set_global_chunk_callback(callback: ThreadsafeFunction<GlobalChunkCallbackArgs>) -> Result<()>` exists at rust/napi/src/session_manager.rs
    let napi = read_napi_shell();
    assert!(
    ...
    assert!(
        ...
        "RPC-041: rust/napi/src/session_manager.rs must declare a CHUNK_FANOUT_TSFN static"
    );
    assert!(
    ...
    // @step And the public TS signature `sessionSetGlobalChunkCallback(callback: ...): void;` in rust/napi/index.d.ts is byte-stable
    let dts_path = workspace_root().join("napi").join("index.d.ts");
    if dts_path.exists() {
        let dts = read(&dts_path);
        assert!(
...
fn scenario_fspec_handler_and_command_emitter_use_helper_gate() {
    ...
    // @step When I inspect the rewritten closures in rust/napi/src/session_manager.rs
    ...
    assert!(
        ...
        "RPC-041: rust/napi/src/session_manager.rs must define and call is_global_chunk_callback_registered"
    );
    ...
    assert!(
        ...
        "RPC-041: rust/napi/src/session_manager.rs must declare `fn is_global_chunk_callback_registered() -> bool`"
    );
...
fn scenario_codelet_sessions_and_codelet_napi_continue_to_build_with_ts_surface_byte_stable() {
    ...
    if std::env::var_os("RPC_041_FULL_TESTS").is_none() {
        ...
        // @step Then `git diff rust/napi/index.d.ts` shows zero removed or renamed TypeScript exports
        let dts_path = workspace_root().join("napi").join("index.d.ts");
        if dts_path.exists() {
            let dts = read(&dts_path);
...
fn scenario_all_codelet_sessions_shape_tests_continue_to_pass_with_inverted_assertions() {
    // @step Given the existing shape-test files `rust/sessions/tests/background_session_shape.rs` and `rust/sessions/tests/session_manager_shape.rs` carry RPC-039/RPC-040 invariants
    let bg_shape = read(
        &workspace_root()
            .join("sessions")
            .join("tests")
    ...
    assert!(
        bg_shape.contains("RPC-041: rust/napi/src/session_manager.rs must no longer reference GLOBAL_CHUNK_CALLBACK"),
