...
//! that originally appeared at `rust/sessions/src/handle_impl.rs:1321`
...
/// Workspace root resolved from `CARGO_MANIFEST_DIR` (= `rust/sessions`).
...
fn handle_impl_has_no_redundant_id_clone() {
    // @step Given the source of `rust/sessions/src/handle_impl.rs`
    ...
    assert_eq!(
        occurrences, 0,
        "rust/sessions/src/handle_impl.rs must NOT contain \
...
fn sessions_cargo_toml_inherits_workspace_lints() {
    // @step Given the source of `rust/sessions/Cargo.toml`
    let src = read_source("sessions/Cargo.toml");

    // @step When I inspect the `[lints]` section
    assert!(
        src.contains("[lints]"),
        "rust/sessions/Cargo.toml must declare a `[lints]` section so \
    ...
    assert!(
        has_workspace_true,
        "rust/sessions/Cargo.toml `[lints]` section must inherit \
