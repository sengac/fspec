...
//!
//! Pattern mirrors `rust/sessions/tests/rpc075_scheduler_format_args_shape.rs`
//! (sibling card RPC-075 — same workspace-lint failure mode, different file).
//!
//! Before the RPC-076 fix, `rust/core/src/session_manager_handle.rs`
...
//! `rust/sessions/tests/skeleton_invariants.rs::scenario_workspace_lints_are_inherited_and_clippy_passes`
//! fail.
//!
...
fn session_manager_handle_use_block_does_not_import_notification_severity() {
    // @step Given the file `rust/core/src/session_manager_handle.rs` exists in the workspace
...
// test in rust/sessions/tests/ invokes
...
fn codelet_core_workspace_lints_pass_with_session_manager_handle_clean() {
    // @step Given the rust/Cargo.toml workspace lints declaration is inherited by codelet-core
    let core_manifest = workspace_root().join("core").join("Cargo.toml");
    assert!(
        core_manifest.exists(),
        "expected rust/core/Cargo.toml at {}",
        core_manifest.display()
    );
    let manifest_body = read(&core_manifest);
    assert!(
        manifest_body.contains("[lints]"),
        "rust/core/Cargo.toml must declare a [lints] section"
    );
    ...
    assert!(
        has_workspace_lints,
        "rust/core/Cargo.toml [lints] must inherit `workspace = true` \
