...
//! `rust/core/src/scheduler/{agent_job,shell_job}.rs`.
//!
//! Feature: spec/features/scheduler-format-args-clippy-compliance.feature
//!
//! Pattern mirrors the source-shape helpers from
//! `rust/sessions/tests/mcp_injection_source_shape.rs` (RPC-062) and
//! `rust/sessions/tests/rpc073_list_providers_wiring.rs` (RPC-073).
//!
...
fn scheduler_agent_job_uses_inline_format_args_exclusively() {
    // @step Given the file `rust/core/src/scheduler/agent_job.rs` exists in the workspace
...
fn scheduler_shell_job_uses_inline_format_args_exclusively() {
    // @step Given the file `rust/core/src/scheduler/shell_job.rs` exists in the workspace
