...
fn architecture_invariants_from_rpc_005_still_hold() {
    // @step Given the rust/rpc-embedded/tests/architecture_invariants.rs source-shape regression
    ...
    // @step And no tokio::runtime::Builder / Runtime::new() construction in rust/fspec/src/ or rust/rpc-server/src/
    // @step And no second envelope format exists (Envelope is the sole wire-format type in rust/rpc-server/src/envelope.rs)
    ...
    assert!(
        ...
        "rust/fspec/Cargo.toml must NOT depend on codelet-napi (rule [13])"
    );
