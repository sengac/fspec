...
| 15 | RPC-212, RPC-190, RPC-192, RPC-171, RPC-167, RPC-219, RPC-221, RPC-218, RPC-314, RPC-315 | 2026-06-14 | Feature-file (.feature) line-based mutation batch (10 commands): create-feature, add-scenario, add-step, add-background, add-architecture, delete-scenario, delete-step, delete-features, update-scenario, update-step. 4 workers (3/2/3/2) + 1 cargo serial worker (user constraint: 5 agents total, 1 reserved for cargo). All reuse `io::gherkin::parse_feature_lenient` + line-based split('\n')/slice/join (NOT AST round-trip); delete-scenario/update-scenario also touch `types::coverage` sidecars. Final: 133 passed / 0 failed across 22 targeted binaries (63 core + 70 CLI+guards); cargo_shape 11p/11ign, cross_frontend_parity 8/8. cargo_shape: allowed lock-list +10 (119→129), main_cap 2500→3000 (main.rs ~2780). Lessons: (a) **supervisor Edit-replace footgun on dispatch.rs** — when run_ported and run_stub both contain the SAME `"cmd" => commands::x::run(args_json, project_root)` line after a half-finished edit, Edit replaces the FIRST (run_ported) occurrence; verify with grep -n that run_ported keeps the real 2-arg arm and run_stub becomes a comment. (b) **delete-features has NO custom -help.ts** → bare Commander.js help; hard-code `DELETE_FEATURES_HELP` const in main.rs + special-case the intercept arm (like list-foundation-sections/register-tag), no help config module. (c) **`link-coverage` APPENDS** — a Phase-B test-only mapping + a Phase-C impl-only mapping do NOT satisfy the validating gate (which wants each scenario to have BOTH in one entry); workers must `unlink-coverage --scenario X --all` then ONE combined `link-coverage` with test-file+impl-file together. cli-subcommand scenarios map impl-file = the CLI bridge. (d) **retroactive state-walk** — supervisor walked specifying→testing→implementing→validating→done with `skipTemporalValidation:true` since all artifacts predated the walk. (e) **missing component/feature-group tags** — one worker's @wip-tagging didn't persist the required @cli/@file-ops + @feature-management tags; the add-tag-to-feature reminder catches it at the done gate — supervisor re-added them. (f) no-Feature-line input: `parse_feature_lenient` is stricter than @cucumber/gherkin and rejects comment-only files at parse time; add an explicit pre-parse `Feature:` check to emit the canonical `No Feature line found in file` (add-background/add-architecture). |

| 18 | RPC-225, RPC-232, RPC-309, RPC-323, RPC-276, RPC-198, RPC-326 + RPC-202, RPC-203, RPC-288 | 2026-06-16 | Event-storm / analysis / work-unit-status / git-checkpoint batch (10 commands): discover-event-storm, generate-example-mapping-from-event-storm, suggest-dependencies, validate-spec-alignment, remove-init-files, auto-advance, workflow-automation + the checkpoint trio (checkpoint, cleanup-checkpoints, restore-checkpoint). 4 workers (W1 checkpoints / W2 event-storm / W3 analysis / W4 status) + 1 cargo serial worker. Checkpoint trio wraps pre-existing `codelet_git::ghost_commit` primitives (create/restore/delete/count/list) + a fspec-core-owned `.git/fspec-checkpoints-index/<wu>.json`; IPC is a NO-OP in Rust. Final: all 10 core + 10 CLI binaries green; cargo_shape 11p/11ign, cross_frontend_parity 8/8. cargo_shape: lock-list 149->159, main_cap 3300->3500 (main.rs ~3340). Lessons: (a) **opus worker sessions stalled mid-task TWICE** (W1 mid-PHASE-B, replacement W1b mid-PHASE-C) - verify deliverables ON DISK (grep `project_root: &Path` / file presence) rather than trusting the report; nudge once with a tight file checklist; if still stalled, close + respawn with full context (prior on-disk artifacts are reusable). (b) **`await_idle` race** - message-then-immediately-await_idle on a just-idle session returns instantly before the worker consumes the queued message; re-await or check `pending_messages`. (c) **default-format contradiction in a worker's own tests** - cleanup-checkpoints had one no-format scenario asserting TEXT and another `parse_data`-ing JSON; fix = JSON-asserting scenarios pass `"format":"json"` (dispatcher default stays text). (d) **parity `.feature` substring guard false-positive** - the standard `//! Feature: ...<name>.feature` bridge doc header trips a `!bridge_src.contains(".feature")` guard; strip `//`-comment lines before scanning (Batch-10 lesson a class). (e) **partial-batch wiring is safe** - wiring the 7 finished commands while the checkpoint trio stayed 1-arg stubs kept the workspace compiling; wire each command's shared arms only after ALL sibling impls in the same crate have landed (a 2-arg rewrite breaks the old 1-arg run_stub arm). |
