    ...
    T: HttpClientExt,
{
    pub fn new(client: Client<T>, model: impl Into<String>) -> Self {
        ...
        Self {
            ...
            model,
            default_max_tokens,
            prompt_caching: true, // Always enable prompt caching for Claude
        }
    }
    ...
    /// Enable automatic prompt caching (enabled by default).
    ///
    ...
    /// Note: Prompt caching is now enabled by default. This method is kept for API compatibility.
...
struct AnthropicCompletionRequest {
    ...
    /// Metadata for OAuth requests (required for Claude Code OAuth)
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct AnthropicRequestParams<'a> {
    ...
    pub prompt_caching: bool,
    /// Whether OAuth mode is active (requires metadata.user_id)
    pub oauth_mode: bool,
}
...
impl TryFrom<AnthropicRequestParams<'_>> for AnthropicCompletionRequest {
    ...
    fn try_from(params: AnthropicRequestParams<'_>) -> Result<Self, Self::Error> {
        ...
        // Generate metadata for OAuth mode using ~/.claude.json values
        let metadata = if oauth_mode {
            Some(Metadata {
                user_id: Some(super::metadata::get_oauth_user_id()),
            })
        } else {
            None
        };

        // For OAuth mode, don't send temperature or tool_choice (Claude Code doesn't send them)
        let temperature = if oauth_mode { None } else { req.temperature };
        let tool_choice = if oauth_mode {
            None
        } else {
            req.tool_choice.and_then(|x| ToolChoice::try_from(x).ok())
        };
    ...
    T: HttpClientExt + Clone + Default + WasmCompatSend + WasmCompatSync + 'static,
{
    ...
    async fn completion(
        ...
        let request = AnthropicCompletionRequest::try_from(AnthropicRequestParams {
            ...
            oauth_mode: self.client.ext().oauth_mode,
        })?;
