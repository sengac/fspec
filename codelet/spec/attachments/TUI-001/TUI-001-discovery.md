...
- `codelet/fspec-tui/src/views/agent/header_build.rs::build_left_line`
...
- `codelet/fspec-tui/src/views/agent/header.rs` — layout / truncation.
...
FspecService::get_model_info (RPC)
  -> codelet/fspec-tui/src/transport/{embedded,websocket}.rs::get_model_info
  -> Action::ModelInfoLoaded(session_id, ModelInfo)
  -> codelet/fspec-tui/src/store/agent_view/chrome_state.rs::set_model_info
...
`codelet/sessions/src/handle_impl.rs` (~lines 870-889):
...
- The session carries `provider_id` and `model_id`
  (`codelet/sessions/src/background_session.rs` — `RwLock<Option<String>>`).
- `codelet_providers::models::ModelRegistry`
  (`codelet/providers/src/models/registry.rs`) exposes:
...
- `codelet/sessions/src/cloud_models.rs` **already performs this exact mapping**
...
1. `codelet_rpc_types::ModelInfo` (`codelet/rpc-types/src/lib.rs:285`) has **no
...
- `codelet/fspec-tui/tests/source_shape_rpc018.rs` — asserts `ModelInfo` shape &
  default impls. Adding a field requires updating any literal `ModelInfo { .. }`
  constructions in tests across `codelet/fspec-tui/tests/**` and
  `codelet/sessions/**` (search for `ModelInfo {`).
...
| Purpose | Path |
|---|---|
| Stub to fix (server) | `codelet/sessions/src/handle_impl.rs` (`get_model_info`) |
| Registry lookup helper to reuse | `codelet/sessions/src/cloud_models.rs` |
| Model registry API | `codelet/providers/src/models/registry.rs` |
| Wire struct (add field) | `codelet/rpc-types/src/lib.rs` (`ModelInfo`) |
| Size-badge selection | `codelet/fspec-tui/src/views/agent/header_build.rs` |
| Renderer (reference only) | `codelet/fspec-tui/src/views/agent/header.rs` |
| Shape tests | `codelet/fspec-tui/tests/source_shape_rpc018.rs` + ModelInfo literals |
