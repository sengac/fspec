//! `remove-command-from-foundation` — Rust port of
//! `src/commands/remove-command-from-foundation.ts` (RPC-270).
//!
//! Soft-deletes a `command` Event Storm item (sets `deleted: true`) within
//! a named `bounded_context` in the foundation's Big Picture
//! `eventStorm.items` array. Mirrors the TS `removeCommandFromFoundation`
//! helper.
//!
...
//! 4. Find the non-deleted `command` whose `text` equals `commandName`
//!    AND whose `boundedContextId` equals the matched context's `id`.
//!    Missing → `"Command '{cmd}' not found in bounded context '{ctx}'"`.
//! 5. Set that command's `deleted` to `true`, write atomically.
//!
...
//! `codelet/fspec/src/remove_command_from_foundation.rs` is JSON
//! marshalling only.
...
/// CLI arguments accepted by `remove-command-from-foundation`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct RemoveCommandFromFoundationArgs {
    context_name: String,
    command_name: String,
}
...
pub async fn run(args_json: &str, project_root: &Path) -> Result<String, FspecCoreError> {
    let args: RemoveCommandFromFoundationArgs =
    ...
    // [4] Find the matching, non-deleted command within that context.
    let command_index = items.iter().position(|i| {
        i.get("type").and_then(Value::as_str) == Some("command")
            && i.get("text").and_then(Value::as_str) == Some(args.command_name.as_str())
            && !i.get("deleted").and_then(Value::as_bool).unwrap_or(false)
    ...
    let command_index = match command_index {
        ...
        None => {
            return Err(FspecCoreError::InvalidArgs {
                command: "remove-command-from-foundation",
                reason: format!(
                    "Command '{}' not found in bounded context '{}'",
    ...
    // [5] Soft-delete the matched command.
    if let Some(arr) = es.get_mut("items").and_then(Value::as_array_mut) {
        if let Some(obj) = arr.get_mut(command_index).and_then(Value::as_object_mut) {
    ...
    let message = format!(
        "Removed command \"{}\" from \"{}\" bounded context",
        args.command_name, args.context_name
    );
...
/// `removeCommandFromFoundation` helper
/// (`src/commands/remove-command-from-foundation.ts` lines 34-45). DELIBERATELY
...
mod tests {
    ...
    fn args_parse_minimal() {
        let a: RemoveCommandFromFoundationArgs = serde_json::from_str(
