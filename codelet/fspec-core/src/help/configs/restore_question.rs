//! `restore-question` help configuration — Rust port of the TS Commander.js
//! help output captured via `node dist/index.js restore-question --help`.
//!
//! Mirrors `src/commands/restore-question-help.ts`.
...
const ARGS: &[CommandArgument] = &[
    ...
    CommandArgument {
        name: "index",
        description: "Question ID (stable index from show-work-unit or show-deleted)",
        required: true,
    },
];
...
const OPTS: &[CommandOption] = &[CommandOption {
    flag: "--ids <ids>",
    description: "Restore multiple questions with comma-separated IDs (e.g., \"2,5,7\")",
    default_value: None,
}];

const EXAMPLES: &[CommandExample] = &[
    CommandExample {
        description: Some("Restore deleted question with ID 2"),
    ...
    CommandExample {
        description: Some("Restore multiple questions at once (bulk restore)"),
        command: "fspec restore-question AUTH-001 --ids 2,5,7",
        output: Some("✓ Restored 3 questions from AUTH-001"),
    },
];
...
pub const CONFIG: CommandHelpConfig = CommandHelpConfig {
    ...
    usage: Some("fspec restore-question <workUnitId> <index>"),
    arguments: ARGS,
    options: OPTS,
    examples: EXAMPLES,
