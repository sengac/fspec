...
fn run_ported(
    ...
    Some(poll_sync_future(async move {
        match name {
            ...
            "validate-tags" => commands::validate_tags::run(args_json, project_root).await,
            "validate-work-units" => {
                commands::validate_work_units::run(args_json, project_root).await
            }
            "validate-hooks" => commands::validate_hooks::run(args_json, project_root).await,
            "validate-foundation-schema" => {
                commands::validate_foundation_schema::run(args_json, project_root).await
            }
            "validate" => commands::validate::run(args_json, project_root).await,
            "search-scenarios" => commands::search_scenarios::run(args_json, project_root).await,
            "search-implementation" => {
                commands::search_implementation::run(args_json, project_root).await
            }
            ...
            "discover-event-storm" => {
                commands::discover_event_storm::run(args_json, project_root).await
            }
            "generate-example-mapping-from-event-storm" => {
                commands::generate_example_mapping_from_event_storm::run(args_json, project_root)
                    .await
            }
            "suggest-dependencies" => {
                commands::suggest_dependencies::run(args_json, project_root).await
            }
            ...
            "checkpoint" => commands::checkpoint::run(args_json, project_root).await,
            "cleanup-checkpoints" => {
                commands::cleanup_checkpoints::run(args_json, project_root).await
            }
            "restore-checkpoint" => {
