...
fn fresh_discovery_creates_draft_with_placeholders_and_first_field_reminder() {
    ...
    assert_eq!(data["valid"].as_bool(), Some(true), "valid must be true: {data}");
    ...
    assert!(draft.contains("\"2.0.0\""), "draft must contain version: {draft}");

    // @step And the draft on disk contains the placeholder "[QUESTION: What is the project name?]"
    assert!(draft.contains("[QUESTION: What is the project name?]"), "missing name placeholder");

    // @step And the draft on disk contains the placeholder "[DETECTED: cli-tool]"
    assert!(draft.contains("[DETECTED: cli-tool]"), "missing detected placeholder");
    ...
    assert!(reminder.contains("Field 1/8: project.name"), "reminder: {reminder}");
}
...
fn rerun_without_force_when_draft_exists_is_blocked() {
    ...
    let data: Value = serde_json::from_str(&result.data).expect("parse data json");
    assert_eq!(data["valid"].as_bool(), Some(false), "valid must be false: {data}");
...
fn running_without_force_when_foundation_exists_is_blocked() {
    ...
    let reminder = data["systemReminder"].as_str().unwrap_or_default();
    assert!(reminder.contains("ERROR: foundation.json already exists!"), "reminder: {reminder}");
...
fn force_overwrite_regenerates_draft_from_scratch_with_warning() {
    ...
    let tmp = TempDir::new().expect("tempdir");
    write_draft_raw(tmp.path(), "{\"version\":\"2.0.0\",\"project\":{\"name\":\"custom\"}}");
    ...
    assert!(draft.contains("[QUESTION: What is the project name?]"), "draft regenerated: {draft}");

    // @step And the returned systemReminder contains a force-overwrite warning
    let reminder = data["systemReminder"].as_str().unwrap_or_default();
    assert!(reminder.contains("overwritten with --force"), "reminder: {reminder}");
...
fn finalize_blocked_when_draft_still_has_placeholder_fields() {
    ...
    assert!(!tmp.path().join("spec/foundation.json").exists(), "foundation.json must not be written");

    // @step And spec/foundation.json.draft still exists on disk
    assert!(tmp.path().join("spec/foundation.json.draft").exists(), "draft must remain");
}
...
fn finalize_blocked_when_filled_draft_violates_schema() {
    ...
    let errors = data["validationErrors"].as_str().unwrap_or_default();
    assert!(errors.starts_with("Schema validation failed."), "validationErrors: {errors}");
...
fn finalize_success_writes_foundation_deletes_draft_and_creates_found_unit() {
    ...
    let result =
        dispatch_command(req(tmp.path(), json!({"finalize": true, "autoGenerateMd": true})));
    ...
    assert!(msg.contains("Discovery complete!"), "completionMessage: {msg}");

    // @step And spec/foundation.json exists on disk
    assert!(tmp.path().join("spec/foundation.json").exists(), "foundation.json must be written");
    ...
    assert!(tmp.path().join("spec/FOUNDATION.md").exists(), "FOUNDATION.md must be generated");
    ...
    assert!(wu.contains("FOUND-"), "work-units.json must contain a FOUND- unit: {wu}");

    // @step And the envelope reports workUnitCreated=true with the FOUND-001 id (parity with the TS CLI "Created work unit" lines)
    assert_eq!(data["workUnitCreated"].as_bool(), Some(true), "workUnitCreated: {data}");
    assert_eq!(data["workUnitId"].as_str(), Some("FOUND-001"), "workUnitId: {data}");
    ...
    assert_eq!(task["type"].as_str(), Some("task"), "FOUND task type: {task}");
    assert!(
    ...
    assert!(task.get("children").is_none(), "FOUND task must NOT have a children array: {task}");
    assert!(
...
fn finalize_found_autocreation_is_idempotent_when_found_unit_already_exists() {
    ...
    fs::write(spec.join("work-units.json"), serde_json::to_string_pretty(&seed).unwrap())
    ...
    let found_count = wu["workUnits"]
        ...
        .unwrap_or(0);
    assert_eq!(found_count, 1, "expected exactly one FOUND- unit; got {found_count}");

    // @step And the envelope reports workUnitCreated=false reusing the existing FOUND-001 id
    assert_eq!(data["workUnitCreated"].as_bool(), Some(false), "workUnitCreated: {data}");
    assert_eq!(data["workUnitId"].as_str(), Some("FOUND-001"), "workUnitId: {data}");
}
