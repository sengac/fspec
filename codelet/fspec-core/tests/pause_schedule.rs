...
// port at `codelet/fspec-core/src/commands/pause_schedule.rs` lands. These
...
fn scenario_pause_already_paused_reports_already_paused() {
    ...
    assert!(!result.success, "expected failure for already-paused schedule, got {result:?}");
    let err = result.error.unwrap_or_default();
    assert!(
...
fn scenario_pause_one_of_several_preserves_others_verbatim() {
  ...
  "schedules": {
    ...
    assert_eq!(after["schedules"]["beta"]["status"].as_str(), Some("paused"));
    assert_eq!(after["schedules"]["alpha"]["status"].as_str(), Some("active"));
