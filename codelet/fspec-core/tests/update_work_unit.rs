...
fn dispatcher_is_the_single_source_of_truth_for_update_work_unit() {
    ...
    // @step And I run `./codelet/target/release/fspec update-work-unit AUTH-001 --title Same` in an identical workspace
    // (The CLI path is exercised by codelet/fspec/tests/cli_update_work_unit.rs; here we assert the
