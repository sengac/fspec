...
async fn dispatch_command_returns_not_yet_ported_when_called_from_inside_a_tokio_runtime() {
    // @step Given the canonical command map registers 'auto-advance' as a Phase 1 stub
    // (precondition: 'auto-advance' is in CANONICAL_COMMANDS and is_ported('auto-advance') == false;
    //  verified in dispatcher_test.rs. We use auto-advance instead of add-rule/add-architecture/audit-coverage
    ...
    // @step When I invoke dispatch_command for the 'auto-advance' command via tokio::task::spawn_blocking
    let started = std::time::Instant::now();
