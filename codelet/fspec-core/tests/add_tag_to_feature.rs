...
fn scenario_adds_multiple_tags_in_a_single_call_preserving_order() {
    ...
    let crit = lines.iter().position(|l| *l == "@critical").expect("@critical line present");
    assert_eq!(lines.get(crit + 1).copied(), Some("@auth"));
    let feat = lines.iter().position(|l| *l == "Feature: Login").expect("Feature line present");
...
fn scenario_appends_new_tag_after_existing_tag_block() {
    ...
    let sec = lines.iter().position(|l| *l == "@security").expect("@security line");
    ...
    assert_eq!(sec + 1, feat, "@security must remain immediately above Feature: Login");
}
