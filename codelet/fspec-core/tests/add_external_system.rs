...
// Feature: spec/features/add-external-system-rust-port.feature
//
// Validates the acceptance criteria for the Rust port of `add-external-system`
// (RPC-182). Each scenario maps to one #[test] fn with @step comments mirroring
...
fn first_add_seeds_event_storm_on_clean_unit() {
    ...
    // @step When I dispatch add-external-system with workUnitId='AUTH-001' and text='Payment Gateway'
    let result = dispatch_command(req(
        tmp.path(),
        json!({"workUnitId": "AUTH-001", "text": "Payment Gateway"}),
    ));

    // @step Then the dispatcher returns success=true
    assert!(result.success, "expected success=true, got {result:?}");

    // @step And the returned data contains externalSystemId=0
    let data: Value = serde_json::from_str(&result.data).expect("parse data json");
    assert_eq!(data["externalSystemId"].as_u64(), Some(0));
    ...
    // @step And spec/work-units.json on disk shows AUTH-001.eventStorm.items[0] has type='external_system', color='pink', text='Payment Gateway', id=0, deleted=false
    let i0 = &es["items"][0];
    assert_eq!(i0["type"].as_str(), Some("external_system"));
    assert_eq!(i0["color"].as_str(), Some("pink"));
    assert_eq!(i0["text"].as_str(), Some("Payment Gateway"));
...
#[test]
fn type_option_maps_to_integration_type_field() {
    // @step Given a project root tempdir with spec/work-units.json containing AUTH-001 status=specifying and no eventStorm field
    let tmp = TempDir::new().expect("tempdir");
    write_work_units(tmp.path(), &seed_unit("AUTH-001", "specifying"));

    // @step When I dispatch add-external-system with workUnitId='AUTH-001', text='Stripe API', type='REST_API'
    let result = dispatch_command(req(
        tmp.path(),
        json!({"workUnitId": "AUTH-001", "text": "Stripe API", "type": "REST_API"}),
    ));
...
#[test]
fn optional_timestamp_and_bounded_context_in_ts_insertion_order() {
    // @step Given a project root tempdir with spec/work-units.json containing AUTH-001 status=specifying and no eventStorm field
    let tmp = TempDir::new().expect("tempdir");
    write_work_units(tmp.path(), &seed_unit("AUTH-001", "specifying"));

    // @step When I dispatch add-external-system with workUnitId='AUTH-001', text='User Database', type='DATABASE', timestamp=1000, boundedContext='Identity'
    let result = dispatch_command(req(
        tmp.path(),
        json!({
            "workUnitId": "AUTH-001",
    ...
    assert_eq!(i0["boundedContext"].as_str(), Some("Identity"));

    // @step And the items[0] JSON key order is type, color, text, integrationType, timestamp, boundedContext, id, deleted, createdAt
    let raw = read_work_units_raw(tmp.path());
    let expected_order = [
        "\"type\"", "\"color\"", "\"text\"", "\"integrationType\"", "\"timestamp\"",
...
fn second_add_increments_next_item_id_and_preserves_order() {
    // @step Given a project root tempdir with spec/work-units.json containing AUTH-001 status=specifying with an existing eventStorm external_system id=0 and nextItemId=1
    let tmp = TempDir::new().expect("tempdir");
    ...
    // @step When I dispatch add-external-system with workUnitId='AUTH-001' and text='RabbitMQ'
    let result = dispatch_command(req(
        tmp.path(),
        json!({"workUnitId": "AUTH-001", "text": "RabbitMQ"}),
    ));

    // @step Then the dispatcher returns success=true
    assert!(result.success, "expected success=true, got {result:?}");

    // @step And the returned data contains externalSystemId=1
    let data: Value = serde_json::from_str(&result.data).expect("parse data json");
    assert_eq!(data["externalSystemId"].as_u64(), Some(1));
    ...
    // @step And spec/work-units.json on disk shows AUTH-001.eventStorm.items[1] has id=1 and text='RabbitMQ'
    let i1 = &es["items"][1];
    assert_eq!(i1["id"].as_u64(), Some(1));
    assert_eq!(i1["text"].as_str(), Some("RabbitMQ"));
}
...
fn missing_work_unit_surfaces_canonical_not_found_error() {
    ...
    // @step When I dispatch add-external-system with workUnitId='NOPE-001' and text='Anything'
    let result = dispatch_command(req(
        tmp.path(),
...
fn done_state_is_rejected_verbatim() {
    ...
    // @step When I dispatch add-external-system with workUnitId='AUTH-001' and text='Anything'
    let result = dispatch_command(req(
        tmp.path(),
