//! Job trigger and persistence — extracted from engine.rs
//!
...
pub async fn trigger_and_update(
    ...
    // Log "triggered" event
    let session_id_for_log = find_scheduled_session_id(name).await.map(|s| s.to_string());
    ...
    let status = match job_type {
        "agent" => {
            let result = trigger_agent_job(name, project_path, entry).await;
            // If agent job succeeded, record the active run for overlap detection
            if result.is_ok() {
                if let Some(sid) = find_scheduled_session_id(name).await {
    ...
    let result_entry = JobLogEntry {
        ...
        schedule: name.to_string(),
        job_type: job_type.to_string(),
        session_id: find_scheduled_session_id(name).await.map(|s| s.to_string()),
...
/// SCHED-004: Trigger an agent job by resolving model and delegating
/// to the agent_job module.
async fn trigger_agent_job(
    name: &str,
    project_path: &str,
    entry: &ScheduleEntry,
) -> Result<(), anyhow::Error> {
