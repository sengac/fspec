...
impl SessionManager {
    ...
    pub async fn create_session_with_id(&self, id: &str, model: &str, project: &str, name: &str) -> Result<()> {
        ...
        // TUI-091: Spawn background footer state poller (CWD + git status)
    ...
    pub async fn create_isolated_session_with_id(
        ...
        // TUI-091: Spawn background footer state poller for isolated session (uses worktree CWD)
        spawn_footer_poller(
            id.to_string(),
    ...
    pub fn destroy_session(&self, id: &str) -> Result<()> {
        ...
        if let Some(session) = session {
            ...
            session.interrupt();
            // TUI-091: Stop footer poller for this session
            stop_footer_poller(id);
            ...
            // TUI-091: Remove per-session footer CWD entry to prevent memory leaks
...
// =============================================================================
// TUI-091: Background Footer State Poller
// =============================================================================

use std::sync::Mutex as StdMutex;

/// Per-session cancellation flags for footer pollers.
static FOOTER_POLLER_TOKENS: once_cell::sync::Lazy<StdMutex<std::collections::HashMap<String, Arc<AtomicBool>>>> =
...
fn spawn_footer_poller(session_id: String, cwd: String, worktree_path: Option<String>) {
    let cancelled = Arc::new(AtomicBool::new(false));
    let cancelled_clone = cancelled.clone();

    {
        let mut map = FOOTER_POLLER_TOKENS.lock().expect("footer poller tokens lock poisoned");
            ...
            if let Ok((is_git, branch)) = git_result {
                    ...
                    || branch != prev_branch
                {
                    ...
                    if let Some(global_cb) = GLOBAL_CHUNK_CALLBACK.get() {
                        ...
                        global_cb.call(sid.clone(), chunk);
                    }
                }
            }
...
fn stop_footer_poller(session_id: &str) {
    let mut map = FOOTER_POLLER_TOKENS.lock().expect("footer poller tokens lock poisoned");
