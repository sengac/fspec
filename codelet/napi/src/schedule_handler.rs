...
pub fn create_handler(project: String) -> ScheduleHandler {
    Arc::new(move |request: ScheduleRequest| -> ScheduleResult {
        match request.action.as_str() {
            ...
            "pause" => handle_pause(&project, &request),
            "resume" => handle_resume(&project, &request),
            "remove" => handle_remove(&project, &request),
...
fn write_schedules_file(project: &str, data: &Value) -> Result<(), String> {
    ...
    std::fs::write(&path, content).map_err(|e| format!("Failed to write schedules.json: {e}"))
}
...
fn handle_add(project: &str, request: &ScheduleRequest) -> ScheduleResult {
    ...
    let name = match &request.name {
        ...
        _ => return ScheduleResult::error("Schedule name is required"),
    };
    let cron = match &request.cron {
        Some(c) if !c.is_empty() => c.clone(),
        _ => return ScheduleResult::error("Cron expression is required"),
    };
    let timezone = match &request.timezone {
        Some(tz) if !tz.is_empty() => tz.clone(),
        _ => return ScheduleResult::error("Timezone is required"),
    };
    let job_type = match &request.job_type {
        Some(jt) if !jt.is_empty() => jt.clone(),
        _ => return ScheduleResult::error("Job type is required"),
    };
