//! Session persistence helpers (RPC-043, originally REFAC-007).
//!
//! Extracted from `codelet/napi/src/session_manager.rs` lines 1865-2127
//! by RPC-043. These helpers persist messages, tool results, token state,
//! and structural annotations to the on-disk session manifest. They are
//! `pub(crate)` because their consumers are the napi-side `agent_loop`
//! and a few `#[napi]` wrappers (`session_restore_messages`,
...
pub(crate) fn persist_assistant_message_internal(
    session_id: &uuid::Uuid,
    provider: &str,
...
pub(crate) fn persist_tool_result_internal(
    session_id: &uuid::Uuid,
    tool_call_id: &str,
    content: &str,
    is_error: bool,
...
pub(crate) fn persist_pending_annotations(
    session_id: &uuid::Uuid,
