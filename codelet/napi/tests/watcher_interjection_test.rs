...
/// Session role (matching session_manager.rs, updated WATCH-024)
...
impl SupervisorRole {
    fn new(name: String, brief: Option<String>) -> Self {
        Self { name, brief, auto_inject: true }
    }
...
mod interjection_parsing_tests {
    ...
    fn test_watcher_autonomously_injects_urgent_security_warning() {
        // @step Given a Security Reviewer watcher with auto-inject enabled is observing a parent session
        ...
        // @step Then supervisor_inject should be called automatically with the content
    ...
    fn test_watcher_continues_without_injection_when_no_issues_found() {
        // @step Given an Architecture Advisor watcher with brief is observing a parent session
    ...
    fn test_evaluation_prompt_includes_proper_format_instructions() {
        ...
        // @step Then the prompt should include role context and brief
    ...
    fn test_auto_inject_disabled_shows_pending_injection_without_injecting() {
        ...
        // When auto_inject = false, even if interjection is parsed, we don't call supervisor_inject
        let interjection = parse_interjection(response);
        assert!(interjection.is_some());
        // When auto_inject is false, caller should NOT call supervisor_inject
        
        // @step And the watcher UI should show the pending injection for manual review
        // This is verified by the supervisor_agent_loop emitting SupervisorPendingInjection chunk
    }
    ...
    fn test_direct_user_prompts_are_not_parsed_for_injection() {
        ...
        // This is handled in supervisor_agent_loop, not in parse_interjection itself
