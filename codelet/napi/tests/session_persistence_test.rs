...
fn test_user_message_persisted_with_envelope() {
    ...
    let mut session = create_session("User Message Test", &project)
...
fn test_assistant_tool_use_persisted_before_execution() {
    ...
    let mut session = create_session("Tool Use Test", &project)
        .expect("create_session should succeed");
...
fn test_tool_result_persisted_on_completion() {
    ...
    let mut session = create_session("Tool Result Test", &project)
        .expect("create_session should succeed");
    ...
    append_message_with_metadata(&mut session, "assistant", "I'll read the config.", assistant_meta)
        .expect("append assistant should succeed");

    // @step When the tool execution completes with result content
    let tool_result_meta = create_tool_result_envelope(
        tool_id,
    ...
    let content = tool_msg.metadata.get("message")
        .and_then(|m| m.get("content"))
        .and_then(|c| c.as_array())
        .expect("Should have content array");
    assert_eq!(content[0].get("tool_use_id").and_then(|t| t.as_str()), Some(tool_id));
}
...
fn test_complete_tool_flow_all_messages_persisted() {
    ...
    let mut session = create_session("Complete Flow Test", &project)
        .expect("create_session should succeed");
    ...
    append_message_with_metadata(&mut session, "assistant", "I'll read that file.", assistant_meta)
        .expect("append assistant should succeed");
...
fn test_session_resume_contains_all_messages() {
    ...
    let mut session = create_session("Resume All Test", &project)
        .expect("create_session should succeed");
    ...
    append_message_with_metadata(&mut session, "assistant", "I'll list the files.", assistant_meta).unwrap();
    
    let tool_result_meta = create_tool_result_envelope(tool_id, "file1.txt\nfile2.txt", false);
    append_message_with_metadata(&mut session, "user", "file1.txt\nfile2.txt", tool_result_meta).unwrap();
    
    let final_meta = create_assistant_text_envelope("The directory contains file1.txt and file2.txt.");
    append_message_with_metadata(&mut session, "assistant", "The directory contains file1.txt and file2.txt.", final_meta).unwrap();
    ...
    for msg in &messages {
        assert!(!msg.content.is_empty(), "No message should have empty content");
    }
}
...
fn test_multiple_tool_uses_in_single_response() {
    ...
    let mut session = create_session("Multi Tool Test", &project)
        .expect("create_session should succeed");
    
    let user_meta = create_user_envelope("Read file A and file B");
    append_message_with_metadata(&mut session, "user", "Read file A and file B", user_meta).unwrap();
    ...
    append_message_with_metadata(&mut session, "assistant", "I'll read both files.", assistant_meta).unwrap();
    ...
    let final_meta = create_assistant_text_envelope("File A contains 'Contents of A' and file B contains 'Contents of B'.");
    ...
    assert_eq!(messages.len(), 5, "Should have 5 messages: user, assistant, tool_result x2, final assistant");
    assert_eq!(messages[0].role, "user");
    assert_eq!(messages[1].role, "assistant");
...
fn test_sequential_tool_calls_all_persisted() {
    ...
    let mut session = create_session("Sequential Tools Test", &project)
        .expect("create_session should succeed");
    ...
    append_message_with_metadata(&mut session, "assistant", "I'll read file A first.", assistant_meta1).unwrap();
    ...
    append_message_with_metadata(&mut session, "assistant", "Now I'll read file B.", assistant_meta2).unwrap();
    ...
    let final_meta = create_assistant_text_envelope("File A contains 'Contents of A' and file B contains 'Contents of B'.");
    append_message_with_metadata(&mut session, "assistant", "File A contains...", final_meta).unwrap();
    ...
    assert_eq!(messages.len(), 6, "Should have 6 messages for sequential tool calls");
    assert_eq!(messages[0].role, "user");
...
fn test_token_state_persisted_and_restored() {
    ...
    let mut session = create_session("Token State Test", &project)
        .expect("create_session should succeed");
...
fn test_compaction_state_persisted_and_restored() {
    ...
    let mut session = create_session("Compaction State Test", &project)
        .expect("create_session should succeed");
...
fn test_resumed_session_restores_compaction_summary() {
    ...
    let mut session = create_session("Compaction Resume Test", &project)
        .expect("create_session should succeed");
    ...
    assert_eq!(messages[0].id, uuid::Uuid::nil(), "First message should be synthetic (nil UUID)");
    assert!(messages[0].content.contains("auth flow"), "Summary should contain the compaction text");
    ...
    assert_eq!(messages.len(), 3, "Should have summary + 2 post-compaction messages");
...
fn test_resumed_session_has_accurate_token_counts() {
    ...
    let mut session = create_session("Token Resume Test", &project)
        .expect("create_session should succeed");
    
    let user_meta = create_user_envelope("Test message for tokens");
    append_message_with_metadata(&mut session, "user", "Test message for tokens", user_meta).unwrap();
    
    let assistant_meta = create_assistant_text_envelope("Here is my response");
    append_message_with_metadata(&mut session, "assistant", "Here is my response", assistant_meta).unwrap();
    ...
    assert_eq!(messages.len(), 2, "Messages should be restored along with token state");
}
...
fn test_concurrent_persistence_serialized() {
    ...
    let mut session = create_session("Concurrent Test", &project)
        .expect("create_session should succeed");
    ...
    for (i, msg) in messages.iter().enumerate() {
        assert!(
            ...
            "Message {} should be at index {}", i, i
        );
    }
}
...
fn test_compaction_boundary_validation() {
    ...
    let mut session = create_session("Boundary Test", &project)
        .expect("create_session should succeed");
    ...
    assert_eq!(reloaded.compaction.as_ref().unwrap().compacted_before_index, 3);
}
...
fn test_session_manifest_integrity() {
    ...
    let mut session = create_session("Integrity Test", &project)
        .expect("create_session should succeed");
    
    append_message(&mut session, "user", "Test message")
        .expect("append should succeed");
...
fn test_message_metadata_preserved_across_reload() {
    ...
    let mut session = create_session("Metadata Test", &project)
        .expect("create_session should succeed");
    
    let metadata = create_user_envelope("Test with metadata");
    let original_uuid = metadata.get("uuid").and_then(|v| v.as_str()).unwrap().to_string();
    let original_timestamp = metadata.get("timestamp").and_then(|v| v.as_str()).unwrap().to_string();
...
fn test_empty_message_content_handled() {
    ...
    let mut session = create_session("Empty Content Test", &project)
        .expect("create_session should succeed");
...
fn test_tool_result_error_flag_persisted() {
    ...
    let mut session = create_session("Tool Error Test", &project)
        .expect("create_session should succeed");
    ...
    append_message_with_metadata(&mut session, "assistant", "Running command...", assistant_meta).unwrap();
    ...
    let content = tool_result.metadata.get("message")
        .and_then(|m| m.get("content"))
        .and_then(|c| c.as_array())
        .expect("Should have content array");
...
fn test_large_session_maintains_message_order() {
    ...
    let mut session = create_session("Large Session Test", &project)
        .expect("create_session should succeed");
    ...
    for (i, msg) in messages.iter().enumerate() {
        ...
        assert_eq!(msg.role, expected_role, "Message {} should have role {}", i, expected_role);
        assert!(
            msg.content.contains(&format!("Message {}", i)) ||
            msg.content.contains(&format!("message {}", i)),
            "Message {} should contain its index", i
        );
    }
}
...
fn test_compaction_at_message_count_boundary() {
    ...
    let mut session = create_session("Compaction End Test", &project)
        .expect("create_session should succeed");
    ...
    // @step Then the compaction should succeed
    assert!(result.is_ok(), "Compaction at message count should be valid");
    ...
    assert_eq!(messages[0].id, uuid::Uuid::nil(), "Should be synthetic message");
}
...
fn test_api_error_midstream_persists_accumulated_content() {
    ...
    let mut session = create_session("API Error Test", &project)
        .expect("create_session should succeed");
    ...
    assert_eq!(messages.len(), 2, "Both user and partial assistant messages should be persisted");
    assert_eq!(messages[1].role, "assistant");
    ...
    assert!(
        ...
        "Partial assistant content should be preserved: {}", messages[1].content
    );
}
...
fn test_user_interrupt_preserves_accumulated_content() {
    ...
    // @step Given a NAPI session with an active streaming response
    let mut session = create_session("Interrupt Test", &project)
        .expect("create_session should succeed");
    ...
    assert_eq!(messages.len(), 2, "Both user and interrupted assistant messages should be persisted");
    assert_eq!(messages[1].role, "assistant");
...
fn test_persistence_failure_propagates_error() {
    ...
    assert!(load_result.is_err(), "Loading non-existent session should fail");
    ...
    assert!(second_load.is_err(), "Second load should also fail consistently");
}
