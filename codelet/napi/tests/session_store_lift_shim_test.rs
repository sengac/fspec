...
fn napi_reexport_paths_route_through_core_for_session_types() {
    // @step Given codelet/napi/src/persistence/storage.rs is deleted and codelet/napi/src/persistence/types.rs is deleted
    ...
    update_session_tokens(&mut session, 42, 21, 0, 0).expect("update_session_tokens via NAPI alias");
