...
pub(super) fn paint_scrollbar(
    ...
    let thumb_height = std::cmp::max(
        1,
        vh.saturating_mul(scrollbar_height) / total_rows,
    );
...
mod tests {
    ...
    fn scrollbar_uses_ts_thumb_and_track_glyphs() {
        ...
        let state = ScrollState { offset: 0, stick_to_bottom: false };
    ...
    fn thumb_pinned_to_bottom_when_sticking() {
        ...
        let area = Rect { x: 0, y: 0, width: 10, height: 10 };
        let state = ScrollState { offset: 0, stick_to_bottom: true };
        let col = run(area, 10, 20, state);
        let glyphs: Vec<&str> = col.iter().map(|(g, _)| g.as_str()).collect();
        assert_eq!(
            glyphs,
    ...
    fn thumb_height_floor_min_one_for_huge_content() {
        ...
        let thumb_count = col
            ...
            .filter(|(g, _)| g == "\u{25A0}")
            .count();
        assert_eq!(thumb_count, 1, "thumb height must clamp to 1 for huge content");
    }
    ...
    fn scrollbar_paints_into_rightmost_column_with_offset_origin() {
        ...
        let area = Rect { x: 5, y: 7, width: 10, height: 4 };
        let mut buf = Buffer::empty(area);
        let state = ScrollState { offset: 0, stick_to_bottom: false };
