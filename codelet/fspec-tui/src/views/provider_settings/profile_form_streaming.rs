...
/// Index of the Streaming field in `PROFILE_FORM_FIELDS` (the 6th, last entry).
pub(super) const STREAMING_FIELD_INDEX: usize = 5;

/// True when `field_index` targets the boolean Streaming field.
...
///
/// While the Streaming field is focused the field is a boolean toggle:
/// Space/Left/Right flip it and every other key (including printable chars) is
/// swallowed so no text is appended. Otherwise the key edits the focused text
...
/// Flip the Streaming toggle on Space/Left/Right; ignore all other keys.
