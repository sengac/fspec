...
fn no_raw_fspec_dialog_literals_outside_render_methods_in_non_test_code() {
    // @step Given the codelet/fspec-tui crate after RPC-079 implementation completes
    use std::fs;
    ...
    // @step When a grep search for "FspecDialog {" is run across codelet/fspec-tui/src/ excluding the components/ directory and excluding all #[cfg(test)] blocks
    ...
    // @step When the same search is run inside codelet/fspec-tui/src/components/
