...
//!   - `codelet/fspec-tui/Cargo.toml [dependencies]` now lists `tui-input`
//!     alongside the existing RPC-008 entries.
//!   - `codelet/Cargo.toml [workspace.dependencies]` declares
//!     `tui-input = "0.10"`.
//!   - Production `[dependencies]` of codelet/fspec-tui still excludes
//!     `codelet-napi` and `codelet-core`.
//!   - `[dev-dependencies]` of codelet/fspec-tui still excludes
...
fn workspace_cargo_toml_declares_tui_input_0_10() {
    // @step Given codelet/Cargo.toml exists
...
fn new_src_views_files_do_not_directly_import_the_encapsulated_transport_crates() {
    // @step Given the source files codelet/fspec-tui/src/views/*.rs exist
    let views_dir = common::workspace_root()
        .join("fspec-tui")
        .join("src")
        .join("views");
