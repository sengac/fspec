...
fn mouse_module_exists_with_the_expected_files() {
    ...
    // @step Then the file codelet/fspec-tui/src/mouse/mod.rs exists
    assert!(
        mouse.join("mod.rs").exists(),
        "codelet/fspec-tui/src/mouse/mod.rs must exist after RPC-023"
    );
    // @step And the file codelet/fspec-tui/src/mouse/hit_test.rs exists
    assert!(
        mouse.join("hit_test.rs").exists(),
        "codelet/fspec-tui/src/mouse/hit_test.rs must exist after RPC-023"
    );
    // @step And the file codelet/fspec-tui/src/mouse/toggle.rs exists
    assert!(
        mouse.join("toggle.rs").exists(),
        "codelet/fspec-tui/src/mouse/toggle.rs must exist after RPC-023"
    );
}
...
fn no_raw_sgr_mouse_escape_strings_appear_anywhere_in_src() {
    ...
    assert!(
        violations.is_empty(),
        "raw SGR mouse escape strings must not appear in codelet/fspec-tui/src — crossterm owns the protocol. Violations: {violations:?}"
    );
}
...
fn action_enum_gains_the_three_new_variants() {
    // @step Given codelet/fspec-tui/src/components/mod.rs after RPC-023 lands
