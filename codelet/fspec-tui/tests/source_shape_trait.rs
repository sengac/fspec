...
fn fspec_backend_trait_surface_exposes_5_rpc_methods_and_3_broadcast_subscriptions() {
    // @step Given codelet/fspec-tui/src/transport/mod.rs exists
...
fn priority_enum_has_the_exact_repr_u32_discriminants_from_rpc_002_doc_09() {
    ...
    // @step Given the Priority enum is defined in codelet/fspec-tui/src/components/mod.rs
    let path = common::workspace_root()
        .join("fspec-tui")
        .join("src")
        .join("components")
...
fn component_trait_surface_exposes_documented_methods_and_defaults() {
    ...
    // @step Given codelet/fspec-tui/src/components/mod.rs declares the Component trait
    let path = common::workspace_root()
        .join("fspec-tui")
        .join("src")
        .join("components")
