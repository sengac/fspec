...
fn codelet_workspace_declares_tui_textarea_as_a_dep() {
    // @step Given codelet/Cargo.toml after RPC-019 lands
    ...
    assert!(
        ...
        "codelet/Cargo.toml must declare tui-textarea as a workspace dep"
    );
}
...
fn codelet_fspec_tui_declares_tui_textarea_as_a_dep() {
    // @step Given codelet/fspec-tui/Cargo.toml after RPC-019 lands
    let body = read_raw(&workspace_codelet_dir().join("fspec-tui").join("Cargo.toml"));
    // @step Then the file contains the substring "tui-textarea"
    assert!(
        body.contains("tui-textarea"),
        "codelet/fspec-tui/Cargo.toml must depend on tui-textarea"
    );
}
...
fn agent_view_orchestrator_now_wires_the_new_widgets() {
    // @step Given codelet/fspec-tui/src/views/agent.rs after RPC-019 lands
...
fn action_enum_gains_four_navigation_variants() {
    // @step Given codelet/fspec-tui/src/components/mod.rs after RPC-019 lands
...
fn views_do_not_directly_import_forbidden_crates() {
    // @step Given the directory codelet/fspec-tui/src/views/ (including views/agent/) after RPC-019 lands
