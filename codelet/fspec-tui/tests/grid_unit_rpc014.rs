...
fn one_hundred_twenty_wide_terminal_divides_into_seven_equal_sixteen_wide_columns() {
    ...
    assert_eq!(total_col_width + 6, 118, "col widths + 6 separators must equal inner width 118");
}
...
fn one_hundred_twenty_five_wide_terminal_spreads_remainder_across_leading_columns() {
    ...
    for idx in 0..5 {
        assert_eq!(column_width_at(idx, widths), 17, "leading column {idx} must be 17 wide");
    }
    // @step And column_width_at(5, ...) and column_width_at(6, ...) each return 16
    for idx in 5..7 {
        assert_eq!(column_width_at(idx, widths), 16, "trailing column {idx} must be 16 wide");
    }
    // @step And the seven column widths plus the six separator chars sum to 123 (= 125 - 2 outer borders)
    let total_col_width: u16 = (0..7).map(|i| column_width_at(i, widths)).sum();
    assert_eq!(total_col_width + 6, 123, "col widths + 6 separators must equal inner width 123");
}
...
fn narrow_terminals_clamp_to_eight_minimum_and_report_zero_remainder() {
    ...
    // @step And the returned remainder equals 0
    assert_eq!(widths.remainder, 0, "remainder must be 0 when base_width was clamped");
}
...
fn build_border_row_produces_four_canonical_separator_strings_for_120_wide_layout() {
    ...
    assert!(plain.starts_with('├'), "plain must start with ├, got `{plain}`");
    ...
    assert_eq!(cross.matches('┼').count(), 6, "cross must contain exactly 6 ┼");
    ...
    assert_eq!(bottom.matches('┴').count(), 6, "bottom must contain exactly 6 ┴");
}
