...
// ─────────────────────────────────────────────────────────────────────
// Scenario 16: Source shape — codelet/fspec-tui/src/app/dispatch_rpc052.rs
// hosts the new debounce + hydration helpers
// ─────────────────────────────────────────────────────────────────────

#[test]
fn source_shape_dispatch_rpc052_module_exists() {
    // @step Given the file codelet/fspec-tui/src/app/dispatch_rpc052.rs exists
    // @step When the file is compiled as part of codelet-fspec-tui
    let manifest_dir = env!("CARGO_MANIFEST_DIR");
    let path = std::path::PathBuf::from(manifest_dir)
        .join("src")
        .join("app")
        .join("dispatch_rpc052.rs");
    assert!(
        path.exists(),
        "codelet/fspec-tui/src/app/dispatch_rpc052.rs must exist at {path:?}"
    );

    let source = std::fs::read_to_string(&path).expect("read dispatch_rpc052.rs");
    // @step Then it must declare impl App methods named handle_pending_input_changed, handle_seed_pending_input, and spawn_hydrate_pending_input
    for name in [
        "handle_pending_input_changed",
        "handle_seed_pending_input",
        "spawn_hydrate_pending_input",
    ] {
        assert!(
            source.contains(name),
            "dispatch_rpc052.rs must declare fn {name} (not found in source)"
        );
    }

    // @step And codelet/fspec-tui/src/app/mod.rs must declare pub mod dispatch_rpc052
    let mod_path = std::path::PathBuf::from(manifest_dir)
        .join("src")
        .join("app")
        .join("mod.rs");
    let mod_source = std::fs::read_to_string(&mod_path).expect("read app/mod.rs");
    assert!(
        mod_source.contains("pub mod dispatch_rpc052")
            || mod_source.contains("mod dispatch_rpc052"),
        "src/app/mod.rs must declare the dispatch_rpc052 module"
    );

    // @step And codelet/fspec-tui/src/app/dispatch.rs must NOT exceed 300 logical lines
    let dispatch_path = std::path::PathBuf::from(manifest_dir)
        .join("src")
        .join("app")
        .join("dispatch.rs");
    let dispatch_source =
    ...
    assert!(
        logical_lines <= 300,
        "src/app/dispatch.rs must stay <= 300 logical lines (found {logical_lines})"
    );
}
