...
pub fn create_handler() -> GraphSearchHandler {
    Arc::new(move |action: GraphSearchAction, _session_id: Uuid| {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async { dispatch_action(action).await })
        })
    })
}
...
async fn get_graph_or_err(name: &str, action_name: &str) -> Result<GraphDatabase, String> {
...
async fn dispatch_action(action: GraphSearchAction) -> String {
    match action {
        ...
        GraphSearchAction::AstNeighbors {
            node_id,
            depth,
            edge_types,
        } => {
            ...
            graph::ast_dispatch::dispatch_ast_neighbors(&db, &node_id, depth, edge_types.as_deref())
                .await
        }
        ...
        GraphSearchAction::AstCallChain {
            from,
            to,
            max_depth,
        } => {
        ...
        GraphSearchAction::AstCallers {
            ...
            graph::ast_transitive::dispatch_ast_callers(&db, &node_id, max_depth, limit).await
        }

        GraphSearchAction::AstCallees {
            node_id,
            max_depth,
            limit,
        } => {
            ...
            graph::ast_transitive::dispatch_ast_callees(&db, &node_id, max_depth, limit).await
        }

        GraphSearchAction::AstHierarchy {
            node_id,
            depth,
            include_methods,
        } => {
            ...
            graph::ast_hierarchy::dispatch_ast_hierarchy(&db, &node_id, depth, include_methods)
                .await
        }

        GraphSearchAction::AstComplexity {
            node_id,
            limit,
            min_threshold,
            path,
        ...
        GraphSearchAction::LearningsSearch {
            query,
            category,
            limit,
        } => {
            let db = match get_graph_or_err(graph::registry::LEARNINGS_GRAPH, "learnings_search")
                .await
            {
                Ok(db) => db,
                Err(err_json) => return err_json,
        ...
        GraphSearchAction::LearningsDecisions { domain, status } => {
            let db = match get_graph_or_err(graph::registry::LEARNINGS_GRAPH, "learnings_decisions")
                .await
            {
                Ok(db) => db,
                Err(err_json) => return err_json,
        ...
        GraphSearchAction::LearningsStats => {
            let db =
                match get_graph_or_err(graph::registry::LEARNINGS_GRAPH, "learnings_stats").await {
                    Ok(db) => db,
                    Err(err_json) => return err_json,
                };
            graph::learnings_dispatch::dispatch_learnings_stats(&db).await
        }
        GraphSearchAction::LearningsRelated {
            topic,
            min_strength,
            limit,
        } => {
            let db = match get_graph_or_err(graph::registry::LEARNINGS_GRAPH, "learnings_related")
                .await
            {
                Ok(db) => db,
                Err(err_json) => return err_json,
            };
            graph::learnings_dispatch::dispatch_learnings_related(&db, &topic, min_strength, limit)
                .await
        }
    }
}
