...
//! `codelet/cli/src/interactive/stream_loop.rs`. This test file pins
//! the contract via:
//!
...
//!      call in `codelet/agent-loop/src/agent_loop.rs`.
//!   4. Structural assertions over the `run_with_provider!` macro
//!      body in `codelet/agent-loop/src/dispatch.rs` forwarding both
...
fn background_session_declares_interrupt_handle_fields() {
    // @step Given the source of `codelet/sessions/src/background_session.rs`
    let src = read_workspace_source("codelet/sessions/src/background_session.rs");
...
fn agent_loop_calls_reset_interrupt_immediately_after_set_status_running() {
    // @step Given the source of `codelet/agent-loop/src/agent_loop.rs`
...
fn run_with_provider_macro_forwards_both_interrupt_handles() {
    // @step Given the source of `codelet/agent-loop/src/dispatch.rs`
...
fn background_output_interrupted_arm_persists_and_emits_interrupted_chunk() {
    // @step Given the source of `codelet/agent-loop/src/background_output.rs`
...
fn stream_chunk_declares_interrupted_variant_and_constructor() {
    // @step Given the source of `codelet/rpc-types/src/lib.rs`
    ...
    assert!(
        ...
        "codelet/rpc-types/src/lib.rs must define `pub enum StreamChunk`"
    );
