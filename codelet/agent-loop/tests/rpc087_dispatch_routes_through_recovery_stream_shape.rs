...
//! `codelet/agent-loop/src/dispatch.rs` funnels through
...
fn workspace_root() -> PathBuf {
    ...
    manifest
        .parent()
        .and_then(|p| p.parent())
        .expect("workspace root above codelet/agent-loop")
        .to_path_buf()
}
...
fn dispatch_contains_exactly_one_run_agent_stream_with_images_call_site() {
    ...
    let path = workspace_root().join("codelet/agent-loop/src/dispatch.rs");
