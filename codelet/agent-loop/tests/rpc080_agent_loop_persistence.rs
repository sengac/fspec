...
//!     codelet/agent-loop/src/ to pin Rules [0]-[7] against future
...
fn persistence_calls_in_agent_loop_use_crate_persist_not_napi() {
    ...
    assert!(
        violations.is_empty(),
        "codelet_napi::persist must not be referenced from codelet/agent-loop/src/. \
         Violations:\n{}",
        violations.join("\n")
    );

    // @step And persist.rs lives at codelet/agent-loop/src/persist.rs
    let persist_path = src_dir.join("persist.rs");
    assert!(
        persist_path.is_file(),
        "codelet/agent-loop/src/persist.rs must exist"
    );
