...
async fn background_session_round_trips_update_tokens_and_get_tokens() {
    ...
    // codelet/sessions/src/background_session.rs:717), so the expected
...
fn token_info_and_context_fill_info_declare_expected_fields_and_stream_event_variants() {
    // @step Given the source of `codelet/cli/src/interactive/output.rs`
    let src = read_workspace_source("codelet/cli/src/interactive/output.rs");
