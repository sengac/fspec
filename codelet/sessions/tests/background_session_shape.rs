...
//!
//! Pattern borrowed from `codelet/sessions/tests/skeleton_invariants.rs`.
...
/// Path to the moved file (`codelet/sessions/src/background_session.rs`).
...
/// `codelet/napi/src/session_manager.rs`. RPC-043 deleted that file and
...
/// `codelet/napi/src/session_bindings.rs`. The helper was retargeted but
...
fn scenario_codelet_sessions_builds_standalone_with_background_session_at_its_new_home() {
    // @step Given the BackgroundSession struct and impl have been moved into codelet/sessions/src/background_session.rs
    let moved = read(&moved_file_path());
    assert!(
        moved.contains("pub struct BackgroundSession"),
        "codelet/sessions/src/background_session.rs must define `pub struct BackgroundSession`"
    );
    assert!(
        moved.contains("impl BackgroundSession"),
        "codelet/sessions/src/background_session.rs must contain an `impl BackgroundSession` block"
    );
...
fn scenario_codelet_napi_still_builds_against_the_re_exported_background_session() {
    // @step Given codelet/napi/src/session_manager.rs now `pub use`s BackgroundSession + its companion types from codelet-sessions
    let napi = read(&napi_shell_path());
    assert!(
        napi.contains("pub use codelet_sessions::background_session::"),
        "codelet/napi/src/session_manager.rs must re-export the moved symbols via `pub use codelet_sessions::background_session::{{..}}`"
    );
    for name in [
        "BackgroundSession",
        "IncomingMessage",
        ...
        assert!(
            napi.contains(name),
            "codelet/napi/src/session_manager.rs must reference `{name}` (typically via the `pub use codelet_sessions::background_session::{{..}}` re-export block)"
        );
    }
    ...
    assert!(
        ...
        "codelet/napi/src/session_manager.rs must surface `ChainOfCommand` (post-RPC-040: via re-export)"
    );
    assert!(
        napi.contains("pub use codelet_sessions::session_manager::SessionManager")
            || napi.contains("pub struct SessionManager"),
        "codelet/napi/src/session_manager.rs must surface `SessionManager` (post-RPC-040: via re-export)"
    );
}
...
fn scenario_the_moved_background_session_rs_has_no_napi_references() {
    ...
    // @step Then I find zero matches in codelet/sessions/src/background_session.rs
    assert!(
        violations.is_empty(),
        "codelet/sessions/src/background_session.rs must not reference napi (found {} violations):\n{}",
        violations.len(),
        violations.join("\n")
    );
}
...
fn scenario_the_moved_background_session_rs_has_no_crate_persistence_or_crate_types_imports() {
    ...
    assert!(
        violations.is_empty(),
        "codelet/sessions/src/background_session.rs must not contain `crate::persistence` or `crate::types` (found {} violations):\n{}",
        violations.len(),
        violations.join("\n")
    );
...
fn scenario_handle_output_uses_the_new_chunks_tx_broadcast_and_no_longer_touches_global_chunk_callback(
    ...
    // @step And the GLOBAL_CHUNK_CALLBACK global has been deleted from codelet/napi/src/session_manager.rs (RPC-041 inverted the prior RPC-039 assertion)
    let napi = read(&napi_shell_path());
    ...
    assert!(
        ...
        "RPC-041: the `struct GlobalChunkCallback` wrapper must be deleted from codelet/napi/src/session_manager.rs"
    );
    assert!(
...
fn scenario_handle_output_emits_unconditionally_on_chunks_tx() {
    // @step Given the BackgroundSession code lives at codelet/sessions/src/background_session.rs
    let moved = read(&moved_file_path());
...
fn scenario_background_session_new_accepts_broadcast_senders_as_trailing_parameters() {
    ...
    // @step When I inspect the parameter list of `BackgroundSession::new` in codelet/sessions/src/background_session.rs
...
fn scenario_pre_existing_in_file_unit_tests_in_codelet_napi_still_pass_via_the_re_exports() {
    ...
    // @step And codelet/napi/src/session_manager.rs re-exports BackgroundSession, IncomingMessage, BridgeImageData, format_incoming_message, PromptInput, CompactionProgress, WorkUnitContext, SessionError from codelet-sessions
    let napi = read(&napi_shell_path());
    assert!(
        napi.contains("pub use codelet_sessions::background_session::"),
        "codelet/napi/src/session_manager.rs must contain a `pub use codelet_sessions::background_session::{{..}}` block"
    );
...
fn scenario_codelet_sessions_tests_assert_background_session_is_reachable_from_its_new_home() {
    ...
    // @step And the integration test codelet/sessions/tests/background_session_shape.rs has been added
    //
    ...
    assert!(
        ...
        "the RPC-039 integration test file must exist at codelet/sessions/tests/background_session_shape.rs"
    );
    ...
    // check, and trust the smoke test (codelet/sessions/tests/smoke.rs)
...
fn scenario_napi_typescript_surface_is_byte_stable_across_the_move() {
    ...
    //
    // The release build regenerates `codelet/napi/index.d.ts`. Running
    ...
    for fragment in [
        ...
        assert!(
            ...
            "codelet/napi/src/session_manager.rs must still expose the `{fragment}` napi function so the TS surface stays byte-stable"
        );
    }
    ...
    if std::env::var_os("RPC_039_FULL_TESTS").is_none() {
        ...
        // requires snapshotting `codelet/napi/index.d.ts` before and
    ...
    for fragment in [
        ...
        assert!(
            ...
            "regenerated codelet/napi/index.d.ts is missing `{fragment}` — the NAPI TypeScript surface is NOT byte-stable across the move"
        );
    }
...
fn scenario_global_chunk_callback_static_struct_and_unsafe_impls_are_removed() {
    // @step Given the napi-side codelet/napi/src/session_manager.rs file previously declared `static GLOBAL_CHUNK_CALLBACK: OnceCell<GlobalChunkCallback>`, a `struct GlobalChunkCallback { callback: ThreadsafeFunction<GlobalChunkCallbackArgs> }`, and `unsafe impl Send for GlobalChunkCallback {}` / `unsafe impl Sync for GlobalChunkCallback {}`
    let napi = read(&napi_shell_path());
    let napi_code = strip_line_comments(&napi);

    // @step When I grep codelet/napi/src/session_manager.rs for the regex `GLOBAL_CHUNK_CALLBACK|struct GlobalChunkCallback|unsafe impl (Send|Sync) for GlobalChunkCallback`
    let mut violations: Vec<String> = Vec::new();
    ...
    assert!(
        violations.is_empty(),
        "RPC-041: codelet/napi/src/session_manager.rs must no longer contain GLOBAL_CHUNK_CALLBACK / struct GlobalChunkCallback / unsafe impl (Send|Sync) for GlobalChunkCallback. Got {} violations:\n{}",
        violations.len(),
        violations.join("\n")
    );
