...
//! both agent-loop twins (codelet/napi/src/agent_loop.rs and
//! codelet/agent-loop/src/agent_loop.rs) must call between re-acquiring the
...
fn both_twins_call_shared_sync_helper_at_dispatch_site() {
    ...
    let twins = [
        ("codelet/napi/src/agent_loop.rs", &napi_src),
        ("codelet/agent-loop/src/agent_loop.rs", &twin_src),
    ];
    ...
    assert!(
        ...
        "codelet/sessions/src/background_session.rs must define `pub fn {HELPER_NAME}`"
    );
}
