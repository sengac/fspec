...
impl FileToolFacade for GeminiReadFileFacade {
    ...
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            description: "Read the contents of a file from the filesystem. Text files exceeding 25,000 tokens will return an error - use offset/limit for large files. This tool can read images (PNG, JPG, GIF, WEBP) - when reading an image file the contents are returned as base64-encoded data with media type. SVG files are always read as text (line-numbered XML source), not as image data. PDFs and Jupyter notebooks (.ipynb) are exempt from the token limit. If the user provides a path to a screenshot or image, use this tool to view it.".to_string(),
            parameters: json!({
                "type": "object",
                "properties": {
                    "file_path": {
