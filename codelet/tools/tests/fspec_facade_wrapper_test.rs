...
use codelet_tools::facade::{claude_fspec_tool, gemini_fspec_tool, wrapper::FacadeArgs};
use codelet_tools::fspec_handler::FspecResult;
use codelet_tools::{clear_all_fspec_handlers, set_fspec_handler_for_session, FspecHandler};
...
fn setup_mock_handler(session_id: Uuid) {
    let handler: FspecHandler = Arc::new(|req| FspecResult {
        success: true,
