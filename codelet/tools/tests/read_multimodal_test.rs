...
async fn test_read_png_image_and_display_visually() {
    ...
    let output: serde_json::Value =
        serde_json::from_str(&result).expect("Result should be valid JSON ReadOutput");
...
async fn test_detect_image_type_by_content_when_extension_missing() {
    ...
    let png_data: [u8; 69] = [
        ...
        0x44, 0x41, 0x54, 0x08, 0xD7, 0x63, 0xF8, 0xFF, 0xFF, 0x3F, 0x00, 0x05, 0xFE, 0x02, 0xFE,
        0xDC, 0xCC, 0x59, 0xE7, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60,
        0x82,
    ];
    ...
    assert_eq!(
        output["type"], "image",
        "Should detect as image by magic bytes"
    );
    assert_eq!(
        output["media_type"], "image/png",
