//! Scheduler engine — SCHED-003 / RPC-058 lift.
//!
...
//! with timezone support, and triggers jobs via the
//! [`crate::scheduler::SchedulerHooks`] indirection.
//!
...
use super::{Hooks, SchedulerHooks};
use chrono::Utc;
use std::path::Path;
use std::sync::Arc;
...
///
/// The `hooks` argument carries the side-channel into the host
/// SessionManager — see [`SchedulerHooks`] for the documented surface.
///
...
pub fn spawn_scheduler(
    ...
    handle.spawn(async move {
        ...
        loop {
            interval.tick().await;
            if let Err(e) = evaluate_and_run(&project_path, &state, hooks.clone()).await {
...
pub async fn evaluate_and_run(
    ...
    let live_ids = hooks.get_live_session_ids().await;
    ...
    for result in &results {
        ...
        if result.job_type == "agent" {
            let current_count = hooks.get_session_count().await;
