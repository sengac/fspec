//! Pure-Rust blob processing helpers for message envelopes.
//!
//! Lifted from `codelet/napi/src/persistence/blob_processing.rs` in
//! RPC-034. Consumes the [`MessageEnvelope`] schema lifted in RPC-031
//! and the [`BlobStore`] facade lifted in the sibling [`super::blob`]
//! module so codelet-rpc-embedded / codelet-rpc-server / the upcoming
//! codelet-sessions crate can process envelope blobs without
//! re-introducing the forbidden `rpc → napi` arrow.

use super::blob::{get_blob, should_use_blob_storage, store_blob};
use super::message_envelope::{
    AssistantContent, DocumentSource, ImageSource, MessageEnvelope, MessagePayload, UserContent,
};

/// Blob reference prefix for content stored in blob storage.
///
/// On-wire format: `blob:sha256:<64-hex>`. MUST stay byte-identical with
/// the pre-lift NAPI constant — downstream agents persist these strings
/// in `messages.jsonl` and must continue to resolve them.
...
/// Extract the hash from a blob reference string.
...
///
/// Returns the blob hash if stored, `None` if content is small enough for
/// inline storage.
...
///
/// Returns the processed envelope and a list of `(key, hash)` pairs for
/// the blob references.
            ...
            for (idx, content) in assistant_msg.content.iter_mut().enumerate() {
                match content {
                    ...
                    AssistantContent::ToolUse { id, input, .. } => {
                        // ToolUse input can contain large content (e.g.
                        // file contents being written). Serialize the
                        // input and check if it should be blobified.
...
/// Rehydrate blob references in an envelope by fetching content from
/// blob storage.
///
/// Restores the original content that was replaced with blob references
/// by [`process_envelope_for_blob_storage`].
