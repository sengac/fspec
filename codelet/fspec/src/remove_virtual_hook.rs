//! `remove-virtual-hook` shell-facing CLI bridge (RPC-283).
//!
...
//! parses argv (the `Mode::RemoveVirtualHook` clap variant in
//! [`crate::main`]) and delegates to the single source-of-truth in
//! [`codelet_fspec_core::commands::remove_virtual_hook::run`] — the SAME
//! function the LLM-facing agent_loop dispatcher invokes.
//!
//! Two-front-doors pattern:
//!   - Shell argv         -> clap -> this module -> fspec_core::commands::remove_virtual_hook::run
//!   - LLM tool call JSON -> fspec_core::dispatch::dispatch_command -> fspec_core::commands::remove_virtual_hook::run
//!
//! Bridge scope: argv -> JSON marshalling + stdout/stderr rendering only.
//! Work-unit lookup, hook filtering, script cleanup, and disk I/O all live
//! in the core. The bridge MUST NOT embed any of those concerns — see the
//! `scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher` test
//! which scans this file for forbidden substrings.

use std::env;
use std::path::PathBuf;

use anyhow::{Context, Result};
use codelet_fspec_core::commands::remove_virtual_hook;
use serde_json::{json, Map, Value};

use crate::common::render_core_error;

/// Strongly-typed args mirrored from the TS Commander.js registration at
/// `src/commands/remove-virtual-hook.ts:80-86`.
...
pub async fn run(args: CliArgs) -> Result<u8> {
    ...
    match remove_virtual_hook::run(&args_json, &project_root).await {
        Ok(data_json) => {
            let envelope: CoreResultEnvelope =
                serde_json::from_str(&data_json).context("parse core result envelope")?;
            println!("✓ Removed virtual hook '{hook_name}' from {work_unit_id}");
            println!("  Remaining virtual hooks: {}", envelope.remaining_count);
            Ok(0)
        }
        Err(err) => {
            eprintln!(
                "✗ Failed to remove virtual hook: {}",
