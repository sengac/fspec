...
mod tests {
    ...
    fn scenario_the_fspec_startup_seam_calls_from_path_and_main_invokes_before_clap_parse() {
        // @step Given the file "codelet/fspec/src/startup_env.rs"
        let startup_env = crate_dir().join("src").join("startup_env.rs");
        assert!(
            startup_env.is_file(),
            "codelet/fspec/src/startup_env.rs must exist (missing: {})",
            startup_env.display()
        );
        ...
        // @step And the file "codelet/fspec/src/main.rs" invokes "startup_env::load_startup_env()" before the clap parse of the Cli struct
    ...
    fn scenario_the_fspec_crate_declares_the_dotenvy_dependency() {
        // @step Given the file "codelet/fspec/Cargo.toml"
