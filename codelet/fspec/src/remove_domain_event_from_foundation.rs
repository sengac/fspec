//! `remove-domain-event-from-foundation` shell-facing CLI bridge (RPC-272).
//!
...
//! parses argv (the `Mode::RemoveDomainEventFromFoundation` clap variant in
//! [`crate::main`]) and delegates to the SAME source-of-truth the
//! LLM-facing dispatcher uses — `remove-domain-event-from-foundation` routed
...
/// registration at `src/commands/remove-domain-event-from-foundation.ts:131-154`.
#[derive(Debug)]
pub struct CliArgs {
...
/// `remove-domain-event-from-foundation` clap subcommand.
