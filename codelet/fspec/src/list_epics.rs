//! `list-epics` shell-facing CLI bridge (RPC-243).
//!
...
//! parses argv (the `Mode::ListEpics` clap variant in [`crate::main`])
//! and delegates to the single source-of-truth in
//! [`codelet_fspec_core::commands::list_epics::run`] — the SAME function
//! the LLM-facing agent_loop dispatcher invokes.
//!
//! Two-front-doors pattern (architecture note [7] on RPC-253, reused here
//! for RPC-243):
//!   - Shell argv         → clap → this module → fspec_core::commands::list_epics::run
//!   - LLM tool call JSON → fspec_core::dispatch::dispatch_command → fspec_core::commands::list_epics::run
//!
//! Both call sites pass a JSON-encoded args shape and a `project_root: &Path`.
//! The CLI surface resolves project_root from CWD (parity with the TypeScript
//! `process.cwd()` default at `src/commands/list-epics.ts:42`). The clap
//! subcommand carries NO flags — matching the flag-less TS Commander.js
//! registration at `src/commands/list-epics.ts:141-146`. No filter or
//! rendering logic is duplicated here.
//!
//! Exit-code contract (RPC-253 rule [14], reused for RPC-243):
...
/// bridge's `run` signature stays symmetric with `list_prefixes::run`
/// for the cross-command parity expected by RPC-003 §7/§11.
#[derive(Debug, Default)]
pub struct CliArgs {}

/// Entry point invoked from `main.rs` for the `list-epics` clap
...
pub async fn run(_args: CliArgs) -> Result<u8> {
    ...
    // list_epics::run validates with serde. With no flags currently
    ...
    match list_epics::run(&args_json, &project_root).await {
        Ok(rendered) => {
