//! `create-task` shell-facing CLI bridge (RPC-215).
//!
...
//! parses argv (the `Mode::CreateTask` clap variant in [`crate::main`]) and
//! delegates to the single source-of-truth in
//! [`codelet_fspec_core::commands::create_task::run`] — the SAME function
//! the LLM-facing agent_loop dispatcher invokes.
//!
//! Two-front-doors pattern:
//!   - Shell argv         → clap → this module → fspec_core::commands::create_task::run
//!   - LLM tool call JSON → fspec_core::dispatch::dispatch_command → fspec_core::commands::create_task::run
//!
//! Both call sites pass a JSON-encoded args shape and a `project_root: &Path`.
//! The CLI surface resolves project_root from CWD (parity with TS
//! `process.cwd()` default at `src/commands/create-task.ts:35`).
//!
//! Exit-code contract:
//!   - 0 on success; the rendered success block (checkmark + Title +
//!     optional Epic/Parent/Description) is written to stdout and the
//!     minimal-requirements reminder is written to stderr (parity with
//!     the TS `output.error(systemReminder)` at create-task.ts:248).
...
//!     path at `src/commands/create-task.ts:264-267`).

use std::env;
use std::path::PathBuf;

use anyhow::{Context, Result};
use codelet_fspec_core::commands::create_task;
use serde_json::{json, Value};

use crate::common::render_core_error;

/// Strongly-typed args mirrored from the TypeScript Commander.js
/// registration at `src/commands/create-task.ts:273-282`.
...
/// Entry point invoked from `main.rs` for the `create-task` clap subcommand.
pub async fn run(args: CliArgs) -> Result<u8> {
    let project_root: PathBuf = env::current_dir().context("resolve current working directory")?;

    // Marshal clap args → JSON object expected by fspec_core::commands::
    // create_task::run. The dispatcher and CLI both feed the SAME serde shape.
    ...
    match create_task::run(&args_json, &project_root).await {
        Ok(rendered) => {
            // The rendered block carries BOTH the success lines AND the
            // trailing reminder. The TS CLI splits these across stdout
            // (success) and stderr (reminder); we mirror that split so the
            // CLI surface matches create-task.ts:234-248.
