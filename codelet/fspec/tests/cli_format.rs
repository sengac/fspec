...
fn scenario_cli_prints_a_no_files_message_when_none_are_found() {
    ...
    // @step When I run `./codelet/target/release/fspec format` from that directory
    let (code, stdout, stderr) = run_format(ws.path(), &[]);
...
fn scenario_cli_errors_when_a_supplied_file_is_missing() {
    ...
    // @step When I run `./codelet/target/release/fspec format spec/features/missing.feature` from that workspace
...
fn scenario_format_help_is_byte_for_byte_identical() {
    ...
    // @step When I run `./codelet/target/release/fspec format --help` piped to non-TTY
    ...
    // @step And stdout matches the captured fixture at codelet/fspec/tests/fixtures/help/format.txt
