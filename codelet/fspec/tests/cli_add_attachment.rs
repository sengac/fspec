...
fn scenario_add_attachment_help_matches_ts_fixture_byte_for_byte() {
    ...
    // @step And stdout matches the canonical help fixture at codelet/fspec/tests/fixtures/help/add-attachment.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/add_attachment.rs contains NO file copy, work-unit lookup, or atomic write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_attachment.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/add_attachment.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
