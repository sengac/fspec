...
fn seed_units(units: &[(&str, &str)]) -> String {
    ...
    for (id, status) in units {
        ...
        obj.insert("title".into(), serde_json::Value::String(format!("title {id}")));
        obj.insert("type".into(), serde_json::Value::String("story".to_string()));
        obj.insert("status".into(), serde_json::Value::String((*status).to_string()));
        obj.insert("createdAt".into(), serde_json::Value::String("2026-06-01T00:00:00.000Z".to_string()));
        obj.insert("updatedAt".into(), serde_json::Value::String("2026-06-01T00:00:00.000Z".to_string()));
...
fn scenario_add_dependency_help_matches_ts_formatcommandhelp_reference() {
    ...
    assert_eq!(code, 0, "add-dependency --help must exit 0; stderr={stderr}");
...
fn scenario_cli_successfully_adds_shorthand_depends_on() {
    ...
    write_work_units(ws.path(), &seed_units(&[("AUTH-001", "specifying"), ("AUTH-002", "specifying")]));
...
fn scenario_cli_successfully_adds_blocks_edge() {
    ...
    let ws = tempfile::tempdir().expect("tempdir");
    write_work_units(ws.path(), &seed_units(&[("AUTH-001", "specifying"), ("API-001", "specifying")]));
    ...
    // @step And stdout contains the substring '✓ Dependency added successfully'
    assert!(stdout.contains("✓ Dependency added successfully"), "got: {stdout}");
...
fn scenario_cli_rejects_invocation_with_no_relationship_args() {
    ...
    assert!(stderr.contains("✗ Failed to add dependency:"), "got stderr: {stderr}");
    // @step And stderr contains the substring 'Must specify at least one relationship'
    assert!(stderr.contains("Must specify at least one relationship"), "got stderr: {stderr}");
...
fn scenario_cli_rejects_conflict_between_shorthand_and_depends_on() {
    ...
    let ws = tempfile::tempdir().expect("tempdir");
    write_work_units(ws.path(), &seed_units(&[("AUTH-001", "specifying"), ("AUTH-002", "specifying"), ("AUTH-003", "specifying")]));
    let pre_bytes = fs::read(ws.path().join("spec/work-units.json")).unwrap();

    // @step When I run `fspec add-dependency AUTH-001 AUTH-002 --depends-on AUTH-003` in that tempdir
    let (code, _stdout, stderr) = run_add_dep(ws.path(), &["AUTH-001", "AUTH-002", "--depends-on", "AUTH-003"]);
...
fn scenario_cli_rejects_circular_blocks_with_exit_1() {
    ...
    let ws = tempfile::tempdir().expect("tempdir");
    let mut pre: serde_json::Value = serde_json::from_str(&seed_units(&[("AUTH-001", "specifying"), ("AUTH-002", "blocked")])).unwrap();
    ...
    assert_eq!(code, 1, "expected exit 1; stderr={stderr}");
    // @step And stderr contains the substring '✗ Failed to add dependency:'
    assert!(stderr.contains("✗ Failed to add dependency:"), "got: {stderr}");
    // @step And stderr contains the substring 'Circular dependency detected'
    assert!(stderr.contains("Circular dependency detected"), "got: {stderr}");
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    assert_eq!(code, 1, "second invocation should fail as duplicate; stderr={stderr}");
    assert!(stderr.contains("Dependency already exists"), "got stderr: {stderr}");
