...
fn scenario_clap_exposes_check_with_verbose_byte_parity_help() {
    ...
    // @step When I run `./codelet/target/release/fspec check --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("check")
        .arg("--help")
    ...
    // @step Then stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/check.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step Then the CLI bridge module codelet/fspec/src/check.rs contains NO inline parsing, tag-validation, or check-aggregation logic — its only computation is JSON arg marshalling and display rendering from the envelope
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/check.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/check.rs must exist as the CLI bridge module"
    );
