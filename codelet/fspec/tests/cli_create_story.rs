...
fn scenario_create_story_help_matches_ts_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec create-story --help` piped to non-TTY
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/create-story.txt
...
fn scenario_clap_exposes_create_story_with_args_and_flags() {
    ...
    // @step When I run `./codelet/target/release/fspec create-story --help`
    let output = Command::new(fspec_bin())
        .arg("create-story")
        .arg("--help")
        .output()
...
fn scenario_cli_creates_minimal_story_and_prints_success_block() {
    ...
    // @step When I run `./codelet/target/release/fspec create-story AUTH "User login"` in that tempdir
...
fn scenario_cli_creates_story_with_epic_flag() {
    ...
    // @step When I run `./codelet/target/release/fspec create-story AUTH "User login" -e auth` in that tempdir
...
fn scenario_cli_rejects_unregistered_prefix_with_exit_1() {
    ...
    // @step When I run `./codelet/target/release/fspec create-story NOPE "User login"` in that tempdir
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/create_story.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
