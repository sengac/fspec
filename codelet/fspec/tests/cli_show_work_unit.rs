...
fn scenario_clap_exposes_show_work_unit_with_positional_and_format_option() {
    ...
    // @step When I run `./codelet/target/release/fspec show-work-unit --help` from a shell
...
fn scenario_cli_prints_text_format_dump_when_work_unit_exists() {
    ...
    // @step When I run `./codelet/target/release/fspec show-work-unit AUTH-001` from that directory
...
fn scenario_cli_prints_json_payload_when_format_json() {
    ...
    // @step When I run `./codelet/target/release/fspec show-work-unit AUTH-001 --format json`
...
fn scenario_cli_unknown_work_unit_exits_1_with_canonical_stderr() {
    ...
    // @step When I run `./codelet/target/release/fspec show-work-unit UNKNOWN-999`
...
fn scenario_cli_no_auto_create_when_work_units_json_absent() {
    ...
    // @step When I run `./codelet/target/release/fspec show-work-unit AUTH-001`
    let (code, stdout, stderr) = run_show_work_unit(ws.path(), &["AUTH-001"]);
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step Then the CLI bridge module codelet/fspec/src/show_work_unit.rs contains NO inline projection, reminder generation, or feature-scan logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/show_work_unit.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/show_work_unit.rs must exist as the CLI bridge module; missing: {}",
        bridge_path.display()
    );
...
fn scenario_show_work_unit_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec show-work-unit --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("show-work-unit")
        .arg("--help")
