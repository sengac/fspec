...
//! codelet/fspec/tests/cli_remove_command_from_foundation.rs (RPC-270).
...
fn scenario_remove_domain_event_from_foundation_help_matches_ts_fixture() {
    ...
    // @step And the stdout matches the canonical help fixture at codelet/fspec/tests/fixtures/help/remove-domain-event-from-foundation.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function() {
    ...
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/remove_domain_event_from_foundation.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
