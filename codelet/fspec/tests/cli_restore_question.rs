...
fn scenario_clap_exposes_restore_question_with_two_positional_args_in_help() {
    ...
    // @step When I run `./codelet/target/release/fspec restore-question --help`
    ...
    assert_eq!(code, 0, "restore-question --help must exit 0; stderr={stderr}");
...
fn scenario_cli_restores_question_and_prints_success_line() {
    ...
    // @step When I run `./codelet/target/release/fspec restore-question AUTH-001 0`
...
fn scenario_cli_rejects_unknown_question_id_with_exit_1_and_error_prefix() {
    ...
    // @step When I run `./codelet/target/release/fspec restore-question AUTH-001 5`
...
fn scenario_cli_rejects_unknown_work_unit_with_exit_1_and_error_prefix() {
    ...
    // @step When I run `./codelet/target/release/fspec restore-question AUTH-999 0`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And running `./codelet/target/release/fspec restore-question AUTH-001 1` afterwards exits 0
    ...
    assert_eq!(arr.len(), 2);
    for q in arr {
        assert_eq!(q["deleted"].as_bool(), Some(false), "all questions active; got {q}");
    }

    // @step And the CLI bridge module codelet/fspec/src/restore_question.rs contains NO inline state mutation or file-write logic — its only computation is JSON arg marshalling
...
fn scenario_restore_question_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec restore-question --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("restore-question")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/restore-question.txt
