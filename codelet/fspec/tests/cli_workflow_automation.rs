...
fn scenario_workflow_automation_help_matches_ts_format_command_help_reference() {
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/workflow-automation.txt
    assert_eq!(
...
fn scenario_cli_bridge_delegates_to_same_fspec_core_function() {
    // @step Given the CLI bridge module codelet/fspec/src/workflow_automation.rs
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/workflow_automation.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/workflow_automation.rs must exist as the CLI bridge module; missing: {}",
        bridge_path.display()
    );
