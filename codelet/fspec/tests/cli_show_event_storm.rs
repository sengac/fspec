...
fn scenario_clap_exposes_show_event_storm_with_help() {
    ...
    // @step When I run `./codelet/target/release/fspec show-event-storm --help` from a shell
...
fn scenario_cli_empty_workspace_exits_1_with_not_found() {
    ...
    // @step When I run `./codelet/target/release/fspec show-event-storm AUTH-001` from that directory
...
fn scenario_cli_exits_1_when_malformed_work_units() {
    ...
    // @step When I run `./codelet/target/release/fspec show-event-storm AUTH-001`
    let (code, stdout, stderr) = run_ses(ws.path(), &["AUTH-001"]);
...
fn scenario_show_event_storm_help_matches_ts_reference() {
    // @step Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    // @step When I run `./codelet/target/release/fspec show-event-storm --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("show-event-storm")
        .arg("--help")
    ...
    // @step Then stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/show-event-storm.txt
