...
//! `codelet/fspec-core/src/commands/resume_schedule.rs` is still a stub, the
//! clap subcommand is NOT yet wired into `codelet/fspec/src/main.rs`, and
//! the bridge module `codelet/fspec/src/resume_schedule.rs` does not exist.
...
fn scenario_resume_schedule_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step And stdout is byte-for-byte identical to the TS reference output at codelet/fspec/tests/fixtures/help/resume-schedule.txt
    assert_eq!(
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/resume_schedule.rs contains NO inline schedule-mutation, validation, or file-writing logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/resume_schedule.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/resume_schedule.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
