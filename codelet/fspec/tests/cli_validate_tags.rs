...
fn scenario_clap_exposes_validate_tags_with_byte_exact_help() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-tags --help` piped to non-TTY
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/validate-tags.txt
...
fn scenario_cli_unregistered_tag_exits_1_prints_violation_block() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-tags spec/features/bad.feature`
...
fn scenario_cli_summary_prints_only_summary_count_lines() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-tags --summary`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step When I dispatch validate-tags through fspec_core::dispatch::dispatch_command and also run `./codelet/target/release/fspec validate-tags` against the same on-disk state
    ...
    // @step Then the CLI bridge module codelet/fspec/src/validate_tags.rs contains NO inline validation or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/validate_tags.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/validate_tags.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
