...
fn scenario_clap_exposes_tag_stats_with_flag_aware_help() {
    ...
    // @step When I run `./codelet/target/release/fspec tag-stats --help` from a shell
...
fn scenario_cli_against_empty_directory_prints_zero_totals() {
    ...
    // @step When I run `./codelet/target/release/fspec tag-stats` from that directory
...
fn scenario_cli_text_output_renders_category_counts() {
  ...
  "categories": [
    ...
    // @step When I run `./codelet/target/release/fspec tag-stats` from the workspace
    let (code, stdout, stderr) = run_tag_stats(ws.path(), &[]);
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "categories": [
    ...
    // @step Then running `./codelet/target/release/fspec tag-stats` against the same on-disk state exits 0 and stdout reports the same counters
    ...
    // @step Then the CLI bridge module codelet/fspec/src/tag_stats.rs contains NO inline tag-counting, category-projection, or rendering logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/tag_stats.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/tag_stats.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_tag_stats_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec tag-stats --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("tag-stats")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/tag-stats.txt
