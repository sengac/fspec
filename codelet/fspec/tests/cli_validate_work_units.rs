...
fn scenario_validate_work_units_help_matches_ts_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-work-units --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("validate-work-units")
        .arg("--help")
...
fn scenario_cli_clean_store_prints_success_exits_0() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-work-units`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step When I dispatch validate-work-units through fspec_core::dispatch::dispatch_command and also run `./codelet/target/release/fspec validate-work-units` against the same on-disk state
    ...
    // @step Then the CLI bridge module codelet/fspec/src/validate_work_units.rs contains NO inline validation logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/validate_work_units.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/validate_work_units.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
