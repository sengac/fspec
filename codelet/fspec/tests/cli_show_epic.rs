...
fn scenario_clap_exposes_show_epic_with_epicid_help() {
    ...
    // @step When I run `./codelet/target/release/fspec show-epic --help` from a shell
...
fn scenario_cli_against_empty_workspace_exits_1() {
    ...
    // @step When I run `./codelet/target/release/fspec show-epic auth` from that directory
...
fn scenario_cli_text_output_renders_epic_header_and_progress() {
    ...
    // @step When I run `./codelet/target/release/fspec show-epic auth`
...
fn scenario_cli_unregistered_epic_id_exits_1() {
    ...
    // @step When I run `./codelet/target/release/fspec show-epic nonexistent`
...
fn scenario_cli_format_json_emits_json() {
    ...
    // @step When I run `./codelet/target/release/fspec show-epic auth --format json`
...
fn scenario_cli_f_short_flag_matches_long_form() {
    ...
    // @step When I run `./codelet/target/release/fspec show-epic auth -f json`
...
fn scenario_cli_delegates_to_same_fspec_core_function() {
    ...
    // @step Then the CLI bridge module codelet/fspec/src/show_epic.rs contains NO inline aggregation, filter, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/show_epic.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/show_epic.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_show_epic_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec show-epic --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("show-epic")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/show-epic.txt
