...
fn scenario_clap_exposes_dependencies_with_help() {
    ...
    // @step When I run `./codelet/target/release/fspec dependencies --help` from a shell
...
fn scenario_cli_prints_header_and_relationship_lines_for_a_unit_with_dependencies() {
    ...
    // @step When I run `./codelet/target/release/fspec dependencies AUTH-001` from that workspace
...
fn scenario_cli_graph_prints_the_depth_first_blocks_tree() {
  ...
  "states": {
    ...
    // @step When I run `./codelet/target/release/fspec dependencies AUTH-001 --graph` from that workspace
...
fn scenario_cli_exits_1_and_writes_to_stderr_when_the_work_unit_does_not_exist() {
  ...
  "states": {
    ...
    // @step When I run `./codelet/target/release/fspec dependencies INVALID-999` from that workspace
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "states": {
    ...
    // @step Then the CLI bridge module codelet/fspec/src/dependencies.rs contains NO inline rendering, traversal, or filter logic — its only computation is JSON arg marshalling and stdout printing
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/dependencies.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/dependencies.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_dependencies_help_matches_ts_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec dependencies --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("dependencies")
        .arg("--help")
    ...
    // @step Then stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/dependencies.txt
