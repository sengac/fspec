...
fn scenario_clap_exposes_show_coverage_with_help() {
    ...
    // @step When I run `./codelet/target/release/fspec show-coverage --help` from a shell
...
fn scenario_show_coverage_missing_feature_exits_1() {
    ...
    // @step When I run `./codelet/target/release/fspec show-coverage missing` from that directory
...
fn scenario_cli_per_feature_markdown_for_fully_covered() {
    ...
    // @step When I run `./codelet/target/release/fspec show-coverage auth` from that workspace
...
fn scenario_cli_per_feature_json_mode() {
    ...
    // @step When I run `./codelet/target/release/fspec show-coverage auth --format json` from that workspace
...
fn scenario_cli_project_wide_missing_features_dir_exits_1() {
    ...
    // @step When I run `./codelet/target/release/fspec show-coverage` from that directory
    let (code, stdout, stderr) = run_show_coverage(ws.path(), &[]);
...
fn scenario_cli_trailing_feature_extension_tolerated() {
    ...
    // @step When I run `./codelet/target/release/fspec show-coverage login.feature` from that workspace
...
fn scenario_show_coverage_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec show-coverage --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("show-coverage")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/show-coverage.txt
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/show_coverage.rs contains NO inline coverage parsing, stats aggregation, or markdown rendering — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/show_coverage.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/show_coverage.rs must exist as the CLI bridge module"
    );
