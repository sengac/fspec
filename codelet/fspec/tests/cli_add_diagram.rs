...
fn scenario_clap_exposes_add_diagram_with_positional_args_in_help() {
    ...
    // @step When I run `./codelet/target/release/fspec add-diagram --help`
...
fn scenario_cli_adds_new_diagram_and_prints_success_block() {
    ...
    // @step When I run `./codelet/target/release/fspec add-diagram Architecture "Component" "graph TD\n  A-->B"`
...
fn scenario_cli_replaces_existing_diagram_and_prints_updated_block() {
    ...
    // @step When I run `./codelet/target/release/fspec add-diagram Architecture "System Overview" "graph LR\n  New-->X"`
    let (code, stdout, stderr) = run_add_diag(
        ws.path(),
...
fn scenario_cli_rejects_invalid_subgraph_identifier() {
    ...
    // @step When I run `./codelet/target/release/fspec add-diagram Architecture "Bad" "graph TB\n  subgraph Id!!!\n  end"`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/add_diagram.rs contains NO inline mermaid validation, ensure_foundation_file, or JSON-mutation logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_diagram.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/add_diagram.rs must exist; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_add_diagram_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec add-diagram --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("add-diagram")
        .arg("--help")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/add-diagram.txt
