...
fn scenario_clap_exposes_validate_spec_alignment_with_help() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-spec-alignment --help` from a shell
...
fn scenario_cli_exits_0_and_prints_success_when_the_work_unit_has_tagged_scenarios() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-spec-alignment AUTH-001` from that workspace
...
fn scenario_cli_exits_1_when_the_work_unit_does_not_exist() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-spec-alignment MISSING-999` from that workspace
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And I run `./codelet/target/release/fspec validate-spec-alignment AUTH-001` against the same workspace
    let (code, stdout, stderr) = run_validate(ws.path(), &["AUTH-001"]);
...
fn scenario_validate_spec_alignment_help_matches_ts_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec validate-spec-alignment --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("validate-spec-alignment")
        .arg("--help")
