...
fn scenario_clap_exposes_review_subcommand_and_prints_bare_help() {
    ...
    // @step When I run `./codelet/target/release/fspec review --help` from a shell
...
fn scenario_cli_reviews_an_existing_work_unit_and_prints_the_report() {
    ...
    // @step When I run `./codelet/target/release/fspec review <id>` from that directory
...
fn scenario_cli_errors_on_a_missing_work_unit_id() {
    ...
    // @step When I run `./codelet/target/release/fspec review BOGUS-999` from that directory
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/review.rs contains NO review logic — its only computation is JSON arg marshalling and stdout printing
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/review.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/review.rs must exist as the CLI bridge module"
    );
