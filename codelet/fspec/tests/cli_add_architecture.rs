...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/add_architecture.rs contains NO inline gherkin parsing or line-splice mutation logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_architecture.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/add_architecture.rs must exist as the CLI bridge module"
    );
