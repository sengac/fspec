...
//! `codelet/fspec/src/main.rs` does not yet register an `add-virtual-hook`
...
fn scenario_clap_exposes_add_virtual_hook_with_flag_aware_help() {
    ...
    // @step When I run `./codelet/target/release/fspec add-virtual-hook --help` from a shell
...
fn scenario_add_virtual_hook_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec add-virtual-hook --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("add-virtual-hook")
        .arg("--help")
