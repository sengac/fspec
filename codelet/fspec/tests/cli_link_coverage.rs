...
fn scenario_help_output_matches_the_captured_ts_fixture() {
    ...
    // @step And the stdout matches the canonical help fixture at codelet/fspec/tests/fixtures/help/link-coverage.txt
...
fn scenario_cli_delegates_to_the_same_fspec_core_function_used_by_the_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/link_coverage.rs contains NO inline mutation, validation, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/link_coverage.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/link_coverage.rs must exist as the CLI bridge module"
    );
