...
fn scenario_add_policy_help_matches_ts_fixture() {
    ...
    // @step When I run `./codelet/target/release/fspec add-policy --help`
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/add-policy.txt
...
fn scenario_cli_successfully_appends_policy_and_prints_success_line() {
    ...
    // @step When I run `./codelet/target/release/fspec add-policy AUTH-001 "Send welcome email"` in that tempdir
...
fn scenario_cli_forwards_when_then_bounded_context_flags() {
    ...
    // @step When I run `./codelet/target/release/fspec add-policy AUTH-001 "Send welcome email" --when UserRegistered --then SendWelcomeEmail --bounded-context Identity` in that tempdir
...
fn scenario_cli_rejects_done_state_with_exit_1_and_error_prefix() {
    ...
    // @step When I run `./codelet/target/release/fspec add-policy AUTH-001 "Anything"` in that tempdir
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/add_policy.rs contains NO inline policy construction, status guard, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_policy.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/add_policy.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
