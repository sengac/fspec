...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/add_tag_to_scenario.rs contains NO inline tag-format validation, scenario lookup, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/add_tag_to_scenario.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/add_tag_to_scenario.rs must exist; missing: {}",
        bridge_path.display()
    );
