...
fn scenario_cli_delegates_to_same_fspec_core_function() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/update_foundation.rs contains NO inline field mapping, validation, or file-write logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/update_foundation.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/update_foundation.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
