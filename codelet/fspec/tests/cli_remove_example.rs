//! CLI surface for the `remove-example` subcommand on the standalone fspec
//! Rust binary — RPC-273.
//!
//! Feature: spec/features/remove-example-cli-subcommand.feature
...
fn run_cmd(cwd: &Path, args: &[&str]) -> (i32, String, String) {
    ...
    cmd.arg("remove-example");
    for a in args {
        cmd.arg(a);
    }
    cmd.current_dir(cwd);
    let output = cmd.output().expect("spawn fspec remove-example");
...
fn help_output_matches_captured_ts_fixture() {
    ...
    // @step When I run `fspec remove-example --help`
    let output = Command::new(fspec_bin())
        .arg("remove-example")
        .arg("--help")
        .output()
        .expect("spawn fspec remove-example --help");
    let code = output.status.code().unwrap_or(-1);
    let stdout = String::from_utf8_lossy(&output.stdout).into_owned();

    // @step Then the exit code is 0
    assert_eq!(code, 0, "fspec remove-example --help must exit 0");

    // @step And stdout matches the canonical help fixture at codelet/fspec/tests/fixtures/help/remove-example.txt
    let fixture = include_str!("fixtures/help/remove-example.txt");
    assert_eq!(stdout, fixture);
}

#[test]
fn happy_path_soft_delete_via_cli() {
    // @step Given a project root tempdir with spec/work-units.json where AUTH-001 status=specifying has one example id=0 'hello'
    ...
    seed_one(
        ...
        "specifying",
        Some(json!([
            {"id": 0, "text": "hello", "deleted": false, "createdAt": "2026-06-01T00:00:00.000Z"}
        ])),
    );

    // @step When I run `fspec remove-example AUTH-001 0` in that tempdir
    let (code, stdout, stderr) = run_cmd(tmp.path(), &["AUTH-001", "0"]);

    // @step Then the exit code is 0
    assert_eq!(code, 0, "exit 0; stderr={stderr}");

    // @step And stdout contains the substring '✓ Removed example: "hello"'
    assert!(
        stdout.contains("✓ Removed example: \"hello\""),
        "stdout: {stdout}"
    );
...
fn missing_work_unit_exits_1_with_canonical_stderr() {
    ...
    // @step When I run `fspec remove-example NOPE-001 0` in that tempdir
...
fn wrong_status_exits_1_with_phase_guard_message() {
    ...
    // @step And stderr contains the substring "Can only remove examples during discovery/specification phase. AUTH-001 is in 'backlog' state."
    assert!(
        stderr.contains("Can only remove examples during discovery/specification phase. AUTH-001 is in 'backlog' state."),
        "stderr: {stderr}"
    );
}
...
fn cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    // @step Given a project root tempdir with spec/work-units.json where AUTH-001 status=specifying has examples id=0 and id=1
    ...
    seed_one(
        ...
        Some(json!([
            ...
            {"id": 1, "text": "one", "deleted": false, "createdAt": "2026-06-01T00:00:00.000Z"}
        ])),
    );
    ...
    // @step When I dispatch remove-example via fspec_core::dispatch with workUnitId='AUTH-001' and index=0
    ...
    // @step And I run the binary `fspec remove-example AUTH-001 1` against the same workspace shape
    let (code, _stdout, stderr) = run_cmd(tmp_cli.path(), &["AUTH-001", "1"]);
    assert_eq!(code, 0, "binary success; stderr={stderr}");

    // @step Then both invocations call commands::remove_example::run with the same JSON-marshalled args
    // (Indirectly verified — both produced soft-deletes.)

    // @step And both examples end up deleted=true on disk
