...
fn scenario_clap_exposes_set_user_story_with_positional_and_required_flags() {
    ...
    // @step When I run `./codelet/target/release/fspec set-user-story --help`
...
fn scenario_cli_writes_the_user_story_and_prints_the_success_block() {
    ...
    // @step When I run `./codelet/target/release/fspec set-user-story AUTH-001 --role developer --action ship --benefit happiness`
...
fn scenario_cli_rejects_unknown_work_unit_with_exit_1_and_error_prefix() {
    ...
    // @step When I run `./codelet/target/release/fspec set-user-story MISSING-001 --role x --action y --benefit z`
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/set_user_story.rs contains NO inline userStory build, file-write, or success-line rendering — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/set_user_story.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/set_user_story.rs must exist as the CLI bridge module; got missing: {}",
        bridge_path.display()
    );
...
fn scenario_set_user_story_help_matches_ts_formatcommandhelp_reference() {
    ...
    // @step When I run `./codelet/target/release/fspec set-user-story --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("set-user-story")
        .arg("--help")
        .env_remove("CLICOLOR_FORCE")
    ...
    // @step And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/set-user-story.txt
