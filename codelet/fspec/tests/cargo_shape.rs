...
fn scenario_fspec_version_prints_the_workspace_version() {
    ...
    // @step And STDOUT contains the workspace version string declared in `codelet/Cargo.toml [workspace.package].version`
...
fn scenario_codelet_fspec_is_registered_as_a_workspace_member() {
    // @step Given the file `codelet/Cargo.toml` exists
    let cargo = codelet_root().join("Cargo.toml");
    assert!(cargo.is_file(), "codelet/Cargo.toml must exist");
...
fn scenario_cargo_toml_declares_a_single_bin_named_fspec() {
    // @step Given the file `codelet/fspec/Cargo.toml` exists
    let cargo = fspec_crate_root().join("Cargo.toml");
    assert!(cargo.is_file(), "codelet/fspec/Cargo.toml must exist");
...
fn scenario_fspec_src_contains_exactly_the_locked_file_layout() {
    // @step Given the directory `codelet/fspec/src/` exists
    let src = fspec_crate_root().join("src");
    assert!(src.is_dir(), "codelet/fspec/src/ must exist");
    ...
    for f in [
        ...
        assert!(
            p.is_file(),
            "codelet/fspec/src/{f} must exist (got missing: {})",
            p.display()
        );
    }
...
fn scenario_no_source_file_constructs_its_own_tokio_runtime() {
    ...
    assert!(
        violations.is_empty(),
        "codelet/fspec/src/ must NOT construct its own tokio runtime: {violations:?}"
    );
}

#[test]
fn scenario_rpc_005_source_shape_invariant_is_widened_to_scan_fspec_src() {
    // @step Given the file `codelet/rpc-embedded/tests/architecture_invariants.rs` exists
...
fn scenario_existing_codelet_rpc_server_dev_helper_binary_stays_in_place() {
    ...
    // @step And `codelet/rpc-server/Cargo.toml` still declares the binary
...
fn scenario_spawn_fspec_daemon_helper_proves_port_line_contract_is_verbatim() {
    ...
    assert!(
        daemon_mode.is_file(),
        "codelet/fspec/tests/daemon_mode.rs must exist"
    );
    ...
    // @step And the same parsing logic mirrors `codelet/rpc-server/tests/websocket_transport.rs::spawn_rpc_server`
...
fn scenario_cargo_build_p_fspec_release_produces_codelet_target_release_fspec() {
    // @step When the developer runs `cargo build -p fspec --release` from `codelet/`
...
fn scenario_existing_codelet_rpc_server_test_harness_still_works_port_line_contract_preserved() {
    // @step Given the test `codelet/rpc-server/tests/websocket_transport.rs::spawn_rpc_server` exists
...
fn scenario_package_json_declares_the_build_rust_fspec_script() {
    ...
    // @step And the script copies `codelet/target/release/fspec` to `dist/fspec`
    assert!(
        script.contains("codelet/target/release") && script.contains("dist"),
        "build:rust:fspec script must copy codelet/target/release artifact to dist/; got: {script}"
    );
}
...
fn scenario_existing_vitest_smoke_at_napi_workunitinfo_shape_test_ts_remains_green() {
    ...
    assert!(
        napi_src.is_dir(),
        "codelet/napi/src must still exist (no NAPI surface change in this card)"
    );
}
...
fn scenario_existing_cargo_test_suites_for_rpc_005_009_remain_green() {
    // @step When the developer runs `cargo test -p codelet-rpc-embedded -p codelet-rpc-server -p codelet-fspec-tui` from `codelet/`
    let output = Command::new("cargo")
        .arg("test")
        .arg("-p")
