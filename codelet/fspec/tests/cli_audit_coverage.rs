...
fn scenario_clap_exposes_audit_coverage_with_byte_parity_help() {
    ...
    // @step When I run `./codelet/target/release/fspec audit-coverage --help` piped to non-TTY
    let output = Command::new(fspec_bin())
        .arg("audit-coverage")
        .arg("--help")
...
fn scenario_cli_reports_all_files_present_and_exits_0() {
    ...
    // @step When I run `./codelet/target/release/fspec audit-coverage user-login` from that directory
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step Then the CLI bridge module codelet/fspec/src/audit_coverage.rs contains NO inline file-existence or rendering logic — its only computation is JSON arg marshalling and envelope decoding
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/audit_coverage.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/audit_coverage.rs must exist as the CLI bridge module"
    );
