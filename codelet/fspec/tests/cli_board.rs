...
fn scenario_clap_exposes_board_with_format_and_limit_byte_parity_help() {
    ...
    // @step When I run `./codelet/target/release/fspec board --help` piped to non-TTY
    ...
    // @step Then stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/board.txt
...
fn scenario_cli_emits_json_board_with_story_point_summary() {
    ...
    // @step When I run `./codelet/target/release/fspec board --format json` from that directory
...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
  ...
  "states": {
    ...
    // @step Then the CLI bridge module codelet/fspec/src/board.rs contains NO inline column-building, point-summing, or rendering logic — its only computation is JSON arg marshalling
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/board.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/board.rs must exist as the CLI bridge module"
    );
