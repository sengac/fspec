...
fn scenario_cli_delegates_to_same_fspec_core_function_as_dispatcher() {
    ...
    // @step And the CLI bridge module codelet/fspec/src/update_scenario.rs contains NO inline gherkin parsing or scenario-rename logic
    let bridge_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/update_scenario.rs");
    assert!(
        bridge_path.exists(),
        "codelet/fspec/src/update_scenario.rs must exist as the CLI bridge module"
    );
