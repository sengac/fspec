#!/usr/bin/env bash
# fspec Source Build Installer (macOS / Linux)
# Builds the Rust binary from source and installs it.
#
# For a prebuilt binary (no Rust toolchain required), use scripts/install.sh:
#   curl -fsSL https://raw.githubusercontent.com/sengac/fspec/main/scripts/install.sh | bash
#
# Usage:
#   ./scripts/build-install.sh                     # Run from repo root
#   curl ... | bash                                 # Piped (auto-clones repo)
#   ./scripts/build-install.sh --dir /usr/local/bin
#   BUILD_PROFILE=release ./scripts/build-install.sh
...
Requires a Rust toolchain (cargo) and git.

For a prebuilt binary instead, use: scripts/install.sh
...
header "fspec Source Build Installer"
echo ""
