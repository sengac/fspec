...
interface ModalLine {
  role: 'user' | 'assistant' | 'tool' | 'supervisor'; // WATCH-012: Added supervisor role
  content: string;
  lineIndex: number;
}
 ...
 */
const getRoleTitle = (role: 'user' | 'assistant' | 'tool' | 'supervisor'): string => {
  switch (role) {
    case 'user':
      return 'User Message';
    case 'assistant':
    ...
    case 'tool':
      return 'Tool Output';
    case 'supervisor':
      return 'Supervisor Input'; // WATCH-012: Supervisor message title
    default:
      return 'Content';
  }
};
...
const renderModalLine = (line: ModalLine): React.ReactNode => {
  ...
  // Default rendering - user is green, supervisor is magenta (WATCH-012), others white
  const baseColor = line.role === 'user' ? 'green' : line.role === 'supervisor' ? 'magenta' : 'white';
  return (
    <Box flexGrow={1}>
