...
export async function cleanupCheckpoints(
  ...
  try {
    ...
    if (result.deletedCount > 0) {
      output.log(`Deleted ${result.deletedCount} checkpoint(s):`);
      result.deleted.forEach(cp => {
        output.log(`  - ${cp.name} (${cp.timestamp})`);
      });
      output.log('');
    }

    if (result.preservedCount > 0) {
      output.log(`Preserved ${result.preservedCount} checkpoint(s):`);
    ...
    output.error(`✗ Failed to cleanup checkpoints: ${errorMessage}`);
    throw error;
  }
}
