...
const config: CommandHelpConfig = {
  ...
  description: 'Create a new work unit for tracking a discrete piece of work through ACDD workflow',
  ...
  arguments: [
    {
      name: 'prefix',
      description: 'Work unit prefix (e.g., AUTH, UI, API). Must be registered with create-prefix first.',
      required: true,
    },
    {
      name: 'title',
  ...
  examples: [
    ...
    {
      command: 'fspec create-work-unit AUTH "OAuth integration" --epic=user-management',
      description: 'Create work unit with epic',
      output: '✓ Created work unit AUTH-002\n  Title: OAuth integration\n  Epic: user-management',
    },
  ],
  prerequisites: [
    'Prefix must be registered first: fspec create-prefix AUTH "Authentication features"',
  ],
  typicalWorkflow: 'create-work-unit → update-work-unit-status (move through workflow) → mark done',
  relatedCommands: ['list-work-units', 'show-work-unit', 'update-work-unit-status', 'create-prefix', 'create-epic'],
  notes: [
