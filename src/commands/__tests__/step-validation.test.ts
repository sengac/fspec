...
Feature: Shopping Cart
  ...
  Scenario: Add items
    ...
    Then I should see the items in my order`;
      ...
      await writeFile(join(featuresDir, 'shopping-cart.feature'), featureContent);

      const coverageData = {
        scenarios: [{ name: 'Add items', testMappings: [] }],
        stats: { totalScenarios: 1, coveredScenarios: 0, coveragePercent: 0, testFiles: [], implFiles: [], totalLinesCovered: 0 }
      };
      await writeFile(join(featuresDir, 'shopping-cart.feature.coverage'), JSON.stringify(coverageData, null, 2));
...
Feature: User Session
  ...
  Scenario: Session management
    ...
    Then I should see my user details`;
      ...
      await writeFile(join(featuresDir, 'user-session.feature'), featureContent);

      const coverageData = {
        scenarios: [{ name: 'Session management', testMappings: [] }],
        stats: { totalScenarios: 1, coveredScenarios: 0, coveragePercent: 0, testFiles: [], implFiles: [], totalLinesCovered: 0 }
      };
      await writeFile(join(featuresDir, 'user-session.feature.coverage'), JSON.stringify(coverageData, null, 2));
  ...
  Scenario: Login
    ...
    Then I should be logged in`;
      ...
      const coverageData = {
        ...
        stats: { totalScenarios: 1, coveredScenarios: 1, coveragePercent: 100, testFiles: ['src/__tests__/auth.test.ts'], implFiles: [], totalLinesCovered: 10 }
      };
      await writeFile(join(featuresDir, 'user-login.feature.coverage'), JSON.stringify(coverageData, null, 2));
...
describe('Login', () => {
  it('should log in successfully', () => {
    ...
    // MISSING: // @step Then I should be logged in
      ...
      expect(validationResult.missingSteps[0]).toContain('Then I should be logged in');
    });
  });
});
