...
describe('Feature: Feature file naming for bug work units', () => {
  ...
  beforeEach(async () => {
    setup = await setupWorkUnitTest('generate-scenarios-bug-naming');
...
`;
      const helpFeaturePath = join(setup.featuresDir, 'help-command.feature');
      await writeFile(helpFeaturePath, helpFeatureContent);

      // When I run "fspec generate-scenarios BUG-001"
      const result = await generateScenarios('BUG-001', { cwd: setup.testDir });
  ...
  describe('Scenario: New capability-oriented feature file created when no match found', () => {
    it('should create capability-oriented feature file when no existing match', async () => {
      ...
      // (setup.featuresDir is empty)

      // When I run "fspec generate-scenarios BUG-002"
      const result = await generateScenarios('BUG-002', { cwd: setup.testDir });
      ...
      const createdFile = capabilityFiles.find(file =>
        existsSync(join(setup.featuresDir, file))
      );
      ...
      if (createdFile) {
        const featureContent = await readFile(
          join(setup.featuresDir, createdFile),
          'utf-8'
        );
...
`;
      ...
      const result = await generateScenarios('BUG-003', { cwd: setup.testDir });
...
`;
      ...
      const result = await generateScenarios('BUG-004', { cwd: setup.testDir });
