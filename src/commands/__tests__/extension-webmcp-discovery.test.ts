...
import { createMessageRouter } from '../../../extension/src/background/message-router';
import { createToolRegistry } from '../../../extension/src/background/tool-registry';
...
describe('Feature: fspec WebMCP Chrome Extension - WebMCP Tool Discovery & Invocation', () => {
  ...
  describe('Scenario: Discover WebMCP tool registered by website', () => {
    it('should detect tool registration and add it to the MCP tool list with origin-based namespacing', () => {
      ...
      // @step And the extension adds "webmcp__travel-demo.bandarra.me__searchFlights" to the MCP tool list
  ...
  describe('Scenario: Remove WebMCP tool when website unregisters it', () => {
    it('should remove the tool from registry and notify when website unregisters', () => {
      ...
      // @step And the extension has a discovered WebMCP tool "webmcp__example.com__oldTool"
