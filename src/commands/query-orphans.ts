...
export function registerQueryOrphansCommand(program: Command): void {
  program
    ...
    .action(async (options: { output?: 'json' | 'text'; excludeDone?: boolean }) => {
      try {
        ...
        if (options.output === 'json') {
          ...
          result.orphans.forEach((orphan, index) => {
            ...
            console.log(`   ${chalk.red('⚠')} No epic or dependency relationships`);
          ...
          console.log(chalk.dim('  fspec update-work-unit <id> --epic=<epic-name>'));
          console.log(
            chalk.dim(
