...
1. Add `getToolStatusIndicators()` function
   - Read environment variables
   - Check config file
   - Return status map

2. Add helper `formatToolList()` function
...
3. Add `workUnitCreatedReminder()` function
   ```typescript
   export function workUnitCreatedReminder(
     workUnitId: string,
     type: WorkUnitType,
     title: string,
     cwd: string
...
4. Update `createWorkUnit()` to return reminder text
   ```typescript
   return {
     workUnitId: id,
     reminder: workUnitCreatedReminder(id, type, title, cwd)
   };
   ```
...
5. Update handlers to output reminder after success message

### Phase 3: Enhanced Specifying Reminder (40 minutes)

**File**: `src/utils/system-reminder.ts`

6. Refactor `specifyingStateReminder()` function
   - Accept full `workUnit` object (not just ID)
   - Analyze title for keywords
   - Generate context-aware tool list
   - Add natural language guidance
   - Include all 5 tools with status

7. Add keyword detection helper
   ```typescript
...
8. Update reminder collection to pass full work unit
   ```typescript
...
function getToolStatusIndicators(cwd: string): Record<string, ToolStatus> {
  const statuses: Record<string, ToolStatus> = {};

  // AST - always available (no config)
  statuses.ast = {
    configured: true,
  ...
  statuses.perplexity = {
    ...
    reason: process.env.PERPLEXITY_API_KEY
      ? 'PERPLEXITY_API_KEY configured'
      : 'Missing PERPLEXITY_API_KEY environment variable'
  };

  // JIRA - requires URL and token
  const hasJira = !!(process.env.JIRA_URL && process.env.JIRA_API_TOKEN);
  statuses.jira = {
    configured: hasJira,
    reason: hasJira
      ? 'JIRA_URL and JIRA_API_TOKEN configured'
      : 'Missing JIRA_URL or JIRA_API_TOKEN'
  };

  // Confluence - requires URL and token
  const hasConfluence = !!(process.env.CONFLUENCE_URL && process.env.CONFLUENCE_API_TOKEN);
  statuses.confluence = {
    configured: hasConfluence,
    reason: hasConfluence
      ? 'CONFLUENCE_URL and CONFLUENCE_API_TOKEN configured'
      : 'Missing CONFLUENCE_URL or CONFLUENCE_API_TOKEN'
  };

  // Stakeholder - requires at least one webhook
  const hasWebhook = !!(
    process.env.TEAMS_WEBHOOK_URL ||
    process.env.SLACK_WEBHOOK_URL ||
    process.env.DISCORD_WEBHOOK_URL
  );
  statuses.stakeholder = {
    configured: hasWebhook,
    reason: hasWebhook
      ? 'Chat webhook configured'
      : 'Missing webhook URL (TEAMS_WEBHOOK_URL, SLACK_WEBHOOK_URL, or DISCORD_WEBHOOK_URL)'
  };

  return statuses;
}

function formatToolList(statuses: Record<string, ToolStatus>): string {
  const tools = [
    {
      name: 'ast',
  ...
  for (const tool of tools) {
    const status = statuses[tool.name];
...
export function workUnitCreatedReminder(
  ...
  const statuses = getToolStatusIndicators(cwd);
  ...
  reminder += formatToolList(statuses);
...
// Update getStatusChangeReminder signature
function getStatusChangeReminder(
  ...
  const reminders: Record<WorkflowState, string | null> = {
    backlog: null,
    specifying: specifyingStateReminder(workUnitId, workUnit, cwd), // Pass workUnit
    testing: testingStateReminder(workUnitId, cwd),
    implementing: implementingStateReminder(workUnitId, cwd),
    validating: validatingStateReminder(workUnitId, cwd),
    done: doneStateReminder(workUnitId, cwd),
    blocked: blockedStateReminder(workUnitId, cwd)
  };

  return reminders[newStatus];
}

// Update call site (around line 650)
const statusReminder = getStatusChangeReminder(
  workUnitId,
  newStatus,
...
describe('getToolStatusIndicators', () => {
  it('should detect configured AST tool', () => {
    const statuses = getToolStatusIndicators('/test/path');
    expect(statuses.ast.configured).toBe(true);
  });

  it('should detect missing Perplexity API key', () => {
    delete process.env.PERPLEXITY_API_KEY;
    const statuses = getToolStatusIndicators('/test/path');
    expect(statuses.perplexity.configured).toBe(false);
    expect(statuses.perplexity.reason).toContain('Missing PERPLEXITY_API_KEY');
  });

  it('should detect configured JIRA', () => {
    process.env.JIRA_URL = 'https://jira.example.com';
    process.env.JIRA_API_TOKEN = 'token123';
    const statuses = getToolStatusIndicators('/test/path');
    expect(statuses.jira.configured).toBe(true);
  });
});
...
describe('workUnitCreatedReminder', () => {
  it('should emphasize AST for code tasks', () => {
    const reminder = workUnitCreatedReminder(
      'TASK-001',
      'task',
  ...
  it('should emphasize Perplexity for research stories', () => {
    const reminder = workUnitCreatedReminder(
      'STORY-001',
      'story',
...
describe('specifyingStateReminder', () => {
  it('should provide context-aware suggestions', () => {
    ...
    const reminder = specifyingStateReminder('TASK-001', workUnit, '/test');
  ...
  it('should list all 5 tools', () => {
    ...
    const reminder = specifyingStateReminder('STORY-001', workUnit, '/test');
