...
### 14. Complete Provider List (from rig)

All 19 providers supported by rig must be configurable:

| Provider | Base URL | Env Variable | Auth Method | Special Config |
|----------|----------|--------------|-------------|----------------|
| **OpenAI** | `https://api.openai.com/v1` | `OPENAI_API_KEY` | Bearer | `baseUrl` override |
| **Anthropic** | `https://api.anthropic.com` | `ANTHROPIC_API_KEY` | x-api-key header | `anthropic-version`, `anthropic-beta` |
| **Cohere** | `https://api.cohere.ai` | `COHERE_API_KEY` | Bearer | - |
| **Google Gemini** | `https://generativelanguage.googleapis.com` | `GEMINI_API_KEY` | Query param | - |
| **Mistral** | `https://api.mistral.ai` | `MISTRAL_API_KEY` | Bearer | - |
| **xAI** | `https://api.x.ai` | `XAI_API_KEY` | Bearer | - |
| **Together AI** | `https://api.together.xyz` | `TOGETHER_API_KEY` | Bearer | - |
| **HuggingFace** | `https://router.huggingface.co` | `HUGGINGFACE_API_KEY` | Bearer | - |
| **OpenRouter** | `https://openrouter.ai/api/v1` | `OPENROUTER_API_KEY` | Bearer | `HTTP-Referer`, `X-Title` headers |
| **Groq** | `https://api.groq.com/openai/v1` | `GROQ_API_KEY` | Bearer | - |
| **Ollama** | `http://localhost:11434` | None | None | `baseUrl` (local only) |
| **DeepSeek** | `https://api.deepseek.com` | `DEEPSEEK_API_KEY` | Bearer | - |
| **Perplexity** | `https://api.perplexity.ai` | `PERPLEXITY_API_KEY` | Bearer | - |
| **Moonshot** | `https://api.moonshot.cn/v1` | `MOONSHOT_API_KEY` | Bearer | - |
| **Hyperbolic** | `https://api.hyperbolic.xyz` | `HYPERBOLIC_API_KEY` | Bearer | - |
| **Mira** | `https://api.mira.network` | `MIRA_API_KEY` | Bearer | - |
| **Galadriel** | `https://api.galadriel.com/v1/verified` | `GALADRIEL_API_KEY` | Bearer | Fine-tune key optional |
| **Azure OpenAI** | Custom endpoint | `AZURE_API_KEY` | api-key header | `endpoint`, `apiVersion` required |
| **Voyage AI** | `https://api.voyageai.com/v1` | `VOYAGE_API_KEY` | Bearer | Embeddings only |

#### Provider-Specific Configuration

**Standard Providers** (most common pattern):
```json
{
  "providers": {
    "openai": { "enabled": true, "apiKey": "from-credentials", "baseUrl": null },
    "anthropic": { "enabled": true, "apiKey": "from-credentials" },
    "cohere": { "enabled": true, "apiKey": "from-credentials" },
    "gemini": { "enabled": true, "apiKey": "from-credentials" },
    "mistral": { "enabled": true, "apiKey": "from-credentials" },
    "xai": { "enabled": true, "apiKey": "from-credentials" },
    "together": { "enabled": true, "apiKey": "from-credentials" },
    "huggingface": { "enabled": true, "apiKey": "from-credentials" },
    "openrouter": { "enabled": true, "apiKey": "from-credentials" },
    "groq": { "enabled": true, "apiKey": "from-credentials" },
    "deepseek": { "enabled": true, "apiKey": "from-credentials" },
    "perplexity": { "enabled": true, "apiKey": "from-credentials" },
    "moonshot": { "enabled": true, "apiKey": "from-credentials" },
    "hyperbolic": { "enabled": true, "apiKey": "from-credentials" },
    "mira": { "enabled": true, "apiKey": "from-credentials" },
    "galadriel": { "enabled": true, "apiKey": "from-credentials" },
    "voyageai": { "enabled": true, "apiKey": "from-credentials" }
  }
}
```
...
**Azure OpenAI** (complex config):
```json
{
  "providers": {
    "azure": {
      "enabled": true,
      "apiKey": "from-credentials",
