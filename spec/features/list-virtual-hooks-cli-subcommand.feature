...
Feature: List Virtual Hooks Cli Subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::list_virtual_hooks::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes list-virtual-hooks as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec list-virtual-hooks --help` from a shell
  ...
  Scenario: CLI prints the populated text layout when the work unit has virtual hooks
    ...
    When I run `./rust/target/release/fspec list-virtual-hooks AUTH-001`
    Then the command exits 0
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher (no duplicated business logic)
    ...
    And the CLI bridge module rust/fspec/src/list_virtual_hooks.rs contains NO inline rendering, hook-grouping, or work-unit-lookup logic — its only computation is JSON arg marshalling

  Scenario: list-virtual-hooks --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec list-virtual-hooks --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-virtual-hooks.txt
