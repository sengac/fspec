...
Feature: RPC-062 MCP Injection Source Shape
  """
  Source-shape regression test for RPC-062. Pins the four MCP wiring touchpoints inside codelet/sessions/src/session_manager.rs (McpInjection import, two init_mcp_session call sites, one cleanup_mcp_session call site, spawn_agent_loop trait signature) and asserts the NAPI-side consumer in codelet/napi/src/agent_loop.rs still consumes the receiver. Also asserts the negative invariant that no MCP method has leaked into codelet/core/src/session_manager_handle.rs, codelet/rpc/src/lib.rs, or codelet/fspec-tui/src/transport/mod.rs.
  ...
  Scenario: codelet-sessions imports the NAPI-free McpInjection type from codelet-tools
    Given the file codelet/sessions/src/session_manager.rs is compiled
  ...
  Scenario: codelet-napi agent_loop consumes mcp_injection_rx inside its select! loop
    Given the file codelet/napi/src/agent_loop.rs is compiled
    When I scan its source bytes after stripping Rust comments
  ...
  Scenario: No MCP injection methods leak into the RPC surface across handle, service, and backend traits
    Given the files codelet/core/src/session_manager_handle.rs, codelet/rpc/src/lib.rs, and codelet/fspec-tui/src/transport/mod.rs are compiled
  ...
  Scenario: codelet-sessions has no transitive napi dependency after the RPC-062 audit
    Given the existing test codelet/sessions/tests/no_napi_dependency.rs is in the codelet-sessions test suite
