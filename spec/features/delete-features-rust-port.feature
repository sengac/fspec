...
Feature: Port delete-features command to Rust
  ...
  Scenario: Dispatcher deletes features carrying a single tag
    Given a project root tempdir with three features, two tagged @deprecated
    When I dispatch delete-features with tags=['@deprecated']
    Then the dispatcher returns success=true and deletedCount=2
    And the two @deprecated feature files no longer exist on disk
    And the untagged feature file still exists

  Scenario: Dispatcher applies AND logic across multiple tags
    Given a project root tempdir with one feature tagged @critical and @spike and one feature tagged only @critical
    When I dispatch delete-features with tags=['@critical','@spike']
    Then the dispatcher returns success=true and deletedCount=1
    And only the feature carrying both tags is deleted

  Scenario: Dispatcher dry-run leaves all files on disk
    Given a project root tempdir with two features tagged @deprecated
    When I dispatch delete-features with tags=['@deprecated'] and dryRun=true
    Then the dispatcher returns success=true and deletedCount=2
    And the dispatcher result lists the matching files
    And both feature files still exist on disk

  Scenario: Dispatcher rejects an empty tag list
    Given a project root tempdir with a feature tagged @deprecated
    When I dispatch delete-features with tags=[]
  ...
  Scenario: Dispatcher reports no matches for an unused tag
    Given a project root tempdir with a feature tagged @deprecated
    When I dispatch delete-features with tags=['@notpresent']
    Then the dispatcher returns success=true and deletedCount=0
    And the dispatcher message equals 'No feature files found matching tags'
