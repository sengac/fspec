...
Feature: Provider settings list: testResult clears on ↑/↓ arrow navigation
  """
  [0] This card complements the full integration coverage already provided by RPC-159 (rust/fspec-tui/tests/provider_settings_clear_test_result_on_nav_rpc159.rs). Pattern matches RPC-077 / RPC-149 / RPC-156 fast structural source-string regression-shape complement to slow integration tests.
  [1] Test file: rust/fspec-tui/tests/rpc151_test_result_clear_on_nav_shape.rs — sub-millisecond execution, no key event simulation, just source-string scanning of list.rs handle_list_key body.
  [2] Source path: rust/fspec-tui/src/views/provider_settings/list.rs handle_list_key (lines 30-87). Up arm at lines 65-77; Down arm at lines 78-87. RPC-159 comments at lines 66-70 and 79-80 document the TS-parity rationale.
  """
