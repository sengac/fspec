...
Feature: Add GitHub Copilot provider
  ...
  Module layout: codelet/providers/src/copilot/{mod.rs (CopilotProvider impl LlmProvider), oauth.rs (device flow), auth.rs (credential persistence mirroring claude_auth.rs/codex_auth.rs), refreshing_client.rs (CopilotHttpClient middleware), behavior_facade.rs (CopilotBehaviorFacade trait + 3 impls + selector), header_facade.rs (CopilotHeaderFacade), classifier.rs (CopilotRequestClassifier), endpoint.rs (CopilotEndpointFacade), models.rs (CopilotModelCatalogService)}
  Provider registration: add ProviderType::GitHubCopilot variant in codelet/providers/src/manager.rs alongside Claude, OpenAI, Codex, Gemini, Zai — the manager remains the single dispatch point and the facade selection happens inside CopilotProvider::new()
  ...
  #   10. CopilotProvider implements LlmProvider (codelet/providers/src/lib.rs:84) and delegates all provider-specific behavior to facades following the same pattern as ClaudeProvider, CodexProvider, GeminiProvider, ZaiProvider, OpenAIProvider
  ...
  #   17. CopilotHttpClient implements rig::http_client::HttpClientExt as a middleware layer that composes CopilotRequestClassifier + CopilotHeaderFacade on every outgoing request — follows the RefreshingCodexClient / RefreshingClaudeClient pattern from codelet/providers/src/
  ...
  #   19. Copilot model catalog is fetched by a CopilotModelCatalogService (single responsibility: GET /models + parse + filter + merge) — not a facade because there is no provider variation; analogous to models/ subdirectory pattern in codelet/providers/src/
  #
  # EXAMPLES:
  ...
  #   3. User selects model gpt-4o-copilot in codelet TUI; codelet sends a /chat/completions request to api.githubcopilot.com with Bearer token, User-Agent codelet/<version>, Openai-Intent conversation-edits, x-initiator user, and receives streamed response
  ...
  Scenario: Chat completion request to gpt-4o-copilot uses /chat/completions endpoint with Copilot headers
    ...
    And the request should include a "User-Agent: codelet/<version>" header
