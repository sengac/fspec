...
Feature: Dependency-rule regression tests for fspec, fspec-tui, sessions
  """
  The new rust/test-helpers/ crate is added to the workspace `members` list AND exposed as `codelet-test-helpers = { path = "test-helpers" }` in [workspace.dependencies] — so consuming crates declare it via `codelet-test-helpers.workspace = true` in [dev-dependencies]
  The helper API has two functions: `assert_no_codelet_napi_in_dependency_graph(crate_name: &str)` (cargo metadata BFS — fails if codelet-napi appears in the transitive set) and `assert_no_codelet_napi_imports_in_sources(crate_dir: &str)` (walks rust/<crate_dir>/src/*.rs, strips comments, fails on `use codelet_napi` or `codelet_napi::` substrings)
  ...
  Refined helper API: `assert_no_transitive_dependency(from_crate: &str, forbidden_pkg: &str)` and `assert_no_import_in_sources(crate_dir_name: &str, forbidden_module: &str)`. The crate_dir_name is the directory name under rust/ (e.g. "fspec", "fspec-tui", "sessions", "core", "rpc-types").
  rust/test-helpers/Cargo.toml depends on serde_json (workspace) ONLY — no other deps. It is a library crate with [lib] (default) plus the workspace lints inherited. The `[lints] workspace = true` block applies.
  """
  ...
  #   2. The helpers use the same cargo metadata JSON-walk strategy already used by rust/fspec/tests/no_napi_dependency.rs (NOT cargo tree text grep) so behaviour is consistent with the existing RPC-044 tests
  ...
  #   4. New regression test file rust/core/tests/no_napi_dependency.rs asserts (a) no codelet-napi in codelet-core's transitive cargo-metadata dependency graph and (b) no `codelet_napi::` / `use codelet_napi` substring under rust/core/src/
  #   5. New regression test file rust/rpc-types/tests/no_napi_dependency.rs asserts (a) no codelet-napi in codelet-rpc-types' transitive dependency graph (the optional `napi` feature pulls the third-party `napi` crate, NOT `codelet-napi`) and (b) no `codelet_napi::` / `use codelet_napi` substring under rust/rpc-types/src/
  #   6. The three existing no_napi_dependency.rs files under rust/fspec/tests, rust/fspec-tui/tests, rust/sessions/tests are migrated to delegate to the shared helpers — each file remains a thin wrapper of two #[test] fns that call the helper with the appropriate crate name + source root
  #   7. rust/rpc-embedded/tests/rpc_006_source_shape.rs is left untouched — it asserts wider invariants (default_fixture cfg-gating, host-runtime handle, loopback-only bind, WorkUnitInfo deduplication, no-bincode push path) that go beyond the no-napi rule and would be lost if collapsed into the helper
  ...
  #
  # EXAMPLES:
  #   1. Sabotage scenario (manually verified): temporarily adding `codelet-napi = { workspace = true }` to rust/core/Cargo.toml makes codelet-core's no_napi_dependency.rs FAIL with a clear message naming codelet-napi
  ...
  #   3. When `cargo test -p codelet-rpc-types --test no_napi_dependency` runs with the default feature set (no `napi` feature), the transitive package walk completes without finding `codelet-napi`, and the source scan of rust/rpc-types/src/ finds no `use codelet_napi` import
  #   4. After migration, the file rust/fspec/tests/no_napi_dependency.rs is under 30 lines and contains only `use codelet_test_helpers::dependency_rules::*` plus two #[test] fns that call assert_no_codelet_napi_in_dependency_graph("codelet-fspec") and assert_no_codelet_napi_imports_in_sources("fspec")
  #   5. If a developer adds `codelet-napi = { workspace = true }` to rust/core/Cargo.toml and re-runs `cargo test -p codelet-core --test no_napi_dependency`, the test FAILS with an error message containing both `codelet-core` and `codelet-napi` and pointing at the forbidden-arrow rule
  ...
  Scenario: codelet-test-helpers crate is wired into the workspace as a library
    Given the codelet workspace root manifest at rust/Cargo.toml
    ...
    And rust/test-helpers/Cargo.toml declares package.name = "codelet-test-helpers"
    And rust/test-helpers/Cargo.toml declares serde_json as its ONLY [dependencies] entry beyond workspace-inherited package fields
    And rust/test-helpers/Cargo.toml inherits [lints] from workspace

  Scenario: codelet-test-helpers exposes the shared dependency-rule helper API
    Given the new crate rust/test-helpers/ exists with a library entry point at rust/test-helpers/src/lib.rs
    When I inspect the public surface of the dependency_rules module
    Then `pub mod dependency_rules` is declared in rust/test-helpers/src/lib.rs
    And rust/test-helpers/src/dependency_rules.rs defines `pub fn assert_no_transitive_dependency(from_crate: &str, forbidden_pkg: &str)`
    And rust/test-helpers/src/dependency_rules.rs defines `pub fn assert_no_import_in_sources(crate_dir_name: &str, forbidden_module: &str)`
  ...
  Scenario: codelet-test-helpers itself has no transitive dependency on codelet-napi
    ...
    And rust/test-helpers/src does not contain any `use codelet_napi` or `codelet_napi::` substring
  ...
  Scenario: codelet-core forbidden-arrow regression test passes against the current workspace
    ...
    And no `.rs` file under rust/core/src contains a `use codelet_napi` or `codelet_napi::` substring after comments are stripped
  ...
  Scenario: codelet-rpc-types forbidden-arrow regression test passes against the current workspace
    ...
    And no `.rs` file under rust/rpc-types/src contains a `use codelet_napi` or `codelet_napi::` substring after comments are stripped

  Scenario: Migrated codelet-fspec regression test delegates to the shared helper
    Given the file rust/fspec/tests/no_napi_dependency.rs
    ...
    And rust/fspec/Cargo.toml declares `codelet-test-helpers.workspace = true` under [dev-dependencies]

  Scenario: Migrated codelet-fspec-tui regression test delegates to the shared helper
    Given the file rust/fspec-tui/tests/no_napi_dependency.rs
    When I inspect its source after the RPC-067 migration
    Then it imports the shared helper module from codelet_test_helpers
    And it contains exactly two #[test] fns
    ...
    And rust/fspec-tui/Cargo.toml declares `codelet-test-helpers.workspace = true` under [dev-dependencies]

  Scenario: Migrated codelet-sessions regression test delegates to the shared helper
    Given the file rust/sessions/tests/no_napi_dependency.rs
    When I inspect its source after the RPC-067 migration
    Then it imports the shared helper module from codelet_test_helpers
    And it contains exactly two #[test] fns
    ...
    And rust/sessions/Cargo.toml declares `codelet-test-helpers.workspace = true` under [dev-dependencies]

  Scenario: rpc_006_source_shape.rs is left untouched
    Given the file rust/rpc-embedded/tests/rpc_006_source_shape.rs
  ...
  Scenario: Sabotaging codelet-core by adding a codelet-napi dependency makes its test fail
    Given a developer adds `codelet-napi = { workspace = true }` to rust/core/Cargo.toml
