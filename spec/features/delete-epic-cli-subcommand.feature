...
Feature: fspec delete-epic CLI subcommand (Rust port)
  ...
  The `fspec delete-epic --help` output is byte-for-byte identical to `node dist/index.js delete-epic --help` (TS reference) — captured as codelet/fspec/tests/fixtures/help/delete-epic.txt.
  """
  ...
  Scenario: Clap exposes delete-epic with a positional arg and --force flag in --help
    ...
    When I run `./codelet/target/release/fspec delete-epic --help`
  ...
  Scenario: CLI deletes an existing epic and prints the success line
    ...
    When I run `./codelet/target/release/fspec delete-epic auth`
    Then the command exits 0
  ...
  Scenario: CLI accepts --force without changing behaviour (TS impl ignores it)
    Given spec/epics.json contains epic 'auth' with title='Authentication'
    When I run `./codelet/target/release/fspec delete-epic auth --force`
    Then the command exits 0
    And stdout contains the line '✓ Epic auth deleted successfully'

  Scenario: CLI exits 1 when the epic does not exist
    Given spec/epics.json contains epic 'dash' with title='Dashboard'
    When I run `./codelet/target/release/fspec delete-epic missing`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./codelet/target/release/fspec delete-epic dash` afterwards exits 0
    And spec/epics.json contains neither 'auth' nor 'dash' epics
    And the CLI bridge module codelet/fspec/src/delete_epic.rs contains NO inline file-read, mutation, or rendering logic — its only computation is JSON arg marshalling

  Scenario: delete-epic --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec delete-epic --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/delete-epic.txt
