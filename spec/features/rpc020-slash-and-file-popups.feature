...
Feature: Slash command palette + @file search popup in AgentView
  ...
  #   8. File-size discipline (RPC-002 rule [10]) is preserved: every new file under rust/fspec-tui/src/views/agent/ and any new file added by this card stays under 300 LoC; if the agent module risks crossing 300 LoC, split into submodules instead of mutating the orchestrator.
