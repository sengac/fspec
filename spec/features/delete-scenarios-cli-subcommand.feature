...
Feature: delete-scenarios CLI subcommand on the standalone fspec Rust binary
  ...
  """
  CLI subcommand wired into codelet/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::delete_scenarios::run(args_json) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: CLI help output matches the captured bare-Commander fixture byte-for-byte
    ...
    Then the process exits with code 0
    And stdout matches the captured fixture at codelet/fspec/tests/fixtures/help/delete-scenarios.txt
