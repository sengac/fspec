...
Feature: fspec restore-question CLI subcommand (Rust port)
  ...
  The bridge module at codelet/fspec/src/restore_question.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::restore_question::run; no validation or rendering logic is duplicated.
  ...
  The `fspec restore-question --help` output is byte-for-byte identical to `node dist/index.js restore-question --help` (TS reference) — captured as codelet/fspec/tests/fixtures/help/restore-question.txt.
  """
  ...
  Scenario: Clap exposes restore-question with two positional args in --help
    ...
    When I run `./codelet/target/release/fspec restore-question --help`
  ...
  Scenario: CLI restores a question and prints the success line
    ...
    When I run `./codelet/target/release/fspec restore-question AUTH-001 0`
    Then the command exits 0
  ...
  Scenario: CLI rejects unknown question ID with exit 1 and stderr Failed prefix
    ...
    When I run `./codelet/target/release/fspec restore-question AUTH-001 5`
  ...
  Scenario: CLI rejects unknown work unit with exit 1 and stderr Failed prefix
    ...
    When I run `./codelet/target/release/fspec restore-question AUTH-999 0`
    Then the command exits with code 1
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./codelet/target/release/fspec restore-question AUTH-001 1` afterwards exits 0
    And spec/work-units.json on disk shows both questions with deleted=false
    And the CLI bridge module codelet/fspec/src/restore_question.rs contains NO inline state mutation or file-write logic — its only computation is JSON arg marshalling

  Scenario: restore-question --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec restore-question --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/restore-question.txt
