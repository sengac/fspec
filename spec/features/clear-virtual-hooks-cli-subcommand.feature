...
Feature: clear-virtual-hooks CLI subcommand
  """
  Shell-facing surface for the Rust port of `fspec clear-virtual-hooks <workUnitId>`. Lives at rust/fspec/src/clear_virtual_hooks.rs as the standard two-front-doors CLI bridge — it owns clap parsing, marshals camelCase JSON into the shared `clear_virtual_hooks::run` core function (defined in rust/fspec-core/src/commands/clear_virtual_hooks.rs and proven by spec/features/clear-virtual-hooks-rust-port.feature), prints the rendered text response to stdout, and surfaces InvalidArgs as a single `Error: <msg>` line to stderr followed by exit code 1.

  The CLI signature mirrors the TS Commander.js definition exactly: one required positional `<workUnitId>` and no flags. Help is intercepted in `rust/fspec/src/main.rs::intercept_ts_help` BEFORE clap parses argv — the intercept arm calls `format_command_help(&configs::clear_virtual_hooks::CONFIG)` which produces output byte-for-byte identical to `node dist/index.js clear-virtual-hooks --help`. The byte-parity contract is enforced by `rust/fspec/tests/fixtures/help/clear-virtual-hooks.txt`.
  """
  ...
  Scenario: CLI prints success message when clearing hooks
    ...
    When I run `./rust/target/release/fspec clear-virtual-hooks AUTH-001` in that project root
  ...
  Scenario: CLI exits 1 with chalk failure prefix when the work unit does not exist
    ...
    When I run `./rust/target/release/fspec clear-virtual-hooks AUTH-999` in that project root
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher (no duplicated business logic)
    ...
    And the CLI bridge module rust/fspec/src/clear_virtual_hooks.rs contains NO inline rendering, file IO, or work-unit-lookup logic — its only computation is JSON arg marshalling
  ...
  Scenario: clear-virtual-hooks --help is byte-for-byte identical to TS formatCommandHelp reference output
    ...
    When I run `./rust/target/release/fspec clear-virtual-hooks --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/clear-virtual-hooks.txt
