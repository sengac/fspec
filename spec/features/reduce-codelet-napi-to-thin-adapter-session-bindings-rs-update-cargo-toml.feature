...
Feature: Reduce codelet-napi to thin adapter (session_bindings.rs); update Cargo.toml
  ...
  # Discovery (2026-05-21): the live codelet/napi/src/session_manager.rs is 6752 LOC
  ...
  # session_bindings.rs. They split into seven sibling modules under codelet/napi/src/:
  ...
  #
  # Behaviour preservation: codelet/napi/index.d.ts diff is EMPTY post-RPC-043;
  # every existing test passes against its new file location; a new shape test
  # (codelet/napi/tests/session_bindings_shape.rs) enforces the static contract;
  # a new smoke test (codelet/napi/tests/session_bindings_smoke.rs) exercises every
  ...
  Background: User Story
    ...
    So that the codelet-napi crate becomes a pure JS-bridge adapter with no agent logic of its own — the agent loop, hooks, and session manager live in codelet-sessions, while napi-internal infrastructure (footer poller, bridges, persistence shims) is properly modularised, unblocking the fspec binary wiring in RPC-044 and keeping the codelet/napi/index.d.ts byte-stable for the TS frontend
  ...
  Scenario: codelet-napi/src/session_manager.rs no longer exists after the move
    ...
    When I run `ls codelet/napi/src/session_manager.rs`
    Then the command exits with a non-zero status
    And stderr contains "No such file or directory"
    And the file at "codelet/napi/src/session_manager.rs" does not exist on disk
  ...
  Scenario: codelet-napi/src/ gains the seven new sibling modules
    ...
    When I run `ls codelet/napi/src/`
  ...
  Scenario: lib.rs declares each new sibling module under the noop feature gate
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/src/lib.rs`
  ...
  Scenario: session_bindings.rs holds 66 to 68 #[napi] free functions and the 12 #[napi(object)] shapes
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/src/session_bindings.rs`
  ...
  Scenario: agent_loop.rs owns the agent loop and its StreamOutput sinks
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/src/agent_loop.rs`
  ...
  Scenario: session_hooks.rs owns the NapiSessionManagerHooks impl and the install helper
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/src/session_hooks.rs`
  ...
  Scenario: persist.rs owns the five persist_* helpers as pub(crate) functions
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/src/persist.rs`
  ...
  Scenario: footer_poller.rs owns the FOOTER_POLLER_TOKENS static and the spawn/stop helpers
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/src/footer_poller.rs`
  ...
  Scenario: bridges.rs owns the bridge init helpers and the handler registration helpers
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/src/bridges.rs`
  ...
  Scenario: interjection.rs owns the parse_interjection function and the Interjection struct
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/src/interjection.rs`
  ...
  @ts_contract
  Scenario: codelet/napi/index.d.ts is byte-identical to the pre-RPC-043 baseline
    Given the RPC-043 changes are applied to the codelet workspace
    And the pre-RPC-043 baseline of `codelet/napi/index.d.ts` is committed
    When I run `git diff codelet/napi/index.d.ts`
  ...
  Scenario: A new shape test enforces the static structural contract
    ...
    Then the command exits with code 0
    And the test asserts `codelet/napi/src/session_manager.rs` does not exist
  ...
  @cargo
  @manifest
  Scenario: codelet/napi/Cargo.toml carries inline RPC-043 comments naming consumer modules
    Given the RPC-043 changes are applied to the codelet workspace
    When I open `codelet/napi/Cargo.toml`
  ...
  Scenario: Dependent shape tests retarget to session_bindings.rs after session_manager.rs deletion
    Given codelet/sessions/tests/background_session_shape.rs was authored by RPC-039 and extended by RPC-041 to grep codelet/napi/src/session_manager.rs for re-export and GLOBAL_CHUNK_CALLBACK invariants
    ...
    Given RPC-043 deletes codelet/napi/src/session_manager.rs and moves its re-exports and #[napi] wrappers verbatim into codelet/napi/src/session_bindings.rs
    Then the `napi_shell_path()` helper in background_session_shape.rs resolves to codelet/napi/src/session_bindings.rs and that file exists on disk
