...
Feature: fspec remove-capability CLI subcommand (Rust port)
  ...
  codelet/fspec/src/remove_capability.rs marshals the clap arg into a JSON
  ...
  captured fixture at codelet/fspec/tests/fixtures/help/remove-capability.txt.
  """
  ...
  Scenario: Clap exposes remove-capability with one positional arg in --help
    ...
    When I run `./codelet/target/release/fspec remove-capability --help`
  ...
  Scenario: CLI removes a capability and prints the success line on stdout
    ...
    When I run `./codelet/target/release/fspec remove-capability "Search"`
    Then the command exits 0
  ...
  Scenario: CLI prints the no-capabilities detail line and exits 1 when none exist
    ...
    When I run `./codelet/target/release/fspec remove-capability "X"`
  ...
  Scenario: CLI lists available capabilities and exits 1 when the name is not found
    ...
    When I run `./codelet/target/release/fspec remove-capability "Login"`
    Then the command exits with code 1
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./codelet/target/release/fspec remove-capability "B"` afterwards exits 0
    And spec/foundation.json solutionSpace.capabilities is empty
    And the CLI bridge module codelet/fspec/src/remove_capability.rs contains NO inline draft probing, matching, or JSON-mutation logic — its only computation is JSON arg marshalling

  Scenario: remove-capability --help is byte-for-byte identical to the captured fixture
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec remove-capability --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/remove-capability.txt
