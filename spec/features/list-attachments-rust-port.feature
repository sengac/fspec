...
Feature: Port list-attachments command to Rust
  """
  Implementation reads the `attachments` array from `WorkUnit.extra` (serde flatten map). A typed field on `WorkUnit` was considered but deferred — see test `shared_infrastructure_and_ported_wiring_are_in_place` in `rust/fspec-core/tests/list_attachments.rs` for the Phase-C architecture-revision rationale.
  ...
  CLI bridge module `rust/fspec/src/list_attachments.rs` defines `pub struct CliArgs { pub work_unit_id: String }` and `pub async fn run(args: CliArgs) -> Result<u8>` — symmetric with the list_prefixes/list_work_units bridges so future flag additions land as field additions only. The bridge contains NO inline rendering or filesystem logic — only JSON arg marshalling + `print!(rendered)` + `eprintln!("Error: {err}")`
  ...
  #   14. The standalone fspec binary at rust/fspec/src/main.rs MUST expose `list-attachments` as a clap v4 derive subcommand with a single required positional argument `<work_unit_id>` and NO flags — matching the TS Commander registration at src/commands/list-attachments.ts:62-66 (no --status, no --prefix, no --format, no --workspace)
  ...
  #   10. Running `./rust/target/release/fspec list-attachments AUTH-001` in a tempdir whose AUTH-001 has no attachments prints "No attachments found for work unit AUTH-001" to stdout and exits 0
  #   11. Running `./rust/target/release/fspec list-attachments --help` prints clap-generated help with the required <WORK_UNIT_ID> positional and NO --status / --prefix / --epic / --format / --workspace flags listed
  #   12. Running `./rust/target/release/fspec list-attachments NONEXISTENT-001` in a tempdir whose work-units.json does not contain NONEXISTENT-001 prints `Error: Work unit 'NONEXISTENT-001' does not exist` to stderr and exits with code 1
  #   13. Running `./rust/target/release/fspec list-attachments` (no positional) exits with clap's standard usage error (code 2) and stderr names the missing required argument — does NOT delegate to fspec_core because clap validates first
  #   14. Both invocation paths produce the SAME structured data: (a) dispatch_command("list-attachments", `{"workUnitId":"AUTH-001","format":"json"}`, project_root) and (b) `./rust/target/release/fspec list-attachments AUTH-001` against the same on-disk state — the only differences are how args are parsed (JSON vs clap positional) and how the result is delivered (DispatchResult.data JSON vs stdout text)
  #
  # ========================================
