...
Feature: show-acceptance-criteria CLI subcommand on the standalone fspec Rust binary
  """
  CLI subcommand wired into codelet/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::show_acceptance_criteria::run(args_json) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes show-acceptance-criteria as a subcommand and prints flag help
    ...
    When I run `./codelet/target/release/fspec show-acceptance-criteria --help` from a shell
  ...
  Scenario: CLI against workspace with no spec/features exits 1
    ...
    When I run `./codelet/target/release/fspec show-acceptance-criteria` from that directory
  ...
  Scenario: CLI default text output prints feature name and scenario steps
    ...
    When I run `./codelet/target/release/fspec show-acceptance-criteria --tag @auth` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --format=markdown prints H1/H2/bullet step output
    ...
    When I run `./codelet/target/release/fspec show-acceptance-criteria --tag @auth --format markdown` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --format=json prints 2-space JSON array
    ...
    When I run `./codelet/target/release/fspec show-acceptance-criteria --tag @auth --format json` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --output writes file and prints message without dumping content
    Given a temp workspace contains spec/features/login.feature tagged '@auth' with one scenario
    When I run `./codelet/target/release/fspec show-acceptance-criteria --tag @auth --format markdown --output out.md` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --tag matching zero features prints 'No features found matching tags'
    ...
    When I run `./codelet/target/release/fspec show-acceptance-criteria --tag @missing` from that workspace
    Then the command exits 0
  ...
  Scenario: show-acceptance-criteria --help is byte-for-byte identical to the TS formatCommandHelp reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec show-acceptance-criteria --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/show-acceptance-criteria.txt
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./codelet/target/release/fspec show-acceptance-criteria --tag @auth --format json` against the same workspace
    Then both invocations produce equivalent JSON for the features array
    And the CLI bridge module codelet/fspec/src/show_acceptance_criteria.rs contains NO inline gherkin parsing, filter, or rendering logic — its only computation is JSON arg marshalling
