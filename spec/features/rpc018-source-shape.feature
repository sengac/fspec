...
Feature: RPC-018 source-shape regression for the AgentView chrome port + shared types + new RPC methods
  ...
  1. `rust/rpc-types/src/lib.rs` — three new types
  `ModelInfo`, `ThinkingLevel`, `WorkspaceInfo` (cfg-gated for napi).
  2. `rust/rpc/src/lib.rs` — `FspecService` trait gains three new
  ...
  6. `rust/fspec-tui/src/views/agent/header.rs` +
  `rust/fspec-tui/src/views/agent/footer.rs` — new widget
  modules under 300 LoC each.
  7. `rust/fspec-tui/src/views/agent.rs` OR
  `rust/fspec-tui/src/views/agent/mod.rs` — orchestrator stays
  under 300 LoC.
  8. `rust/napi/src/git.rs` (or sibling) — additive
  `napi::get_workspace_info(cwd)` export delegates to
  `codelet_git::status::get_current_branch`.
  9. `rust/napi/src/session_manager.rs` (or sibling) — additive
  ...
  Scenario: New shared types live in rpc-types
    Given rust/rpc-types/src/lib.rs after RPC-018 lands
  ...
  Scenario: FspecService trait gains three new RPC methods
    Given rust/rpc/src/lib.rs after RPC-018 lands
  ...
  Scenario: SessionManagerHandle trait gains get_model_info / get_thinking_level with default impls
    Given rust/core/src/session_manager_handle.rs after RPC-018 lands
  ...
  Scenario: FspecBackend trait declares the three new methods
    Given rust/fspec-tui/src/transport/mod.rs after RPC-018 lands
  ...
  Scenario: Both transports implement the three new FspecBackend methods
    Given the rust/fspec-tui crate after RPC-018 lands
    Then rust/fspec-tui/src/transport/embedded.rs contains the substring "async fn get_model_info"
    And rust/fspec-tui/src/transport/embedded.rs contains the substring "async fn get_thinking_level"
    And rust/fspec-tui/src/transport/embedded.rs contains the substring "async fn get_workspace_info"
    And rust/fspec-tui/src/transport/websocket.rs contains the substring "async fn get_model_info"
    And rust/fspec-tui/src/transport/websocket.rs contains the substring "async fn get_thinking_level"
    And rust/fspec-tui/src/transport/websocket.rs contains the substring "async fn get_workspace_info"

  Scenario: New agent widget modules exist as separate files
    Given the rust/fspec-tui crate after RPC-018 lands
    Then the file rust/fspec-tui/src/views/agent/header.rs exists
    And the file rust/fspec-tui/src/views/agent/footer.rs exists

  Scenario: New and modified agent modules stay under 300 lines
    Given the directory rust/fspec-tui/src/views/agent/ plus the views/agent.rs orchestrator (or views/agent/mod.rs)
  ...
  Scenario: Action enum gains three new variants
    Given rust/fspec-tui/src/components/mod.rs after RPC-018 lands
  ...
  Scenario: NAPI surface exposes additive get_workspace_info export
    Given rust/napi/src/git.rs after RPC-018 lands
