...
Feature: fspec remove-aggregate-from-foundation CLI subcommand (Rust port)
  ...
  rust/fspec/src/remove_aggregate_from_foundation.rs marshals the clap args into a JSON
  ...
  fixture at rust/fspec/tests/fixtures/help/remove-aggregate-from-foundation.txt).
  """
  ...
  Scenario: Clap exposes remove-aggregate-from-foundation with positional args in --help
    ...
    When I run `./rust/target/release/fspec remove-aggregate-from-foundation --help`
  ...
  Scenario: CLI soft-deletes an aggregate and prints the success message on stdout
    ...
    When I run `./rust/target/release/fspec remove-aggregate-from-foundation Sales Order`
    Then the command exits 0
  ...
  Scenario: CLI rejects an unknown aggregate with exit 1 and stderr Error prefix
    Given spec/foundation.json contains a bounded_context 'Sales' with id=0 and an aggregate 'Order' linked to it
    When I run `./rust/target/release/fspec remove-aggregate-from-foundation Sales Ghost`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec remove-aggregate-from-foundation Sales Shipment` afterwards exits 0
    And both the 'Order' and 'Shipment' aggregates have deleted=true
    And the CLI bridge module rust/fspec/src/remove_aggregate_from_foundation.rs contains NO inline bounded-context lookup, ensure_foundation_file, or JSON-mutation logic — its only computation is JSON arg marshalling

  Scenario: remove-aggregate-from-foundation --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec remove-aggregate-from-foundation --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/remove-aggregate-from-foundation.txt
