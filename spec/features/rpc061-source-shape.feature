...
Feature: RPC-061 source-shape — supervisor surface file layout
  ...
  Action enum, SessionHeader, SessionFooter, dispatch_rpc061,
  ...
  Scenario: dispatch_rpc061.rs has the documented helper surface
    Given the file codelet/fspec-tui/src/app/dispatch_rpc061.rs is compiled
    Then it declares method "handle_supervisors_loaded"
    And it declares method "handle_send_to_subordinate"
    And it declares method "try_dispatch_rpc061"
  ...
  Scenario: app/dispatch.rs catch-all routes through try_dispatch_rpc061
    ...
    Then it calls self.try_dispatch_rpc061
    And the file stays under 300 lines
