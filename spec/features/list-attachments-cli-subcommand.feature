...
Feature: List attachments CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::list_attachments::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: list-attachments --help is byte-for-byte identical to TS reference output
    ...
    When I run `./rust/target/release/fspec list-attachments --help` piped to non-TTY (no color codes)
  ...
  Scenario: CLI exits 2 when the required positional argument is missing
    ...
    When I run `./rust/target/release/fspec list-attachments` (no positional) from that directory
  ...
  Scenario: CLI prints the empty-attachments sentinel and exits 0 when the work unit has no attachments
    ...
    When I run `./rust/target/release/fspec list-attachments AUTH-001`
    Then the command exits 0
  ...
  Scenario: CLI exits 1 and writes to stderr when the requested work unit does not exist
    ...
    When I run `./rust/target/release/fspec list-attachments NONEXISTENT-001`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher (no duplicated business logic)
    ...
    Then the CLI bridge module rust/fspec/src/list_attachments.rs contains NO inline rendering, file-stat, or work-unit-lookup logic — its only computation is JSON arg marshalling
