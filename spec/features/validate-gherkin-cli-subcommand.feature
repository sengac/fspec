...
Feature: Validate Gherkin CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::validate::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: CLI validates a single valid file and exits 0
    ...
    When I run `./rust/target/release/fspec validate spec/features/login.feature` from that directory
  ...
  Scenario: CLI exits 1 against a syntactically broken file
    ...
    When I run `./rust/target/release/fspec validate spec/features/broken.feature` from that directory
  ...
  Scenario: CLI exits 2 when no feature files are found
    ...
    When I run `./rust/target/release/fspec validate` from that directory
  ...
  Scenario: Clap exposes validate as a subcommand and prints help byte-identical to the TS reference
    ...
    When I run `./rust/target/release/fspec validate --help` from a shell
    Then the command exits 0
