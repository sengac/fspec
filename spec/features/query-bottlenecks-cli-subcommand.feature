...
Feature: query-bottlenecks clap subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/query_bottlenecks.rs → fspec_core::commands::query_bottlenecks::run
  ...
  --help is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/query-bottlenecks.txt.
  """
  ...
  Scenario: Clap exposes query-bottlenecks as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec query-bottlenecks --help` from a shell
  ...
  Scenario: CLI without options prints '✓ No bottlenecks found' against an empty workspace
    ...
    When I run `./rust/target/release/fspec query-bottlenecks` from that directory
    Then the command exits 0
    And stdout contains the exact line '✓ No bottlenecks found'

  Scenario: CLI with --output=json prints empty bottlenecks array
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./rust/target/release/fspec query-bottlenecks --output json` from that directory
    Then the command exits 0
  ...
  Scenario: CLI text output lists each qualifying bottleneck with its score
    ...
    When I run `./rust/target/release/fspec query-bottlenecks` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./rust/target/release/fspec query-bottlenecks --output json` against the same workspace
    Then both invocations produce JSON with bottlenecks[0].id='A' and bottlenecks[0].score=3
    And the CLI bridge module rust/fspec/src/query_bottlenecks.rs contains NO inline DFS, filtering, or rendering logic — its only computation is JSON arg marshalling and stdout printing

  Scenario: query-bottlenecks --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec query-bottlenecks --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/query-bottlenecks.txt
