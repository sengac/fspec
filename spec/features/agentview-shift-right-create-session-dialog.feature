...
Feature: AgentView Shift+Right does not open CreateSessionDialog and option styling drifts from TS Ink — dialog flag set but never pushed onto compositor
  ...
  """
  TS canonical source: src/components/CreateSessionDialog.tsx (135 lines). Rust port target: codelet/fspec-tui/src/components/create_session_dialog.rs (rewrite render). Dispatch wiring: codelet/fspec-tui/src/app/dispatch_rpc024.rs::handle_session_cycle NavTarget::CreateDialog branch.
  ...
  Mount path (single unified entry point): Action::OpenCreateSessionDialog{preselect} → App::handle_open_create_session_dialog (dispatch_rpc060.rs) → compositor.push(CreateSessionDialog::new(preselect, work_unit).with_action_tx(tx)). Already idempotent on CREATE_SESSION_DIALOG_ID. Shift+Right path is rewired to use this same helper instead of the orphan store-flag setter.
