...
Feature: Provider settings list: Tab keybind emits SwitchToModels event
  """
  [0] This card complements the full integration coverage already provided by RPC-160 (rust/fspec-tui/tests/provider_settings_tab_switch_to_models_rpc160.rs). Pattern matches RPC-077 / RPC-149 / RPC-151 / RPC-156 fast structural source-string regression-shape complement to slow integration tests.
  [1] Test file: rust/fspec-tui/tests/rpc152_tab_switch_to_models_shape.rs — sub-millisecond execution, no key event simulation, just source-string scanning of mod.rs (enum) + list.rs (Tab arm + filter_mode gate).
  [2] Source paths: rust/fspec-tui/src/views/provider_settings/mod.rs (enum `ProviderSettingsEvent` lines 57-70 — `SwitchToModels,` variant at line 69) + rust/fspec-tui/src/views/provider_settings/list.rs (`handle_list_key` line 30, `if view.filter_mode { return handle_filter_key(view, key); }` line 34, `KeyCode::Tab => ProviderSettingsEvent::SwitchToModels,` line 64, `handle_filter_key` line 161).
  """
  ...
  Scenario: ProviderSettingsEvent enum declares SwitchToModels variant
    Given I read the source of rust/fspec-tui/src/views/provider_settings/mod.rs
  ...
  Scenario: handle_list_key body binds Tab to SwitchToModels via an expression arm
    Given I read the source of rust/fspec-tui/src/views/provider_settings/list.rs
