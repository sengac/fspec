...
Feature: validate-spec-alignment clap subcommand on the standalone fspec Rust binary
  ...
  --help is byte-for-byte identical to the captured TS fixture at codelet/fspec/tests/fixtures/help/validate-spec-alignment.txt.
  """
  ...
  Scenario: Clap exposes validate-spec-alignment as a subcommand and prints --help
    ...
    When I run `./codelet/target/release/fspec validate-spec-alignment --help` from a shell
  ...
  Scenario: CLI exits 0 and prints success when the work unit has tagged scenarios
    ...
    When I run `./codelet/target/release/fspec validate-spec-alignment AUTH-001` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI exits 1 when the work unit does not exist
    ...
    When I run `./codelet/target/release/fspec validate-spec-alignment MISSING-999` from that workspace
    Then the command exits with code 1
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./codelet/target/release/fspec validate-spec-alignment AUTH-001` against the same workspace
    Then both invocations agree the work unit is valid
    And the CLI bridge module codelet/fspec/src/validate_spec_alignment.rs contains NO inline scan logic — its only computation is JSON arg marshalling and stdout/stderr printing

  Scenario: validate-spec-alignment --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec validate-spec-alignment --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/validate-spec-alignment.txt
