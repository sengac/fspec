...
Feature: init CLI subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/init.rs → fspec_core::commands::init::run
  ...
  rust/fspec/tests/fixtures/help/init.txt.
  """
  ...
  Scenario: Clap exposes init as a subcommand and prints --help
    ...
    When I run `./rust/target/release/fspec init --help` from a shell
  ...
  Scenario: CLI without --agent fails because the shell is non-TTY
    Given an empty directory is set as the current working directory
    When I run `./rust/target/release/fspec init` from that directory
  ...
  Scenario: CLI rejects an unknown agent id
    Given an empty directory is set as the current working directory
    When I run `./rust/target/release/fspec init --agent bogus` from that directory
    Then the command exits with code 1
  ...
  Scenario: init --help is byte-for-byte identical to the TS reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec init --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/init.txt
