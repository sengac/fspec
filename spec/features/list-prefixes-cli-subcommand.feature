...
Feature: List prefixes CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::list_prefixes::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes list-prefixes as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec list-prefixes --help` from a shell
  ...
  Scenario: CLI against empty directory prints sentinel and does not auto-create files
    ...
    When I run `./rust/target/release/fspec list-prefixes` from that directory
    Then the command exits 0
  ...
  Scenario: CLI text output renders prefix progress for the populated case
    ...
    When I run `./rust/target/release/fspec list-prefixes`
    Then the command exits 0
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher (no duplicated business logic)
    ...
    Then the CLI bridge module rust/fspec/src/list_prefixes.rs contains NO inline prefix-aggregation, filter, or rendering logic — its only computation is JSON arg marshalling

  Scenario: list-prefixes --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec list-prefixes --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-prefixes.txt
