...
Feature: show-feature CLI subcommand on the standalone fspec Rust binary
  """
  The CLI bridge module rust/fspec/src/show_feature.rs marshals argv into JSON
  ...
  Scenario: Clap exposes show-feature as a subcommand and prints flag-aware help
    ...
    When I run `./rust/target/release/fspec show-feature --help` from a shell
  ...
  Scenario: show-feature against a workspace with no spec/features prints feature-not-found and exits 1
    ...
    When I run `./rust/target/release/fspec show-feature missing` from that directory
  ...
  Scenario: CLI text output renders feature contents and Work Units None for a tag-free feature
    ...
    When I run `./rust/target/release/fspec show-feature login` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI text output renders Work Unit progress block when the feature carries a work-unit tag
    ...
    When I run `./rust/target/release/fspec show-feature auth` from that workspace
    Then the command exits 0
  ...
  Scenario: show-feature --help is byte-for-byte identical to TS formatCommandHelp reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec show-feature --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-feature.txt
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./rust/target/release/fspec show-feature auth` against the same workspace
    Then both invocations produce the exact line '  AUTH-001 (feature-level) - Login'
    And the CLI bridge module rust/fspec/src/show_feature.rs contains NO inline gherkin parsing, work-unit aggregation, or text rendering — its only computation is JSON arg marshalling
