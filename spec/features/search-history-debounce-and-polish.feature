...
Feature: /search slash command end-to-end (UI view)
  ...
  [E] File layout: changes confined to (1) `rust/fspec-tui/src/views/agent/search_history_view.rs` — add j/k branch, add `highlight_query` helper, extend handle_key tests; (2) `rust/fspec-tui/src/app/dispatch_resume_search_views.rs` — add debounce + stale-discard logic; (3) `rust/fspec-tui/src/components/mod.rs` — widen `Action::HistorySearchResults` variant; (4) new integration test file `rust/fspec-tui/tests/search_view_rpc064.rs` covering debounce + stale + insert + dismiss + j/k + highlight. NO new dispatch_rpc064.rs file expected (logic fits in existing dispatch_resume_search_views.rs). All affected files must stay under 300 lines — search_history_view.rs is at 297 lines currently, so the j/k + highlight changes will push it past the budget; extract the renderer body to a new sibling file `search_history_view_render.rs` if needed.
  [F] Backwards-compat impact: widening `Action::HistorySearchResults` from a tuple variant `(Vec<HistoryMatch>)` to a struct variant breaks every test that constructed it. Affected tests today: `rpc026_app_dispatch.rs` does NOT construct it directly (the spawned task does). Only the dispatcher path constructs it via `action_tx.send(Action::HistorySearchResults(matches))` in `dispatch_resume_search_views.rs` line 158. So the impact is limited to one production-code line plus the new test file. Confirmed by `Grep HistorySearchResults` — only 2 hits.
  [G] App state for debounce: add `last_search_debounce: Option<tokio::task::JoinHandle<()>>` to `App` (in `src/app/state.rs`). Each `handle_search_history` call aborts the previous handle (if any), then spawns a new tokio task that does `tokio::time::sleep(Duration::from_millis(150)).await` then calls `backend.persistence_search_history(query)` and dispatches `Action::HistorySearchResults { query, matches }`. The spawn ALSO pushes the new handle into `App::pending_tasks` so the existing test harness drains it. The 150ms constant lives as `const SEARCH_DEBOUNCE_MS: u64 = 150;` in `dispatch_resume_search_views.rs`.
  """
