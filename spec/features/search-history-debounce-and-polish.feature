...
Feature: /search slash command end-to-end (UI view)
  ...
  [E] File layout: changes confined to (1) `codelet/fspec-tui/src/views/agent/search_history_view.rs` — add j/k branch, add `highlight_query` helper, extend handle_key tests; (2) `codelet/fspec-tui/src/app/dispatch_resume_search_views.rs` — add debounce + stale-discard logic; (3) `codelet/fspec-tui/src/components/mod.rs` — widen `Action::HistorySearchResults` variant; (4) new integration test file `codelet/fspec-tui/tests/search_view_rpc064.rs` covering debounce + stale + insert + dismiss + j/k + highlight. NO new dispatch_rpc064.rs file expected (logic fits in existing dispatch_resume_search_views.rs). All affected files must stay under 300 lines — search_history_view.rs is at 297 lines currently, so the j/k + highlight changes will push it past the budget; extract the renderer body to a new sibling file `search_history_view_render.rs` if needed.
