...
Feature: Mux focus flash — settled final frame persists on the active pane
  """
  Implementation:
  - the render path keeps calling flash_cells but clamps the clock to the settle boundary — after the 350ms window the strip stays parked at the top row (the LAST_PAINT_MS frame — the scan's final bottom-to-top frame) instead of vanishing. The paint pass must render for the focused pane whenever mux is enabled, not only while is_flash_active(); the tick-gate operand (is_mux_flash_active) keeps its 350ms semantics (animation only, no perpetual redraws). State stays live-only on MultiplexLayout (MuxConfig serde shape unchanged).
  """
  ...
  #
  # BUSINESS RULES:
  #   1. R1: after the 350ms scan window (MUX-006) elapses, the focused pane keeps the scan's FINAL frame painted on every subsequent mux frame — a 1-ROW-HIGH dark-purple bar (MUX_FOOTER_BG, RGB 74, 44, 112) across the pane's TOP row (full pane width) — until focus moves or mux is disabled (1-row panes settle to that same single row) (SUPERSEDED 2026-08-31 by MUX-008: the settled frame was a full-height 2-column left-edge strip)
  #   2. R2: the settled strip moves with focus — when focus changes, the previously focused pane loses its settled strip (no purple remains in that pane) and the newly focused pane plays a fresh 350ms scan that settles into its own top-row bar (MUX-006 R4 re-arm behavior is preserved)
  ...
  #
  # EXAMPLES:
  #   1. With Board | Agent active and focus on the Board pane: after the 350ms scan ends, the Board pane keeps a 1-row-high dark-purple bar across its TOP row (full width) on every frame — the Board pane's top row is purple across all columns, nothing else in the body is purple
  #   2. After the Agent pane's strip has settled, pressing Shift+Left moves focus to the Board pane: the Agent pane's purple strip vanishes immediately (the next frame shows zero purple in the Agent pane) and a fresh 350ms scan sweeps the Board pane bottom-to-top before its own top-row bar settles
  ...
  #   4. With the session idle and the scan finished, the mux pane shows the settled top-row bar with no continuous redrawing — the run-loop draw gate reports idle (no animation in flight) yet the bar is still visible on the next user-triggered frame
  #
  # ========================================
  Background: User Story
    As a developer supervising multiple agents in mux mode
    I want to keep the focus-flash final frame (dark-purple 1-row top bar) painted on the active pane after the 350ms scan ends
  ...
  # R1: after the scan ends, the focused pane keeps the final (top-row) frame
  Scenario: the active pane keeps the settled 1-row top bar after the scan ends
    ...
    Then the Board pane shows the dark purple background (RGB 74, 44, 112) as a 1-row-high bar across the full pane width on the pane's top row
    And the Agent pane rect has no dark purple background
    And rendering more frames later the Board pane still shows the same 1-row top bar
  ...
  Scenario: the settled strip is repaint content that does not force continuous redrawing
    ...
    And the next user-triggered frame still paints the settled 1-row top bar on the Agent pane
