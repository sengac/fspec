...
Feature: dependencies clap subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/dependencies.rs → fspec_core::commands::dependencies::run
  ...
  --help is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/dependencies.txt.
  """
  ...
  Scenario: Clap exposes dependencies as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec dependencies --help` from a shell
  ...
  Scenario: CLI prints the header and relationship lines for a unit with dependencies
    ...
    When I run `./rust/target/release/fspec dependencies AUTH-001` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --graph prints the depth-first blocks tree
    ...
    When I run `./rust/target/release/fspec dependencies AUTH-001 --graph` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI exits 1 and writes to stderr when the work unit does not exist
    ...
    When I run `./rust/target/release/fspec dependencies INVALID-999` from that workspace
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    Then the DispatchResult.data equals the stdout produced by running `./rust/target/release/fspec dependencies AUTH-001`
    Then the CLI bridge module rust/fspec/src/dependencies.rs contains NO inline rendering, traversal, or filter logic — its only computation is JSON arg marshalling and stdout printing

  Scenario: dependencies --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec dependencies --help` piped to non-TTY
    Then the command exits 0
    Then stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/dependencies.txt
