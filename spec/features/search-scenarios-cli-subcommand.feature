...
Feature: Port search-scenarios command to Rust
  ...
  Scenario: Clap exposes search-scenarios as a subcommand and prints flag help
    ...
    When I run `./codelet/target/release/fspec search-scenarios --help` from a shell
  ...
  Scenario: CLI without --query flag exits non-zero
    ...
    When I run `./codelet/target/release/fspec search-scenarios` from that directory
  ...
  Scenario: CLI default output prints the green summary line
    ...
    When I run `./codelet/target/release/fspec search-scenarios --query Login` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --json prints the 2-space JSON envelope to stdout
    Given a temp workspace contains spec/features with a feature whose scenario is named "Login with valid credentials"
    When I run `./codelet/target/release/fspec search-scenarios --query Login --json` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --regex sets searchMode to regex
    ...
    When I run `./codelet/target/release/fspec search-scenarios --query valid.* --regex --json` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI exits non-zero on an invalid regex pattern
    ...
    When I run `./codelet/target/release/fspec search-scenarios --query [ --regex` from that workspace
    Then the command exits with a non-zero status
  ...
  Scenario: search-scenarios --help is byte-for-byte identical to the TS formatCommandHelp reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec search-scenarios --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/search-scenarios.txt
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./codelet/target/release/fspec search-scenarios --query Login --json` against the same workspace
    Then both invocations produce a JSON envelope with the same scenarios array length
    And the CLI bridge module codelet/fspec/src/search_scenarios.rs contains NO inline filtering or rendering logic — its only computation is JSON arg marshalling
