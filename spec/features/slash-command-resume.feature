...
Feature: /resume durable restore via restore_session_messages + restore_session_token_state
  ...
  #   7. Source-shape regression: a grep over `codelet/fspec-tui/src/` finds zero matches for `codelet_napi`; every file under `codelet/fspec-tui/src/` (including the new dispatch_rpc049.rs if introduced) is < 300 LoC; `codelet/fspec-tui/src/app/dispatch.rs` is < 300 LoC after the new AttachToSession + SessionResumeComplete arms are wired.
  #
  # ========================================
