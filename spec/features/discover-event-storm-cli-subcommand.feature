...
Feature: discover-event-storm clap subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/discover_event_storm.rs → fspec_core::commands::discover_event_storm::run
  ...
  --help is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/discover-event-storm.txt.
  """
  ...
  Scenario: Clap exposes discover-event-storm as a subcommand and prints flag-free --help
    ...
    When I run `./rust/target/release/fspec discover-event-storm --help` from a shell
    Then the command exits 0
    Then stdout is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/discover-event-storm.txt
  ...
  Scenario: CLI emits guidance for a work unit in specifying status and exits 0
    ...
    When I run `./rust/target/release/fspec discover-event-storm AUTH-001` from that directory
