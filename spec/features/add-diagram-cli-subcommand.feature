...
Feature: fspec add-diagram CLI subcommand (Rust port)
  ...
  `<section>`, `<title>`, and `<code>`. The CLI bridge at codelet/fspec/src/add_diagram.rs
  ...
  codelet/fspec/tests/fixtures/help/add-diagram.txt).
  """
  ...
  Scenario: Clap exposes add-diagram with three positional args in --help
    ...
    When I run `./codelet/target/release/fspec add-diagram --help`
  ...
  Scenario: CLI adds a new diagram and prints the success block on stdout
    ...
    When I run `./codelet/target/release/fspec add-diagram Architecture "Component" "graph TD\n  A-->B"`
    Then the command exits 0
  ...
  Scenario: CLI replaces an existing diagram and prints the Updated success block
    ...
    When I run `./codelet/target/release/fspec add-diagram Architecture "System Overview" "graph LR\n  New-->X"`
    Then the command exits 0
  ...
  Scenario: CLI rejects an empty code argument with exit 1 and stderr Error prefix
    Given spec/foundation.json exists with architectureDiagrams=[]
    When I run `./codelet/target/release/fspec add-diagram Architecture "X" ""`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./codelet/target/release/fspec add-diagram Architecture "Via CLI" "graph TD\n  C-->D"` afterwards exits 0
    And spec/foundation.json architectureDiagrams contains two entries
    And the CLI bridge module codelet/fspec/src/add_diagram.rs contains NO inline mermaid validation, ensure_foundation_file, or JSON-mutation logic — its only computation is JSON arg marshalling

  Scenario: add-diagram --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec add-diagram --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/add-diagram.txt
