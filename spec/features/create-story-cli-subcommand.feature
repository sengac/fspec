...
Feature: fspec create-story CLI subcommand (Rust port)
  ...
  rust/fspec/src/create_story.rs marshals the clap args into a JSON object {prefix, title, description?,
  ...
  (TS reference) — captured as rust/fspec/tests/fixtures/help/create-story.txt.
  """
  ...
  Scenario: create-story --help is byte-for-byte identical to TS reference
    ...
    When I run `./rust/target/release/fspec create-story --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/create-story.txt
  ...
  Scenario: Clap exposes create-story with positional args and the three optional flags
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec create-story --help`
    Then the command exits 0
  ...
  Scenario: CLI creates a minimal story and prints the success block
    ...
    When I run `./rust/target/release/fspec create-story AUTH "User login"` in that tempdir
    Then the command exits 0
  ...
  Scenario: CLI creates a story with -e epic and includes the Epic line
    ...
    When I run `./rust/target/release/fspec create-story AUTH "User login" -e auth` in that tempdir
    Then the command exits 0
    And stdout contains the line '✓ Created story AUTH-001'
  ...
  Scenario: CLI rejects an unregistered prefix with exit 1 and stderr Error prefix
    ...
    When I run `./rust/target/release/fspec create-story NOPE "User login"` in that tempdir
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec create-story AUTH "Second"` afterwards exits 0
    And spec/work-units.json on disk contains both 'AUTH-001' and 'AUTH-002'
    And the CLI bridge module rust/fspec/src/create_story.rs contains NO inline foundation check, prefix validation, id generation, or file-write logic — its only computation is JSON arg marshalling
