...
Feature: rpc-types: field-level #[cfg_attr(feature = "napi", napi(js_name = ...))] uses unscoped `napi` attribute
  ...
  # codelet/rpc-types/src/lib.rs uses TWO different attribute spellings.
  ...
  Scenario: Reproducing the bug before the fix shows 34 unscoped-napi-attribute errors
    ...
    And the file codelet/rpc-types/src/lib.rs still contains 34 `#[cfg_attr(feature = "napi", napi(js_name = "..."))]` field-level decorations
  ...
  Scenario: After the fix, codelet-rpc-types builds cleanly with the napi feature enabled
    Given the fix (replacing every field-level `napi(js_name = "X")` with `serde(rename = "X")`) has been applied to codelet/rpc-types/src/lib.rs
  ...
  Scenario: TypeScript surface preserves every camelCase field after regeneration
    Given the fix has been applied to codelet/rpc-types/src/lib.rs
    When I run `cargo build -p codelet-napi --release` to regenerate codelet/napi/index.d.ts
  ...
  Scenario: BUG-146 fix unblocks the RPC-043 noop-build assertion
    Given the fix has been applied to codelet/rpc-types/src/lib.rs
    And RPC-043 (the 7-module split of codelet/napi/src/session_manager.rs) has NOT yet landed
    When I run `cargo build -p codelet-napi --features noop`
