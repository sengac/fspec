...
Feature: Port update-work-unit-estimate command to Rust
  ...
  Scenario: Dispatcher estimates a task without any feature file
    ...
    When I dispatch update-work-unit-estimate with workUnitId='TASK-001' and estimate=3
    Then the dispatcher returns success=true
    And spec/work-units.json work unit 'TASK-001' has estimate 3
    And the updatedAt of 'TASK-001' is set to a non-empty ISO-8601 string

  Scenario: Dispatcher rejects a non-Fibonacci estimate
    Given spec/work-units.json contains work unit 'TASK-001' of type 'task'
    When I dispatch update-work-unit-estimate with workUnitId='TASK-001' and estimate=7
    Then the dispatcher returns success=false
    And the error message contains the substring 'Failed to update work unit estimate'
    And the error message contains the substring 'Invalid estimate: 7. Must be one of: 1,2,3,5,8,13,21'

  Scenario: Dispatcher rejects a missing work unit
    Given an empty project root directory with no spec/ subdirectory
    When I dispatch update-work-unit-estimate with workUnitId='MISSING-999' and estimate=5
    Then the dispatcher returns success=false
  ...
  Scenario: Dispatcher blocks estimating a story without a feature file
    Given spec/work-units.json contains work unit 'AUTH-001' of type 'story'
    And spec/features contains no feature file tagged @AUTH-001
    When I dispatch update-work-unit-estimate with workUnitId='AUTH-001' and estimate=5
    Then the dispatcher returns success=false
    And the error message contains the substring 'ACDD VIOLATION'
    And the error message contains the substring 'without completed feature file'

  Scenario: Dispatcher blocks estimating a story whose feature file has prefill placeholders
    Given spec/work-units.json contains work unit 'AUTH-001' of type 'story'
    And spec/features contains a feature file tagged @AUTH-001 that contains a role-placeholder token
    When I dispatch update-work-unit-estimate with workUnitId='AUTH-001' and estimate=5
    Then the dispatcher returns success=false
    And the error message contains the substring 'ACDD VIOLATION'
    And the error message contains the substring 'prefill placeholders'

  Scenario: Dispatcher estimates a story with a clean tagged feature file
    Given spec/work-units.json contains work unit 'AUTH-001' of type 'story'
    And spec/features contains a clean feature file tagged @AUTH-001 with no prefill placeholders
    When I dispatch update-work-unit-estimate with workUnitId='AUTH-001' and estimate=5
    Then the dispatcher returns success=true
    And spec/work-units.json work unit 'AUTH-001' has estimate 5
