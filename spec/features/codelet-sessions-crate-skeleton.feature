...
Feature: Create codelet-sessions crate skeleton
  ...
  The integration test `rust/sessions/tests/smoke.rs` is the test artifact for ACDD coverage linking — `cargo test -p codelet-sessions` is the verifying command
  """
  ...
  #
  # BUSINESS RULES:
  #   1. The crate's package name is `codelet-sessions` and the path is `rust/sessions/`
  ...
  #   4. `rust/sessions/src/lib.rs` declares two placeholder modules: `pub mod background_session;` and `pub mod session_manager;` — both populated by later cards (RPC-039 / RPC-040)
  ...
  #   9. Smoke test exists at `rust/sessions/tests/smoke.rs` with a `#[test] fn crate_compiles() {}` to lock in the compile-only contract
  ...
  #   4. Developer opens `rust/sessions/src/lib.rs` and sees `pub mod background_session;` and `pub mod session_manager;` with placeholder doc comments referring forward to RPC-039 / RPC-040
  #   5. Developer opens the root `Cargo.toml` and sees `rust/sessions` listed in the workspace `members` array alongside the existing crates
  ...
  Scenario: Cargo workspace recognises the new codelet-sessions crate
    Given the root `Cargo.toml` lists `rust/sessions` as a workspace member and `codelet-sessions` as a workspace dependency
    When I run `cargo metadata -p codelet-sessions --format-version 1`
    Then the output JSON includes a package named `codelet-sessions` at version 0.1.0 with manifest path ending in `rust/sessions/Cargo.toml`
  ...
  Scenario: Smoke test runs and passes
    Given `rust/sessions/tests/smoke.rs` contains a `crate_compiles` test
  ...
  Scenario: lib.rs declares the placeholder modules for later RPC cards
    ...
    When I read `rust/sessions/src/lib.rs`
  ...
  Scenario: Workspace lints are inherited and clippy passes
    Given `rust/sessions/Cargo.toml` declares `[lints]` with `workspace = true`
