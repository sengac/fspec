...
Feature: review CLI subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/review.rs → fspec_core::commands::review::run
  ...
  Scenario: Clap exposes review as a subcommand and prints bare --help
    ...
    When I run `./rust/target/release/fspec review --help` from a shell
  ...
  Scenario: CLI reviews an existing work unit and prints the report
    ...
    When I run `./rust/target/release/fspec review <id>` from that directory
    Then the command exits 0
    Then stdout contains the substring 'REVIEW:'
  ...
  Scenario: CLI errors on a work unit id that does not exist
    ...
    When I run `./rust/target/release/fspec review BOGUS-999` from that directory
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And the CLI bridge module rust/fspec/src/review.rs contains NO review logic — its only computation is JSON arg marshalling and stdout printing
