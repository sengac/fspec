...
Feature: Lift SessionStore manifest + load_session + append_message_with_metadata into codelet-core::persistence::manifest
  ...
  #
  # BUSINESS RULES:
  #   1. SessionStore (struct + impl) plus SessionManifest TokenUsage MergeRecord PastedContent ForkPoint CompactionState SessionLineage all live in codelet/core/src/persistence/manifest.rs with identical serde derives, rename_all, and field order — no #[napi] attributes added
