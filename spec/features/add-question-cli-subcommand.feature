...
Feature: fspec add-question CLI subcommand (Rust port)
  """
  Clap derive subcommand `add-question` exposes the same surface as the TS Commander.js registration at src/commands/add-question.ts:86-100 — two positional arguments `<workUnitId>` and `<question>`, no flags. The bridge module at rust/fspec/src/add_question.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::add_question::run; no validation or rendering logic is duplicated.
  ...
  The `fspec add-question --help` output is byte-for-byte identical to `node dist/index.js add-question --help` (TS reference) — captured as rust/fspec/tests/fixtures/help/add-question.txt.
  """
  ...
  Scenario: Clap exposes add-question with two positional args in --help
    ...
    When I run `./rust/target/release/fspec add-question --help`
  ...
  Scenario: CLI adds a question and prints the success line
    ...
    When I run `./rust/target/release/fspec add-question AUTH-001 "Should we add OAuth?"`
    Then the command exits 0
  ...
  Scenario: CLI rejects unknown work unit with exit 1 and stderr Failed prefix
    ...
    When I run `./rust/target/release/fspec add-question AUTH-999 "Q?"`
  ...
  Scenario: CLI rejects wrong status with exit 1 and stderr Failed prefix
    ...
    When I run `./rust/target/release/fspec add-question AUTH-001 "Q?"`
    Then the command exits with code 1
    And stderr contains the substring '✗ Failed to add question:'
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec add-question AUTH-001 "from-cli"` afterwards exits 0
    And spec/work-units.json now contains both 'dispatched' and 'from-cli' question texts on AUTH-001
    And the CLI bridge module rust/fspec/src/add_question.rs contains NO inline state mutation or file-write logic — its only computation is JSON arg marshalling

  Scenario: add-question --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec add-question --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/add-question.txt
