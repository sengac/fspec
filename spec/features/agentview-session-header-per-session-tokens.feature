...
Feature: AgentView SessionHeader per-session token tracking parity — reasoning_tokens, tokens_per_second, cache_read/cache_creation, compaction_reduction not per-session in Rust port
  ...
  #
  # BUSINESS RULES:
  #   1. TokenState in AgentViewStore (rust/fspec-tui/src/store/agent_view.rs:40-67) MUST be extended to track all TokenTracker fields: input_tokens, output_tokens, cache_read_input_tokens, cache_creation_input_tokens, reasoning_tokens, tokens_per_second, AND context_fill_pct — all persisted per SessionId in token_state_by_session HashMap
  ...
  #   3. chrome_paint::paint_header_and_role (rust/fspec-tui/src/views/agent/chrome_paint.rs:25-71) MUST source tokens_per_second, reasoning_tokens, and compaction_reduction from store.token_state_for(sid) — currently these are hardcoded None/0/None at lines 58-60
