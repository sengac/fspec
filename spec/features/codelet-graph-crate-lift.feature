...
@codelet
@RPC-092
Feature: Lift rust/napi/src/graph/ (52 files, ~15,013 LOC) into a NAPI-free codelet-graph workspace crate — verbatim relocation, NO cut-downs
  """
  Source-of-truth: rust/napi/src/graph/ (52 .rs files, ~15,013 LOC, zero direct NAPI coupling). Lift target: rust/graph/ as a peer workspace crate. See spec/attachments/RPC-092/inventory.md for the complete file-by-file LOC table.
  Why this card was carved (instead of inlining into RPC-072 Phase B): the body port at rust/napi/src/agent_loop.rs:299-1456 calls graph_search_handler::create_handler() and register_deep_search_handler(...), both of which transitively pull crate::graph::*. Inlining a 15,013-LOC mechanical relocation into the 1,170-LOC body port would produce a single ~30,000-LOC commit with no checkpoint between stages — context-window-exhausting and impossible to bisect. RPC-092 is the discrete, atomic relocation step. See spec/attachments/RPC-092/root-cause-analysis.md.
  ...
  NAPI shim contract: replace the deleted rust/napi/src/graph/ directory with a single file rust/napi/src/graph.rs containing only `pub use codelet_graph::*;`. This preserves the crate::graph::* import surface for the 24 rust/napi/tests/ast_*_test.rs integration tests so they pass with ZERO test-source modification.
  ...
  Test pyramid (4 tiers): Tier 1 = NEW rust/graph/tests/no_napi_dependency.rs boundary guard (THE acceptance bar — mirrors RPC-067 pattern). Tier 2 = lifted graph_reset_test.rs (8 tests). Tier 3 = preserved rust/napi/tests/ast_*_test.rs suite (24 tests, must pass through shim with zero modification). Tier 4 = `cargo clippy --workspace -- -D warnings` clean. See spec/attachments/RPC-092/test-plan.md for the full Done definition.
  """
  ...
  #
  # BUSINESS RULES:
  #   1. codelet-graph MUST exist as a peer workspace member under rust/graph/ — added to rust/Cargo.toml [workspace] members and [workspace.dependencies]
  #   2. codelet-graph MUST have zero dependency edges to codelet-napi, napi, or napi-derive — enforced by a new rust/graph/tests/no_napi_dependency.rs that shells out to cargo metadata and asserts on the dependencies array
  ...
  #   4. All 4 bundled schemas (ast-code.pg, ast-queries.gq, learnings.pg, learnings-queries.gq) MUST move from rust/napi/schemas/ to rust/graph/schemas/ — they are compiled into the binary via include_str! and have no runtime path dependency
  #   5. rust/napi/src/graph/ directory MUST be replaced by a single file rust/napi/src/graph.rs containing only `pub use codelet_graph::*;` — this preserves the existing crate::graph::* import surface for the 24 rust/napi/tests/ast_*_test.rs integration tests
  #   6. All 24 existing rust/napi/tests/ast_*_test.rs integration tests MUST pass post-lift with zero modification to test source — the shim re-export is the contract
  #   7. The existing in-module graph_reset_tests.rs (8 #[cfg(test)] tests) MUST be lifted to rust/graph/tests/graph_reset_test.rs as a standalone integration test using codelet_graph::* imports — same 8 scenarios, same assertions
  #   8. codelet-agent-loop MUST be able to add `codelet-graph = { workspace = true }` to its [dependencies] and lift deep_search_handler.rs + graph_search_handler.rs without re-introducing a codelet-napi edge — verified by rust/agent-loop/tests/no_napi_dependency.rs still passing
  ...
  #   5. After the lift, the maintainer adds `codelet-graph = { workspace = true }` to rust/agent-loop/Cargo.toml and `pub mod deep_search_handler;` to rust/agent-loop/src/lib.rs — `cargo check -p codelet-agent-loop` succeeds AND `cargo test -p codelet-agent-loop --test no_napi_dependency` still passes
  ...
  #   8. Running `git log --follow rust/graph/src/ast_pipeline/ast_ts_extractor.rs` shows the full history dating back to the file's original creation under rust/napi/src/graph/ast_pipeline/ — git mv preserved blame, satisfying the verbatim-lift rule
  ...
  Background: User Story
    ...
    I want to lift rust/napi/src/graph/ verbatim into a NAPI-free codelet-graph crate
  ...
  Scenario: codelet-graph exists as a workspace member with the expected dependency surface
    Given the lift has been completed against the workspace at rust/Cargo.toml
  ...
  Scenario: codelet-graph boundary guard test passes
    Given the lift has been completed and rust/graph/tests/no_napi_dependency.rs is in place
  ...
  Scenario: Lifted graph reset integration tests still prove the reset semantics
    Given the old in-module rust/napi/src/graph/graph_reset_tests.rs has been lifted to rust/graph/tests/graph_reset_test.rs as a standalone integration test
  ...
  Scenario: All 24 existing NAPI ast_*_test.rs integration tests still pass through the shim
    Given the rust/napi/src/graph/ directory has been replaced by rust/napi/src/graph.rs containing only `pub use codelet_graph::*;`
    And no source under rust/napi/tests/ has been modified
  ...
  Scenario: codelet-agent-loop can depend on codelet-graph without re-introducing a NAPI edge
    ...
    When I add `codelet-graph = { workspace = true }` to rust/agent-loop/Cargo.toml and `pub mod deep_search_handler;` to rust/agent-loop/src/lib.rs
  ...
  Scenario: Git blame history is preserved through git mv
    ...
    When I run `git log --follow rust/graph/src/ast_pipeline/ast_ts_extractor.rs`
    Then the log shows the file's full history dating back to its original creation under rust/napi/src/graph/ast_pipeline/
    And the same property holds for every other file under rust/graph/src/
