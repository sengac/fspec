...
Feature: Agent loop: MCP injection drain (mcp_injection_rx tokio::select! arm + mcp_channel_open flag)
  ...
  Test file: codelet/agent-loop/tests/rpc089_mcp_injection_drain.rs — integration test that reads agent_loop.rs from CARGO_MANIFEST_DIR-relative path; sub-millisecond execution; pairs with the existing single behavioural scenario in spec/features/agent-loop-mcp-injection.feature
  """
  ...
  #
  # BUSINESS RULES:
  #   1. The canonical `agent_loop` function in `codelet/agent-loop/src/agent_loop.rs` MUST declare a parameter `mut mcp_injection_rx: mpsc::Receiver<McpInjection>` so MCP server messages can flow into the per-session loop
