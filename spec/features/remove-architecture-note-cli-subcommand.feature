...
Feature: fspec remove-architecture-note CLI subcommand (Rust port)
  """
  Clap derive subcommand `remove-architecture-note` exposes the same surface as the TS Commander.js registration at src/commands/remove-architecture-note.ts:88-107 — two positional arguments `<workUnitId>` and `<index>` (integer). The bridge module at rust/fspec/src/remove_architecture_note.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::remove_architecture_note::run; no soft-delete or rendering logic is duplicated.
  ...
  The `fspec remove-architecture-note --help` output is byte-for-byte identical to `node dist/index.js remove-architecture-note --help` — captured as rust/fspec/tests/fixtures/help/remove-architecture-note.txt.
  """
  ...
  Scenario: Clap exposes remove-architecture-note with positional args in --help
    ...
    When I run `./rust/target/release/fspec remove-architecture-note --help`
  ...
  Scenario: CLI soft-deletes an architecture note and prints the success line
    ...
    When I run `./rust/target/release/fspec remove-architecture-note AUTH-001 0`
    Then the command exits 0
  ...
  Scenario: CLI rejects an unknown work unit with exit 1 and stderr Error prefix
    ...
    When I run `./rust/target/release/fspec remove-architecture-note MISSING-001 0`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And the CLI bridge module rust/fspec/src/remove_architecture_note.rs contains NO inline soft-delete, ID-lookup, or file-write logic — its only computation is JSON arg marshalling

  Scenario: remove-architecture-note --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec remove-architecture-note --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/remove-architecture-note.txt
