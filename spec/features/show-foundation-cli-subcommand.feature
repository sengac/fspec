...
Feature: show-foundation clap subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/show_foundation.rs → fspec_core::commands::show_foundation::run
  ...
  --help is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/show-foundation.txt.
  """
  ...
  Scenario: Clap exposes show-foundation as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec show-foundation --help` from a shell
  ...
  Scenario: CLI default render prints PROJECT section to stdout
    ...
    When I run `./rust/target/release/fspec show-foundation` from that directory
    Then the command exits 0
  ...
  Scenario: CLI positional section emits raw string in text format
    ...
    When I run `./rust/target/release/fspec show-foundation projectName` from that directory
    Then the command exits 0
    Then stdout equals exactly 'fspec' (with a trailing newline)

  Scenario: CLI --format=json emits JSON
    Given spec/foundation.json contains project.name='fspec'
    When I run `./rust/target/release/fspec show-foundation projectName --format json` from that directory
    Then the command exits 0
    Then stdout starts with the bytes '"fspec"'

  Scenario: CLI exits 1 when section is unknown
    Given spec/foundation.json contains project.name='fspec'
    When I run `./rust/target/release/fspec show-foundation nonexistent` from that directory
  ...
  Scenario: CLI --draft surfaces missing-draft error
    ...
    When I run `./rust/target/release/fspec show-foundation --draft` from that directory
    Then the command exits with code 1
    Then stderr contains the substring 'Error:'
  ...
  Scenario: CLI --output writes file and stdout prints success line
    Given spec/foundation.json contains project.name='fspec'
    When I run `./rust/target/release/fspec show-foundation projectName --output out/name.txt` from that directory
    Then the command exits 0
  ...
  Scenario: CLI --list-sections is parsed but ignored (no-op parity)
    Given spec/foundation.json contains project.name='fspec'
    When I run `./rust/target/release/fspec show-foundation --list-sections` from that directory
    Then the command exits 0
    Then stdout contains the exact line '=== PROJECT ==='

  Scenario: CLI --line-numbers is parsed but ignored (no-op parity)
    Given spec/foundation.json contains project.name='fspec'
    When I run `./rust/target/release/fspec show-foundation --line-numbers` from that directory
    Then the command exits 0
    Then stdout contains the exact line '=== PROJECT ==='
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    Then the CLI bridge module rust/fspec/src/show_foundation.rs contains NO inline FIELD_MAP, formatter, or filesystem logic — its only computation is JSON arg marshalling and stdout printing

  Scenario: show-foundation --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec show-foundation --help` piped to non-TTY
    Then the command exits 0
    Then stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-foundation.txt
