...
Feature: Validate foundation schema CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::validate_foundation_schema::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: CLI validates a valid foundation and exits 0
    ...
    When I run `./rust/target/release/fspec validate-foundation-schema` from that directory
  ...
  Scenario: Clap exposes validate-foundation-schema as a subcommand and prints help byte-identical to the TS reference
    ...
    When I run `./rust/target/release/fspec validate-foundation-schema --help` from a shell
    Then the command exits 0
