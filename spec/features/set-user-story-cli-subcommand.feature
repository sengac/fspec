...
Feature: fspec set-user-story CLI subcommand (Rust port)
  """
  Clap derive subcommand `set-user-story` exposes the same surface as the TS Commander.js registration at src/commands/set-user-story.ts:65-80 — positional `<work-unit-id>` plus required `--role`, `--action`, `--benefit` flags. The bridge at rust/fspec/src/set_user_story.rs marshals the clap args into JSON and delegates to codelet_fspec_core::commands::set_user_story::run; no rendering or persistence logic is duplicated.
  Exit codes: 0 on success, 1 on any FspecCoreError. Errors are written to stderr prefixed with 'Error:'.
  The `fspec set-user-story --help` output is byte-for-byte identical to `node dist/index.js set-user-story --help` — captured as rust/fspec/tests/fixtures/help/set-user-story.txt.
  """
  ...
  Scenario: Clap exposes set-user-story with positional and required flags in --help
    ...
    When I run `./rust/target/release/fspec set-user-story --help`
  ...
  Scenario: CLI writes the user story and prints the success block
    ...
    When I run `./rust/target/release/fspec set-user-story AUTH-001 --role developer --action ship --benefit happiness`
    Then the command exits 0
  ...
  Scenario: CLI rejects an unknown work unit with exit 1 and stderr Error prefix
    ...
    When I run `./rust/target/release/fspec set-user-story MISSING-001 --role x --action y --benefit z`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And the CLI bridge module rust/fspec/src/set_user_story.rs contains NO inline userStory build, file-write, or success-line rendering — its only computation is JSON arg marshalling

  Scenario: set-user-story --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec set-user-story --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/set-user-story.txt
