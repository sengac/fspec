...
Feature: fspec remove-question CLI subcommand (Rust port)
  """
  Clap derive subcommand `remove-question` exposes the same surface as the TS Commander.js registration at src/commands/remove-question.ts:88-107 — two positional arguments `<workUnitId>` and `<index>`, no flags. The bridge module at rust/fspec/src/remove_question.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::remove_question::run; no validation or rendering logic is duplicated.
  ...
  The `fspec remove-question --help` output is byte-for-byte identical to `node dist/index.js remove-question --help` (TS reference) — captured as rust/fspec/tests/fixtures/help/remove-question.txt.
  """
  ...
  Scenario: Clap exposes remove-question with two positional args in --help
    ...
    When I run `./rust/target/release/fspec remove-question --help`
  ...
  Scenario: CLI removes a question and prints the success line
    ...
    When I run `./rust/target/release/fspec remove-question AUTH-001 0`
    Then the command exits 0
  ...
  Scenario: CLI rejects unknown question ID with exit 1 and stderr Failed prefix
    ...
    When I run `./rust/target/release/fspec remove-question AUTH-001 5`
  ...
  Scenario: CLI rejects unknown work unit with exit 1 and stderr Failed prefix
    ...
    When I run `./rust/target/release/fspec remove-question AUTH-999 0`
    Then the command exits with code 1
    And stderr contains the substring '✗ Failed to remove question:'
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec remove-question AUTH-001 1` afterwards exits 0
    And spec/work-units.json on disk shows both questions with deleted=true
    And the CLI bridge module rust/fspec/src/remove_question.rs contains NO inline state mutation or file-write logic — its only computation is JSON arg marshalling

  Scenario: remove-question --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec remove-question --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/remove-question.txt
