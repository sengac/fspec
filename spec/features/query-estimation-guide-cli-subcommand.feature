...
Feature: query-estimation-guide clap subcommand on the standalone fspec Rust binary
  ...
  --help is byte-for-byte identical to the captured TS fixture at codelet/fspec/tests/fixtures/help/query-estimation-guide.txt.
  """
  ...
  Scenario: Clap exposes query-estimation-guide as a subcommand and prints flag-aware --help
    ...
    When I run `./codelet/target/release/fspec query-estimation-guide --help` from a shell
  ...
  Scenario: CLI without --format prints nothing to stdout (TS silent-text parity)
    ...
    When I run `./codelet/target/release/fspec query-estimation-guide ANY-001` from that directory
    Then the command exits 0
  ...
  Scenario: CLI with --format=text also prints nothing to stdout (explicit text bug parity)
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./codelet/target/release/fspec query-estimation-guide ANY-001 --format text` from that directory
    Then the command exits 0
    And stdout is exactly empty

  Scenario: CLI with --format=json prints empty patterns array against an empty workspace
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./codelet/target/release/fspec query-estimation-guide ANY-001 --format json` from that directory
    Then the command exits 0
    And stdout parses as JSON whose root object has patterns=[]

  Scenario: CLI requires the positional workUnitId argument
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./codelet/target/release/fspec query-estimation-guide` from that directory with no positional argument
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./codelet/target/release/fspec query-estimation-guide ANY-001 --format json` against the same workspace
    Then both invocations produce JSON with patterns[0].points=3 and patterns[0].confidence='low'
    And the CLI bridge module codelet/fspec/src/query_estimation_guide.rs contains NO inline grouping, bucketing, or rendering logic — its only computation is JSON arg marshalling and stdout printing

  Scenario: query-estimation-guide --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec query-estimation-guide --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/query-estimation-guide.txt
