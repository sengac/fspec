...
Feature: Provider settings delete-confirm dialog: n/N cancel shortcut
  """
  [0] This card complements the full integration coverage already provided by RPC-164 (rust/fspec-tui/tests/confirm_dialog_yn_shortcut_rpc164.rs, 13 tests). Pattern matches RPC-077 / RPC-149 fast structural source-string regression-shape complement to slow integration tests.
  [1] Test file: rust/fspec-tui/tests/rpc156_delete_confirm_yn_shortcut_shape.rs — sub-millisecond execution, no key event simulation, just source-string scanning of confirm_dialog.rs.
  [2] Source path: rust/fspec-tui/src/views/agent/confirm_dialog.rs handle_key method (lines 138-168). Lines 162-165 contain the y/Y → outcome_for_index(0) and n/N → outcome_for_index(self.cancel_index()) arms added by RPC-164.
  """
  ...
  Scenario: confirm_dialog.rs handle_key binds n/N as cancel shortcut
    Given I read the source of rust/fspec-tui/src/views/agent/confirm_dialog.rs
