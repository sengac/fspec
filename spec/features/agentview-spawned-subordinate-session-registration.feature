...
Feature: Spawned subordinate agents are not registered/visible in the Rust TUI
  ...
  TUI subscriber: in codelet/fspec-tui/src/app/bootstrap.rs::spawn_subscriber_tasks, add a fifth subscriber task that consumes session_created_rx and folds each event into Action::SessionCreated(id). Mirror the work_units_rx lag-recovery pattern (RecvError::Lagged -> debug log + continue, RecvError::Closed -> break).
