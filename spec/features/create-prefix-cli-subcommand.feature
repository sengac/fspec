...
Feature: Create prefix CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::create_prefix::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes create-prefix as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec create-prefix --help` from a shell
  ...
  Scenario: CLI against empty directory creates the prefixes file and prints the success message
    ...
    When I run `./rust/target/release/fspec create-prefix AUTH "Auth features"` from that directory
    Then the command exits 0
  ...
  Scenario: CLI rejects a lowercase prefix with exit 1 and stderr Error prefix
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./rust/target/release/fspec create-prefix auth "bad case"` from that directory
  ...
  Scenario: CLI rejects a duplicate prefix without touching the file
    ...
    When I run `./rust/target/release/fspec create-prefix AUTH "Different desc"` from that directory
    Then the command exits with code 1
    Then stderr contains the substring 'Error:'
  ...
  Scenario: CLI missing positional argument fails with a clap usage error
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./rust/target/release/fspec create-prefix` from that directory
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher (no duplicated business logic)
    ...
    Then the CLI bridge module rust/fspec/src/create_prefix.rs contains NO inline validation, file IO, or rendering logic — its only computation is JSON arg marshalling and stdout/stderr printing

  Scenario: create-prefix --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec create-prefix --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/create-prefix.txt
