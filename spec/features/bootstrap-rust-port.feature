...
Feature: Port bootstrap command to Rust
  ...
  Scenario: CLI and dispatcher converge on the same fspec_core run function
    ...
    Then the CLI bridge module codelet/fspec/src/bootstrap.rs contains no documentation-building or transform logic — its only computation is JSON arg marshalling
