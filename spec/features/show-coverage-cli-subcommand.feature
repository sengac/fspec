...
Feature: show-coverage CLI subcommand on the standalone fspec Rust binary
  """
  The CLI bridge module rust/fspec/src/show_coverage.rs marshals argv into JSON
  ...
  Scenario: Clap exposes show-coverage as a subcommand and prints help on --help
    ...
    When I run `./rust/target/release/fspec show-coverage --help` from a shell
  ...
  Scenario: show-coverage with a missing feature exits 1 and writes the TS-parity error to stderr
    ...
    When I run `./rust/target/release/fspec show-coverage missing` from that directory
  ...
  Scenario: CLI per-feature mode renders markdown report to stdout for a fully covered feature
    ...
    When I run `./rust/target/release/fspec show-coverage auth` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI per-feature JSON mode renders 2-space-indented JSON for the requested feature
    ...
    When I run `./rust/target/release/fspec show-coverage auth --format json` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI project-wide mode aggregates and renders Project Coverage Report
    ...
    When I run `./rust/target/release/fspec show-coverage` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI project-wide mode exits 1 when spec/features/ is missing
    ...
    When I run `./rust/target/release/fspec show-coverage` from that directory
    Then the command exits 1
    And stderr contains the substring 'Error:'
  ...
  Scenario: CLI tolerates a trailing .feature on the positional feature-name
    ...
    When I run `./rust/target/release/fspec show-coverage login.feature` from that workspace
    Then the command exits 0
  ...
  Scenario: show-coverage --help is byte-for-byte identical to TS formatCommandHelp reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec show-coverage --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-coverage.txt
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./rust/target/release/fspec show-coverage auth --format json` against the same workspace
    Then both invocations produce byte-equal JSON content
    And the CLI bridge module rust/fspec/src/show_coverage.rs contains NO inline coverage parsing, stats aggregation, or markdown rendering — its only computation is JSON arg marshalling
