...
Feature: Final TS-frontend regression + boundary audit
  ...
  #
  # BUSINESS RULES:
  #   1. codelet/napi/src/session_manager.rs MUST be deleted
  #   2. codelet/napi/src/persistence/ MUST contain only mod.rs + napi_bindings.rs (plus optional test helpers)
  #   3. GLOBAL_CHUNK_CALLBACK static and the GlobalChunkCallback unsafe Send/Sync impls MUST be deleted from executable code
  #   4. tokio::broadcast::Sender<(SessionId, StreamChunk)> MUST be wired in codelet/sessions/src/{session_manager.rs, background_session.rs}
  #   5. No `.rs` file under codelet/{core,rpc,rpc-types,rpc-embedded,rpc-server,fspec,fspec-tui,sessions}/src/ MAY contain `use codelet_napi` or `codelet_napi::`
  #   6. No Cargo.toml in codelet/{core,rpc,rpc-types,rpc-embedded,rpc-server,fspec,fspec-tui,sessions} MAY declare an active codelet-napi dependency
  #   7. codelet/napi/index.d.ts MUST NOT remove any export from the pre-RPC-030 baseline (function-surface regression)
  ...
  #   5. Running `npm test` produces 4747 passing tests and the watch-024 supervisor-terminology suite reports 16/16 passing after the stale-path fix updates the test to read from the new union of `codelet/sessions/src/*.rs` + `codelet/napi/src/{session_bindings,agent_loop,bridges}.rs`
  ...
  Scenario: GLOBAL_CHUNK_CALLBACK is removed from executable code
    ...
    When I run `rg "static GLOBAL_CHUNK_CALLBACK" codelet/` from the repository root
    Then the search returns zero matches in executable Rust code
    And the only references that remain are doc-string comments in `codelet/sessions/src/*.rs` and assertion-only references inside `codelet/napi/tests/global_chunk_callback_napi_test.rs` and `codelet/sessions/tests/background_session_shape.rs`

  Scenario: TS-facing NAPI export surface is a strict superset of the pre-RPC-030 baseline
    Given the pre-RPC-030 baseline `codelet/napi/index.d.ts` from commit `ea0ed0a0`
    When I extract the `export declare function` identifiers from the baseline and from the current `codelet/napi/index.d.ts`
  ...
  Scenario: codelet-napi persistence collapses to a thin adapter
    ...
    When I list `codelet/napi/src/persistence/`
    Then the directory contains exactly `mod.rs` and `napi_bindings.rs`
    And `codelet/core/src/persistence/` contains the lifted pure-Rust modules `message_envelope.rs`, `messages.rs`, `manifest.rs`, `blob.rs`, `blob_processing.rs`, and `history.rs`

  Scenario: TS test suite remains green after the watch-024 path fix
    Given the RPC-030 chain has split `codelet/napi/src/session_manager.rs` across `codelet/sessions/src/*.rs` and `codelet/napi/src/{session_bindings,agent_loop,bridges}.rs`
