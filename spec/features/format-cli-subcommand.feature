...
Feature: format CLI subcommand on the standalone fspec Rust binary
  """
  CLI subcommand wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::format::run(args_json) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: CLI formats all feature files and prints the green summary
    ...
    When I run `./rust/target/release/fspec format` from that workspace
  ...
  Scenario: CLI formats a single supplied file
    ...
    When I run `./rust/target/release/fspec format spec/features/login.feature` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI prints a no-files message when none are found
    ...
    When I run `./rust/target/release/fspec format` from that directory
    Then the command exits 0
  ...
  Scenario: CLI errors when a supplied file is missing
    ...
    When I run `./rust/target/release/fspec format spec/features/missing.feature` from that workspace
  ...
  Scenario: format --help is byte-for-byte identical to the TS formatCommandHelp reference
    ...
    When I run `./rust/target/release/fspec format --help` piped to non-TTY
    Then the command exits 0
    And stdout matches the captured fixture at rust/fspec/tests/fixtures/help/format.txt
