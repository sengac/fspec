...
Feature: Remove Virtual Hook Cli Subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::remove_virtual_hook::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes remove-virtual-hook as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec remove-virtual-hook --help` from a shell
  ...
  Scenario: CLI removes an existing hook and prints the canonical success lines
    ...
    When I run `./rust/target/release/fspec remove-virtual-hook AUTH-001 eslint`
    Then the command exits 0
  ...
  Scenario: CLI fails with exit 1 when the work unit does not exist
    ...
    When I run `./rust/target/release/fspec remove-virtual-hook AUTH-999 eslint`
    Then the command exits 1
    And stderr contains the substring '✗ Failed to remove virtual hook:'
  ...
  Scenario: CLI fails with exit 1 when the named hook is not found
    Given spec/work-units.json contains AUTH-001 with virtualHooks=[{name:'eslint',event:'post-implementing',command:'eslint .',blocking:true}]
    When I run `./rust/target/release/fspec remove-virtual-hook AUTH-001 missing`
    Then the command exits 1
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher (no duplicated business logic)
    ...
    And the CLI bridge module rust/fspec/src/remove_virtual_hook.rs contains NO inline script-removal, retain, or work-unit-lookup logic — its only computation is JSON arg marshalling

  Scenario: remove-virtual-hook --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec remove-virtual-hook --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/remove-virtual-hook.txt
