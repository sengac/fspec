...
Feature: Port list-features command to Rust
  ...
  #   10. The standalone fspec binary at rust/fspec/src/main.rs MUST expose `list-features` as a clap v4 derive subcommand with ONE flag: `--tag <TAG>` (matching the single TS Commander.js `.option('--tag <tag>', ...)` registration at src/commands/list-features.ts:156); no --format, no --cwd, no --workspace
  ...
  #   11. Running `./rust/target/release/fspec list-features --help` prints clap-generated help showing the --tag flag and NOT --status/--prefix/--epic/--format/--workspace
  #   12. Running `./rust/target/release/fspec list-features` from an empty directory (no spec/) prints `Error: ... Directory not found: spec/features/` to stderr and exits with code 2 (NOT 1 — parity with TS exit-code 2 branch at lines 138-145)
  #   13. Running `./rust/target/release/fspec list-features --tag @critical` against a populated spec/features/ where one feature is tagged @critical → exit 0; stdout contains 'Found 1 feature files matching @critical' and lists the matching feature only
  #   14. Both invocation paths produce the SAME structured data: (a) dispatch_command('list-features', {format:'json',tag:'@critical'}, project_root) and (b) `./rust/target/release/fspec list-features --tag @critical` against the same on-disk state — only the rendering and delivery channel differ
  #
  # ========================================
