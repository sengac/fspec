...
Feature: Anthropic device auth flow for headless login
  ...
  #   6. On success, tokens persisted to ~/.fspec/credentials/claude_auth.json using existing write_claude_auth() from PROV-021 (claude_auth.rs). Returns ClaudeAuthJson with access_token, refresh_token, and expires timestamp — identical output to browser OAuth (PROV-021).
