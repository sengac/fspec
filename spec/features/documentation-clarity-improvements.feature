...
Feature: Documentation Clarity Improvements
  ...
  #   2. Make Feature Event Storm decision interactive and research-first: 1) Research codebase using 'fspec research --tool=ast --pattern="function $NAME" --lang=typescript --path=src/' FIRST to understand domain, 2) Self-assess based on research findings, 3) If uncertain after research, ASK USER (share findings, let them decide), 4) Proceed with chosen approach. Decision is SUBJECTIVE and COLLABORATIVE - emphasize no guessing, always ask if unsure. Research builds familiarity before judgment.
  ...
  #   2. AI is about to do Feature Event Storm but hasn't seen the codebase yet. Instead of guessing, AI first runs 'fspec research --tool=ast --pattern="function $NAME" --lang=typescript --path=src/auth/' to understand authentication domain. Then shares findings: 'I found 3 domain events: UserRegistered, LoginAttempted, SessionExpired. Should we do Feature Event Storm to map the full authentication flow?' Human decides based on actual code analysis.
  ...
  Scenario: Research codebase before deciding to use Feature Event Storm
    ...
    Then the AI runs AST research with pattern matching to understand the domain
