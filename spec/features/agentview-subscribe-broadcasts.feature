...
Feature: AgentView: subscribe to chunks + status broadcasts; handle every new StreamChunk variant
  ...
  App::dispatch additions for the new chunk-variant branches live in a new app/dispatch_rpc045.rs file (following the dispatch_rpc022/024/025/026 pattern) so app/dispatch.rs stays under the 300-LoC ceiling pinned by rpc024-source-shape.feature. A new try_dispatch_rpc045 helper is invoked from the existing ChunkReceived arm AFTER the per-session record_chunk + token-state apply but BEFORE the navigator/compositor fanout.
