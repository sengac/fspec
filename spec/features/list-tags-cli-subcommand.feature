...
Feature: List tags CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::list_tags::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes list-tags as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec list-tags --help` from a shell
  ...
  Scenario: CLI against empty directory auto-creates tags.json and prints all 9 default categories
    ...
    When I run `./rust/target/release/fspec list-tags` from that directory
    Then the command exits 0
  ...
  Scenario: CLI text output renders alphabetically sorted tags per category
    ...
    When I run `./rust/target/release/fspec list-tags`
    Then the command exits 0
  ...
  Scenario: CLI --category filter restricts output to the matching category
    ...
    When I run `./rust/target/release/fspec list-tags --category 'Phase Tags'`
    Then the command exits 0
  ...
  Scenario: CLI --category filter exits 1 and writes 'Category not found' to stderr for unknown category
    ...
    When I run `./rust/target/release/fspec list-tags --category 'No Such Category'`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher (no duplicated business logic)
    ...
    And the CLI bridge module rust/fspec/src/list_tags.rs contains NO inline category-filter, tag-sorting, or rendering logic
  ...
  Scenario: list-tags --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec list-tags --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-tags.txt
