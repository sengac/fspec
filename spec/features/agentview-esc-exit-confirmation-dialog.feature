...
Feature: Port AgentView ESC exit confirmation dialog (Detach/Close Session/Cancel) from TS Ink to Rust ratatui
  """
  New file: rust/fspec-tui/src/components/exit_confirmation_dialog.rs (≈250 LoC) — modelled on create_session_dialog.rs but with ExitChoice enum {Detach, CloseSession, Cancel}, Accent::Yellow, Priority::Critical, ID 'exit-confirmation-dialog', is_busy-driven description, no work-unit binding.
  New Action variants in rust/fspec-tui/src/components/mod.rs: enum ExitChoice {Detach, CloseSession, Cancel}; Action::AgentExitChoice { choice: ExitChoice }. Register the dialog module via `pub mod exit_confirmation_dialog;` in components/mod.rs.
