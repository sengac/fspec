...
Feature: Per-Model Context Window and Max Output Configuration
  ...
  TypeScript changes in src/tui/services/modelSelectionService.ts: Pass selection.contextWindow and selection.maxOutput as additional arguments to sessionSetModel() and sessionSetModelProfile(). The NAPI bindings (codelet/napi/index.d.ts) will need updated type signatures.
  No compaction engine changes needed — calculate_usable_context() in codelet/cli/src/compaction_threshold.rs and CompactionHook in codelet/core/src/compaction_hook.rs both already read from ProviderManager::context_window() and max_output_tokens(). The fix is entirely at the data source level.
