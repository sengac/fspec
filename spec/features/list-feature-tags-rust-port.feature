...
Feature: Port list-feature-tags command to Rust
  """
  New impl file at codelet/fspec-core/src/commands/list_feature_tags.rs replaces the NotYetPorted stub. The module exposes `pub async fn run(args_json: &str, project_root: &Path) -> Result<String, FspecCoreError>` with the same signature shape as list_features::run. Args struct deserializes `{file: String (required), showCategories?: bool, format?: 'text'|'json'}` with `#[serde(default)]` on the optional fields.
  ...
  Both invocation paths (LLM dispatcher and shell-facing CLI bridge) converge on this single `pub async fn run` — the CLI bridge module at codelet/fspec/src/list_feature_tags.rs is delivered as part of RPC-244 itself and performs only JSON arg marshalling + CWD resolution; see spec/features/list-feature-tags-cli-subcommand.feature for the clap surface contract.
  """
