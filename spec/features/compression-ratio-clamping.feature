...
Feature: Negative compression_ratio reachable on the wire — clamp missing in shared helper, three live unclamped producers
  """
  Fix lives in the shared helper compression_ratio() (rust/cli/src/interactive_helpers.rs:190-196) via .clamp(0.0, 1.0). Post-RPC-421 reality: the live callers are BOTH inject_summary_handler twins (agent-loop/src/inject_summary_handler.rs and napi/src/inject_summary_handler.rs — the StreamChunk::CompactionComplete producers whose honest post-injection numbers feed every notice/badge path that still formats a ratio) plus the debug-only recovery_compaction.rs:450 capture. The compact_session RPC twins (sessions/src/handle_impl.rs, napi/src/session_bindings.rs session_compact) and the plain-CLI repl_loop.rs /compact handler NO LONGER call the helper — RPC-421 made them ship the acknowledgement sentinel (compression_ratio 0.0, compacted_tokens 0) directly, and the REPL prints no ratio at all. The clamp still protects every surface that renders a ratio because all such surfaces source from the CompactionComplete chunk. TUI-side clamping would be the wrong layer per the RPC-420 wire contract.
  """
