...
Feature: skeleton_invariants clippy fails on unused_imports NotificationSeverity in codelet-core session_manager_handle
  ...
  #
  # BUSINESS RULES:
  #   1. The `NotificationSeverity` symbol MUST NOT appear in the `use codelet_rpc_types::{...}` import list in rust/core/src/session_manager_handle.rs
  ...
  #   4. A source-shape regression test MUST assert that `NotificationSeverity` is not re-introduced as an unused import in rust/core/src/session_manager_handle.rs
  ...
  #   3. A new source-shape regression test `rpc076_session_manager_handle_imports_shape.rs` in rust/sessions/tests/ scans the use-block in session_manager_handle.rs and panics if `NotificationSeverity` is re-introduced without a use-site
  ...
  Scenario: session_manager_handle.rs use-block does not import NotificationSeverity
    Given the file `rust/core/src/session_manager_handle.rs` exists in the workspace
  ...
  Scenario: codelet-core skeleton-invariants workspace-lint precondition: codelet-core itself passes -D warnings
    Given the rust/Cargo.toml workspace lints declaration is inherited by codelet-core
