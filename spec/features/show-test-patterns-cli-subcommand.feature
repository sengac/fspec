...
Feature: show-test-patterns CLI subcommand on the standalone fspec Rust binary
  ...
  Scenario: Clap exposes show-test-patterns as a subcommand and prints flag help
    ...
    When I run `./codelet/target/release/fspec show-test-patterns --help` from a shell
  ...
  Scenario: CLI without --tag flag exits 1
    ...
    When I run `./codelet/target/release/fspec show-test-patterns` from that directory
  ...
  Scenario: CLI default output prints the green summary line
    ...
    When I run `./codelet/target/release/fspec show-test-patterns --tag @cli` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --json prints 2-space JSON envelope to stdout
    ...
    When I run `./codelet/target/release/fspec show-test-patterns --tag @cli --json` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --include-coverage includes deduplicated test file paths
    ...
    When I run `./codelet/target/release/fspec show-test-patterns --tag @cli --include-coverage --json` from that workspace
    Then the command exits 0
    And the JSON.testFiles array has 3 unique elements

  Scenario: CLI exits 1 when work-units.json is missing
    Given an empty directory with no spec/ subdirectory is the current working directory
    When I run `./codelet/target/release/fspec show-test-patterns --tag @cli` from that directory
    Then the command exits with a non-zero status
  ...
  Scenario: show-test-patterns --help is byte-for-byte identical to the TS formatCommandHelp reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec show-test-patterns --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/show-test-patterns.txt
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./codelet/target/release/fspec show-test-patterns --tag @cli --json` against the same workspace
    Then both invocations produce a JSON envelope with workUnits.length=2
    And the CLI bridge module codelet/fspec/src/show_test_patterns.rs contains NO inline filtering or rendering logic — its only computation is JSON arg marshalling
