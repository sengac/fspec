...
Feature: Implement SessionManagerHandle for the extracted SessionManager
  """
  The impl block is placed in `rust/sessions/src/session_manager.rs` immediately after the existing `impl SessionManager { ... }` block. This co-locates the impl with the struct so future maintainers find it next to the methods it delegates to. Alternatively, if the file approaches the 300-line agent guideline limit (it already is ~982 LOC), the impl block goes in a sibling `rust/sessions/src/handle_impl.rs` re-exported via `pub mod handle_impl;` from `lib.rs`. The shape test accepts EITHER placement to avoid lock-in.
  ...
  Scenario: The impl block exists with explicit overrides for every trait method
    Given the file "rust/sessions/src/session_manager.rs" (and any sibling "handle_impl.rs" if used)
  ...
  Scenario: The conversions module bridges tool_pause and rpc-types pause families
    Given the file "rust/sessions/src/conversions.rs"
    And the module is declared from "rust/sessions/src/lib.rs" via "pub mod conversions;"
