...
Feature: query-work-units CLI subcommand on the standalone fspec Rust binary
  ...
  invariant (RPC-003 §7/§11), the CLI bridge (codelet/fspec/src/query_work_units.rs)
  ...
  Scenario: Standalone fspec binary exposes query-work-units as a clap subcommand
    ...
    When I run `./codelet/target/release/fspec query-work-units --help` from a shell
  ...
  Scenario: CLI query-work-units --format=json prints parseable JSON to stdout
    ...
    When I run `./codelet/target/release/fspec query-work-units --status=implementing --format=json`
    Then the command exits 0
  ...
  Scenario: CLI query-work-units --format=text prints NOTHING to stdout per TS quirk
    ...
    When I run `./codelet/target/release/fspec query-work-units --status=implementing --format=text`
    Then the command exits 0
  ...
  Scenario: CLI query-work-units --tag filter matches dispatcher behavior
    ...
    When I run `./codelet/target/release/fspec query-work-units --tag=@cli --format=json`
    Then the command exits 0
  ...
  Scenario: CLI query-work-units exits 1 and writes to stderr when spec/work-units.json is missing
    ...
    When I run `./codelet/target/release/fspec query-work-units` from that directory
  ...
  Scenario: query-work-units --help matches TS formatCommandHelp reference fixture
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec query-work-units --help` piped to non-TTY
    Then the command exits 0
    Then stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/query-work-units.txt
