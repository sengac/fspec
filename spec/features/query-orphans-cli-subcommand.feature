...
Feature: query-orphans clap subcommand on the standalone fspec Rust binary
  ...
  --help is byte-for-byte identical to the captured TS fixture at codelet/fspec/tests/fixtures/help/query-orphans.txt.
  """
  ...
  Scenario: Clap exposes query-orphans as a subcommand and prints flag-aware --help
    ...
    When I run `./codelet/target/release/fspec query-orphans --help` from a shell
  ...
  Scenario: CLI without options prints success message against an empty workspace
    ...
    When I run `./codelet/target/release/fspec query-orphans` from that directory
    Then the command exits 0
    And stdout contains the exact line '✓ No orphaned work units found.'

  Scenario: CLI with --output=json prints empty orphans array
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./codelet/target/release/fspec query-orphans --output json` from that directory
    Then the command exits 0
  ...
  Scenario: CLI text output lists each orphan with suggested actions
    ...
    When I run `./codelet/target/release/fspec query-orphans` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --exclude-done flag suppresses done orphans
    ...
    When I run `./codelet/target/release/fspec query-orphans --exclude-done --output json` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./codelet/target/release/fspec query-orphans --output json` against the same workspace
    Then both invocations produce JSON with orphans[0].id='MISC-001'
    And the CLI bridge module codelet/fspec/src/query_orphans.rs contains NO inline orphan-detection or rendering logic — its only computation is JSON arg marshalling and stdout printing

  Scenario: query-orphans --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec query-orphans --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/query-orphans.txt
