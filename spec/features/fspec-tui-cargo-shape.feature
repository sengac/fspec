...
@rpc
@RPC-008
Feature: rust/fspec-tui Cargo.toml + workspace shape
  Source-shape regressions for the rust/fspec-tui crate's workspace
  registration, [dependencies] / [dev-dependencies] policy, and the
  ban on own-runtime construction. Mirrors the pattern in
  rust/rpc-embedded/tests/architecture_invariants.rs and
  rpc_006_source_shape.rs — these tests do not exercise any runtime
  code path.

  Background: User Story
    As a fspec developer building the ratatui frontend
    I want rust/fspec-tui's Cargo.toml + source tree to be tightly bounded (only the RPC-seam crates in [dependencies], codelet-core allowed only in [dev-dependencies], and zero own-runtime construction)
  ...
  Scenario: rust/fspec-tui is a fifth RPC-family workspace member with no binary entry point
    Given the codelet workspace currently lists rpc, rpc-types, rpc-embedded, and rpc-server as RPC-family members
    When I add rust/fspec-tui to rust/Cargo.toml's [workspace] members and create rust/fspec-tui/Cargo.toml plus rust/fspec-tui/src/lib.rs
    Then "codelet-fspec-tui" appears in the workspace member list
    And rust/fspec-tui/Cargo.toml contains a [lib] section but NO [[bin]] section
    And running "cargo build -p codelet-fspec-tui" from rust/ exits with status code 0

  Scenario: rust/fspec-tui production dependencies include only RPC seam crates plus ratatui dependencies
  ...
  Scenario: rust/fspec-tui dev-dependencies allow codelet-core for fixtures but never codelet-napi
    Given rust/fspec-tui/Cargo.toml exists
  ...
  Scenario: rust/fspec-tui contains no own-runtime construction calls
    Given a source-shape integration test that scans rust/fspec-tui/src/
  ...
  Scenario: The existing scenario_7_* test in rpc-embedded is widened to scan fspec-tui
    Given the existing test rust/rpc-embedded/tests/architecture_invariants.rs::scenario_7_embedded_transport_requires_tokio_handle_at_construction
