...
Feature: RPC-006 source-shape architecture invariants
  ...
  Scenario: default_fixture is unreachable from production code
    Given the codelet/rpc crate after the watcher lift
  ...
  Scenario: Embedded transport reuses the host tokio runtime Handle for any fan-out it spawns
    Given the codelet/rpc-embedded crate after RPC-006
  ...
  Scenario: Shared service crate codelet-rpc may depend on codelet-core but not on codelet-napi
    Given codelet/rpc/Cargo.toml after the watcher lift
    When I inspect its [dependencies] table and the codelet/rpc/src/ source for use statements naming codelet_napi or codelet_core
  ...
  Scenario: WorkUnitInfo continues to be defined exactly once in rpc-types
    ...
    When I inspect codelet/napi and codelet/core for any local definition of a struct named WorkUnitInfo and run cargo check on the workspace
    Then no other crate defines WorkUnitInfo locally, both crates re-export the type from codelet/rpc-types where they reference it, and the workspace builds successfully

  Scenario: Embedded push path contains no bincode encode call
    Given the codelet/rpc-embedded crate after RPC-006 exposes work_units_rx via a broadcast::Receiver
