...
Feature: fspec restore-architecture-note CLI subcommand (Rust port)
  ...
  The bridge module at rust/fspec/src/restore_architecture_note.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::restore_architecture_note::run; no validation or rendering logic is duplicated.
  Exit codes: 0 on success, 1 on any FspecCoreError. Errors are written to stderr prefixed with 'Error:'.
  The `fspec restore-architecture-note --help` output is byte-for-byte identical to `node dist/index.js restore-architecture-note --help` (TS reference) — captured as rust/fspec/tests/fixtures/help/restore-architecture-note.txt.
  """
  ...
  Scenario: Clap exposes restore-architecture-note with two positional args in --help
    ...
    When I run `./rust/target/release/fspec restore-architecture-note --help`
  ...
  Scenario: CLI restores an architecture note and prints the success line
    ...
    When I run `./rust/target/release/fspec restore-architecture-note AUTH-001 0`
    Then the command exits 0
  ...
  Scenario: CLI rejects unknown note ID with exit 1 and stderr Error prefix
    ...
    When I run `./rust/target/release/fspec restore-architecture-note AUTH-001 5`
  ...
  Scenario: CLI rejects unknown work unit with exit 1 and stderr Error prefix
    ...
    When I run `./rust/target/release/fspec restore-architecture-note AUTH-999 0`
    Then the command exits with code 1
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec restore-architecture-note AUTH-001 1` afterwards exits 0
    And spec/work-units.json on disk shows both architectureNotes with deleted=false
    And the CLI bridge module rust/fspec/src/restore_architecture_note.rs contains NO inline state mutation or file-write logic — its only computation is JSON arg marshalling

  Scenario: restore-architecture-note --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec restore-architecture-note --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/restore-architecture-note.txt
