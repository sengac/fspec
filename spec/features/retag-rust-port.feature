...
Feature: Port retag command to Rust
  ...
  CLI bridge codelet/fspec/src/retag.rs marshals {from,to,dryRun} JSON, NO domain logic. Renders: failure -> 'Error: <error>' exit 1; dryRun -> 'Dry run mode - no files modified' + summary + file list exit 0; real with files -> '✓ <message>' + 'Modified files:' + list exit 0. Help config help/configs/retag.rs uses CommandHelpConfig (CommonError type). Uses existing regex crate dep (no new Cargo deps).
  """
