...
Feature: skeleton_invariants clippy fails on redundant_clone in codelet-sessions handle_impl
  ...
  #
  # BUSINESS RULES:
  #   1. rust/sessions/src/handle_impl.rs must NOT contain `id: id.clone(),` or `prompt: prompt.clone(),` inside a struct literal whose owned bindings (`id`, `prompt`) are dropped at the end of the block
  ...
  #
  # EXAMPLES:
  #   1. Searching rust/sessions/src/handle_impl.rs for the regex `id: id\.clone\(\),` returns zero matches
  #   2. Searching rust/sessions/src/handle_impl.rs for the regex `prompt: prompt\.clone\(\),` returns zero matches
  ...
  Scenario: handle_impl.rs has no redundant `id: id.clone(),` struct literal
    Given the source of `rust/sessions/src/handle_impl.rs`
  ...
  Scenario: rust/sessions/Cargo.toml inherits workspace lints
    Given the source of `rust/sessions/Cargo.toml`
