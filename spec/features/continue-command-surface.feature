...
Feature: Continue Command Surface
  ...
  TUI wiring (verified 2026-07-09): registry codelet/fspec-tui/src/views/agent/slash_commands.rs (SlashCommandAction enum :21, name() :45, SLASH_COMMANDS :85); typed args codelet/fspec-tui/src/app/slash_parser.rs::parse_slash_command (:74, /loop routed :123-125) gains a '/continue' branch returning SlashCommandParse::ContinueSubcommand backed by a new continue_parser.rs modeled on loop_parser.rs (LoopSubcommand :19); dispatch codelet/fspec-tui/src/app/dispatch_slash_commands.rs handle_slash_command :27 (palette) and handle_input_submitted :175 (typed) with the /compact backend round-trip pattern (:65-97) driving a new session-state setter in codelet/napi/src/session_bindings.rs; status-bar indicator '⏩ auto-continue (n/N)' while armed via a pure formatting helper.
  CLI repl wiring: codelet/cli/src/interactive/repl_loop.rs::repl_loop (:16) — /continue handler inserted after the /compact block (:68-134) and BEFORE the provider-switch '/' catch-all (:137); grammar applied by a shared pure apply function in codelet_cli's auto_continue module so repl and NAPI setter agree; state printed via println per repl conventions.
  Session state mutated: continue_enabled/continue_budget on codelet/cli/src/session/mod.rs Session (CONT-002 fields).
  """
