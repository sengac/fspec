...
Feature: Provider settings: empty-Enter in API-key edit cancels silently (drop inline 'cannot be empty' validation)
  ...
  Implementation:
  - codelet/fspec-tui/src/views/provider_settings/detail.rs handle_edit_key — Enter arm with empty draft now sets view.mode = ProviderSettingsMode::List, clears view.status, and returns ProviderSettingsEvent::Consumed (no Action). Esc arm now sets view.mode = ProviderSettingsMode::List (was Detail::Summary { last_status: None }). Enter arm with non-empty draft now sets view.mode = ProviderSettingsMode::List before emitting Action::SaveProviderCredentials (was Detail::Summary { SavingCredentials }). The DetailSub::Summary variant and DetailStatus enum remain untouched for legacy callers and existing tests.
  """
