...
Feature: RPC-024 source-shape regressions for the multi-session AgentViewStore refactor
  ...
  Scenario: SessionContext lives in its own sub-module under the 300-LoC ceiling
    ...
    Then a file exists at codelet/fspec-tui/src/store/agent_view/session_context.rs
    And that file is under 300 lines
    And the file codelet/fspec-tui/src/store/agent_view.rs is under 300 lines
    And no file under codelet/fspec-tui/src/views/ imports codelet_core, codelet_napi, tarpc, or tokio_tungstenite
  ...
  Scenario: AgentViewStore exposes the multi-session surface
    Given codelet/fspec-tui/src/store/agent_view.rs after RPC-024 lands
  ...
  Scenario: AgentView no longer owns the scrollback / next_seq fields
    Given codelet/fspec-tui/src/views/agent.rs after RPC-024 lands
