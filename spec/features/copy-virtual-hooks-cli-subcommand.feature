...
Feature: copy-virtual-hooks CLI subcommand
  ...
  The CLI signature mirrors the TS Commander.js definition exactly: three options `--from <workUnitId>`, `--to <workUnitId>`, `--hook-name <name>`. Help is intercepted in `codelet/fspec/src/main.rs::intercept_ts_help` BEFORE clap parses argv — the intercept arm calls `format_command_help(&configs::copy_virtual_hooks::CONFIG)` which produces output byte-for-byte identical to `node dist/index.js copy-virtual-hooks --help`. The byte-parity contract is enforced by `codelet/fspec/tests/fixtures/help/copy-virtual-hooks.txt`.
  """
  ...
  Scenario: CLI prints success message when copying all hooks
    ...
    When I run `./codelet/target/release/fspec copy-virtual-hooks --from AUTH-001 --to AUTH-002` in that project root
  ...
  Scenario: CLI prints success message when copying a single named hook
    ...
    When I run `./codelet/target/release/fspec copy-virtual-hooks --from AUTH-001 --to AUTH-002 --hook-name eslint` in that project root
    Then the command exits 0
  ...
  Scenario: CLI exits 1 when --from is omitted
    ...
    When I run `./codelet/target/release/fspec copy-virtual-hooks --to AUTH-002` in that project root
    Then the command exits 1
    And stderr contains the substring "--from option is required"

  Scenario: CLI exits 1 when --to is omitted
    Given an empty project root with no spec/ subdirectory
    When I run `./codelet/target/release/fspec copy-virtual-hooks --from AUTH-001` in that project root
    Then the command exits 1
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher (no duplicated business logic)
    ...
    And the CLI bridge module codelet/fspec/src/copy_virtual_hooks.rs contains NO inline rendering, file IO beyond cwd resolution, or work-unit-lookup logic — its only computation is JSON arg marshalling plus the --from/--to presence guard

  Scenario: copy-virtual-hooks --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec copy-virtual-hooks --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/copy-virtual-hooks.txt
