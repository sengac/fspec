...
Feature: Show work unit CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::show_work_unit::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes show-work-unit as a subcommand with positional workUnitId and a format option
    ...
    When I run `./rust/target/release/fspec show-work-unit --help` from a shell
  ...
  Scenario: CLI prints text-format dump when the work unit exists
    ...
    When I run `./rust/target/release/fspec show-work-unit AUTH-001` from that directory
    Then the command exits 0
  ...
  Scenario: CLI prints JSON payload when --format json is supplied
    ...
    When I run `./rust/target/release/fspec show-work-unit AUTH-001 --format json`
    Then the command exits 0
  ...
  Scenario: CLI exits 1 and writes the canonical message to stderr when the work unit does not exist
    ...
    When I run `./rust/target/release/fspec show-work-unit UNKNOWN-999`
  ...
  Scenario: CLI exits 1 when spec/work-units.json is absent (no auto-create)
    ...
    When I run `./rust/target/release/fspec show-work-unit AUTH-001`
    Then the command exits with code 1
    Then stderr contains the substring 'Error:'
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    Then the CLI bridge module rust/fspec/src/show_work_unit.rs contains NO inline projection, reminder generation, or feature-scan logic — its only computation is JSON arg marshalling

  Scenario: show-work-unit --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec show-work-unit --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-work-unit.txt
