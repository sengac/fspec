...
Feature: fspec add-capability CLI subcommand (Rust port)
  ...
  rust/fspec/src/add_capability.rs marshals the clap args into a JSON object
  ...
  captured fixture at rust/fspec/tests/fixtures/help/add-capability.txt.
  """
  ...
  Scenario: Clap exposes add-capability with two positional args in --help
    ...
    When I run `./rust/target/release/fspec add-capability --help`
  ...
  Scenario: CLI adds a capability and prints the success block on stdout
    ...
    When I run `./rust/target/release/fspec add-capability "Search" "Full text search"`
    Then the command exits 0
  ...
  Scenario: CLI writes to the draft and reports the draft file name
    ...
    When I run `./rust/target/release/fspec add-capability "Data Export" "Export to CSV"`
    Then the command exits 0
  ...
  Scenario: CLI prints the placeholder-removal line when only placeholders existed
    ...
    When I run `./rust/target/release/fspec add-capability "Login" "Authenticate users"`
    Then the command exits 0
  ...
  Scenario: CLI fails with exit 1 when foundation.json is missing
    ...
    When I run `./rust/target/release/fspec add-capability "X" "Y"`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec add-capability "Via CLI" "d"` afterwards exits 0
    And spec/foundation.json solutionSpace.capabilities contains two entries
    And the CLI bridge module rust/fspec/src/add_capability.rs contains NO inline draft probing, placeholder detection, or JSON-mutation logic — its only computation is JSON arg marshalling

  Scenario: add-capability --help is byte-for-byte identical to the captured fixture
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec add-capability --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/add-capability.txt
