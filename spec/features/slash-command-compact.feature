...
Feature: /compact slash command + compaction progress footer
  ...
  Notice-line format constant: helper `format_compaction_notice(result: &CompactionResult) -> String` lives in app/dispatch_rpc020.rs (next to the /compact handler). Used by BOTH the slash-handler success branch AND the CompactionComplete chunk handler in dispatch_rpc045.rs so the formatting stays single-sourced.
  """
