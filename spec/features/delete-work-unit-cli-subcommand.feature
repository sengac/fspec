...
Feature: fspec delete-work-unit CLI subcommand (Rust port)
  ...
  The `fspec delete-work-unit --help` output is byte-for-byte identical to `node dist/index.js delete-work-unit --help` (TS reference) — captured as rust/fspec/tests/fixtures/help/delete-work-unit.txt.
  """
  ...
  Scenario: Clap exposes delete-work-unit with a positional arg and flags in --help
    ...
    When I run `./rust/target/release/fspec delete-work-unit --help`
  ...
  Scenario: CLI deletes an existing leaf work unit and prints the success line
    ...
    When I run `./rust/target/release/fspec delete-work-unit AUTH-001`
    Then the command exits 0
  ...
  Scenario: CLI exits 1 when the work unit does not exist
    Given spec/work-units.json contains work unit AUTH-001 with status='backlog' and no dependencies
    When I run `./rust/target/release/fspec delete-work-unit MISSING-999`
  ...
  Scenario: CLI cascades dependencies and prints a blocks warning
    ...
    When I run `./rust/target/release/fspec delete-work-unit AUTH-001 --cascade-dependencies`
    Then the command exits 0
    And stdout contains the line '✓ Work unit AUTH-001 deleted successfully'
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec delete-work-unit DASH-001` afterwards exits 0
    And spec/work-units.json contains neither AUTH-001 nor DASH-001 work units
    And the CLI bridge module rust/fspec/src/delete_work_unit.rs contains NO inline file-read, mutation, or rendering logic — its only computation is JSON arg marshalling

  Scenario: delete-work-unit --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec delete-work-unit --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/delete-work-unit.txt
