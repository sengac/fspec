...
Feature: List work units CLI subcommand
  """
  CLI subcommand is wired into codelet/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::list_work_units::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  """
  ...
  Scenario: CLI against empty directory creates default files and prints sentinel
    ...
    When I run `./codelet/target/release/fspec list-work-units` from that directory
  ...
  Scenario: CLI emits 2-space indented JSON when --format=json is passed
    ...
    When I run `./codelet/target/release/fspec list-work-units --format=json`
  ...
  Scenario: CLI --status filter matches dispatcher behavior
    ...
    When I run `./codelet/target/release/fspec list-work-units --status=backlog --format=json`
  ...
  Scenario: CLI exits 1 and writes to stderr when work-units.json is malformed
    ...
    When I run `./codelet/target/release/fspec list-work-units`
  ...
  Scenario: CLI tolerates unknown work-unit type values for parity with the TypeScript runtime
    ...
    When I run `./codelet/target/release/fspec list-work-units --type=story --format=json`
