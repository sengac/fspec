...
Feature: Show deleted CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::show_deleted::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes show-deleted as a subcommand with positional workUnitId and no flags
    ...
    When I run `./rust/target/release/fspec show-deleted --help` from a shell
  ...
  Scenario: CLI prints sentinel when the work unit exists but has no deleted items
    ...
    When I run `./rust/target/release/fspec show-deleted AUTH-001` from that directory
    Then the command exits 0
  ...
  Scenario: CLI text output renders header and item lines for the populated case
    ...
    When I run `./rust/target/release/fspec show-deleted AUTH-001`
    Then the command exits 0
  ...
  Scenario: CLI exits 1 and writes to stderr when the work unit does not exist
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./rust/target/release/fspec show-deleted UNKNOWN-999`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    Then the CLI bridge module rust/fspec/src/show_deleted.rs contains NO inline deleted-item collection, filter, or rendering logic — its only computation is JSON arg marshalling

  Scenario: show-deleted --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec show-deleted --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-deleted.txt
