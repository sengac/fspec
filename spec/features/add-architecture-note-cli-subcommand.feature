...
Feature: fspec add-architecture-note CLI subcommand (Rust port)
  """
  Clap derive subcommand `add-architecture-note` exposes the same surface as the TS Commander.js registration at src/commands/add-architecture-note.ts:89-108 — two positional arguments `<workUnitId>` and `<note>`. The bridge module at rust/fspec/src/add_architecture_note.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::add_architecture_note::run; no validation or rendering logic is duplicated.
  ...
  The `fspec add-architecture-note --help` output is byte-for-byte identical to `node dist/index.js add-architecture-note --help` (TS reference) — captured as rust/fspec/tests/fixtures/help/add-architecture-note.txt.
  """
  ...
  Scenario: Clap exposes add-architecture-note with positional args in --help
    ...
    When I run `./rust/target/release/fspec add-architecture-note --help`
  ...
  Scenario: CLI appends an architecture note and prints the success block
    ...
    When I run `./rust/target/release/fspec add-architecture-note AUTH-001 "Uses bcrypt"`
    Then the command exits 0
  ...
  Scenario: CLI rejects an unknown work unit with exit 1 and stderr Error prefix
    ...
    When I run `./rust/target/release/fspec add-architecture-note MISSING-001 "any note"`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec add-architecture-note AUTH-001 "via cli"` afterwards exits 0
    And spec/work-units.json work unit 'AUTH-001' contains two architectureNotes
    And the CLI bridge module rust/fspec/src/add_architecture_note.rs contains NO inline note-append, nextNoteId, or system-reminder rendering logic — its only computation is JSON arg marshalling

  Scenario: add-architecture-note --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec add-architecture-note --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/add-architecture-note.txt
