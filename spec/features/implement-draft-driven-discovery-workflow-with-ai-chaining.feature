...
Feature: Implement draft-driven discovery workflow with AI chaining
  ...
  #   4. System-reminder instructs AI to: analyze codebase, ask human, use fspec update-foundation command
  ...
  Scenario: Fill project.name field via feedback loop
    ...
    When AI reads package.json and confirms name with human
    And AI runs "fspec update-foundation --field project.name --value fspec"
  ...
  Scenario: Verify DETECTED project type with human
    ...
    When AI analyzes commander.js usage and bin field
