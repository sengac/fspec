...
Feature: Move BackgroundSession from codelet-napi into codelet-sessions, replace NAPI references
  ...
  Architecture: send_input's return type narrows from `napi::bindgen_prelude::Result<()>` to `Result<(), String>`. The NAPI free function session_send_input (at rust/napi/src/session_manager.rs:6555) which forwards into BackgroundSession::send_input is updated to map the String error back into `napi::Error::from_reason(err)` so the TS Promise<void> signature is preserved verbatim — this is the only NAPI-shell adapter change forced by the move and it MUST live in napi (NOT inside codelet-sessions)
  ...
  #
  # BUSINESS RULES:
  #   1. The complete BackgroundSession struct + impl block currently at rust/napi/src/session_manager.rs lines 459-1356 lives verbatim in rust/sessions/src/background_session.rs after the move
  #   2. No napi:: references appear inside rust/sessions/src/background_session.rs — the only NAPI-typed return inside the moved code (napi::bindgen_prelude::Result<()> on send_input) is rewritten to Result<(), String>, and the napi::Error::from_reason(...) call site is rewritten to a plain format!-produced String error
  ...
  #   2. Developer runs `cargo build -p codelet-napi` (default features) — build succeeds because rust/napi/src/session_manager.rs now `pub use codelet_sessions::background_session::{BackgroundSession, IncomingMessage, BridgeImageData, format_incoming_message, PromptInput, CompactionProgress, WorkUnitContext, SessionError};` keeps every existing import path resolvable
  #   3. Developer greps `rust/sessions/src/background_session.rs` for the regex `napi::|use napi|#\[napi` and finds zero matches — the moved file has no NAPI references
  #   4. Developer greps `rust/sessions/src/background_session.rs` for the regex `crate::persistence|crate::types` and finds zero matches — all such imports were rewritten to codelet_core::persistence and codelet_rpc_types respectively
  ...
  #   10. Developer runs `cargo build -p codelet-napi --release` regenerating rust/napi/index.d.ts — git diff shows no changes (or only whitespace/comment changes) compared to the pre-move version, proving the NAPI public surface is byte-stable
  #
  # ========================================
  ...
  Scenario: codelet-sessions builds standalone with BackgroundSession at its new home
    Given the BackgroundSession struct and impl have been moved into rust/sessions/src/background_session.rs
  ...
  Scenario: codelet-napi still builds against the re-exported BackgroundSession
    Given rust/napi/src/session_manager.rs now `pub use`s BackgroundSession + its companion types from codelet-sessions
  ...
  Scenario: The moved background_session.rs has no napi:: references
    Given the BackgroundSession code has been moved into rust/sessions/src/background_session.rs
    When I grep the moved file for the regex `napi::|use napi|#\[napi`
    Then I find zero matches in rust/sessions/src/background_session.rs
  ...
  Scenario: handle_output uses the new chunks_tx broadcast and no longer touches GLOBAL_CHUNK_CALLBACK
    ...
    And the GLOBAL_CHUNK_CALLBACK global itself still lives in rust/napi/src/session_manager.rs (deletion is explicitly deferred to RPC-041)
  ...
  Scenario: Pre-existing in-file unit tests in codelet-napi still pass via the re-exports
    Given the BackgroundSession code has been moved into rust/sessions/src/background_session.rs
    And rust/napi/src/session_manager.rs re-exports BackgroundSession, IncomingMessage, BridgeImageData, format_incoming_message, PromptInput, CompactionProgress, WorkUnitContext, SessionError from codelet-sessions
  ...
  Scenario: codelet-sessions tests assert BackgroundSession is reachable from its new home
    Given the BackgroundSession code has been moved into rust/sessions/src/background_session.rs
    And the integration test rust/sessions/tests/background_session_shape.rs has been added
  ...
  Scenario: NAPI TypeScript surface is byte-stable across the move
    Given the BackgroundSession code has been moved into rust/sessions/src/background_session.rs
    When I run `cargo build -p codelet-napi --release` regenerating rust/napi/index.d.ts
    Then no TypeScript interface, function, enum, or type alias in the regenerated rust/napi/index.d.ts is removed
