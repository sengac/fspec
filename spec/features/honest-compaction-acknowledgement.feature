...
Feature: Honest compaction acknowledgement from compact_session
  """
  RPC-421 engine-side twin of single-sourced-compaction-notice.feature. compact_session in BOTH twins (rust/sessions/src/handle_impl.rs and rust/napi/src/session_bindings.rs session_compact) measures compacted_tokens immediately after execute_compaction clears the context to reminders + instruction — BEFORE the agent builds the DAG summary. The returned CompactionResult therefore carried a fabricated ~90-99% reduction with turns_summarized 0.
  ...
  The plain-CLI REPL (rust/cli/src/interactive/repl_loop.rs /compact arm) is the third instance: it printed "[Context compacted: X→Y tokens, Z% compression]" from the same trough measurement. It now prints a compaction-started message with no fabricated numbers. Debug-capture events (BUG-134) keep recording the real trough measurements — diagnostics, not display.
  Test file: rust/napi/tests/rpc421_honest_ack_test.rs — the napi test crate is the only crate that sees codelet-cli, codelet-sessions and codelet-napi at once (cmpct038/cmpct039 precedent), so both engine twins and the repl_loop source shape are validated there.
  """
