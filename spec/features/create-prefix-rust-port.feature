...
Feature: Port create-prefix command to Rust
  ...
  Help config at codelet/fspec-core/src/help/configs/create_prefix.rs ports `src/commands/create-prefix-help.ts` 1:1 — same description, when_to_use, two required arguments, single example with output `✓ Created prefix AUTH\n  Description: Authentication features`, related commands list, notes.
  ...
  #   9. The CLI bridge at codelet/fspec/src/create_prefix.rs marshals two required positional args (<prefix> and <description>) to JSON `{ "prefix": "...", "description": "..." }` and delegates to `fspec_core::commands::create_prefix::run` — NO validation or filesystem logic in the bridge (RPC-003 §7/§11 two-front-doors)
  ...
  Scenario: Shared infrastructure is reused without duplication
    ...
    When I inspect codelet/fspec-core/src/commands/create_prefix.rs
