...
Feature: Lift MessageStore + message_index into codelet-core::persistence::messages
  ...
  #   2. MessageStore, compute_hash, StoredMessage, MessageSource, MessageRef and the binary index helpers (IndexEntry, load_index, save_index, scan_jsonl_range, read_message_at) all live in rust/core/src/persistence/messages.rs (the index helpers as a nested `mod index` submodule)
  ...
  #   4. rust/napi/src/persistence/message_index.rs is deleted (the `mod message_index;` declaration is removed from persistence/mod.rs) — the binary index helpers are reached exclusively through codelet_core::persistence::messages
  #   5. rust/napi/src/persistence/types.rs no longer defines StoredMessage, MessageSource, or MessageRef; it re-exports those three types from codelet_core::persistence::messages so SessionManifest (which still lives in NAPI for RPC-033) keeps the same field type Vec<MessageRef>
