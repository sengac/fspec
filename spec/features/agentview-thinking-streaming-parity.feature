...
Feature: AgentView Thinking streaming accumulation parity (port appendThinking + finalizeThinkingBlock to Rust SessionContext) — fix [Thinking] per-delta repetition
  ...
  Rust touch points: codelet/fspec-tui/src/store/agent_view/session_context.rs (add in_flight_thinking, replace Thinking arm), chunk_processor.rs (add append_thinking + finalize_in_flight_thinking, wire from handle_tool_call), views/agent/scrollback.rs (move [Thinking]\n prefix to render-time), views/agent.rs ScrollbackList (add insert_chunk_at helper)
