...
Feature: List features CLI subcommand
  """
  CLI subcommand is wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::list_features::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  #
  # EXAMPLES:
  #   1. `./rust/target/release/fspec list-features --help` exits 0 and stdout describes the subcommand and the --tag flag (and does NOT advertise --status/--prefix/--epic/--format/--workspace)
  ...
  Scenario: Clap exposes list-features as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec list-features --help` from a shell
  ...
  Scenario: CLI against directory with no spec/ exits 2 with Directory-not-found error
    ...
    When I run `./rust/target/release/fspec list-features` from that directory
  ...
  Scenario: CLI text output renders feature listing for the populated case
    ...
    When I run `./rust/target/release/fspec list-features`
    Then the command exits 0
  ...
  Scenario: CLI --tag filter narrows results and updates summary line
    ...
    When I run `./rust/target/release/fspec list-features --tag @critical`
    Then the command exits 0
  ...
  Scenario: CLI bridge module embeds NO duplicated business logic
    Given the file rust/fspec/src/list_features.rs exists as the CLI bridge module
  ...
  Scenario: list-features --help is byte-for-byte identical to TS formatCommandHelp reference output
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec list-features --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/list-features.txt
