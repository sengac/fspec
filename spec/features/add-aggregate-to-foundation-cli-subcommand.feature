...
Feature: fspec add-aggregate-to-foundation CLI subcommand (Rust port)
  ...
  <text>` flag. The CLI bridge at rust/fspec/src/add_aggregate_to_foundation.rs marshals
  ...
  at rust/fspec/tests/fixtures/help/add-aggregate-to-foundation.txt).
  """
  ...
  Scenario: Clap exposes add-aggregate-to-foundation with positional args and description flag in --help
    ...
    When I run `./rust/target/release/fspec add-aggregate-to-foundation --help`
  ...
  Scenario: CLI adds an aggregate and prints the success message on stdout
    ...
    When I run `./rust/target/release/fspec add-aggregate-to-foundation Sales Order`
    Then the command exits 0
  ...
  Scenario: CLI persists the optional description via the -d flag
    ...
    When I run `./rust/target/release/fspec add-aggregate-to-foundation Billing Invoice -d "Billing root"`
    Then the command exits 0
  ...
  Scenario: CLI rejects an unknown bounded context with exit 1 and stderr Error prefix
    Given spec/foundation.json contains a bounded_context item 'Sales' with id=0 in eventStorm.items
    When I run `./rust/target/release/fspec add-aggregate-to-foundation Unknown Order`
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec add-aggregate-to-foundation Sales ViaCli` afterwards exits 0
    And spec/foundation.json eventStorm.items contains two aggregate items
    And the CLI bridge module rust/fspec/src/add_aggregate_to_foundation.rs contains NO inline bounded-context lookup, ensure_foundation_file, or JSON-mutation logic — its only computation is JSON arg marshalling

  Scenario: add-aggregate-to-foundation --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec add-aggregate-to-foundation --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/add-aggregate-to-foundation.txt
