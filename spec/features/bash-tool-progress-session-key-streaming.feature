...
Feature: Bash/tool output does not stream incrementally in Rust TUI (session-id key mismatch)
  """
  Root cause: tool-progress registry (rust/tools/src/session_registry.rs:95-100) uses exact HashMap lookup with no fallback. Registration was under Uuid::nil() (cli/src/interactive/stream_loop.rs:459-462, BUG-126) while BashTool emits under the real session.id (bash.rs:66,268 -> bash_streams.rs:73).
