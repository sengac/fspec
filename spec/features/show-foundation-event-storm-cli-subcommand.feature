...
Feature: show-foundation-event-storm CLI subcommand on the standalone fspec Rust binary
  """
  CLI subcommand wired into rust/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::show_foundation_event_storm::run(args_json) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes show-foundation-event-storm as a subcommand and prints flag help
    ...
    When I run `./rust/target/release/fspec show-foundation-event-storm --help` from a shell
  ...
  Scenario: CLI against workspace with no foundation.json exits 1 with error
    ...
    When I run `./rust/target/release/fspec show-foundation-event-storm` from that directory
  ...
  Scenario: CLI prints empty array when foundation.json has no eventStorm
    ...
    When I run `./rust/target/release/fspec show-foundation-event-storm` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --type filter narrows to matching items
    ...
    When I run `./rust/target/release/fspec show-foundation-event-storm --type aggregate` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --context filter returns bounded context plus linked items
    ...
    When I run `./rust/target/release/fspec show-foundation-event-storm --context "Work Management"` from that workspace
    Then the command exits 0
  ...
  Scenario: show-foundation-event-storm --help is byte-for-byte identical to the TS formatCommandHelp reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec show-foundation-event-storm --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-foundation-event-storm.txt
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./rust/target/release/fspec show-foundation-event-storm --context "Work Management"` against the same workspace
    Then both invocations produce JSON arrays with 3 elements
    And the CLI bridge module rust/fspec/src/show_foundation_event_storm.rs contains NO inline filtering or rendering logic — its only computation is JSON arg marshalling
