...
Feature: Pause / HITL UI (ConfirmDialog + HitlDialog end-to-end)
  ...
  Create codelet/fspec-tui/src/app/dispatch_pause_hitl.rs hosting impl App methods: handle_pause_chunk(session_id) — fired from dispatch_stream_chunks SessionStateChange{Paused} arm, spawns parallel backend.get_pause_state + get_hitl_request and dispatches OpenPauseDialog OR OpenHitlDialog (HITL wins on tie); handle_open_pause_dialog(session, state) — idempotent compositor push; handle_open_hitl_dialog(session, request) — idempotent compositor push; handle_pause_confirmed/handle_pause_triple/handle_pause_resumed — fire-and-forget backend writes, pop dialog before await (best-effort UX); handle_hitl_submitted — fire-and-forget backend.send_hitl_response, pop dialog before await; handle_pause_cleared — pop any mounted PauseDialog/HitlDialog for the session.
  Wire into dispatch_stream_chunks::handle_stream_chunk_state_updates: branch SessionStateChange{state} on Running/Idle to dispatch Action::PauseCleared (so any mounted dialog is popped on resume), AND on Paused to dispatch a NEW Action::PauseChunkReceived(session_id) which routes through handle_pause_chunk (the parallel get_pause_state/get_hitl_request fetcher). Route both new actions in app/dispatch.rs (or via try_dispatch_pause_hitl catch-all helper).
  ...
  Source-shape regression test (similar to source_shape_rpc050.rs) at codelet/fspec-tui/tests/source_shape_rpc053.rs asserting: dispatch_pause_hitl.rs exists; pub mod dispatch_pause_hitl declared in app/mod.rs; pause_dialog.rs and hitl_dialog.rs exist; pub mod declarations in components/mod.rs; app/dispatch.rs stays under 300 logical lines.
  """
