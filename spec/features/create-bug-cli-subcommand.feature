...
Feature: fspec create-bug CLI subcommand (Rust port)
  """
  Clap derive subcommand `create-bug` exposes the same surface as the TS Commander.js registration at src/commands/create-bug.ts:278-288 — two positional arguments `<prefix>` and `<title>` plus optional `-d, --description`, `-e, --epic`, and `-p, --parent` flags. The bridge module at rust/fspec/src/create_bug.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::create_bug::run; no validation or rendering logic is duplicated.
  ...
  The `fspec create-bug --help` output is byte-for-byte identical to the captured fixture at rust/fspec/tests/fixtures/help/create-bug.txt.
  """
  ...
  Scenario: Clap exposes create-bug with positional args and option flags in --help
    ...
    When I run `./rust/target/release/fspec create-bug --help`
  ...
  Scenario: CLI creates a minimal bug and prints the success block
    ...
    When I run `./rust/target/release/fspec create-bug BUG "Login crash"`
    Then the command exits 0
  ...
  Scenario: CLI creates a bug with description, epic, and parent printing all detail lines
    ...
    When I run `./rust/target/release/fspec create-bug BUG "Login crash" -d "Crashes on submit" -e auth -p BUG-001`
    Then the command exits 0
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec create-bug BUG "Second"` afterwards exits 0
    And spec/work-units.json now contains both 'BUG-001' and 'BUG-002'
    And the CLI bridge module rust/fspec/src/create_bug.rs contains NO inline validation, id-generation, or file-write logic — its only computation is JSON arg marshalling

  Scenario: create-bug --help is byte-for-byte identical to the fixture
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec create-bug --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/create-bug.txt
