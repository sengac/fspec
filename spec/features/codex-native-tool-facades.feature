...
Feature: Codex-Native Tool Facades - Map Tools to Codex CLI Tool Schemas
  ...
  Reuse existing OpenAIFspecFacade and OpenAIBridgeFacade for Codex.

  BUG-107: glob is NOT part of the native Codex CLI tool set (codex-rs/core/src/tools/spec.rs)
  and must not be exposed as a Codex facade tool.
  """
  ...
  #   4. Only tools defined in codex-rs/core/src/tools/spec.rs may be exposed as Codex facade tools (BUG-107: glob is NOT in the native tool set)
  #   5. Tools without a Codex equivalent (AstGrep, AstGrepRefactor) MUST be kept with current naming
  #   6. CodexProvider::create_rig_agent() MUST use FacadeToolWrapper for tools with Codex-equivalent schemas, replacing direct tool registration
  #
  # EXAMPLES:
  ...
  Scenario: Codex agent does not expose non-native glob tool
    ...
    Then the tool list does not contain "glob"
    And the tool list contains "shell_command"
    And the tool list contains "read_file"
    And the tool list contains "list_dir"
    And the tool list contains "grep_files"
    And the tool list contains "apply_patch"
