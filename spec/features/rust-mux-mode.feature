...
Feature: Mux mode — multiplexed top-level views with /mux configuration
  ...
  NOTE: the divider is mouse-drag-resizable ONLY. Tab pane/divider cycling and keyboard divider resize were removed (2026-08-26 user directive): Tab is reserved for the agent view's turn-select mode and Esc never exits MUX MODE (mux exits are /mux off / config dialog). Mux keybindings: Shift+Left/Right (pane focus cycling) + Esc on the focused Files/Checkpoints pane closes THAT PANE only (BUG-183: transient — removed from the live rendered grid, saved tui.mux untouched; Esc stays the board-pane exit dialog / agent-pane exit cascade elsewhere; superseded the old "Esc is never a mux exit" blanket rule).
  NOTE (BUG-183, 2026-09-14): Esc on a focused ChangedFiles/Checkpoints mux pane closes the pane from the LIVE rendered grid only (saved layout untouched — /mux off then /mux on restores it); degenerate no-panes-left exit lands on the single Board view. See spec/features/mux-pane-esc-close.feature.
  ...
  #   14. The divider is mouse-drag-resizable ONLY. Tab pane/divider cycling and keyboard divider resize were removed (2026-08-26 user directive): Tab is reserved for the agent view's turn-select mode; Esc exits mux MODE only via /mux off (the 2026-08-26 blanket "Esc is never a mux exit" rule is superseded by BUG-183 — Esc on the focused Files/Checkpoints pane closes that PANE from the live grid, see spec/features/mux-pane-esc-close.feature).
