...
Feature: RPC-014 source-shape regressions — board grid + details strip modules
  ...
  - rust/core/src/work_units.rs parses `attachments` with
  `#[serde(default)]` so legacy JSON without the field still loads;
  - the new modules `rust/fspec-tui/src/views/board/grid.rs` and
  `rust/fspec-tui/src/views/board/details_strip.rs` exist and stay
  under the 300 LoC ceiling;
  - the orchestrator `rust/fspec-tui/src/views/board.rs` stays under
  the 300 LoC ceiling;
  ...
  Scenario: Grid helpers and details_strip modules exist as separate files
    Given the rust/fspec-tui crate after RPC-014 lands
    When a developer scans the views directory
    Then the file rust/fspec-tui/src/views/board/grid.rs exists
    And the file rust/fspec-tui/src/views/board/details_strip.rs exists
    And the file rust/fspec-tui/src/views/board.rs exists

  Scenario: New and modified board modules stay under 300 lines
    Given the directory rust/fspec-tui/src/views/board/
  ...
  Scenario: WorkUnitInfo gains the attachments field
    Given rust/rpc-types/src/lib.rs after RPC-014 lands
  ...
  Scenario: Core work_units parser reads attachments with serde default
    Given rust/core/src/work_units.rs after RPC-014 lands
  ...
  Scenario: RPC-013 invariants preserved
    Given rust/fspec-tui/src/views/navigator.rs after RPC-014 lands
  ...
  Scenario: Views still avoid encapsulated transport crates and host runtime construction
    Given the directory rust/fspec-tui/src/views/ (including the new board/ subdir)
