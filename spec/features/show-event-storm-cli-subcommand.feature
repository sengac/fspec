...
Feature: show-event-storm clap subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/show_event_storm.rs → fspec_core::commands::show_event_storm::run
  ...
  --help is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/show-event-storm.txt.
  """
  ...
  Scenario: Clap exposes show-event-storm as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec show-event-storm --help` from a shell
  ...
  Scenario: CLI against empty workspace exits 1 with Work unit not found
    ...
    When I run `./rust/target/release/fspec show-event-storm AUTH-001` from that directory
  ...
  Scenario: CLI exits 1 when work-units.json is malformed
    ...
    When I run `./rust/target/release/fspec show-event-storm AUTH-001`
    Then the command exits with code 1
    Then stderr contains the substring 'Error:'
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    Then the CLI bridge module rust/fspec/src/show_event_storm.rs contains NO inline filter or rendering logic — its only computation is JSON arg marshalling and stdout printing

  Scenario: show-event-storm --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec show-event-storm --help` piped to non-TTY
    Then the command exits 0
    Then stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/show-event-storm.txt
