...
Feature: fspec create-task CLI subcommand (Rust port)
  """
  Clap derive subcommand `create-task` exposes the same surface as the TS Commander.js registration at src/commands/create-task.ts:273-283 — two positional arguments `<prefix>` and `<title>` plus optional `-d, --description`, `-e, --epic`, and `-p, --parent` flags. The bridge module at codelet/fspec/src/create_task.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::create_task::run; no validation or rendering logic is duplicated.
  ...
  The `fspec create-task --help` output is byte-for-byte identical to the captured fixture at codelet/fspec/tests/fixtures/help/create-task.txt.
  """
  ...
  Scenario: Clap exposes create-task with positional args and option flags in --help
    ...
    When I run `./codelet/target/release/fspec create-task --help`
  ...
  Scenario: CLI creates a minimal task and prints the success block
    ...
    When I run `./codelet/target/release/fspec create-task INFRA "Setup CI pipeline"`
    Then the command exits 0
  ...
  Scenario: CLI creates a task with description, epic, and parent printing all detail lines
    ...
    When I run `./codelet/target/release/fspec create-task INFRA "Configure monitoring" -d "Datadog dashboards" -e ops -p INFRA-001`
    Then the command exits 0
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./codelet/target/release/fspec create-task INFRA "Second"` afterwards exits 0
    And spec/work-units.json now contains both 'INFRA-001' and 'INFRA-002'
    And the CLI bridge module codelet/fspec/src/create_task.rs contains NO inline validation, id-generation, or file-write logic — its only computation is JSON arg marshalling

  Scenario: create-task --help is byte-for-byte identical to the fixture
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec create-task --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/create-task.txt
