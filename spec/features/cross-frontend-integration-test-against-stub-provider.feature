...
Feature: Cross-frontend integration test against stub provider
  ...
  #
  # BUSINESS RULES:
  #   1. Test file lives at codelet/fspec/tests/cross_frontend_parity.rs and exercises the real fspec binary, NOT a MockBackend (that is RPC-065's surface)
  #   2. The test uses daemon mode (fspec daemon --workspace <tmp>) NOT combined mode — combined-mode tests grab /dev/tty via ratatui::init and the existing combined_smoke.rs tests are all #[ignore]'d for that reason; daemon mode exercises the exact same SessionManager + RPC surface (codelet/fspec/src/common.rs::build_service is shared) with zero TTY contention
  #   3. The stub provider is a NEW LlmProvider impl in codelet/providers/src/stub_provider.rs, gated by the existing `test-support` cargo feature, that returns deterministic responses with NO network I/O — the existing minimal StubProvider struct only exposes canned_chunks() and does not implement LlmProvider; this card widens it
  ...
  #   9. Regeneration procedure for the TS reference fixture is documented in codelet/fspec/tests/README.md (new file) so the team can re-record after a TS frontend change without spelunking the test source
  ...
  #   11. Ship a Rust-pinned golden file at codelet/fspec/tests/fixtures/cross_frontend_run.jsonl as the initial regression baseline. TS-side fixture regeneration is a follow-up RPC card once the TS stub-provider boot recipe is documented.
  #   12. Widen codelet/providers/src/stub_provider.rs to implement the LlmProvider trait behind the existing `test-support` feature; register under slug `stub` via the existing custom-provider mechanism so SessionManager::create_session("stub/canned", ...) reaches it via ProviderType::Custom. This keeps the agent loop in the test path.
  ...
  #   10. When the new codelet/fspec/tests/README.md is read, it contains a section titled `## Regenerating cross_frontend_run.jsonl` describing the FSPEC_RPC_066_REGENERATE=1 step AND a section titled `## Future: TS-recorded reference fixture` documenting the (deferred) follow-up procedure for re-recording chunks from the TS Ink frontend
  #
  # QUESTIONS (ANSWERED):
  ...
  Scenario: FSPEC_RPC_066_REGENERATE=1 re-records the golden file when it is missing
    Given the file codelet/fspec/tests/fixtures/cross_frontend_run.jsonl does not exist
  ...
  Scenario: Injected regression in BackgroundSession's chunk forwarding fails the parity test
    ...
    When a developer edits codelet/sessions/src/background_session.rs to substitute StreamChunk::text("oops") for the forwarded provider chunk
  ...
  Scenario: codelet/fspec/tests/README.md documents regeneration and the deferred TS-fixture path
    Given the codelet/fspec/tests/README.md file exists after this card
