...
Feature: Add profile action to AgentManager for Rust runtime diagnostics
  ...
  Initial instrumentation targets (must be wired during implementing — not optional): rust/tools/src/bridge_relay.rs relay_loop outer loop, connect_and_relay inbound for-loop, connect_and_relay outbound select! arms (one label per arm: control_recv, stream_recv, subordinate_recv, shutdown_recv); rust/napi/src/agent_manager_handler.rs handle_await_idle outer loop and join_set.join_next branches, spawn_subordinate_forwarding_task recv loop; rust/napi/src/scheduler/engine.rs spawn_scheduler tick loop; rust/napi/src/scheduler/loop_store.rs register_with_task_and_idle_check inner loop
  Channel instrumentation: introduce TrackedBroadcast<T>, TrackedMpsc<T>, and TrackedUnboundedMpsc<T> wrapper types in rust/tools/src/profile/channels.rs that delegate to the tokio primitives but register themselves in a global ChannelRegistry on construction (with a stable name) and unregister on Drop. Migrate supervisor_broadcast in rust/napi/src/session_manager.rs:509, OUTBOUND_CONTROL_SENDERS in bridge_relay.rs, and SUBORDINATE_CHUNK_SENDERS in bridge_relay.rs to use these wrappers
  ...
  Process metrics: use the existing dependency tree where possible — RSS via mach2::task on macOS, /proc/self/status on Linux, GetProcessMemoryInfo on Windows. Wrap in a single get_process_metrics() helper in rust/common/src/process_metrics.rs to avoid duplicating platform code
  AgentManager dispatch: extend AgentManagerAction enum in rust/tools/src/agent_manager/types.rs with `Profile { duration_secs: Option<u32>, top_n: Option<usize>, label_prefix: Option<String> }`. Profile is ASYNC (it awaits tokio::time::sleep for the duration of the window), so it dispatches through the existing async handler in rust/napi/src/agent_manager_handler.rs::create_async_handler — NOT through the sync handler. The handler validates duration_secs is in range 1..=60 (defaulting to 10), then awaits ProfileSession::run(duration_secs, top_n, label_prefix)
  ...
  Module path: rust/tools/src/profile/ — new submodule containing registry.rs (ProfileRegistry singleton with DashMap<&'static str, ScopeMetrics> and PROFILING_ACTIVE: AtomicBool), scope.rs (ProfileScope RAII guard + profile_scope!() macro), session.rs (ProfileSession::run() — the time-bounded capture orchestrator), result.rs (ProfileResult, ScopeReport, ChannelReport types), and channels.rs (instrumented broadcast/mpsc wrappers used by bridge_relay and session_manager)
  rig::tool::Tool integration: extend AgentManagerTool::call() in rust/tools/src/agent_manager/handler.rs to handle AgentManagerAction::Profile by awaiting ProfileSession::run(duration_secs, top_n, label_prefix). The handler must use the same singleton ProfileRegistry instance as the NAPI dispatch path so both call paths see the same counters and the same PROFILING_ACTIVE gate
  ...
  #   3. Every long-running async loop in rust/tools/src/bridge_relay.rs (relay_loop, inbound_handle for-loop, outbound select! loop, subordinate forwarding loop), rust/napi/src/agent_manager_handler.rs (await_idle outer loop, spawn_subordinate_forwarding_task receive loop), rust/napi/src/scheduler/engine.rs (30s tick loop), and rust/napi/src/scheduler/loop_store.rs (per-entry interval loop) MUST be instrumented with a unique label so each loop iteration is counted individually
  ...
  #
  # QUESTIONS (ANSWERED):
  #   Q: Should the proc-macro-based instrumentation cover ALL functions in rust/tools and rust/napi automatically (via #[instrument] on every fn), or should it be opt-in per call site so we keep the registry small and only profile the loops we care about?
