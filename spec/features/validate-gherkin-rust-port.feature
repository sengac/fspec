...
Feature: Port validate command to Rust
  ...
  """
  RPC-329 KNOWN DIVERGENCE (do NOT block): for malformed .feature input the error CLASSIFICATION (valid=false), exit code 1, and the Suggestion line all match TS exactly, but the embedded raw parser-error TEXT differs — TS @cucumber/gherkin emits e.g. "expected: #EOF, #StepLine, got '...'" with its token vocabulary, while Rust gherkin-0.16 emits a different (line:col) + expected-token-set message. A cucumber-compatible error formatter in io/gherkin.rs is tracked separately under RPC-329. Tests MUST assert on structural facts (file path, valid/invalid, exit code, Suggestion presence, error line number) and on substrings that DO match, NOT on the exact raw parser message. Reuse io::gherkin::parse_feature_lenient and surface its Display string as the message, matching the sibling commands' form (add_scenario.rs:90-108).
  checkForCommonIssues + getSuggestion are content-string heuristics independent of the gherkin crate — port them verbatim so the Suggestion lines and the two custom error messages (Unescaped triple quotes / Excessive blank lines) match TS byte-for-byte. These do NOT depend on RPC-329; only the embedded parser-error message text diverges.
  Exit-code transport: follow list-features precedent. Core run returns Ok(rendered_text) on the all-valid path; on the has-invalid path it returns a structured failure the bridge maps to exit 1; on the no-feature-files / unexpected-error path the core returns Err(FspecCoreError) which the bridge maps to exit 2. Concretely: introduce a small carrier so the bridge can distinguish 0/1/2 — e.g. core returns Ok(String) where the String IS the rendered block, AND for json/dispatcher path returns a JSON object {success, results:[...], summary, exitCode}. The shell bridge inspects the dispatcher-shape's exitCode (0/1/2) OR re-derives it from invalidCount. DECISION (mirrors list-features DirectoryNotFound→2): zero-feature-files and unexpected errors → Err(FspecCoreError) → bridge exit 2; invalid files → Ok with an embedded exitCode/invalidCount the bridge reads → exit 1; all valid → exit 0. Supervisor to confirm whether to add a dedicated FspecCoreError variant or carry exitCode in the JSON payload.
  """
