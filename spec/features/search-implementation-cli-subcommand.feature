...
Feature: Port search-implementation command to Rust
  ...
  Scenario: Clap exposes search-implementation as a subcommand and prints flag help
    ...
    When I run `./rust/target/release/fspec search-implementation --help` from a shell
  ...
  Scenario: CLI without --function flag exits non-zero
    ...
    When I run `./rust/target/release/fspec search-implementation` from that directory
  ...
  Scenario: CLI default output prints the green summary line
    ...
    When I run `./rust/target/release/fspec search-implementation --function loadConfig` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI --json prints the 2-space JSON envelope to stdout
    Given a temp workspace has a coverage sidecar whose implMappings reference an on-disk file containing "loadConfig"
    When I run `./rust/target/release/fspec search-implementation --function loadConfig --json` from that workspace
    Then the command exits 0
  ...
  Scenario: search-implementation --help is byte-for-byte identical to the TS formatCommandHelp reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec search-implementation --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/search-implementation.txt
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And I run `./rust/target/release/fspec search-implementation --function loadConfig --json` against the same workspace
    Then both invocations produce a JSON envelope with the same files array length
    And the CLI bridge module rust/fspec/src/search_implementation.rs contains NO inline filtering or rendering logic — its only computation is JSON arg marshalling
