...
Feature: TS frontend NAPI WorkUnitInfo shape preserved after rpc-types lift
  ...
  After WorkUnitInfo is lifted from codelet/napi/src/types.rs into codelet/rpc-types, the existing TypeScript frontend MUST continue to see the same shape with no breaking changes. This feature is a single Vitest smoke test that imports get_all_work_units via the codelet/napi binding and asserts the returned object keys.
  ...
  Background: User Story
    As a TypeScript developer consuming codelet/napi from the existing TUI
  ...
  Scenario: TS frontend smoke test confirms get_all_work_units shape after the lift
    Given the WorkUnitInfo type has been lifted from codelet/napi into codelet/rpc-types and codelet/napi has been rebuilt
    When the Vitest smoke test imports get_all_work_units from the codelet/napi binding and calls it
