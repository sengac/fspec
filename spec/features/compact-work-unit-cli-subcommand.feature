...
Feature: fspec compact-work-unit CLI subcommand (Rust port)
  ...
  The `fspec compact-work-unit --help` output is byte-for-byte identical to `node dist/index.js compact-work-unit --help` (TS reference) — captured as codelet/fspec/tests/fixtures/help/compact-work-unit.txt.
  """
  ...
  Scenario: Clap exposes compact-work-unit with a positional arg and --force flag in --help
    ...
    When I run `./codelet/target/release/fspec compact-work-unit --help`
  ...
  Scenario: CLI compacts a done work unit and prints the removed-items summary
    ...
    When I run `./codelet/target/release/fspec compact-work-unit AUTH-001`
    Then the command exits 0
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And the CLI bridge module codelet/fspec/src/compact_work_unit.rs contains NO inline file-read, mutation, or rendering logic — its only computation is JSON arg marshalling

  Scenario: compact-work-unit --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec compact-work-unit --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/compact-work-unit.txt
