...
Feature: Widen codelet-rpc-types with every wire-portable shape AgentView needs
  ...
  #   6. Phase 2.4 supporting type: IsolatedSessionInfo { session_id: SessionId, worktree_path: String, base_commit: String } is added. The pre-existing FspecResult type is left byte-compatible: its current shape (success, data: String, error: Option<String>, system_reminder, tool_call_id) is preserved so rust/napi/src/types.rs::FspecResult round-trips through NAPI unchanged
  #   7. Every added struct and enum is exercised by a JSON round-trip test (serialize via serde_json → deserialize → assert equality with the original instance) in rust/rpc-types/src/lib.rs under a #[cfg(test)] mod tests block — establishing the first test suite for rpc-types and proving the types are wire-portable by construction
  ...
  #   7. Cross-frontend usage example: a downstream consumer in rust/fspec-tui writes `use codelet_rpc_types::{SessionTokens, PauseState, HitlRequest, IsolatedSessionInfo, ThinkingConfig};` and the imports resolve cleanly — proving every new type is publicly re-exported from the rpc-types crate root, matching the existing pattern for WorkUnitInfo, SessionId, and StreamChunk
  #
  # ========================================
  ...
  Scenario: Phase 2.1 per-session derived-state types are added with the documented field shapes
    Given the engineer opens rust/rpc-types/src/lib.rs after RPC-036 is implemented
  ...
  Scenario: All new types JSON-round-trip cleanly via serde_json
    Given a test suite under #[cfg(test)] in rust/rpc-types/src/lib.rs
    And serde_json is declared in rust/rpc-types/Cargo.toml under [dev-dependencies]
  ...
  Scenario: codelet-napi continues to compile after the rpc-types widening
    ...
    And rust/napi/src/types.rs::stream_chunk_to_json_value destructures the IsolationStateChange variant with the new base_commit field accounted for
