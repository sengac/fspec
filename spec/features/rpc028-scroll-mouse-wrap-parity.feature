...
Feature: Add proper scrolling, mouse wheel support, and wrap-around to all Rust dialog/popup/picker views
  ...
  #   6. A new shared module rust/fspec-tui/src/components/scroll_viewport.rs owns wrap_index(current, delta, total), ensure_visible(scroll_offset, selected, visible_rows, total), and a WheelVelocity helper — every refactored view consumes it instead of duplicating math.
