...
Feature: Auto-continue engine: done() tool and /continue toggle with nudge budget
  """
  VERIFIED integration points (2026-07-09): Session struct codelet/cli/src/session/mod.rs:31-62 (precedent field thinking_exhaustion_cross_turn_count at :57; constructors Session::new :72-88 and from_provider_manager :99-109). Settle point stream_loop.rs run_agent_stream_internal (:269), FinalResponse arm ends with emit_done_with_stop_reason at :1429 (doc said 1428-1430 — exact). Non-nudging emit sites confirmed: :763 (interrupt), :813/:843/:864 (stall_timeout), :1903 (error). PROV-041 re-prompt recipe at :1301-1358 (fresh TokenState+CompactionHook, prompt_streaming_with_history_and_hook, push Message::User, reset per-stream locals, continue).
  DECISION (doc §3 'verify' resolved): agents are rebuilt PER USER MESSAGE — CLI: agent_runner.rs:42 create_rig_agent inside run_agent_with_interruption, with a FRESH session_id per message (agent_runner.rs:32 Uuid::new_v4); NAPI/TUI: agent_loop.rs:982 per dispatched input with stable session.id. So conditional (armed-only) registration IS feasible and takes effect from the next dispatched message. Constraint: create_rig_agent(session_id, preamble, thinking) 3-arg signature is enforced by shape tests rpc082/rpc083/rpc085 — must NOT add a parameter. Chosen mechanism: per-session armed registry in codelet_tools (new codelet/tools/src/done.rs, modeled on INJECT_SUMMARY_HANDLERS RwLock registry, inject_summary.rs:90-98); each of the 7 builder chains conditionally adds DoneTool::new(session_id) when codelet_tools::done::is_continue_armed(session_id). Session.continue_enabled remains the single source of truth; the registry is synced immediately before create_rig_agent at the two dispatch sites (agent_runner.rs ~:33 after generating the fresh uuid; agent_loop.rs ~:960 before the provider match). Fallback (always-register + inert) NOT needed.
  Pure decision function: new module codelet/cli/src/interactive/auto_continue.rs (<300 lines) exporting ContinueDecision {Finish, Nudge, FinishWithWarning, FinishWithSummary} + decide_continuation(armed, done_summary: Option<&str>, stop_reason: Option<&str>, nudges_used, budget, is_interrupted) implementing the doc §5 table; called in the FinalResponse arm just before stream_loop.rs:1429. Only stop_reason stop/end_turn/None may nudge — max_tokens/length returns Finish (defers to PROV-040/041 which run earlier in the arm). Test pattern mirrors PROV-041: pure fns re-exported via codelet_cli::interactive, tested from codelet/cli/tests/ (see thinking_exhaustion_recovery_test.rs:10-15); wiring asserted via source-shape tests (rpc082/083 precedent) rather than a full scripted stream harness — codelet's stream_loop has no ScriptedModel-style harness (the CONT-001 harness drives the rig-core loop, not run_agent_stream_internal), so shape tests + pure-fn tests are the feasible red-first surface.
  Command surface wiring (verified): TUI registry codelet/fspec-tui/src/views/agent/slash_commands.rs — SlashCommandAction enum :21, name() :45, SLASH_COMMANDS :85. Typed args codelet/fspec-tui/src/app/slash_parser.rs::parse_slash_command (:74; /loop routed :123-125) — add '/continue' branch producing SlashCommandParse::ContinueSubcommand(ContinueSubcommand) with a dedicated continue_parser.rs modeled on loop_parser.rs (LoopSubcommand :19). Dispatch codelet/fspec-tui/src/app/dispatch_slash_commands.rs — handle_slash_command :27 (palette pick), handle_input_submitted :175 (typed form); backend round-trip per /compact arm :65-97 → new session-state setter in codelet/napi/src/session_bindings.rs. CLI repl codelet/cli/src/interactive/repl_loop.rs::repl_loop (:16): /continue handler inserted after /compact (:68-134) and BEFORE the provider-switch catch-all at :137; grammar parsing shared via codelet_cli auto_continue module (small duplicate of TUI parser is acceptable — crates don't share a parser crate; ~20 lines, mirrors existing repl ad-hoc pattern).
  """
