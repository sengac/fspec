...
Feature: Provider settings: inline testResult rendering on focused row in list mode
  ...
  Implementation:
  - rust/fspec-tui/src/views/provider_settings/mod.rs — add `pub test_result: Option<ProviderTestResult>` field on `ProviderSettingsView` (default None in `new()`); add `pub fn set_test_result(&mut self, provider_id: impl Into<String>, status: ProviderTestStatus)` and `pub fn clear_test_result(&mut self)`. Define `ProviderTestResult { pub provider_id: String, pub status: ProviderTestStatus }` and `pub enum ProviderTestStatus { Testing, Ok { latency_ms: u32 }, Err { message: String } }` — derive Debug/Clone/PartialEq/Eq. Re-export both from `mod.rs`.
  """
