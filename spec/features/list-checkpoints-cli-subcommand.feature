...
Feature: List checkpoints CLI subcommand
  """
  CLI subcommand is wired into codelet/fspec/src/main.rs's Mode enum as a clap v4 derive variant per RPC-003 §7/§11. The action arm delegates to fspec_core::commands::list_checkpoints::run(args_json, &cwd) so business logic is not duplicated between the LLM-facing dispatcher and the shell-facing CLI.
  ...
  Scenario: Clap exposes list-checkpoints as a subcommand with flag-aware --help
    ...
    When I run `./codelet/target/release/fspec list-checkpoints --help` from a shell
  ...
  Scenario: list-checkpoints --help is byte-for-byte identical to the TS formatCommandHelp reference output
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec list-checkpoints --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at codelet/fspec/tests/fixtures/help/list-checkpoints.txt
  ...
  Scenario: Missing positional argument exits non-zero with clap's required-arg error
    Given the fspec Rust binary at codelet/target/release/fspec has been compiled
    When I run `./codelet/target/release/fspec list-checkpoints` from a shell with NO positional argument
  ...
  Scenario: CLI against empty directory prints sentinel and does not auto-init a git repo
    ...
    When I run `./codelet/target/release/fspec list-checkpoints AUTH-001` from that directory
    Then the command exits 0
  ...
  Scenario: CLI text output renders manual checkpoint progress for the populated case
    ...
    When I run `./codelet/target/release/fspec list-checkpoints AUTH-001`
    Then the command exits 0
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    When I run `./codelet/target/release/fspec list-checkpoints AUTH-001` against the same on-disk state
    Then stdout contains the substring '📌  baseline (manual)'
    Then the CLI bridge module codelet/fspec/src/list_checkpoints.rs contains NO inline checkpoint-listing, classification or rendering logic — its only computation is JSON arg marshalling
