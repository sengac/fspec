...
Feature: OpenAI Chat Completions drops tool-returned images (Read/PDF/MCP)
  """
  Primary fix location: rust/patches/rig-core/src/providers/openai/completion/mod.rs. The existing `impl TryFrom<message::ToolResult> for Message` at lines 393-414 returns a single Message — NOT suitable because we may need to emit a follow-up user message. Change the integration at lines 507-540 (`TryFrom<OneOrMany<message::UserContent>> for Vec<Message>`) so the tool-results branch (lines 517-524) routes through a NEW helper `tool_result_to_messages(tool_result) -> Result<Vec<Message>, MessageError>` that returns [tool_message] for text-only and [tool_message, user_image_message] for image-bearing results. Flatten the Vec<Vec<Message>> into Vec<Message>. Keep the original `TryFrom<message::ToolResult> for Message` impl for backward compat (it can return ConversionError for image inputs, now an unreachable path inside the crate).
  Reference implementation: the Responses API path at rust/patches/rig-core/src/providers/openai/responses_api/mod.rs:295-322 already handles mixed text+image tool results correctly using `ToolResultOutput::ContentItems`. We do NOT copy that wholesale (Chat Completions uses a different message shape), but the same pattern — iterate parts, partition into text vs image, emit appropriately — applies. Similar reference: Anthropic at anthropic/completion.rs:485-499 which uses content blocks.
  Tests in rust/patches/rig-core/src/providers/openai/completion/mod.rs (`#[cfg(test)] mod prov_084_tests`). Each test builds a rig `message::Message::User { content: OneOrMany<UserContent::ToolResult(...)> }`, converts to Vec<Message> via `TryFrom`, and asserts the shape of the result: number of messages, roles, tool_call_ids, content parts. Serialize to JSON and spot-check `image_url.url` prefix. Reuse base64 decoding helpers from PROV-081's test infrastructure if needed.
