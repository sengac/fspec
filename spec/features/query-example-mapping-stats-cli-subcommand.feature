...
Feature: query-example-mapping-stats clap subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/query_example_mapping_stats.rs → fspec_core::commands::query_example_mapping_stats::run
  ...
  --help is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/query-example-mapping-stats.txt.
  """
  ...
  Scenario: Clap exposes query-example-mapping-stats as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec query-example-mapping-stats --help` from a shell
  ...
  Scenario: CLI with --format=json prints the canonical empty stats against an empty workspace
    ...
    When I run `./rust/target/release/fspec query-example-mapping-stats --format json` from that directory
    Then the command exits 0
  ...
  Scenario: CLI without --format prints nothing to stdout (TS silent-text parity)
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./rust/target/release/fspec query-example-mapping-stats` from that directory
    Then the command exits 0
  ...
  Scenario: CLI with --format=text also prints nothing to stdout (explicit text bug parity)
    Given an empty directory with no spec/ subdirectory is set as the current working directory
    When I run `./rust/target/release/fspec query-example-mapping-stats --format text` from that directory
    Then the command exits 0
    Then stdout is exactly empty
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    Then the CLI bridge module rust/fspec/src/query_example_mapping_stats.rs contains NO inline aggregation, filter, or rendering logic — its only computation is JSON arg marshalling and stdout printing

  Scenario: query-example-mapping-stats --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec query-example-mapping-stats --help` piped to non-TTY
    Then the command exits 0
    Then stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/query-example-mapping-stats.txt
