...
Feature: Behaviour-parity test suite for every slash command + keyboard shortcut
  ...
  [K] /provider vs /providers alias verification: both branches share the `Provider | Providers` match arm in dispatch_rpc020.rs::handle_slash_command (line 103). The parity test fires Provider and Providers from two fresh harnesses and asserts both produce `navigator.active_view == ViewMode::ProviderSettings` after draining (the OpenProviderSettingsView action is delivered via action_tx → navigator.apply_action).
  [L] Ctrl+C requires the session to be in a state where the dispatch path emits `Action::Interrupt` (codelet/fspec-tui/src/views/agent/dispatch.rs line 240). The harness press_key helper synthesises the KeyEvent and routes it through `app.handle_event(Event::Key(_))`. Status gating: the App::dispatch handler for Interrupt only spawns backend.interrupt when the focused session's status is Running/Compacting (per dispatch_rpc051.rs). The parity test seeds `SessionStatus::Running` via `mock.push_status_change(s-1, Running)` then drains, then presses Ctrl+C.
  """
  ...
  #   10. Dispatching SlashCommandSelected(Provider) and SlashCommandSelected(Providers) both result in agent_view_store.provider_settings_view becoming Some(_) — the two slash commands are aliases and both routes converge on Action::OpenProviderSettingsView
  ...
  #   16. Dispatching SlashCommandSelected(Schedule) appends a static help notice into the focused session's scrollback (the bare-popup path documented in dispatch_rpc058.rs::handle_slash_schedule_help)
  #   17. Dispatching SlashCommandSelected(Loop) appends a static help notice into the focused session's scrollback (dispatch_rpc059.rs::handle_slash_loop_help)
  ...
  Scenario: /providers is an alias for /provider and activates the same view
    Given a fresh AppTestHarness with focused session s-1
    When I dispatch the slash command "/providers"
    And I drain pending tasks and actions
    Then the navigator's active_view is ViewMode::ProviderSettings
