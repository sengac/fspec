...
Feature: Behaviour-parity test suite for every slash command + keyboard shortcut
  """
  [A] File layout: (1) new module `rust/fspec-tui/tests/common/harness.rs` declared from `tests/common/mod.rs` via `pub mod harness;`. The harness wraps `App` + `Arc<MockBackend>` (the existing 2876-LoC double already exports every counter we need). (2) new integration test file `rust/fspec-tui/tests/behaviour_parity_rpc065.rs` declaring `mod common;` to pull in the harness module.
  ...
  #   4. A single new integration test file `rust/fspec-tui/tests/behaviour_parity_rpc065.rs` walks the full matrix from the RPC-065 attachment; existing detailed `*_rpcNNN.rs` test files are NOT deleted or migrated
