...
Feature: Scheduler engine lift into codelet-core::scheduler
  ...
  currently in rust/napi/src/scheduler/ (engine, state, cron_utils,
  types, trigger, agent_job, shell_job, catch_up, job_log) MUST be
  lifted into rust/core/src/scheduler/ following the same lift
  pattern used by RPC-031..RPC-034 for the persistence layer.

  After the lift:

  - rust/core/src/scheduler/mod.rs hosts all engine modules and a
  ...
  - rust/napi/src/scheduler/mod.rs collapses to `pub use
  ...
  out of rust/napi/src/schedule_handler.rs into a new
  rust/core/src/scheduler/crud.rs (NAPI-free). The remaining
  ...
  Scenario: Scheduler engine modules live under rust/core/src/scheduler/
    Given the directory rust/core/src/scheduler/ exists
  ...
  Scenario: codelet-core declares a SchedulerHooks trait
    Given the file rust/core/src/scheduler/mod.rs is compiled
  ...
  Scenario: spawn_scheduler accepts an Arc<dyn SchedulerHooks>
    Given the file rust/core/src/scheduler/engine.rs is compiled
  ...
  Scenario: The lifted engine has no crate::session_bindings references
    Given the directory rust/core/src/scheduler/ exists
    Then no file under rust/core/src/scheduler/ contains the text "crate::session_bindings"
    And no file under rust/core/src/scheduler/ contains the text "session_bindings::SessionManager"
  ...
  Scenario: The pure CRUD helpers live in codelet-core::scheduler::crud
    Given the file rust/core/src/scheduler/crud.rs is compiled
  ...
  Scenario: codelet-sessions handle_impl wires the five new methods to crud.rs
    Given the file rust/sessions/src/handle_impl.rs is compiled
  ...
  Scenario: Lifted scheduler engine and cron_utils use captured-identifier format args (RPC-058 retro 2026-05-27)
    ...
    Given rust/core/src/scheduler/engine.rs and rust/core/src/scheduler/cron_utils.rs live at their post-RPC-058 location
