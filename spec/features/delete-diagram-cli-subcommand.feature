...
Feature: fspec delete-diagram CLI subcommand (Rust port)
  ...
  `<section>` and `<title>`. The CLI bridge at rust/fspec/src/delete_diagram.rs
  ...
  rust/fspec/tests/fixtures/help/delete-diagram.txt).
  """
  ...
  Scenario: Clap exposes delete-diagram with two positional args in --help
    ...
    When I run `./rust/target/release/fspec delete-diagram --help`
  ...
  Scenario: CLI removes a diagram by title and prints the success block on stdout
    ...
    When I run `./rust/target/release/fspec delete-diagram Architecture "Component Flow"`
    Then the command exits 0
  ...
  Scenario: CLI fails with exit 1 when foundation.json is missing
    ...
    When I run `./rust/target/release/fspec delete-diagram Architecture "X"`
  ...
  Scenario: CLI fails with exit 1 when title is not found
    ...
    When I run `./rust/target/release/fspec delete-diagram Architecture "Missing"`
    Then the command exits with code 1
    And stderr contains the substring 'Error:'
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec delete-diagram Architecture "B"` afterwards exits 0
    And spec/foundation.json architectureDiagrams is empty
    And the CLI bridge module rust/fspec/src/delete_diagram.rs contains NO inline file IO, JSON-parse, or splice logic — its only computation is JSON arg marshalling

  Scenario: delete-diagram --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec delete-diagram --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/delete-diagram.txt
