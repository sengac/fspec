...
Feature: get-scenarios clap subcommand on the standalone fspec Rust binary
  ...
  - Shell argv         → clap → rust/fspec/src/get_scenarios.rs → fspec_core::commands::get_scenarios::run
  ...
  --help is byte-for-byte identical to the captured TS fixture at rust/fspec/tests/fixtures/help/get-scenarios.txt.
  """
  ...
  Scenario: Clap exposes get-scenarios as a subcommand and prints flag-aware --help
    ...
    When I run `./rust/target/release/fspec get-scenarios --help` from a shell
  ...
  Scenario: CLI with --format=json prints a JSON array of scenario objects
    ...
    When I run `./rust/target/release/fspec get-scenarios --tag @auth --format json` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI default text output prints the count message and groups scenarios by feature
    Given a temp workspace contains spec/features/login.feature tagged '@auth' with two scenarios
    When I run `./rust/target/release/fspec get-scenarios --tag @auth` from that workspace
    Then the command exits 0
  ...
  Scenario: CLI against a workspace with no spec/features exits 1
    ...
    When I run `./rust/target/release/fspec get-scenarios` from that directory
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    Then the CLI bridge module rust/fspec/src/get_scenarios.rs contains NO inline parsing, filtering, or rendering logic — its only computation is JSON arg marshalling and stdout printing

  Scenario: get-scenarios --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec get-scenarios --help` piped to non-TTY
    Then the command exits 0
    Then stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/get-scenarios.txt
