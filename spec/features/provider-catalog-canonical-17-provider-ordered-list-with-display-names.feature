...
Feature: Provider catalog: canonical 17-provider ordered list with display names
  ...
  #   3. The list_provider_credentials impl in codelet/sessions/src/handle_impl.rs:869-895 returns ALL 17 canonical providers FIRST in canonical order (regardless of whether their env var is set), then appends any custom providers discovered on disk. The hard-coded 6-built-in loop in codelet/providers/src/custom/management.rs:99-118 is REPLACED with iteration over CANONICAL_PROVIDERS
  #   4. The Rust slug for Anthropic is 'anthropic' (TS-canonical) NOT 'claude' (the current Rust internal slug at codelet/providers/src/custom/management.rs:100). Any internal callers using 'claude' must be aliased or migrated — the wire/UX surface uses 'anthropic' exclusively
  #   5. ProviderCredentialInfo.display_name (codelet/rpc-types/src/lib.rs:395) is populated from the canonical catalog NOT from the slug — the TUI list and title row use ONLY display_name, never provider_id, for human-facing strings
  #
  # EXAMPLES:
