...
Feature: fspec create-epic CLI subcommand (Rust port)
  """
  Clap derive subcommand `create-epic` exposes the same surface as the TS Commander.js registration at src/commands/create-epic.ts:115-127 — two positional arguments `<epicId>` and `<title>` plus an optional `-d, --description <description>` flag. The bridge module at rust/fspec/src/create_epic.rs marshals the clap args into a JSON object and delegates to codelet_fspec_core::commands::create_epic::run; no validation or rendering logic is duplicated.
  ...
  The `fspec create-epic --help` output is byte-for-byte identical to `node dist/index.js create-epic --help` (TS reference) — captured as rust/fspec/tests/fixtures/help/create-epic.txt.
  """
  ...
  Scenario: Clap exposes create-epic with positional args and a --description flag in --help
    ...
    When I run `./rust/target/release/fspec create-epic --help`
  ...
  Scenario: CLI creates a minimal epic and prints the success block
    ...
    When I run `./rust/target/release/fspec create-epic auth Authentication`
    Then the command exits 0
  ...
  Scenario: CLI creates an epic with -d description and includes the Description line
    Given an empty working directory with no spec/ subdirectory
    When I run `./rust/target/release/fspec create-epic auth Authentication -d "Login features"`
    Then the command exits 0
    And stdout contains the line '✓ Created epic auth'
    And stdout contains the line '  Title: Authentication'
  ...
  Scenario: CLI rejects an invalid epicId with exit 1 and stderr Error prefix
    Given an empty working directory with no spec/ subdirectory
    When I run `./rust/target/release/fspec create-epic INVALID Authentication`
  ...
  Scenario: CLI rejects creating an epic that already exists with exit 1
    ...
    When I run `./rust/target/release/fspec create-epic auth NewTitle`
    Then the command exits with code 1
    And stderr contains the substring 'Error:'
  ...
  Scenario: CLI delegates to the same fspec_core function used by the dispatcher
    ...
    And running `./rust/target/release/fspec create-epic dash Dashboard` afterwards exits 0
    And spec/epics.json now contains both 'auth' and 'dash' epics
    And the CLI bridge module rust/fspec/src/create_epic.rs contains NO inline epic-id validation, duplicate-check, or file-write logic — its only computation is JSON arg marshalling

  Scenario: create-epic --help is byte-for-byte identical to TS reference
    Given the fspec Rust binary at rust/target/release/fspec has been compiled
    When I run `./rust/target/release/fspec create-epic --help` piped to non-TTY
    Then the command exits 0
    And stdout is byte-for-byte identical to the fixture at rust/fspec/tests/fixtures/help/create-epic.txt
